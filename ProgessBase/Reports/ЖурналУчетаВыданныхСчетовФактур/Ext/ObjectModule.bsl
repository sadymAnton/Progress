...
Функция ВыводСтроки(Документ, Секция, МассивПрефиксовДляРИБиОрганизации)
	...
	/// Кунов О.В., 12.03.2015 - 37421
	Секция.Параметры.ПредставлениеСФОснования = "";
	///
...
Функция ВыводСтрокиКорректировочногоСФ(Документ, Секция, МассивПрефиксовДляРИБиОрганизации)
	...
	/// Кунов О.В., 12.03.2015 - 37421
	Секция.Параметры.ПредставлениеСФОснования = " № "
		+ ОбщегоНазначения.ПолучитьНомерНаПечать(Новый Структура("Номер, Организация", Документ.НомерСФОснования, Организация), МассивПрефиксовДляРИБиОрганизации)
		+ " от " + Формат(Документ.ДатаСФОснования, "ДФ=dd.MM.yyyy") + " г.";
	///
...
Функция ПодготовитьОтчетКВыводуНаПечать()
	...
	/// Кунов О.В., 12.03.2015 - 37421
	|	РеестрСчетовФактур.НомерСФОснования,
	|	РеестрСчетовФактур.ДатаСФОснования,
	///
	|	ВЫБОР
	...
	/// Кунов О.В., 12.03.2015 - 37421
	|		,СчетФактураВыданный.НомерИсходногоДокумента КАК НомерСФОснования,
	|		СчетФактураВыданный.ДатаИсходногоДокумента КАК ДатаСФОснования
	///
	|	ИЗ
	...
	/// Кунов О.В., 12.03.2015 - 37421
	|		,КорректировочныйСчетФактура.НомерИсходногоДокумента КАК НомерСФОснования,
	|		КорректировочныйСчетФактура.ДатаИсходногоДокумента КАК ДатаСФОснования
	///
	|	ИЗ
