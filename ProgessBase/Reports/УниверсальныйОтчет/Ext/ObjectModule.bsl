...
Процедура СформироватьОтчет(ТабличныйДокумент, Очищать = Истина, ФиксироватьЗаголовок = Истина, Отчет = Неопределено) Экспорт
	...
	Если ВыводВозможен() Тогда
		...
		//начало изменений Ожиганов 21.05.2015 срок годности 
...
//начало изменений Ожиганов 21.05.2015 срок годности
Функция ПРГЭтоПримитивныйТип(Типы)
	...
	Если ТипЗнч(Типы) = Тип("Массив") Тогда
		Если Типы.Количество() = 1 Тогда
			Если Типы[0] = Тип("Дата") или Типы[0]= Тип("Булево") или Типы[0] = Тип("Число") Тогда
				возврат истина;
			Иначе
				возврат ложь;
			КонецЕсли;	
	...
	Если ТипЗнч(Типы) = тип("тип") Тогда
		Если Типы = Тип("Дата") или Типы = Тип("Булево") или Типы = Тип("Число") Тогда
			возврат истина;
		Иначе
			возврат ложь;
		КонецЕсли;	
...
Процедура УстановитьНачальныеНастройкиПоМетаданнымРегистра(ИмяРегистра)
	...
	//	|	ПРЕДСТАВЛЕНИЕ(ИсточникДанных." + Измерение.Имя + ") КАК " + Измерение.Имя + "Представление";
		|	ИсточникДанных." + Измерение.Имя + " КАК " + Измерение.Имя + ?(ПРГЭтоПримитивныйТип(Измерение.Тип.Типы()),"",",
		|	ПРЕДСТАВЛЕНИЕ(ИсточникДанных." + Измерение.Имя + ") КАК " + Измерение.Имя + "Представление");
	...
	Если мМассивПоляГруппировка <> Неопределено Тогда
		...
		Для каждого ПолеГруппировка из мМассивПоляГруппировка Цикл
			...
			|	" + ПолеВыборки + " КАК " + ПолеГруппировка.Имя + ?(ПРГЭтоПримитивныйТип(ПолеГруппировка.Тип),"",",
			|	ПРЕДСТАВЛЕНИЕ(" + ПолеВыборки + ") КАК " + ПолеГруппировка.Имя + "Представление");
			//конец изменений   
...
Процедура СформироватьСтруктуруДляСохраненияНастроек(СтруктураСНастройками) Экспорт
	...
	/// Кунов О.В., 27.12.2016 - 60663
	Если мНазваниеОтчета = "Товары на складах" Тогда
		СтруктураСНастройками.Вставить("Отбор", ЗначениеВСтрокуВнутр(ПостроительОтчета.Отбор));
	КонецЕсли;
	///
...
Функция ВосстановитьНастройкиИзСтруктуры(СтруктураСНастройками, Отчет = Неопределено) Экспорт
	...
	Если мНазваниеОтчета = "Товары на складах" Тогда
		Отбор = Неопределено;
		Если СтруктураСНастройками.Свойство("Отбор", Отбор) Тогда
			Отбор = ЗначениеИзСтрокиВнутр(Отбор);
			Если ТипЗнч(Отбор) = Тип("Отбор") И Отбор.Количество() > 0 Тогда
				Для Н = 1 По ПостроительОтчета.Отбор.Количество() Цикл
					ПостроительОтчета.Отбор.Удалить(0);
				КонецЦикла;
				Для Каждого ЭлементОтбора Из Отбор Цикл
					Если ЭлементОтбора.Имя = "Периодичность" Или Не ЗначениеЗаполнено(ЭлементОтбора.ПутьКДанным) Тогда
						Продолжить;
					КонецЕсли;
					Попытка
						НовыйЭлементОтбора = ПостроительОтчета.Отбор.Добавить(ЭлементОтбора.ПутьКДанным);
						НовыйЭлементОтбора.ВидСравнения = ЭлементОтбора.ВидСравнения; // т.к. Значение копируется раньше, чем ВидСравнения, ВидСравнения нужно заполнить заранее
						ЗаполнитьЗначенияСвойств(НовыйЭлементОтбора, ЭлементОтбора, , "ВидСравнения");
					Исключение
					КонецПопытки;
				КонецЦикла;
			КонецЕсли;
