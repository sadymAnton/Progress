...
//начало изменений ИТИ
Перем мОбязательнаяСортировка Экспорт; 
Перем мМассивОграничениНаБыстрыеОтборы Экспорт;
//конец изменений ИТИ
...
Процедура ОформитьМакетДляРезЗапроса(РезЗапрос,ПостроительИтоговый,ПолучаемыйМакет)
   ...
   //ПостроительИтоговый.Макет.Показать();
	...
	НачСтрока  = 7;
	//ЗАГОЛОВОК 
	///ТабМакет.Область(1,,НачСтрока).Имя = "Заголовок";
	ДобавитьОбластьЗаголовок(ТабМакет);
	//ТабМакет.Показать();
	НачСтрока  = ТабМакет.ВысотаТаблицы;
	...
	Если УниверсальныйОтчет.мРазмещениеПоложениеВыбранныхПолей <> Неопределено Тогда
		СтараяСтруктура = УниверсальныйОтчет.мРазмещениеПоложениеВыбранныхПолей;
		УниверсальныйОтчет.мРазмещениеПоложениеВыбранныхПолей = Новый Структура;
		 
		Для каждого _ТекЭлемент Из ПостроительОтчета.ВыбранныеПоля Цикл
			НайденноеЗначение = Неопределено;
			Если СтараяСтруктура.Свойство(_ТекЭлемент.Имя,НайденноеЗначение)  Тогда
				 УниверсальныйОтчет.мРазмещениеПоложениеВыбранныхПолей.Вставить(_ТекЭлемент.Имя,НайденноеЗначение);
			КонецЕсли;	
		КонецЦикла; 
	...
	Для Каждого _ТекЗначение1 ИЗ УниверсальныйОтчет.Показатели.Строки Цикл
		 ...
		 Если _ТекЗначение1.Использование  Тогда
			  КолВоИспПодч = 0;
			  Для каждого _ТекЗначение2 Из _ТекЗначение1.Строки Цикл
			  ...
			  Если НайдРекв.Количество() > 0 или Истина Тогда
					  ...
					  КонецЦикла; 
					   ...
					   Если  МассПарметров.Количество()= 1 Тогда
							  ...
							  КонецЕсли;
							  ТекОбласть.АвтоОтступ = 1;//НомерГруппировки-1;
							  //ТекОбласть.Отступ = НомерГруппировки-1;
					   Иначе
						   шбл  = "";  Зап = "";
		 ...
		 Пока м < КолВоПустыхДополнительно Цикл
			 ...
			 Если м < ТаблДетЗаписей.Количество() Тогда
				  ...
				  Если м = ТаблРеквГруппДо.Количество()  Тогда
					  Если  ПостроительИтоговый.ИзмеренияСтроки.Количество() > 0 Тогда
					  		ТекОбласть.АвтоОтступ = 1; //ПостроительИтоговый.ИзмеренияСтроки.Количество()+1;
							//ТекОбласть.Отступ	  = ПостроительИтоговый.ИзмеренияСтроки.Количество()+1;
					  КонецЕсли;	
				  КонецЕсли; 
			  КонецЕсли;	
			 ТекКолонка = ТекКолонка+1;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Область <> Неопределено Тогда  
				...
				//Если ЭтоГруппировка Тогда
					ВыводОбласть = ТабличныйДокумент.Вывести(Область,Уровень);
		КонецЕсли;		
...
Процедура ВывестиДанныеВТабличнДокумент(ТабличныйДокумент,ПостроительИтоговый,РезЗапрос,Отчет) Экспорт
	Перем ПолучаемыйМакет;
	
	  УниверсальныйОтчет = ЭтотОбъект;
	  
	  ОформитьМакетДляРезЗапроса(РезЗапрос,ПостроительИтоговый,ПолучаемыйМакет);
	...
	//ПолучаемыйМакет.Показать();
	 ...
	 КонецЕсли; 
	  ...
	  ТабличныйДокумент.Вывести(ПолучаемыйМакет.ПолучитьОбласть("Заголовок"));
	  ...
	  Иначе
		  Если ПостроительИтоговый.ВыводитьОбщиеИтоги  Тогда
				...
				ВывестиРезЗапроса(Выборка,ТаблОбластей,ТабличныйДокумент,0,ИндексДеталей,1,КолВоЗап);
				ТабличныйДокумент.ЗакончитьАвтогруппировкуСтрок();
				ТабличныйДокумент.Вывести(Область);
		  Иначе		
		    	Выборка = РезЗапрос.Выбрать();
...
Процедура СформироватьОтчет(ТабличныйДокумент, Очищать = Истина, ФиксироватьЗаголовок = Истина, Отчет = Неопределено) Экспорт
	...
	Если ВыводВозможен() Тогда
		...
		//ПостроительОтчета.МакетОформления.Показать();
		...
		Если мОбязательнаяСортировка <> Неопределено Тогда
			//ПостроительОтчета.ВыбранныеПоля.Добавить("Дата");
			//строительОтчета.Порядок.Добавить("Регистратор");
			для Каждого ТекЭл ИЗ мОбязательнаяСортировка Цикл;
				Если ПостроительОтчета.ВыбранныеПоля.Найти(ТекЭл) = Неопределено Тогда
					ПостроительОтчета.ВыбранныеПоля.Добавить(ТекЭл)
				КонецЕсли;	
				ПостроительОтчета.Порядок.Добавить(ТекЭл);
			КонецЦикла;
		КонецЕсли;
...
Процедура УстановитьПредставленияПолей(СтруктураПредставлениеПолей, ПостроительОтчета,БезПериодов=Ложь) Экспорт

	Если Не БезПериодов Тогда
...
Процедура ОбработкаРасшифровкиУниверсальногоОтчета(Расшифровка, Отчет, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		Иначе
			...
			Если ТипЗнч(ОтчетРасшифровка) = Тип("ОтчетОбъект.АП_ДебиторскаяЗадолженностьПоСрокамДолга")
				И ТипЗнч(Отчет) = Тип("ОтчетОбъект.АП_ДебиторскаяЗадолженностьПоСрокамДолга") Тогда
				ОтчетРасшифровка.Интервал = Отчет.Интервал;
			КонецЕсли;
			//----m.ionov@a-prof.ru---
...
Процедура ДобавитьПанельБыстрыеОтборы(Объект, Форма) Экспорт
	...
	//начало изменений ИТИ
	Если мМассивОграничениНаБыстрыеОтборы <> Неопределено Тогда
		Попытка
			КоличествоБыстрыхОтборов = Мин(мМассивОграничениНаБыстрыеОтборы.Количество(),КоличествоБыстрыхОтборов);
		Исключение
		КонецПопытки	
	КонецЕсли;	
	//конец изменений ИТИ
	...
	Иначе
		...
		Для каждого ЭлементОтбора из мМассивБыстрыеОтборы Цикл
			...
			Если мМассивОграничениНаБыстрыеОтборы <> Неопределено Тогда
				Попытка
					Если мМассивОграничениНаБыстрыеОтборы.Найти(ЭлементОтбора.Имя) = Неопределено Тогда
						продолжить;
					КонецЕсли;	
				Исключение
	...
	КонецЕсли;
	//начало изменений ИТИ
	мМассивОграничениНайБыстрыеОтборы = Неопределено;
	//конец изменений ИТИ
