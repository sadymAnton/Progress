...
Функция СформироватьЗаголовок() Экспорт
	...
	Если ЗначениеЗаполнено(ВидУчета) Тогда
		СтрФильтры = СтрФильтры + ", Вид учета = " + ВидУчета;
	КонецЕсли;
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	ЗапросПоОстаткам.УстановитьПараметр("ВидУчета",    ВидУчета);
	...
	ЗапросПоПроводкам.УстановитьПараметр("ВидУчета",    ВидУчета);
	...
	БухгалтерскиеОтчеты.ДополнитьСтрокуОграниченийПоРеквизитам(СтрокаОграниченийПоРеквизитам, "Организация", Организация);
	Если ЗначениеЗаполнено(ВидУчета) Тогда
		СтрокаОграниченийПоРеквизитам = БухгалтерскиеОтчеты.ОбъединитьОграничения(СтрокаОграниченийПоРеквизитам, "((СчетДт В ИЕРАРХИИ (&СчетАнализа)) И ВидУчетаДт = &ВидУчета ИЛИ (СчетКт В ИЕРАРХИИ (&СчетАнализа)) И ВидУчетаКт = &ВидУчета)");
	Иначе
