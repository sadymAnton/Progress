...
Функция ТекстЗапросаПоСценарию(СтрокаСценарий,СтруктураПараметров,Номер,ПериодДанных, СтруктураГруппировки)
	...
	ИначеЕсли СтрокаСценарий.ВидДанных=Перечисления.ВидыДанныхДляПланФактногоАнализаДДС.ФактическиеДанные Тогда
		...
		ИначеЕсли СтрокаСценарий.ВидДанных=Перечисления.ВидыДанныхДляПланФактногоАнализаДДС.ФактическиеИПланируемые Тогда 
			...
			|		    И НЕ (Проект ССЫЛКА Справочник.ВидыРаспределенияПоПроектам)) КАК ПланируемыеПоступленияДенежныхСредствОстатки
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ
			...
			|		    И НЕ (Проект ССЫЛКА Справочник.ВидыРаспределенияПоПроектам)) КАК ЗаявкиНаРасходованиеСредствОстатки
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ
...
Процедура РасшифровкаДвижений(Расшифровка, ЭтотОтчет, Выбор)
	...
	Если Расшифровка.Свойство("ЦФО") 
		И ЗначениеЗаполнено(Расшифровка.ЦФО)
		Тогда
	
		Если Отчет.УниверсальныйОтчет.ПостроительОтчета.Отбор.Найти("Подразделение") = Неопределено Тогда
			Отчет.УниверсальныйОтчет.ДобавитьОтбор("Подразделение");
		КонецЕсли;
		Отчет.УниверсальныйОтчет.ПостроительОтчета.Отбор.Подразделение.Установить(Расшифровка.ЦФО);
