...
Функция СформироватьЗаголовок() Экспорт
	...
	СтрОтбор = ?(ЗначениеЗаполнено(ВидУчета), ", Вид учета = "+ВидУчета,"");

	ТекстОтбора = УправлениеОтчетами.СформироватьСтрокуОтборов(ПостроительОтчета.Отбор);
	
	СтрОтбор = Сред(СтрОтбор + ", " + ТекстОтбора, 3);
...
Процедура ЗаполнитьПараметрыРасшифровки(Область, Выборка, ОтборСубконто = Неопределено)
	...
	ПараметрыКарточкиСубконто.Вставить("ВидУчета", ВидУчета);
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	|ИЗ (ВЫБРАТЬ
	|	(ВС.Ссылка.Количественный) Количественный
	|ИЗ
