					...
					Если СсылкаНаВерсиюТаблицы[Индекс-1]._ИдСтроки = Неопределено Тогда
						...
						Для Каждого ЭлементКоллекции Из СсылкаНаВерсиюТаблицы.Колонки Цикл
							Если (ЭлементКоллекции.Имя <> "_ИдСтроки") И (ЭлементКоллекции.Имя <> "_Модификация") Тогда
								...
								Если ТаблицаИзмененийТабличныхЧастей[ИмяТаблицы][НомерПредыдущейВерсии].Колонки.Найти(ЭлементКоллекции.Имя) <> Неопределено Тогда
...
Функция РазборВерсии(НомерВерсии, Ссылка)
	...
	Если ПараметрыСеанса.ПРГ_КаталогХраненияФайлов <> "" Тогда
		ИмяФайлаХран = ПРГ_ДопФункцииКлиентСервер.ПРГ_ВнФайлы_Версии_ПолучитьСущИмяФайла(Ссылка,НомерВерсии);
		Если ИмяФайлаХран <> "" Тогда
			Расшир = Нрег(Прав(ИмяФайлаХран,3));
			ИмяВрФайлаXml = ПолучитьИмяВременногоФайла("xml");
			ИмяВрФайлаZip = ПолучитьИмяВременногоФайла("zip");
			УдалятьКаталог = Ложь;
			Если Расшир = "xml" Тогда
				ОбФайлxml = Новый ДвоичныеДанные(ИмяФайлаХран);
				ОбФайлxml.Записать(ИмяВрФайлаXml);
			ИначеЕсли Расшир = "zip" Тогда
				ОбФайлZip = Новый ДвоичныеДанные(ИмяФайлаХран);
				ОбФайлZip.Записать(ИмяВрФайлаZip);
				
				ЧтениеЗип = Новый ЧтениеZipФайла(ИмяВрФайлаZip);
				НужныйЭлемент = Неопределено;
				Для каждого ТекЭлем Из ЧтениеЗип.Элементы Цикл
					Если Нрег(ТекЭлем.Расширение) = "xml" или Нрег(ТекЭлем.Расширение) = ".xml" Тогда
						НужныйЭлемент = ТекЭлем;
						прервать;
					Иначе
						НужныйЭлемент = ?(НужныйЭлемент<>Неопределено,ТекЭлем,НужныйЭлемент);
					КонецЕсли;	
				Конеццикла;	
				...
				Если НужныйЭлемент  = Неопределено Тогда
					...
					///ВызватьИсключение  " не удалось извлечь информацию из архива";
					возврат Ложь;
				КонецЕсли;
				
				ОбВрДляКаталога = Новый Файл(ПолучитьИмяВременногоФайла("tmp"));
				ВрКаталогЗип    = Лев(ОбВрДляКаталога.ПолноеИмя,СтрДлина(ОбВрДляКаталога.ПолноеИмя)-4);
				
				СоздатьКаталог(ВрКаталогЗип);
				Если Прав(ВрКаталогЗип,1) <> "\" Тогда
					ВрКаталогЗип = ВрКаталогЗип+"\";
				Конецесли;	
				
				ЧтениеЗип.Извлечь(НужныйЭлемент,ВрКаталогЗип);
				ЧтениеЗип.Закрыть();
				
				УдалитьФайлы(ИмяВрФайлаZip);
				
				ИмяВрФайлаXml = ВрКаталогЗип+НужныйЭлемент.Имя;
				УдалятьКаталог = Истина;
				
			Иначе
				вызватьисключение "неизвестный формат файла !!!";
			КонецЕсли;	
			
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.Прочитать(ИмяВрФайлаXml, КодировкаТекста.UTF8);
			СтрокаXML = ТекстовыйДокумент.ПолучитьТекст();
			ТекстовыйДокумент = Неопределено;
			//Сообщить(ИмяВрФайлаXml);
			Если УдалятьКаталог Тогда
				УдалОбъект = Новый Файл(ИмяВрФайлаXml);
				ПРГ_ДопФункцииКлиентСервер.ПРГ_ВнФайлы_УдалитьФайлыВКаталоге(УдалОбъект.Путь,Истина);
			Иначе	
				УдалитьФайлы(ИмяВрФайлаXml);
			КонецЕсли;	
