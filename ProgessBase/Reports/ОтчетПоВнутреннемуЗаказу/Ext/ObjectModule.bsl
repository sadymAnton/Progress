...
Функция ПолучитьРезультатЗапросаПоНоменклатуре(Заказ, ДатаАнализа) Экспорт 	
	...
	Запрос = Новый Запрос;
	Если ТипЗаказа = "Производство" Тогда
		Запрос.Текст=
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ОтчетПроизводстваЗаСменуМатериалы.Номенклатура.Код + "" "" + ОтчетПроизводстваЗаСменуМатериалы.Номенклатура.Наименование КАК Номенклатура,
		|	ОстаткиТоваровКомпании.СерияНоменклатуры.Представление КАК Партия,
		|	ОстаткиТоваровКомпании.СерияНоменклатуры.СрокГодности КАК СрокГодности,
		|	ОстаткиТоваровКомпании.КоличествоОстаток КАК Остаток
		|ИЗ
		|	РегистрНакопления.ТоварыНаСкладах.Остатки(&ДатаАнализа, Склад = &СкладЗаказа) КАК ОстаткиТоваровКомпании
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПроизводстваЗаСмену.Материалы КАК ОтчетПроизводстваЗаСменуМатериалы
		...
		|		ПО (ОтчетПроизводстваЗаСменуМатериалы.Номенклатура = ОстаткиТоваровКомпании.Номенклатура)
		|ГДЕ
		|	ОтчетПроизводстваЗаСмену.СкладМатериалов = &СкладЗаказа
		|	И ОтчетПроизводстваЗаСмену.Ссылка = &Заказ
		|ИТОГИ
		|	СУММА(Остаток)
		|ПО
		|	Номенклатура";
	...
	ИначеЕсли ТипЗаказа = "Внутренний" Тогда
		Запрос.Текст=
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ВнутреннийЗаказТовары.Номенклатура.Код + "" "" + ВнутреннийЗаказТовары.Номенклатура.Наименование КАК Номенклатура,
		|	ОстаткиТоваровКомпании.СерияНоменклатуры.Представление КАК Партия,
		|	ОстаткиТоваровКомпании.СерияНоменклатуры.СрокГодности КАК СрокГодности,
		...
		|		ПО ВнутреннийЗаказТовары.Номенклатура = ОстаткиТоваровКомпании.Номенклатура
		|ГДЕ
		|	ВнутреннийЗаказТовары.Ссылка = &Заказ
		|ИТОГИ
		|	СУММА(Остаток)
		|ПО
		|	Номенклатура,
	...
	ИначеЕсли ТипЗаказа = "Производство" Тогда
		Запрос.УстановитьПараметр("СкладЗаказа",  Заказ.СкладМатериалов);
	КонецЕсли;
	...
	Если НЕ СтруктураПараметров.Свойство("СтруктураДанныхТекущегоЗаказа",СтруктураДанныхТекущегоЗаказа) Тогда 
		...
		Если НЕ СтруктураПараметров.Свойство("ЭтоМатериалы",ЭтоМатериалы) Тогда 
			...
			Если ИмяГруппировки<>"ТоварТара" Тогда
				...
				Если СтруктураПараметров.ТипЗаказа = "Производство" ИЛИ СтруктураПараметров.ТипЗаказа = "Внутренний" Тогда
					Партия = ?( Выборка["Партия"] = NULL, "", Выборка["Партия"]);
					ЗначениеСвободныйОстаток	= Формат(?(Выборка["Остаток"]=NULL,0,
																Выборка["Остаток"]), ФорматПоказателя);
					СрокГодности			= ?(Выборка["СрокГодности"]=NULL,"",Выборка["СрокГодности"]);

					//Если НЕ СтруктураПараметров.ТипЗаказа = "Производство" Тогда 
					//	Склад			= ?(Выборка["Склад"]=NULL,"",Выборка["Склад"]);
					//КонецЕсли;
			...
			ОбластьЗначенияПоказателя.Параметры.Партия     = СокрЛП(Партия);
			ОбластьЗначенияПоказателя.Параметры.СрокГодности = СокрЛП(СрокГодности);
...
//  Макет = ПолучитьМакет( ?(ТипЗаказа = "Поставщику", "МакетЗаказПоставщику", ?(ТипЗаказа = "Внутренний", "МакетВнутреннийЗаказ", "Макет")));
  	Макет = ПолучитьМакет("МакетВнутреннийЗаказ");
	...
	СтруктураСдвигУровняГруппировок.Вставить("Номенклатура", 0);
	Если ТипЗаказа = "Внутренний" Тогда 
		СтруктураСдвигУровняГруппировок.Вставить("Склад", 1);
	КонецЕсли;
	...
	СтруктураПараметров.Вставить("СтруктураСдвигУровняГруппировок",  СтруктураСдвигУровняГруппировок);
	СтруктураПараметров.Вставить("ВсегоГруппировок", ?(ТипЗаказа="Внутренний",2,1));
	СтруктураПараметров.Вставить("ТипЗаказа", ТипЗаказа);
	...
	ОбластьЗначение.Параметры.ЗаголовокОтчета = "Наличие материалов по документу "
			+ ПредставлениеЗаказа+Символы.ПС;
			//+" на "
