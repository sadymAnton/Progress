...
Функция СформируемТаблица_АдресаПоставки_ТипыЦен()
	...
	Если ПараметрНачалоПериода <> Неопределено Тогда
		...
		ДатаЦен = НачалоДня(ПараметрНачалоПериода.Значение);
		//25.01.2016 Островерхий} 
	...
	Результат.Свернуть("Контрагент, АдресПоставки, Номенклатура, Скидка, ТипЦены, Цена, ЦенаСНДС","");
	...
	Пока Сч < Результат.Количество() Цикл
		...
		Строка.КаналПродаж = Строка.Контрагент.НСИ_КаналПродаж;
		...
		//{25.01.2016 Островерхий заявка №б/н 
		ПЦ_ДатаЦены = ПЦ_Ценообразование.ПолучимДатуЦеныПоОтсрочкеПовышенияЦены(ДатаЦен,
		...
		ДатаЦен = ?(ПЦ_ДатаЦены = Дата(1,1,1), ДатаЦен, ПЦ_ДатаЦены);	
		//25.01.2016 Островерхий}
		СпТипЦен = ПЦ_Ценообразование.ПолучитьТипЦенПродажи(ДатаЦен,
														Строка.Регион,
														Строка.ПроектГП,
														Строка.КаналПродаж,
														Строка.УсловиеПоставки,
														Строка.Контрагент,
														Неопределено,//Справочники.ДоговорыКонтрагентов.ПустаяСсылка(),
		...
		Если ЗначениеЗаполнено(Строка.ТипЦены) Тогда
			...
			Если Строка.ТипЦены.ЦенаВключаетНДС	тогда
				Строка.ЦенаСНДС = Строка.Цена;
				Строка.Цена = Ценообразование.ПересчитатьЦенуПриИзмененииФлаговНалогов(
					Строка.Цена, 
					Неопределено, //СпособЗаполненияЦен //Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатуры, 
					Строка.ТипЦены.ЦенаВключаетНДС,
		       		истина,//УчитыватьНДС, 
					Ложь,//СуммаВключаетНДС, 
					УчетНДС.ПолучитьСтавкуНДС(Строка.Номенклатура.СтавкаНДС));
			Иначе	
				Строка.ЦенаСНДС = Ценообразование.ПересчитатьЦенуПриИзмененииФлаговНалогов(
					Строка.Цена, 
					Неопределено, //СпособЗаполненияЦен //Перечисления.СпособыЗаполненияЦен.ПоЦенамНоменклатуры, 
					Строка.ТипЦены.ЦенаВключаетНДС,
		       		истина,//УчитыватьНДС, 
