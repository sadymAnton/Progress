...
Функция СформироватьОтчет(Результат = Неопределено, ДанныеРасшифровки = Неопределено, ВыводВФормуОтчета = Истина) Экспорт
	...
	СтруктураВнешнихДанных.Вставить("ИсходныеДанные", Ценообразование.ПолучитьЦенуНоменклатурыНоваяМетодика(СтруктураФильтров.Период, СтруктураФильтров.ПериодПолученияКурсовВалют, Ложь,
	СтруктураФильтров.Дивизион, СтруктураФильтров.Регион, СтруктураФильтров.КаналПродаж, СтруктураФильтров.Контрагент, СтруктураФильтров.УсловиеПоставки, СтруктураФильтров.АдресПоставки,
	СтруктураФильтров.Проект, СтруктураФильтров.ТоварнаяКатегория, СтруктураФильтров.Брендообъем, СтруктураФильтров.Номенклатура, СтруктураФильтров.ТипЦен));
	
	//{19.02.2016 Островерхий заявка №48274 
	СтруктураВнешнихДанных.ИсходныеДанные.Колонки.Добавить("ДатаЦены",Новый ОписаниеТипов("Дата"));
	СтруктураВнешнихДанных.ИсходныеДанные.ЗаполнитьЗначения(СтруктураФильтров.Период, "ДатаЦены"); 
	
	//Скорректируем цены и даты цен в отчете по имеющейся отсрочке цен.
	СкорректироватьТаблицуЦенСУчетомОтсрочки(СтруктураВнешнихДанных.ИсходныеДанные,СтруктураФильтров);
	//19.02.2016 Островерхий} 
	
	//{10.01.2017 Островерхий заявка №60462 
	СкорректироватьТаблицуЦенСУчетомДопСкидки(СтруктураВнешнихДанных.ИсходныеДанные,СтруктураФильтров);
	//10.01.2017 Островерхий} 
...
Процедура СкорректироватьТаблицуЦенСУчетомДопСкидки(ТаблицаЦен,СтруктураФильтров)

	ТаблицаЦен.Колонки.Добавить("ПроцентДопИзмененияЦены",Новый ОписаниеТипов("Число"));

	МассивАдресовПоставки = ТаблицаЦен.ВыгрузитьКолонку("АдресПоставки");
	МассивНоменклатур = ТаблицаЦен.ВыгрузитьКолонку("Номенклатура");
	
	УдалитьДублирующиеЭлементыМассива(МассивАдресовПоставки);
	УдалитьДублирующиеЭлементыМассива(МассивНоменклатур);
	
	ТаблицаУсловийДопИзмененияЦены = Ценообразование.ПолучитьУсловияДопИзмененияЦеныПоАПИНоменклатуре(СтруктураФильтров.Период, МассивАдресовПоставки, МассивНоменклатур);
	...
	Запрос.Текст = 
		...
		|	ТаблицаЦен.Дивизион,
		|	ТаблицаЦен.Регион,
		|	ТаблицаЦен.КаналПродаж,
		|	ТаблицаЦен.Контрагент,
		|	ТаблицаЦен.УсловиеПоставки,
		|	ТаблицаЦен.АдресПоставки,
		|	ТаблицаЦен.ПроектГП,
		|	ТаблицаЦен.ТоварнаяКатегория,
		|	ТаблицаЦен.Брендообъем,
		|	ТаблицаЦен.Номенклатура,
		|	ТаблицаЦен.ТипЦен,
		|	ТаблицаЦен.ЦенаБезНДС,
		|	ТаблицаЦен.ЦенаСНДС,
		|	ТаблицаЦен.Валюта,
		|	ТаблицаЦен.ЭтоИндивидуальнаяЦена,
		|	ТаблицаЦен.КоличествоЦен,
		|	ТаблицаЦен.КодНоменклатуры,
		|	ТаблицаЦен.Штрихкод,
		|	ТаблицаЦен.ДатаЦены
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаУсловийДопИзмененияЦены.АдресПоставки,
		|	ТаблицаУсловийДопИзмененияЦены.Номенклатура,
		|	ТаблицаУсловийДопИзмененияЦены.ПроцентИзмененияЦены,
		|	ТаблицаУсловийДопИзмененияЦены.Условие
		|ПОМЕСТИТЬ втТаблицаУсловийДопИзмененияЦены
		|ИЗ
		|	&ТаблицаУсловийДопИзмененияЦены КАК ТаблицаУсловийДопИзмененияЦены
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втТаблицаЦен.Дивизион,
		|	втТаблицаЦен.Регион,
		|	втТаблицаЦен.КаналПродаж,
		|	втТаблицаЦен.Контрагент,
		|	втТаблицаЦен.УсловиеПоставки,
		|	втТаблицаЦен.АдресПоставки,
		|	втТаблицаЦен.ПроектГП,
		|	втТаблицаЦен.ТоварнаяКатегория,
		|	втТаблицаЦен.Брендообъем,
		|	втТаблицаЦен.Номенклатура,
		|	втТаблицаЦен.Валюта,
		|	втТаблицаЦен.ЭтоИндивидуальнаяЦена,
		|	втТаблицаЦен.КоличествоЦен,
		|	втТаблицаЦен.КодНоменклатуры,
		|	втТаблицаЦен.Штрихкод,
		|	втТаблицаЦен.ДатаЦены,
		|	ЕСТЬNULL(ВложенныйЗапрос.ПроцентИзмененияЦены, 0) КАК ПроцентДопИзмененияЦены,
		|	втТаблицаЦен.ТипЦен,
		|	втТаблицаЦен.ЦенаБезНДС КАК ЦенаБезНДС,
		|	ВЫРАЗИТЬ(втТаблицаЦен.ЦенаСНДС КАК ЧИСЛО(10, 2)) КАК ЦенаСНДС
		|ИЗ
		|	втТаблицаЦен КАК втТаблицаЦен
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			втТаблицаУсловийДопИзмененияЦены.АдресПоставки КАК АдресПоставки,
		|			втТаблицаУсловийДопИзмененияЦены.Номенклатура КАК Номенклатура,
		|			СУММА(втТаблицаУсловийДопИзмененияЦены.ПроцентИзмененияЦены) КАК ПроцентИзмененияЦены
		|		ИЗ
		|			втТаблицаУсловийДопИзмененияЦены КАК втТаблицаУсловийДопИзмененияЦены
		|		
		|		СГРУППИРОВАТЬ ПО
		|			втТаблицаУсловийДопИзмененияЦены.АдресПоставки,
		|			втТаблицаУсловийДопИзмененияЦены.Номенклатура) КАК ВложенныйЗапрос
		|		ПО втТаблицаЦен.АдресПоставки = ВложенныйЗапрос.АдресПоставки
		|			И втТаблицаЦен.Номенклатура = ВложенныйЗапрос.Номенклатура";

	Запрос.УстановитьПараметр("ТаблицаЦен",ТаблицаЦен);
	Запрос.УстановитьПараметр("ТаблицаУсловийДопИзмененияЦены",ТаблицаУсловийДопИзмененияЦены);	
...
Процедура СкорректироватьТаблицуЦенСУчетомОтсрочки(ТаблицаЦен,СтруктураФильтров)

	Если НЕ ВыводитьСУчетомОтсрочкиЦен Тогда
		Возврат;
	КонецЕсли; 
	
	ТаблицаКорректировокЦен = Новый ТаблицаЗначений;     
	ТаблицаКорректировокЦен.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
    ТаблицаКорректировокЦен.Колонки.Добавить("АдресПоставки", Новый ОписаниеТипов("СправочникСсылка.АдресаПоставки"));
	ТаблицаКорректировокЦен.Колонки.Добавить("ТипЦен", Новый ОписаниеТипов("СправочникСсылка.ТипыЦенНоменклатуры"));
	ТаблицаКорректировокЦен.Колонки.Добавить("ДатаЦены", Новый ОписаниеТипов("Дата"));
	ТаблицаКорректировокЦен.Колонки.Добавить("ЦенаБезНДС", Новый ОписаниеТипов("Число"));
	ТаблицаКорректировокЦен.Колонки.Добавить("ЦенаСНДС", Новый ОписаниеТипов("Число"));
	
    //Сформируем таблицу отсрочки АдресПоставки+Номенклатура+ДатаЦены
	ТаблицаОтсрочкиЦен = Ценообразование.СформироватьТаблицуОтсрочкиЦенНоменклатурыНоваяМетодика(СтруктураФильтров.Период,
		СтруктураФильтров.Дивизион, СтруктураФильтров.Регион, СтруктураФильтров.КаналПродаж, СтруктураФильтров.Контрагент, СтруктураФильтров.УсловиеПоставки, СтруктураФильтров.АдресПоставки,
		СтруктураФильтров.Проект, СтруктураФильтров.ТоварнаяКатегория, СтруктураФильтров.Брендообъем, СтруктураФильтров.Номенклатура);
		
	Если ТаблицаОтсрочкиЦен.Количество() = 0 Тогда
		Возврат;
	КонецЕсли; 	
		
	ТаблицаПериодов = ТаблицаОтсрочкиЦен.Скопировать();
	ТаблицаПериодов.Свернуть("ДатаЦены");

	Для каждого СтрокаПериода Из ТаблицаПериодов Цикл
		
		ОбработкаПрерыванияПользователя();

		МассивАдресовПоставки = Новый Массив;
		МассивНоменклатур = Новый Массив;
		
		НайденныеСтроки = ТаблицаОтсрочкиЦен.НайтиСтроки(Новый Структура("ДатаЦены", СтрокаПериода.ДатаЦены));
		
		Для каждого ТекСтрока Из НайденныеСтроки Цикл
		
			МассивАдресовПоставки.Добавить(ТекСтрока.АдресПоставки);		
		...
		Если МассивАдресовПоставки.Количество()> 0 И МассивНоменклатур.Количество()> 0 Тогда
		
			ВременнаяТаблицаЦен = Ценообразование.ПолучитьЦенуНоменклатурыНоваяМетодика(СтрокаПериода.ДатаЦены, , Ложь,	, , , 
				, , МассивАдресовПоставки,	, , , МассивНоменклатур, );
				
			ВременнаяТаблицаЦен.Колонки.Добавить("ДатаЦены",Новый ОписаниеТипов("Дата"));
			ВременнаяТаблицаЦен.ЗаполнитьЗначения(СтрокаПериода.ДатаЦены, "ДатаЦены");
	
			УправлениеПланированием.ДополнитьТаблицу(ТаблицаКорректировокЦен,ВременнаяТаблицаЦен);	
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаКорректировокЦен.АдресПоставки,
		|	ТаблицаКорректировокЦен.Номенклатура,
		|	ТаблицаКорректировокЦен.ТипЦен,
		|	ТаблицаКорректировокЦен.ДатаЦены,
		|	ТаблицаКорректировокЦен.ЦенаБезНДС,
		|	ТаблицаКорректировокЦен.ЦенаСНДС
		|ПОМЕСТИТЬ втТаблицаКорректировокЦен
		|ИЗ
		|	&ТаблицаКорректировокЦен КАК ТаблицаКорректировокЦен
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	втТаблицаЦен.Брендообъем,
		|	втТаблицаЦен.Номенклатура,
		|	ЕСТЬNULL(втТаблицаКорректировокЦен.ТипЦен, втТаблицаЦен.ТипЦен) КАК ТипЦен,
		|	ЕСТЬNULL(втТаблицаКорректировокЦен.ЦенаБезНДС, втТаблицаЦен.ЦенаБезНДС) КАК ЦенаБезНДС,
		|	ЕСТЬNULL(втТаблицаКорректировокЦен.ЦенаСНДС, втТаблицаЦен.ЦенаСНДС) КАК ЦенаСНДС,
		|	втТаблицаЦен.Валюта,
		|	втТаблицаЦен.ЭтоИндивидуальнаяЦена,
		|	втТаблицаЦен.КоличествоЦен,
		|	втТаблицаЦен.КодНоменклатуры,
		|	втТаблицаЦен.Штрихкод,
		|	ЕСТЬNULL(втТаблицаКорректировокЦен.ДатаЦены, втТаблицаЦен.ДатаЦены) КАК ДатаЦены
		|ИЗ
		|	втТаблицаЦен КАК втТаблицаЦен
		|		ЛЕВОЕ СОЕДИНЕНИЕ втТаблицаКорректировокЦен КАК втТаблицаКорректировокЦен
		|		ПО втТаблицаЦен.АдресПоставки = втТаблицаКорректировокЦен.АдресПоставки
		|			И втТаблицаЦен.Номенклатура = втТаблицаКорректировокЦен.Номенклатура";

	Запрос.УстановитьПараметр("ТаблицаЦен",ТаблицаЦен);
	Запрос.УстановитьПараметр("ТаблицаКорректировокЦен",ТаблицаКорректировокЦен);	
	ТаблицаЦен = Запрос.Выполнить().Выгрузить();
...
Функция СформироватьСтруктуруФильтров(КомпоновщикНастроек)
	...
	Для каждого ЭлементОтбора Из ЭлементыОтбора Цикл
		...
		Если ЭлементОтбора.Использование
			...
			И СтруктураФильтров.Свойство(Строка(ЭлементОтбора.ЛевоеЗначение)) Тогда
