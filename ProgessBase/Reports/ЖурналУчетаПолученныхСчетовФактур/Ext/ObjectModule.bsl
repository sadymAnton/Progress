...
Функция ПодготовитьОтчетКВыводуНаПечать()
	...
	|	АО
	|;
	// <- Шевченков 46446 20151126
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СчетФактураПолученныйДокументыОснования.Ссылка) КАК ДокСФ,
	...
	|		КОГДА СчетФактураПолученный.Ссылка.Продавец <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА СчетФактураПолученный.Ссылка.Продавец
	...
	// <- Шевченков 46446 20151126
	|		ЛЕВОЕ СОЕДИНЕНИЕ втМножСФ КАК втМножСФ
	|		ПО СчетФактураПолученный.ДокументОснование = втМножСФ.ДокументОснование
	// ->
	|ГДЕ
	...
	|	РеестрСчетовФактур.ВидСчетаФактуры КАК ВидСчетаФактуры
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВТ_НеКорректировочныеСФ.СчетФактура КАК СчетФактура,
	|		ВТ_НеКорректировочныеСФ.ДатаРегистрации КАК ДатаРегистрации,
	|		ВТ_НеКорректировочныеСФ.ДокументОснование КАК ДокументОснование,
	|		ВТ_НеКорректировочныеСФ.СуммаДокумента КАК СуммаДокумента,
	|		ВТ_НеКорректировочныеСФ.ДатаВходящегоДокумента КАК ДатаВходящегоДокумента,
	|		ВТ_НеКорректировочныеСФ.НомерВходящегоДокумента КАК НомерВходящегоДокумента,
	|		ВТ_НеКорректировочныеСФ.Контрагент КАК Контрагент,
	|		ВТ_НеКорректировочныеСФ.ВалютаДокумента КАК ВалютаДокумента,
	|		ВТ_НеКорректировочныеСФ.ОпределитьПараметрыСчетаФактуры КАК ОпределитьПараметрыСчетаФактуры,
	|		ВТ_НеКорректировочныеСФ.ДокументОснованиеДата КАК ДокументОснованиеДата,
	|		ВТ_НеКорректировочныеСФ.ДокументОснованиеНомер КАК ДокументОснованиеНомер,
	|		ВТ_НеКорректировочныеСФ.ВидСчетаФактуры КАК ВидСчетаФактуры
	|	ИЗ
	|		ВТ_НеКорректировочныеСФ КАК ВТ_НеКорректировочныеСФ
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыПолученного.ПустаяСсылка)
	|	ИЗ
