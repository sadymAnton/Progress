...
Процедура ЗаполнитьСписокНастроек(КлючВарианта = Неопределено)
	...
	Запрос = Новый Запрос;
	ОбработкаСкладскихДокументов = ХранилищаНастроек.НастройкиФормированияРегулярныхДокументов.ТекущийВладелецЭтоСкладскиеДокументы(КлючОбъекта);
	...
	Иначе
		...
		Запрос.Текст = 
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	НастройкиФормированияДокументовВыпускаПродукции.Наименование,
			|	НастройкиФормированияДокументовВыпускаПродукции.Ссылка,
			|	""Выпуск продукции: "" КАК НаименованиеДополнительно,
			|	1 КАК Сортировка
			|ИЗ
			...
			|
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	НастройкиФормированияДокументовОтработанногоВремени.Наименование,
			|	НастройкиФормированияДокументовОтработанногоВремени.Ссылка,
			|	""Отработанное время: "",
			|	2
			|ИЗ
			|	Справочник.НастройкиФормированияДокументовОтработанногоВремени КАК НастройкиФормированияДокументовОтработанногоВремени
			|ГДЕ
			|	(НЕ НастройкиФормированияДокументовОтработанногоВремени.ПометкаУдаления)
			|	И (НЕ НастройкиФормированияДокументовОтработанногоВремени.ЭтоГруппа)
			|
			|УПОРЯДОЧИТЬ ПО
			|	Сортировка";
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			СтрокаСписка.Наименование       = Выборка.НаименованиеДополнительно + Выборка.Наименование;
		КонецЕсли; 
