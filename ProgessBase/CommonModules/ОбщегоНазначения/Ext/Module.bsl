...
Процедура ПриУстановкеНовогоНомераДляДокументов(Источник, СтандартнаяОбработка, Префикс) Экспорт
	...
	//++ Spl_Апроф 24.12.2014 (k.russkih@a-prof.ru)
	АП_ОбщегоНазначенияСервер.УстановитьПрефиксОтделения(Источник, Префикс);
	//-- Spl_Апроф
	
	//начало изменений нумерация объектов
	ПРГ_ДопФункцииКлиентСервер.ПрисвоитьНомер(Источник,Префикс,"",СтандартнаяОбработка);
	//конец изменений нумерация объектов
КонецПроцедуры
...
Процедура ДублироватьСообщениеВЖурналРегистрации(ТекстСообщения, Статус, Заголовок="")
	...
	ДублироватьСообщенияВЖурналРегистрации = Истина;
...
Функция ПолучитьРегистрыДляОптимизацииПерезаписиДвижений(ТекущийРежимПроведенияДокумента)
	...
	Если ТекущийРежимПроведенияДокумента = РежимПроведенияДокумента.Неоперативный Тогда
		...
		//начало изменений Ожиганов 29.07.2015 добавим списанные товары, чтобы избавиться от блокировок 
		МассивРегистров.Добавить("СписанныеТовары");
		//конец изменений 
		//начало изменений Ожиганов 14.09.2015 добавим размещение заказов
		МассивРегистров.Добавить("РазмещениеЗаказовПокупателей");
		//конец изменений 
		//m.ionov@a-prof.ru 22.05.2014
		Если ПараметрыСеанса.ПроведениеДокументов Тогда
			//МассивРегистров.Добавить("СписанныеТовары");
			МассивРегистров.Добавить("РасчетыПоРеализацииВУсловныхЕдиницахОрганизации");
			
			//МассивРегистров.Добавить("НДСНачисленный");
			МассивРегистров.Добавить("РасчетыПоРеализацииОрганизации");		
		КонецЕсли;
		//----m.ionov@a-prof.ru---
	КонецЕсли;
...
//	РежимПроведенияДокумента 	- режим проведения (оперативный / неоперативный), 
//								нужен для составления списка регистров, которые не надо очищать
Процедура УдалитьЗаписанныеДвиженияДокумента(ДокументОбъект, Отказ, ВыборочноОчищатьРегистры, ТекущийРежимПроведенияДокумента)
	
	//{26.01.2016 Островерхий заявка №б/н 
	//Изменено Имя переменной 4го параметра 
	//Было РежимПроведенияДокумента стало ТекущийРежимПроведенияДокумента
	...
	Если ВыборочноОчищатьРегистры Тогда
		РегистрыДляОптимизацииПерезаписиДвижений = ПолучитьРегистрыДляОптимизацииПерезаписиДвижений(ТекущийРежимПроведенияДокумента);
		
		//{26.01.2016 Островерхий заявка №46457, добавим РН свободные остатки в массив РегистрыДляОптимизацииПерезаписиДвижений для видов документов:
		//поступление товаров на складах, реализации товаров услуг, требование-накладной,
		//списание товаров, отчет производства за смену,Перемещение товаров, Корректировка реализации,
		ТипОбъекта = ТипЗнч(ДокументОбъект);
		
		Если ТекущийРежимПроведенияДокумента = РежимПроведенияДокумента.Неоперативный
			И (ТипОбъекта = Тип("ДокументОбъект.ПоступлениеТоваровУслуг")
			ИЛИ ТипОбъекта = Тип("ДокументОбъект.РеализацияТоваровУслуг") ИЛИ ТипОбъекта = Тип("ДокументОбъект.КорректировкаРеализации")
			ИЛИ ТипОбъекта = Тип("ДокументОбъект.ТребованиеНакладная") ИЛИ ТипОбъекта = Тип("ДокументОбъект.СписаниеТоваров")
			ИЛИ ТипОбъекта = Тип("ДокументОбъект.ОтчетПроизводстваЗаСмену") ИЛИ ТипОбъекта = Тип("ДокументОбъект.ПеремещениеТоваров"))
			Тогда
		
			РегистрыДляОптимизацииПерезаписиДвижений.Добавить("СвободныеОстатки");	
	...
	//начало изменений Ожиганов 26.05.2015 немножко оптимизируем 
	ПРГДопОтложПроведение   = Ложь;
	ОтражениеВРеглУчетеБыло = Истина;
	Если ДокументОбъект.ДополнительныеСвойства.Свойство("ПРГСтароеОтражВрегл",ОтражениеВРеглУчетеБыло) Тогда
		ПРГДопОтложПроведение   = Не ОтражениеВРеглУчетеБыло; 
	КонецЕсли;	
	//конец изменений 
	...
	Иначе
		РегистрыОтложенногоПроведения =  ОтложенноеПроведениеДокументов.ПолучитьРегистрыОтложенногоПроведения();
		//начало изменений Ожиганов 01.06.2015 немножко оптимизируем 
		Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаказПокупателя")
		или  ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РезервированиеТоваров")
		или  ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗакрытиеЗаказовПокупателей") Тогда
			 РегистрыОтложенногоПроведения.Добавить("РегистрСведений.СписанныеТовары"); 
		КонецЕсли;
		ПараметрыПартионногоУчета = глЗначениеПеременной("ПараметрыПартионногоУчета");
		
		Если  Не ПараметрыПартионногоУчета.СписыватьПартииПриПроведенииДокументов  Тогда
			 РегистрыОтложенногоПроведения.Добавить("РегистрНакопления.ПартииТоваровНаСкладах");
		КонецЕсли;
		//конец изменений 	
		//начало изменений Ожиганов 05.06.2015 немножко оптимизируем 
		ПРГМассНеучРегистров = Неопределено;
		Если ДокументОбъект.ДополнительныеСвойства.Свойство("МассивНеучРегистров",ПРГМассНеучРегистров) Тогда  
			Для Каждого ТекЭлем ИЗ ПРГМассНеучРегистров Цикл
				 РегистрыОтложенногоПроведения.Добавить(ТекЭлем);
			КонецЦикла;	
		КонецЕсли;	
		//конец изменений 
	//конец изменений 
	...
	Для Каждого ПолноеИмяРегистра ИЗ МассивРегистров Цикл
		...
		//начало изменений Ожиганов 26.05.2015 немножко оптимизируем 
		Если ПРГДопОтложПроведение Тогда
...
Процедура ВыполнитьДвиженияПоРегиструСвободныеОстатки(НаборДвижений, Регистратор, Замещение, ВидРегистраОснования, Отказ = Ложь) Экспорт
	...
	Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.КорректировкаЗаписейРегистров") 
		//начало изменений
		или ТипЗнч(Регистратор) = Тип("ДокументСсылка.ПРГ_СторноНакладной") 
		//конец изменений
		Тогда
...
//начало изменений Ожиганов 26.05.2015 немножко оптимизируем 
Процедура ПРГЗаполнитьСубконтоВНабореЗаписей(Запись,Строка,ПРГМассЗнчСубк,ПРГМассВидСубк);
	//начало изменений Ожиганов 25.05.2015 немножко оптимизируем 
	Для Сч = 0 по 2 Цикл	
		 ВидСубконто = Строка[ПРГМассВидСубк[Сч]];
		 Если ЗначениеЗаполнено(ВидСубконто) Тогда
			 Запись.СубконтоДт.Вставить(ВидСубконто,Строка[ПРГМассЗнчСубк[Сч]]);
		 КонецЕсли; 
	КонецЦикла;	
	Для Сч = 3 по 5 Цикл	
		 ВидСубконто = Строка[ПРГМассВидСубк[Сч]];
		 Если ЗначениеЗаполнено(ВидСубконто) Тогда
			 Запись.СубконтоКт.Вставить(ВидСубконто,Строка[ПРГМассЗнчСубк[Сч]]);
		 КонецЕсли; 
	КонецЦикла;	
...
Процедура ДобавитьСтрокиВНаборЗаписей(НаборЗаписей, Таблица) Экспорт
	...
	//начало изменений Ожиганов 25.05.2015 немножко оптимизируем 
	ПРГМассЗнчСубк = Новый Массив;
	ПРГМассВидСубк = Новый Массив;
	Если ЭтоРегистрБухгалтерии Тогда
		Для НомерСчета = 1 по 2 Цикл
			...
			КонецЕсли;
			Для НомерСубконто = 1 по 3 цикл
				ПРГМассЗнчСубк.Добавить("Субконто"+СчетДтКТ+НомерСубконто);
				ПРГМассВидСубк.Добавить("ВидСубконто"+СчетДтКТ+НомерСубконто);
			КонецЦикла;
		КонецЦикла;	
	КонецЕсли;	
	//конец изменений 
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Если ЭтоРегистрБухгалтерии тогда
			...
			ПРГЗаполнитьСубконтоВНабореЗаписей(Запись,Строка,ПРГМассЗнчСубк,ПРГМассВидСубк);
			//конец изменений 
		КонецЕсли;
...
Функция ПолучитьНомерНаПечать(Документ, МассивПрефиксов = Неопределено,НомерДок = Неопределено) Экспорт
//конец изменений 	
