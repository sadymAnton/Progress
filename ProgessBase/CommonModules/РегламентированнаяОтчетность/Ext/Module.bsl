	...
	Для Каждого ВыгружаемыйДокумент Из ВыгружаемыеДокументы Цикл
		...
		Если ЭтоБП ИЛИ ЭтоУППКА Тогда
			...
			Если ЭтоДокументРегистраУчетаНДС Тогда
				...
				ХранилищаФайловВыгрузки = ОбъектВыгрузки.ЭлектронноеПредставление(УникальныйИдентификатор);
				
				Результат.Вставить(ВыгружаемыйДокумент, ХранилищаФайловВыгрузки);
...
//
Функция СоздатьЭкземплярРСВ1(Организация, НаДату, ИмяФормы = "") Экспорт
	
	ИдОтчета = "РегламентированныйОтчетРСВ1";
	
	ОбъектОтчета = РеглОтчеты(ИдОтчета);
	Если НЕ ЭтоВнешнийОтчет(ИдОтчета) Тогда
	...
	Если ЗначениеЗаполнено(ИмяФормы) Тогда
		ИмяСоздаваемойФормы = ИмяФормы;
	Иначе
		ИмяСоздаваемойФормы = ОсновнаяФормаОтчета.ПолучитьФормуДляПериода(НаДату);
	КонецЕсли;
	
	ДействующаяФорма = ОбъектОтчета.ПолучитьФорму(ИмяСоздаваемойФормы);
...
// Возвращает значение разделителя пути, используемого в операционной системе,
// в контексте которой выполняется вызов.
//
Функция ПолучитьРазделительПути82() Экспорт
	
	РазделительПути = "\";
	
	ХарактеристикиПК = Новый СистемнаяИнформация;
	
	Если ХарактеристикиПК.ТипПлатформы = ТипПлатформы.Linux_x86
	 ИЛИ ХарактеристикиПК.ТипПлатформы = ТипПлатформы.Linux_x86_64 Тогда
		РазделительПути = "/";
	КонецЕсли;
...
Функция ДанныеРегистраДополнительныхФайлов(ПараметрыЧтения)
...
// Возвращает Истина, если у справочника ПодразделенияОрганизаций есть реквизиты КодПоОКПО и КодОрганаФСГС
//
Функция ЕстьРеквизитыРосстатаПодразделений() Экспорт
	Если Метаданные.Справочники.Найти("ПодразделенияОрганизаций") = Неопределено Тогда 
		Возврат Ложь;
	КонецЕсли;
	
	Если Метаданные.Справочники.ПодразделенияОрганизаций.Реквизиты.Найти("КодПоОКПО") = Неопределено 
		Или Метаданные.Справочники.ПодразделенияОрганизаций.Реквизиты.Найти("КодПоОКПО") = Неопределено Тогда 
		Возврат Ложь;
	КонецЕсли;
...
// Возвращает направление сдачи по ссылке на отчет в Росстат, Строка в формате **-**
// Подразумевается, что есть реквизиты КодПоОКПО и КодОрганаФСГС у справочника ПодразделенияОрганизаций
//
Функция ПолучитьКодПодразделенияСтатотчета(СсылкаНаОтчет) Экспорт 
	КодФСГС = "";
	Организация = СсылкаНаОтчет.Организация;
	...
	Если ТипЗнч(ДанныеОтчета) = Тип("Структура") Тогда 
		...
		Если Не ЗначениеЗаполнено(ОбособленноеПодразделение) Тогда
			Возврат ПолучитьКодОрганаФСГСПоОрганизации(Организация);
		Иначе
			КодФСГС = ПолучитьКодОрганаФСГСПоПодразделению(ОбособленноеПодразделение);
			Если Не ЗначениеЗаполнено(КодФСГС) Тогда
				Возврат ПолучитьКодОрганаФСГСПоОрганизации(Организация);
			КонецЕсли;
			Возврат КодФСГС;
		КонецЕсли;
...
// Возвращает код орагана Росстата для подразедения организации
//
Функция ПолучитьКодОрганаФСГСПоПодразделению(ОбособленноеПодразделение) Экспорт
	Если Не ЗначениеЗаполнено(ОбособленноеПодразделение) Тогда 
		Возврат "";
	КонецЕсли;
	
	КодФСГС = ОбособленноеПодразделение.КодОрганаФСГС;
	Если Не ЗначениеЗаполнено(СтрЗаменить(КодФСГС, "-", "")) Тогда
		Возврат "";
	КонецЕсли;
...
// Возвращает код орагана Росстата для организации
//
Функция ПолучитьКодОрганаФСГСПоОрганизации(Организация) Экспорт
	КодОрганаФСГС = СокрЛП(ПолучитьСведенияОбОрганизации(Организация, , "КодОрганаФСГС").КодОрганаФСГС);
	Если Не ЗначениеЗаполнено(СтрЗаменить(КодОрганаФСГС, "-", "")) Тогда 
		Возврат "";
	КонецЕсли;
	Возврат КодОрганаФСГС;
КонецФункции

// Возвращает код ОКПО по ссылке на отчет в Росстат, Строка(14)
// Подразумевается, что есть реквизиты КодПоОКПО и КодОрганаФСГС у справочника ПодразделенияОрганизаций
//
Функция ПолучитьКодОКПОСтатотчета(СсылкаНаОтчет) Экспорт 
	КодОКПО = "";
	Организация = СсылкаНаОтчет.Организация;
	...
	Если ТипЗнч(ДанныеОтчета) = Тип("Структура") Тогда 
		...
		ДанныеОтчета.Свойство("ОбособленноеПодразделение", ОбособленноеПодразделение);
		Если Не ЗначениеЗаполнено(ОбособленноеПодразделение) Тогда
			Возврат ПолучитьОКПОПоОрганизации(Организация);
		Иначе
			КодОКПО = ПолучитьОКПОПоПодразделению(ОбособленноеПодразделение);
			Если Не ЗначениеЗаполнено(КодОКПО) Тогда
				Возврат ПолучитьОКПОПоОрганизации(Организация);
			КонецЕсли;
			Возврат КодОКПО;
		КонецЕсли;
...
// Возвращает код ОКПО Росстата для подразедения организации
//
Функция ПолучитьОКПОПоПодразделению(ОбособленноеПодразделение) Экспорт
	Если Не ЗначениеЗаполнено(ОбособленноеПодразделение) Тогда 
		Возврат "";
	КонецЕсли;
	
	Возврат ОбособленноеПодразделение.КодПоОКПО;
КонецФункции

// Возвращает код ОКПО Росстата для организации
//
Функция ПолучитьОКПОПоОрганизации(Организация) Экспорт
	КодПоОКПО = СокрЛП(ПолучитьСведенияОбОрганизации(Организация, , "ОКПО").ОКПО);
	Возврат КодПоОКПО;
КонецФункции
...
Функция ПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	СлужебныйТелефонФизЛица = ?(СвойствоОпределено(Справочники.ВидыКонтактнойИнформации, "ТелефонСлужебныйФизЛица"), Справочники.ВидыКонтактнойИнформации.ТелефонСлужебныйФизЛица, Справочники.ВидыКонтактнойИнформации.ТелефонСлужебный);
	...
	Для Каждого Показатель Из СписокПоказателей Цикл
		...
		ИначеЕсли Имяпоказателя = "ОснВидДеят2" Тогда
			ИмяРеквизита = "НаименованиеОКВЭД2";
			
			Если Метаданные.Справочники.Организации.Реквизиты.Найти(ИмяРеквизита) <> Неопределено Тогда
				Значение = Организация[ИмяРеквизита];
			Иначе
				Значение = "";
			КонецЕсли;
			...
			Если Результат <> Неопределено И Результат.Количество() > 0 Тогда
				Если ИдКонф = "ББУ" ИЛИ ИдКонф = "БГУ" Тогда
					...
					Если НЕ ЗначениеЗаполнено(ФЛ) Тогда
						Значение = "";
					Иначе
						Данные = Новый Структура("Объект, Тип, Вид", ФЛ, Перечисления.ТипыКонтактнойИнформации.Телефон, СлужебныйТелефонФизЛица);
						Результат2 = РегистрыСведений.КонтактнаяИнформация.Получить(Данные);
						Если Результат2 <> Неопределено Тогда
							Значение = Результат2.Представление;
						Иначе
			...
			Если Результат <> Неопределено И Результат.Количество() > 0 Тогда
				...
				Иначе
					...
					Если УдЛичн = Неопределено Тогда
						Значение = "";
					Иначе
						Значение = УдЛичн.ДокументКодПодразделения;
					КонецЕсли;
				КонецЕсли;
		...
		ИначеЕсли ИмяПоказателя = "ТелУпПред" Тогда
			...
			Если Результат <> Неопределено И Результат.Количество() > 0 Тогда
				...
				Иначе
					...
					Данные = Новый Структура("Объект, Тип, Вид",Результат[0].ФизическоеЛицо, Перечисления.ТипыКонтактнойИнформации.Телефон, СлужебныйТелефонФизЛица);
			...
			Если Результат <> Неопределено И Результат.Количество() > 0 Тогда
				Если ИдКонф = "ББУ" ИЛИ ИдКонф = "БГУ" Тогда
					...
					Если НЕ ЗначениеЗаполнено(ФизЛицо) Тогда
						Значение = "";
					Иначе
						Данные = Новый Структура("Объект, Тип, Вид", ФизЛицо, Перечисления.ТипыКонтактнойИнформации.Телефон, СлужебныйТелефонФизЛица);
						Результат2 = РегистрыСведений.КонтактнаяИнформация.Получить(Данные);
						Если Результат2 <> Неопределено Тогда
							Значение = Результат2.Представление;
						Иначе
...
Функция ДатаРожденияФЛ(ФЛ) Экспорт
...
Функция МестоРожденияФЛ(ФЛ) Экспорт
...
Функция ПолФЛ(ФЛ) Экспорт
	...
	Если ИдКонф = "ББУ" ИЛИ ИдКонф = "БГУ" Тогда
		...
		Иначе
			Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
			                      |	ХарактеристикиКонтрагентов.ЗначениеХарактеристики КАК Пол
			                      |ИЗ
			                      |	РегистрСведений.ХарактеристикиКонтрагентов КАК ХарактеристикиКонтрагентов
			                      |ГДЕ
			                      |	ХарактеристикиКонтрагентов.Контрагент = &Контрагент
			                      |	И ХарактеристикиКонтрагентов.ВидХарактеристики = &ВидХарактеристики");
			Запрос.УстановитьПараметр("Контрагент", ФЛ);
			Запрос.УстановитьПараметр("ВидХарактеристики", ПланыВидовХарактеристик.ВидыХарактеристикКонтрагентов.Пол);
			Выборка = Запрос.Выполнить().Выбрать();
	...
	Иначе
		Если НЕ ЗначениеЗаполнено(ФЛ) Тогда
			Значение = 0;
		Иначе
			Если ФЛ.Пол = Перечисления.ПолФизическихЛиц.Женский Тогда
				Значение = 2;
			ИначеЕсли ФЛ.Пол = Перечисления.ПолФизическихЛиц.Мужской Тогда
				Значение = 1;
			Иначе
				Значение = 0;
			КонецЕсли;
...
// Возвращает значение ОКВЭД, которым необходимо заполнить отчет.
// Параметры:
//   ОКВЭД - Строка - ОКВЭД по ОК 029-2007 (КДЕС Ред. 1.1).
//   ОКВЭД2 - Строка - ОКВЭД по ОК 0292014 (КДЕС Ред. 2).
//   ДатаПериодаОтчета - Дата - дата, на которую определяется период за который представляется отчет.
//   ДатаПодписиОтчета - Дата - дата, подписи отчета.
//   НомерКорректировки - Число - номер корректировки отчета.
//
Функция ДействующийОКВЭД(ОКВЭД, ОКВЭД2, ДатаПериодаОтчета, ДатаПодписиОтчета, НомерКорректировки = 0) Экспорт
	
	ДействующийОКВЭД = "";
	
	Если НомерКорректировки > 0 И ДатаПериодаОтчета < '2016-12-01' Тогда
		ДействующийОКВЭД = ОКВЭД;
	Иначе
		Если ДатаПодписиОтчета < '2017-01-01' Тогда
			ДействующийОКВЭД = ОКВЭД;
		Иначе
			ДействующийОКВЭД = ОКВЭД2;
		КонецЕсли;
		
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДействующийОКВЭД) Тогда
		ДействующийОКВЭД = ОКВЭД2;
	КонецЕсли;
	
	Возврат ДействующийОКВЭД;
...
Процедура роПоказатьСтраницуМногостраничногоРазделаПриВосстановлении(Форма, Шаг, ИмяТекущейСтраницыПанели, УчестьАктивнуюСтраницу = Ложь, КолМногострочныхЧастей = -1) Экспорт
	...
	Для Каждого Элемент Из СтруктураДанныхСледующаяСтраница Цикл
        ...
        Если ТекТабличноеПоле.Области.Найти(ИмяПоказателя) <> Неопределено
		   И ТекТабличноеПоле.Области[ИмяПоказателя].СодержитЗначение = Истина Тогда
			ТекТабличноеПоле.Области[ИмяПоказателя].Значение = ЗначениеПоказателя;
		КонецЕсли;
...
// Получает из дерева свойств разделов отчета необходимый параметр.
// Параметры
...
Функция роПоказательВходитВМногострочнуюЧасть(Форма, ИмяПоказателя) Экспорт
	...
	Если Форма.мСтруктураМногострочныхРазделов.Свойство(ИмяГруппы, СтруктураГруппы) Тогда
		Возврат СтруктураГруппы.Количество();
		
	Иначе
		ИмяПоказателя13 = СокрЛП(Лев(ИмяПоказателя, 13));
		Для Каждого ЭлементСтруктурыМногострочныхЧастей Из Форма.мСтруктураМногострочныхРазделов Цикл
			ТаблицаМСЧ = ЭлементСтруктурыМногострочныхЧастей.Значение;
			Если ТаблицаМСЧ.Колонки.Найти(ИмяПоказателя13) <> Неопределено Тогда
				Возврат ТаблицаМСЧ.Количество();
			КонецЕсли;
		КонецЦикла;
...
Процедура роПриАктивизацииОбластиРегламентированногоОтчета(Форма, Элемент, ИмяТекущейСтраницыПанели = Неопределено) Экспорт
	...
	Если НЕ Форма.Открыта() Тогда
		// Обход проблемы со срабатыванием события в неоткрываемой форме под платформой 8.3.7.
		Возврат;
	КонецЕсли;
...
// Выполняет очистку структуры мСтруктураВариантовЗаполнения.
...
// Возвращает значения с табличного поля
// Параметры:
//  Форма          - ссылка на форму
...
Процедура ОчиститьПоказателиОтчетаЗаполняемыеИзИБ(Форма, НазванияМногостраничныхРазделовДляУдаления = "") Экспорт
	...
	Для каждого СтрокаУровня1 из Форма.мСвойстваРазделовДекларации.Строки Цикл
		...
		Если НазваниеСтраницы = "Титульный"
		 ИЛИ (НазванияМногостраничныхРазделовДляУдаления <> ""
		 И НЕ НазванияМнгостраничныхРазделов.Свойство(НазваниеСтраницы)) Тогда
			Продолжить;
		КонецЕсли;
...
// Формирует состав кнопки-меню "Очистить" в регламентированных отчетах.
//
Процедура СформироватьСоставКнопкиОчиститьДляРегОтчетов(Форма) Экспорт
	...
	РазделОчищаемый = ИмяСтраницы <> "Титульный" И 
	                  роПолучитьСвойствоРаздела(Форма, ИмяСтраницы, "ОчищатьРаздел", Ложь) <> Ложь; // Возможны состояния отличные от булевых.
	
	Если РазделОчищаемый Тогда
...
Функция ПолучитьСведенияОПредставителе(Форма, Знач МожетБытьТолькоЮРЛицо = Неопределено, Знач ИФНС = Неопределено, КПП = Неопределено) Экспорт
	...
	Если ПредставительЯвляетсяФизЛицом(Представитель) Тогда
		...
		Данные = Новый Структура("Объект, Тип, Вид", Представитель, Перечисления.ТипыКонтактнойИнформации.Телефон, СлужебныйТелефонФизЛица);
		Результат2 = РегистрыСведений.КонтактнаяИнформация.Получить(Данные);
...
// Параметры:
//   мСохраненныйДок - ДокументОбъект - документ, блокировку которого следует выполнить.
// Возвращаемое значение:
...
Функция ЗаблокироватьДокумент(мСохраненныйДок)
	...
	Исключение
		...
		Если Найти(Врег(ОписаниеОшибки), "ОБЪЕКТ УЖЕ ЗАБЛОКИРОВАН") Тогда
			ОписаниеОшибкиДляПользователя = СтрПолучитьСтроку(ОписаниеОшибки, 2) + Символы.ПС
			                              + СтрПолучитьСтроку(ОписаниеОшибки, 3);
			ОбщегоНазначения.ПредупреждениеОбОшибке(ОписаниеОшибкиДляПользователя);
			
		Иначе
			ЗаписьЖурналаРегистрации("Блокировка регламентированного отчета",
				УровеньЖурналаРегистрации.Ошибка,
				,
				мСохраненныйДок.Ссылка,
				ОписаниеОшибки);
			#Если ТолстыйКлиентОбычноеПриложение Тогда
			ТекстСообщения = "Не удается заблокировать документ. Подробности смотрите в журнале регистрации событий";
...
Функция ИнициализацияОтчета(Док, Форма = Неопределено)
	...
	Если СвойствоОпределено(Отчет, "мПериодичность") Тогда
...
Функция ВыбратьФормуОтчетаИзДействующегоСписка(Форма) Экспорт
	...
	Если Не ВыбСтрока = Неопределено Тогда
		...
		СтрОтбора.Вставить("ДатаНачалоДействия", ВыбСтрока.ДатаНачалоДействия);
...
Функция ЭтоФормаАлкогольнойОтчетности(Форма)
	...
	ИмяОбъектаМетаданных = Форма.ЭтотОбъект.Метаданные().Имя;
	Возврат (Найти(ИмяОбъектаМетаданных, "РегламентированныйОтчетАлкоПриложение") > 0);
...
Функция ВывестиМашиночитаемуюФорму(Форма, ИмяКнопки) Экспорт
	...
	Если СвойствоОпределено(Форма, "мПроверятьСоотношенияПриПечатиИВыгрузки")
	   И Форма.мПроверятьСоотношенияПриПечатиИВыгрузки = Истина Тогда // возможны 3 состояния
		ПроверитьКонтрольныеСоотношенияВОтчете(Форма, Форма.мСохраненныйДок, Форма.мБезОткрытияФормы);
	КонецЕсли;
	
	// Текст выгрузки во временном файле.
	...
	РазделительПути = ПолучитьРазделительПути82();
	
	КаталогВремФайлов = КаталогВременныхФайлов();
	КаталогВремФайлов = ?(Прав(КаталогВремФайлов, 1) = РазделительПути, КаталогВремФайлов, КаталогВремФайлов + РазделительПути);
	
	ВремКаталог = КаталогВремФайлов + Строка(Новый УникальныйИдентификатор) + РазделительПути;
	СоздатьКаталог(ВремКаталог);
	
	// Выгрузка во временный каталог шаблонов печати, соответствующих сформированному файлу выгрузки.
...
Функция ВсеЮрФизЛица(ЮрЛицо) Экспорт 
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если Не (ЭтоПБОЮЛ(Выборка.Ссылка) = ЮрЛицо) Тогда 
...
Процедура УстановитьОтборВФормеВыбораОтчета(Форма) Экспорт
	...
	Если НЕ Найти(ИмяОтчета, "ОтчетОбъект.РегламентированныйОтчетСтатистика") Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПолучитьМассивФормСтатистикиТолькоДляОрганизации().Найти(ИмяОтчета) <> Неопределено Тогда
		...
		Иначе
			Форма.ЭлементыФормы.Организация.Значение = Форма.Организация;
		КонецЕсли;
...
Процедура ПриНажатииНаКнопкуОтправкиВОбъектеОтправкиВФНС(Форма, Кнопка) Экспорт
	...
	Иначе
		ИмяДокументаЖурналУчетаСчетовФактурДляПередачиВЭлектронномВиде = "ЖурналУчетаСчетовФактурДляПередачиВЭлектронномВиде";
		Если Метаданные.Документы.Найти(ИмяДокументаЖурналУчетаСчетовФактурДляПередачиВЭлектронномВиде) <> Неопределено
		И ТипЗнч(Форма.Ссылка) = Тип("ДокументСсылка." + ИмяДокументаЖурналУчетаСчетовФактурДляПередачиВЭлектронномВиде) Тогда
			КонтекстЭДО.ОтправкаЖурналаСчетовФактурВФНС(Форма.Ссылка);
		Иначе
...
Процедура ЗаполнитьПредопределенныеВидыОтправляемыхДокументов(СправочникЦеликом = Ложь) Экспорт
	...
	Иначе
		...
		Пока ВыборкаРезультата.Следующий() Цикл
			ИмяЭлемента = ИмяПредопределенного(Справочники.ВидыОтправляемыхДокументов, ВыборкаРезультата.Ссылка);
	...
	Пока ВыборкаРезультата.Следующий() Цикл
		...
		СпрИмя = ИмяПредопределенного(Справочники.ВидыОтправляемыхДокументов, ВыборкаРезультата.Ссылка);
		Если НЕ ЗначениеЗаполнено(СпрИмя) Тогда
			Продолжить;
		КонецЕсли;
...
КонецПроцедуры
 ...
 // Безопасное получение имени предопределенного для платформ версий 8.2 и 8.3
Функция ИмяПредопределенного(МенеджерОбъектаСсылки, Ссылка) Экспорт
	
	Попытка
		// работает везде при отсутствии задвоений
		Результат = МенеджерОбъектаСсылки.ПолучитьИмяПредопределенного(Ссылка);
	Исключение
		Результат = "";
	КонецПопытки;
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		Попытка
			Результат = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Ссылка, "ИмяПредопределенныхДанных");
		Исключение
			Результат = "";
		КонецПопытки;
	КонецЕсли;
...
Процедура НазначитьНомераПачекОтчетовПФР() Экспорт
	...
	Пока ВыборкаОтчетов.Следующий() Цикл
		...
		Попытка
			...
			ОтчетОбъект.НомерПачки = НомерПачки;
			ОтчетОбъект.Записать();
			УстановитьСчетчикВыгруженныхФайловПФР(ВыборкаОтчетов.Организация, ВыборкаОтчетов.Год, НомерПачки);
...
// Изменяет состояние флажка, выполненного средствами табличного документа, на противоположное.
//
// Параметры
// Область - Область ячеек табличного документа - область, в которой реализован флажок.
// СтандартнаяОбработка - ссылка на переменную СтандартнаяОбработка функции обработки выбора табличного документа.
// Модифицированность - ссылка на переменную Модифицированность формы отчета.
//
Процедура ПереключитьФлажок(Область, СтандартнаяОбработка, Модифицированность) Экспорт
...
// Проверяет соответствие даты требованиям типа ДатаТип файлов выгрузки налоговой отчетности форматов версии 5.
//
Функция УдовлетворяетТребованиямНО5_ДатаТип(ПроверяемаяДата) Экспорт
	
	Возврат (ПроверяемаяДата >= '1900-01-01' И ПроверяемаяДата <= '2099-12-31');
	
КонецФункции

#Если ТолстыйКлиентОбычноеПриложение Тогда

Функция АдресОрганизацииПоУмолчаниюБухОтчетности(Организация) Экспорт
	
	ПолныйАдрес = ",,,,,,,,,";
	ПредставлениеАдреса = "";
	
	Если ЭтоПБОЮЛ(Организация) Тогда
		ВидыАдресов = "АдрМЖ, АдрПрописки";
	Иначе
		ВидыАдресов = "АдрФакт, АдрЮР, АдрПочт";
	КонецЕсли;
	
	Адреса = ПолучитьСведенияОбОрганизации(Организация, , ВидыАдресов);
	
	ИменаВидовАдресов = ОбщегоНазначения.РазложитьСтрокуВМассивПодстрок(ВидыАдресов);
	
	Для Каждого ИмяВидаАдреса Из ИменаВидовАдресов Цикл
		ИмяВидаАдреса = СокрЛП(ИмяВидаАдреса);
		
		ПолныйАдрес = Адреса[ИмяВидаАдреса];
		ПредставлениеАдреса = ПредставлениеАдресаВФормате9Запятых(ПолныйАдрес);
...
Процедура ВыбратьАдресОрганизацииВБухОтчетности(Организация, Область, Модифицированность, АдресВФормате9Запятых) Экспорт
	
	ЭтоПБОЮЛ = ЭтоПБОЮЛ(Организация);
	
	СписокАдресов = ?(ЭтоПБОЮЛ, "АдрМЖ, АдрПрописки", "АдрФакт, АдрЮР, АдрПочт");
	Адреса = ПолучитьСведенияОбОрганизации(Организация, , СписокАдресов);
	
	Для Каждого АдресОрганизации Из Адреса Цикл
		АдресПустой = НЕ ЗначениеЗаполнено(СтрЗаменить(АдресОрганизации.Значение, ",", ""));
		
		Если АдресПустой Тогда
			ПредставлениеАдреса = "";
		Иначе
			ПредставлениеАдреса = ПредставлениеАдресаВФормате9Запятых(АдресОрганизации.Значение);
		КонецЕсли;
		Адреса.Вставить(АдресОрганизации.Ключ, ПредставлениеАдреса);
	КонецЦикла;
	...
	Если Адреса.Свойство("АдрФакт") И ЗначениеЗаполнено(Адреса.АдрФакт) Тогда
		НовСтр = ТаблицаАдресов.Добавить();
		НовСтр.НазваниеАдреса = "Фактический адрес";
		НовСтр.Адрес = Адреса.АдрФакт;
	КонецЕсли;
	
	Если Адреса.Свойство("АдрФакт") И ЗначениеЗаполнено(Адреса.АдрЮР) Тогда
		НовСтр = ТаблицаАдресов.Добавить();
		НовСтр.НазваниеАдреса = "Юридический адрес";
		НовСтр.Адрес = Адреса.АдрЮР;
	КонецЕсли;
	
	Если Адреса.Свойство("АдрПочт") И ЗначениеЗаполнено(Адреса.АдрПочт) Тогда
		НовСтр = ТаблицаАдресов.Добавить();
		НовСтр.НазваниеАдреса = "Почтовый адрес";
		НовСтр.Адрес = Адреса.АдрПочт;
	КонецЕсли;
	
	Если Адреса.Свойство("АдрМЖ") И ЗначениеЗаполнено(Адреса.АдрМЖ) Тогда
		НовСтр = ТаблицаАдресов.Добавить();
		НовСтр.НазваниеАдреса = "Адрес места жительства";
		НовСтр.Адрес = Адреса.АдрМЖ;
	КонецЕсли;
	
	Если Адреса.Свойство("АдрПрописки") И ЗначениеЗаполнено(Адреса.АдрПрописки) Тогда
		НовСтр = ТаблицаАдресов.Добавить();
		НовСтр.НазваниеАдреса = "Адрес прописки";
		НовСтр.Адрес = Адреса.АдрПрописки;
	КонецЕсли;
	
	Если ТаблицаАдресов.Количество() = 1 Тогда
		Предупреждение("В контактной информации не указаны адреса организации.");
		Возврат;
	КонецЕсли;
	...
	Если УказанныйАдрес.НазваниеАдреса = "Фактический адрес" Тогда
		АдресВФормате9Запятых = ПолучитьСведенияОбОрганизации(Организация, , "АдрФакт").АдрФакт;
	КонецЕсли;
	
	Если УказанныйАдрес.НазваниеАдреса = "Юридический адрес" Тогда
		АдресВФормате9Запятых = ПолучитьСведенияОбОрганизации(Организация, , "АдрЮР").АдрЮР;
	КонецЕсли;
	
	Если УказанныйАдрес.НазваниеАдреса = "Почтовый адрес" Тогда
		АдресВФормате9Запятых = ПолучитьСведенияОбОрганизации(Организация, , "АдрПочт").АдрПочт;
	КонецЕсли;
	
	Если УказанныйАдрес.НазваниеАдреса = "Адрес места жительства" Тогда
		АдресВФормате9Запятых = ПолучитьСведенияОбОрганизации(Организация, , "АдрМЖ").АдрМЖ;
	КонецЕсли;
	
	Если УказанныйАдрес.НазваниеАдреса = "Адрес прописки" Тогда
		АдресВФормате9Запятых = ПолучитьСведенияОбОрганизации(Организация, , "АдрПрописки").АдрПрописки;
	КонецЕсли;
	
КонецПроцедуры

Функция ВыбратьОтправкуОтчета(Ссылка, Наименование) Экспорт
	...
	ФормаВсеОтправки = КонтекстЭДО.ПолучитьФорму("ВсеОтправки");
	
	ФормаВсеОтправки.Ссылка       = Ссылка;
	ФормаВсеОтправки.Наименование = Наименование;
	
	Возврат ФормаВсеОтправки.ОткрытьМодально();
	
КонецФункции

Функция ИдФайлаДекларацииНДС(ДекларацияНДС, ЦиклОбмена = Неопределено) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Если ЦиклОбмена = Неопределено Тогда
		ЦиклОбмена = КонтекстЭДО.ПолучитьПоследнийЦиклОбмена(ДекларацияНДС);
	КонецЕсли;
	
	Если ЦиклОбмена = Справочники.ЦиклыОбмена.ПустаяСсылка() Тогда
		Возврат Неопределено;
	КонецЕсли;
		
	СообщенияЦикла 	= КонтекстЭДО.ПолучитьСообщенияЦиклаОбмена(ЦиклОбмена,,,Истина);
	...
	ВложенияТранспортногоСообщения = КонтекстЭДО.ПолучитьВложенияТранспортногоСообщения(
		ТранспортноеСообщение, 
		, 
		Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ФайлОтчетности);
	
	Если ВложенияТранспортногоСообщения.Количество() > 0 Тогда
		
		ИмяФайла     = ВложенияТранспортногоСообщения[0].ИмяФайла;
		ДатаОтправки = ЦиклОбмена.ДатаСоздания;
		
		Результат = Новый Структура;
		Результат.Вставить("ИмяФайла",     КонтекстЭДО.ИмяФайлаБезРасширения(ИмяФайла));
...
// Выбирает валюту из справочника валют.
//
// Параметры:
//   ОбластьКода - Область ячеек табличного документа - область, в которую выбирается код валюты.
//   ОбластьНаименования - Область ячеек табличного документа - область, в которую дополнительно помещается наименование валюты.
//   СтандартнаяОбработка - ссылка на переменную СтандартнаяОбработка функции обработки выбора табличного документа.
//   Модифицированность - ссылка на переменную Модифицированность формы отчета.
//
Процедура ВыбратьСтрану(ОбластьКода, ОбластьНаименования = Неопределено, СтандартнаяОбработка, Модифицированность) Экспорт
	
	ФормаВыбора = Справочники.КлассификаторСтранМира.ПолучитьФормуВыбора();
	ФормаВыбора.РежимВыбора = Истина;
	ФормаВыбора.Заголовок = "Выбор страны";
	
	Если НЕ РегламентированнаяОтчетность.ПустоеЗначение(ОбластьКода.Значение) Тогда
		НайденнаяСтрока = Справочники.КлассификаторСтранМира.НайтиПоКоду(ОбластьКода.Значение);
	...
	Если ФормаВыбора.ОткрытьМодально() <> Неопределено Тогда
		Если ОбластьКода.Значение = ФормаВыбора.ТекущийЭлемент.ТекущиеДанные.Код Тогда
			ОбластьКода.Значение = "";
			Если ОбластьНаименования <> Неопределено Тогда
				ОбластьНаименования.Значение = "";
			КонецЕсли;
		Иначе
			ОбластьКода.Значение = СокрЛП(ФормаВыбора.ТекущийЭлемент.ТекущиеДанные.Код);
			Если ОбластьНаименования <> Неопределено Тогда
				ОбластьНаименования.Значение = СокрЛП(ФормаВыбора.ТекущийЭлемент.ТекущиеДанные.Наименование);
			КонецЕсли;
		КонецЕсли;
		Модифицированность = Истина;
	КонецЕсли;
...
// Выбирает страну из справочника стран мира.
//
// Параметры:
//   ОбластьКода - Область ячеек табличного документа - область, в которую выбирается код страны.
//   ОбластьНаименования - Область ячеек табличного документа - область, в которую дополнительно помещается наименование страны.
//   СтандартнаяОбработка - ссылка на переменную СтандартнаяОбработка функции обработки выбора табличного документа.
//   Модифицированность - ссылка на переменную Модифицированность формы отчета.
//
Процедура ВыбратьВалюту(ОбластьКода, ОбластьНаименования = Неопределено, СтандартнаяОбработка, Модифицированность) Экспорт
	...
	Если НЕ РегламентированнаяОтчетность.ПустоеЗначение(ОбластьКода.Значение) Тогда
		НайденнаяСтрока = Справочники.Валюты.НайтиПоКоду(ОбластьКода.Значение);
		Если ЗначениеЗаполнено(НайденнаяСтрока) Тогда
			ФормаВыбора.ПараметрТекущаяСтрока = НайденнаяСтрока;
		КонецЕсли;
	КонецЕсли;
...
// Процедура, предназначенная для бесшовного подключения к 1С-ЭДО из мастера по 1С-Отчетности.
Процедура ЗаполнитьПараметрыДляФормыПодключенияКСервисуЭлектронныхДокументов(Организация, СтруктураПараметров) Экспорт
	...
	//заполнение полей ОтпечатокСертификатаПодписи, ЭлектроннаяПодписьВМоделиСервиса, КодНО
	Запрос = Новый Запрос;
	Запрос.Текст = 
	...
	Если Выборка.Следующий() Тогда
		...
		Если СтруктураПараметров.Свойство("ЭлектроннаяПодписьВМоделиСервиса") Тогда
			СтруктураПараметров.Вставить("ЭлектроннаяПодписьВМоделиСервиса", Ложь);
		КонецЕсли;
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Если СтруктураПараметров.Свойство("ТипКриптоПровайдера") Тогда
			...
			Если Крипт <> Неопределено Тогда
				...
				Если КонтекстЭДО <> Неопределено Тогда
					...
					Инфотекс = КонтекстЭДО.ПолучитьПараметрыКриптопровайдераПоЗначениюПеречисления(Перечисления.ТипыКриптоПровайдеров.VipNet);
					КриптоПро = КонтекстЭДО.ПолучитьПараметрыКриптопровайдераПоЗначениюПеречисления(Перечисления.ТипыКриптоПровайдеров.CryptoPro);
					
					Имя = "";
					Тип = 0;
					
					Пока Крипт.ПолучитьСледующийКриптопровайдер(Имя, Тип) Цикл
					КонецЦикла;
		...
		КонецЕсли;
	#Иначе	
		Тип = Константы.ДокументооборотСКонтролирующимиОрганами_ТипКриптопровайдера.Получить();
		Если Тип = 75 Тогда
			СтруктураПараметров.Вставить("ТипКриптоПровайдера", "CryptoPro");
		ИначеЕсли Тип = 2 Тогда
			СтруктураПараметров.Вставить("ТипКриптоПровайдера", "VipNet");
		КонецЕсли;
	#КонецЕсли
