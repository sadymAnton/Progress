...
Функция jsonПрочитатьОпределитьДату(Значение)
	...
	//спасибо лОГИНЕТУ, формат дат "dd/MM/yy<пробел>HH:mm" ("26/08/1916 12:12")
	//{08.09.2016 Островерхий заявка №б/н 
	//Так же допустим формат  "dd/MM/yy<пробел>HH:mm:ss" ("26/08/16 12:12:15") 
	//08.09.2016 Островерхий} 
	
	//начало изменений Ожиганов А. б/н меняем алгоритм определения даты 24.11.2016 
//	Если (СтрДлина(Значение) = 14 ИЛИ СтрДлина(Значение) = 17)
//		//начало изменений Ожиганов А. б/н меняем алгоритм определения даты 24.11.2016 
//		ИЛИ (СтрДлина(Значение) = 16 ИЛИ СтрДлина(Значение) = 19)
		//конец изменений 
//		И Найти(Значение,"/") = 3 И Найти(Значение,":") = 12 Тогда
		Если ((СтрДлина(Значение) = 14 ИЛИ СтрДлина(Значение) = 17)  И Найти(Значение,"/") = 3 И Найти(Значение,":") = 12)
			ИЛИ ( (СтрДлина(Значение) = 16 ИЛИ СтрДлина(Значение) = 19) И Найти(Значение,"/") = 3 И Найти(Значение,":") = 14)
		 Тогда
	...
	//конец изменений 	
		...
		Попытка
			...
			//начало изменений Ожиганов А. б/н меняем алгоритм определения даты 24.11.2016 
			//тЗначение = СтрЗаменить(Значение,"/",".");
			//тЗначение = ?(СтрЧислоВхождений(тЗначение,":")=1,тЗначение+":00",тЗначение);
			//тЗначение = СтрЗаменить(Строка(Дата(тЗначение)),".00",".20");
			//конец изменений 
			тЗначение = ПРГ_ПрочитатьДатуИзСтроки(Значение);
			
			Возврат ТипЗнч(Дата(тЗначение)) = Тип("Дата");
...
Функция jsonПрочитатьДату(Значение, Индекс)
	...
	Попытка
		...
		//спасибо лОГИНЕТУ, формат дат "dd/MM/yy<пробел>HH:mm" ("26/08/16 12:12")
		//{08.09.2016 Островерхий заявка №б/н 
		//Так же допустим формат  "dd/MM/yy<пробел>HH:mm:ss" ("26/08/16 12:12:15") 
		//08.09.2016 Островерхий} 
		//начало изменений Ожиганов А. б/н меняем алгоритм определения даты 24.11.2016 
...
Функция jsonЗаписать(Значение, Стандарт, ПредставленияСсылок, Типы, Колонки, Смещение, Отступ, ПереносСтроки, НеПолноеМаскированиеСимволов)
	...
	Если Типы.ПримитивныеТипы.СодержитТип(Тип) Тогда 
		#Если ВебКлиент Или ТонкийКлиент Тогда                                                                                                       
		Если (Значение = Null) Или (Значение = Неопределено) Тогда Возврат "null"; ИначеЕсли (Тип = Типы.Дата) Тогда Возврат """" + Формат(Значение, "ДФ=""дд/ММ/гггг ЧЧ:мм:сс""") + """" ИначеЕсли (Тип = Типы.УникальныйИдентификатор) Тогда Возврат """" + Значение + """"; Иначе Возврат Формат(Значение, "ЧРД=.; ЧН=; ЧГ=; БЛ=false; БИ=true"); КонецЕсли;
		#Иначе
		Если (Значение = Null) Или (Значение = Неопределено) Тогда Возврат "null"; ИначеЕсли (Тип = Типы.Дата) Тогда Возврат """" + Формат(Значение, "ДФ=""дд/ММ/гггг ЧЧ:мм:сс""") + """" ИначеЕсли (Тип = Типы.УникальныйИдентификатор) Тогда Возврат """" + XMLСтрока(Значение) + """"; Иначе Возврат XMLСтрока(Значение); КонецЕсли;
		#КонецЕсли
	КонецЕсли;
...
//начало изменений Ожиганов А. б/н меняем алгоритм определения даты 24.11.2016 
Функция ПРГ_ПрочитатьДатуИзСтроки(тЗначение)
	
		//разбираем из строки даты "dd/MM/yy<пробел>HH:mm:ss" ("26/08/16 12:12:15") 
		МассДанных = Новый Массив;
		к = 1;
		ДлСтр = СтрДлина(тЗначение);
		ТекСтр ="";
		ДобавляемВМассив = Ложь;
		Пока к <= ДлСтр Цикл
			Если Сред(тЗначение,к,1) >= "0" и Сред(тЗначение,к,1) <= "9" Тогда
				ТекСтр = ТекСтр + Сред(тЗначение,к,1);
				ДобавляемВМассив = Ложь;
			Иначе
				ДобавляемВМассив = Истина;
			КонецЕсли;	
			...
			Если ДобавляемВМассив  Тогда
				Если СтрДлина(ТекСтр) > 0 Тогда
					МассДанных.Добавить( ?(СтрДлина(ТекСтр)=1,"0","")+ТекСтр);
				КонецЕсли;	
				ТекСтр = "";
			КонецЕсли;	
			к = к + 1;
		...
		Если МассДанных.Количество() < 3 или МассДанных.Количество() > 6 Тогда
			//исключение
			ВызватьИсключение "неправильный формат даты "+тЗначение;
		Иначе
			Для к = МассДанных.Количество() По 5 Цикл 
				МассДанных.Добавить("00");
			КонецЦикла;	
		КонецЕсли;	
		
		Если СтрДлина(МассДанных[2]) = 2 Тогда //год
			МассДанных[2] = "20"+МассДанных[2];
		КонецЕсли;
		
		возврат Дата(МассДанных[2]+МассДанных[1]+МассДанных[0]+МассДанных[3]+МассДанных[4]+МассДанных[5]);
