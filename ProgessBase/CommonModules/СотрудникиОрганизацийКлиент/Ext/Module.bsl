...
// Измайлова Полина 14.09.2011 14:09 
// ОПИСАНИЕ: добавлен параметр ЭтоНовыйОбъект [Булево] - указывает на то, что сотрудник еще не сохранен.
////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
Процедура СформироватьЗаголовокПерсональныхНадбавок(ЭтаФорма, AVT_ЭтоНовыйОбъект = Ложь) Экспорт
	...
	///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 23.08.2011 17:14 
	// ОПИСАНИЕ: если объект еще не был записан, берем надбавки из реквизита формы, иначе из регистра.
	Если AVT_ЭтоНовыйОбъект Тогда
		Если ЭтаФорма.AVT_ТЗНадбавки.Количество() > 0 Тогда
			ПерсональныеНадбавки = ЭтаФорма.AVT_ТЗНадбавки;
		Иначе
	...
	Иначе
		...
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ПерсональныеНадбавки.Период,
		|	ПерсональныеНадбавки.Сотрудник,
		|	ПерсональныеНадбавки.Надбавка,
		|	ПерсональныеНадбавки.Показатель1
		|ИЗ
		...
		|				И Период В
		|					(ВЫБРАТЬ
		|						МАКСИМУМ(ПерсональныеНадбавки.Период) КАК Период
		|					ИЗ
		|						РегистрСведений.AVT_ПерсональныеНадбавкиСотрудников.СрезПоследних(, Сотрудник = &Сотрудник) КАК ПерсональныеНадбавки)) КАК ПерсональныеНадбавки";
		Запрос.УстановитьПараметр("Сотрудник", ЭтаФорма.Ссылка);
		ПерсональныеНадбавки = Запрос.Выполнить().Выгрузить();
	КонецЕсли;
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
	
	// ИЗМЕНЕНИЕ: Измайлова Полина 23.08.2011 17:16  
...
Процедура ОчиститьНеИспользуемыеРеквизиты(ЭтаФорма) Экспорт
	...
	Если ВидыДоговоров.Найти(ЭтаФорма.ВидДоговора) = Неопределено Тогда
		...
		ЭтаФорма.ВалютаТарифнойСтавки = Справочники.Валюты.ПустаяСсылка();
				///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 20.07.2011 13:54 
		ЭтаФорма.AVTШтатноеРасписание = Справочники.AVT_ШтатноеРасписание.ПустаяСсылка();
		////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА} 
...
Процедура ПодразделениеДолжностьНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка, ТипЭлемента, ЭтаФорма) Экспорт
	...
	Если ЭлементСписка <> Неопределено  и Элемент.Значение <> ЭлементСписка.Значение Тогда
			///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Петрухин Андрей 17.06.2011 15:23 
		Если ТипЗнч(ЭлементСписка.Значение) = Тип("СправочникСсылка.AVT_ШтатноеРасписание") Тогда
			Элемент.Значение = ЭлементСписка.Значение.Должность;
			ЭтаФорма.AVTШтатноеРасписание = ЭлементСписка.Значение; 
		Иначе
...
Процедура ФормаЭлементаНадбавкиПоТрудовомуДоговоруТекстНажатие(Элемент, ЭтаФорма) Экспорт
	...
	Если Не Отказ Тогда
		...
		КонецПопытки;
			...
			// ИЗМЕНЕНИЕ: Измайлова Полина 23.08.2011 10:58  
		//ФормаПерсональныеНадбавки = ЭтотОбъект.ПолучитьФорму("ФормаПерсональныеНадбавки", ЭтаФорма, ЭтаФорма.Ссылка);
		ФормаПерсональныеНадбавки = ЭтотОбъект.ПолучитьФорму("AVT_ФормаПерсональныеНадбавки", ЭтаФорма, ЭтаФорма.Ссылка);
		// ИЗМЕНЕНИЕ: Измайлова Полина 23.08.2011 11:35  
		//ФормаПерсональныеНадбавки.СправочникОбъект	= ЭтотОбъект;
		ФормаПерсональныеНадбавки.Сотрудник	        = ЭтотОбъект.Ссылка;
