...
Функция ДобавитьСтрокуВДеревоКнопок(СтрокаПодменю, Имя, Знач ТипКнопки = Неопределено ,Текст = "", Действие = Неопределено, Расшифровка = Неопределено, Пометка = Ложь, Подсказка = "", Картинка = Неопределено, Позиция = Неопределено, ИзменяетДанные = Ложь)
	...
	//Blik 42521 060815 исправлена ошибка - передавалась по кнопке печать модифицированность
	//СтрокаКнопки.ИзменяетДанные     = 6;  //Blik  закком
	СтрокаКнопки.ИзменяетДанные     = ИзменяетДанные;  //Blik добавлено
...
Функция ПолучитьДеревоКнопокЗаполненияТабличныхЧастей(Ссылка, Действие) Экспорт
	...
	|	ЕСТЬNULL(ВнешниеОбработкиПринадлежность.Ссылка.ПРГ_ИзменяетДанные, ИСТИНА) КАК ПРГ_ИзменяетДанные
	|ИЗ
	|	Справочник.ВнешниеОбработки.Принадлежность КАК ВнешниеОбработкиПринадлежность
	|ГДЕ
	|	НЕ ВнешниеОбработкиПринадлежность.Ссылка.ПометкаУдаления
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаДействий.Следующий() Цикл
			...
			//начало изменений Ожиганов 28.07.2015
			НоваяКнопка = ДобавитьСтрокуВДеревоКнопок(СтрокаПодменю, "ЗаполнениеТЧ"+НомерКнопки, ТипКнопкиКоманднойПанели.Действие, ВыборкаДействий.Наименование, Действие , СтруктураРасшифровки, , , , , Истина);
			//ДобавитьСтрокуВДеревоКнопок(СтрокаПодменю, "ЗаполнениеТЧ"+НомерКнопки, ТипКнопкиКоманднойПанели.Действие, ВыборкаДействий.Наименование, Действие , СтруктураРасшифровки, , , , , ВыборкаДействий.ПРГ_ИзменяетДанные);
			НоваяКнопка.ИзменяетДанные = ВыборкаДействий.ПРГ_ИзменяетДанные;
			//конец изменений 
...
Функция НапечататьДокумент(ПечДокумент, КоличествоЭкземпляров = 1, НаПринтер = Ложь, Заголовок = "", Ссылка = Неопределено, ПараметрыПечДокумента = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(НаПринтер) = Тип("Структура") Тогда
	    ПРГ_Печатать 	= Ложь;
		ПРГ_ИмяПринтера = "";
		Если Не НаПринтер.Свойство("ИмяПринтера",ПРГ_ИмяПринтера) Тогда
			ПРГ_ИмяПринтера = "";
		Иначе 
			 ПРГ_ИмяПринтера  = ?(ТипЗнч(ПРГ_ИмяПринтера)<>Тип("Строка"),"",ПРГ_ИмяПринтера);
		КонецЕсли;	
		
		Если Не НаПринтер.Свойство("НаПринтер",ПРГ_Печатать) Тогда
			ПРГ_Печатать = Ложь;
		Иначе
			ПРГ_Печатать =  ?(ТипЗнч(ПРГ_Печатать) <> Тип("Булево"),Ложь,ПРГ_Печатать);
		КонецЕсли;	
		
		Если Не ПустаяСтрока(ПРГ_ИмяПринтера) Тогда
			ПечДокумент.ИмяПринтера = ПРГ_ИмяПринтера;
		КонецЕсли;	
		
		Если ПРГ_Печатать Тогда
			ПечДокумент.Вывод = ИспользованиеВывода.Разрешить;
			ПечДокумент.Напечатать();
		Иначе
			ФормаПечати = ПолучитьОбщуюФорму("ПечатьДокументов",, Новый УникальныйИдентификатор);
			...
			ФормаПечати.Открыть();
			Если Не ПустаяСтрока(ПРГ_ИмяПринтера) Тогда
				ФормаПечати.ЭлементыФормы.ПолеТабличногоДокумента.ИмяПринтера = ПРГ_ИмяПринтера;
			КонецЕсли;	
			Возврат ФормаПечати;
		КонецЕсли;
	//конец изменений 
