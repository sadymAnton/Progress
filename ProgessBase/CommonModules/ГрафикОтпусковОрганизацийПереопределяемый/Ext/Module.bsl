...
Процедура ОбработкаЗаполнения(Объект, Основание) Экспорт
	...
	Если ТипОснования = Тип("ДокументСсылка.ПланированиеОтпуска") Тогда
		...
		Если Основание.Проведен
			...
			Запрос.УстановитьПараметр("ГоловнаяОрганизация",	ОбщегоНазначенияЗК.ГоловнаяОрганизация(Объект.Организация));
...
Процедура ДополнитьПодменюЗаполненияТЧ(ПодменюЗаполнить, Действие) Экспорт
	...
	Кнопка = Кнопки.Вставить(0,"ЗаполнитьПоУпрУчету",ТипКнопкиКоманднойПанели.Действие,ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("По управленческому графику отпусков"),Действие);
	Кнопка.Подсказка = ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("Заполнить утвержденными отпусками управленческого учета");
	Кнопка.Пояснение = ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("Заполнить утвержденными отпусками управленческого учета");
