...
//Функция разделяет массив на массив массивов согласно разделителю
//
Функция РазделитьМассив(ИсходныйМассив,Делитель) Экспорт 
	...
	Для каждого ЭлементМассива Из ИсходныйМассив Цикл
		...
		Попытка
			Если Результ.Получить(Группа) = Неопределено Тогда
				Результ[Группа] = Новый Массив;
			КонецЕсли; 
		Исключение
			Результ.Вставить(Группа,Новый Массив);	
		КонецПопытки; 
		
		Результ[Группа].Добавить(ЭлементМассива);	
		
		сч = сч + 1;
		
	КонецЦикла;
	
	Если НЕ Результ.Найти(Неопределено) = Неопределено Тогда
	
		Результ.Удалить(Результ.Найти(Неопределено));	
	
	КонецЕсли; 
	
	Возврат Результ;

КонецФункции // РазделитьМассив()

//Процедура удаляет дублирующие элементы массива
Процедура СвернутьМассив(Массив) Экспорт 
