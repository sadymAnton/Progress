...
Функция СформироватьПечатнуюФормуСправкиПоУправленческимОтпускамСотрудника(Физлицо, ФИОСотрудника, Сотрудник = Неопределено) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		ОбластьМакета.Параметры.Текст			= "" + Выборка.КоличествоДнейОтпускаВГод + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",Выборка.КоличествоДнейОтпускаВГод) + " отпуска в год";
		ТабДокумент.Вывести(ОбластьМакета);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.НомерСтроки = 0 Тогда // Остаток отпуска на рабочую дату/дату увольнения
			...
			Если ОбщееКоличествоДней >= 0 Тогда
				...
				ОбластьМакета.Параметры.КоличествоДней	= "" + ОбщееКоличествоДней + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",ОбщееКоличествоДней);
			Иначе
			...
			Если ОбщееКоличествоДней >= 0 Тогда
				Если Выборка.КоличествоДнейОтпускаБезТекущегоГода > 0 Тогда
					...
					ОбластьМакета.Параметры.КоличествоДней	= "" + Выборка.КоличествоДнейОтпускаБезТекущегоГода + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",Выборка.КоличествоДнейОтпускаБезТекущегоГода);
					МакетыНаСегодня.Добавить(ОбластьМакета);
				КонецЕсли;
				Если Выборка.ОстатокДополнительногоОтпуска > 0 Тогда
					ОбластьМакета = Макет.ПолучитьОбласть("Строка");
					ОбластьМакета.Параметры.Текст			= ОбщийТекст + "дополнительный отпуск";
					ОбластьМакета.Параметры.КоличествоДней	= "" + Выборка.ОстатокДополнительногоОтпуска + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",Выборка.ОстатокДополнительногоОтпуска);
					МакетыНаСегодня.Добавить(ОбластьМакета);
				КонецЕсли;
		...
		Иначе
			...
			ОбластьМакета.Параметры.КоличествоДней	= "" + Выборка.ДнейОтпуска + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",Выборка.ДнейОтпуска);
			МассивМакетов.Добавить(ОбластьМакета);
			...
			Если Выборка.ОтпускВСчетЕжегодногоБезТекущегоГода > 0 Тогда
				...
				ОбластьМакета.Параметры.КоличествоДней	= "" + Выборка.ОтпускВСчетЕжегодногоБезТекущегоГода + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",Выборка.ОтпускВСчетЕжегодного);
				МассивМакетов.Добавить(ОбластьМакета);
			КонецЕсли;
			...
			Если Выборка.ОтпускВСчетДополнительного > 0 Тогда
				...
				ОбластьМакета.Параметры.КоличествоДней	= "" + Выборка.ОтпускВСчетДополнительного + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",Выборка.ОтпускВСчетДополнительного);
				МассивМакетов.Добавить(ОбластьМакета);
			КонецЕсли;
			...
			Если Выборка.ОтпускАвансом > 0 Тогда
				...
				ОбластьМакета.Параметры.КоличествоДней	= "" + Выборка.ОтпускАвансом + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",Выборка.ОтпускАвансом);
				МассивМакетов.Добавить(ОбластьМакета);
			КонецЕсли;
	...
	Для Каждого ТекОстаток Из ТаблицаОстатков Цикл
		...
		ОбластьМакета.Параметры.КоличествоДней	= "" + ТекОстаток.Количество + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",ТекОстаток.Количество);
		МассивМакетов.Добавить(ОбластьМакета);
	...
	Для Каждого ТекДопОтпуск Из ТаблицаДопОтпусков Цикл
		...
		ОбластьМакета.Параметры.КоличествоДней	= "" + ТекДопОтпуск.ДнейОтпуска + " " + ОбщегоНазначенияЗК.ФормаМножественногоЧисла("день","дня","дней",ТекДопОтпуск.ДнейОтпуска);
		МассивМакетов.Добавить(ОбластьМакета);
...
Функция ТекстЗапросаПоОтпускамДляЛицевогоСчета() Экспорт
...
КонецФункции // ТекстЗапросаПоОтпускамДля()
