...
Процедура ПроверитьНапоминанияПользователя(ВыбПользователь, ПроверятьДеньРождения = Ложь) Экспорт
	...
	|
	|ВЫБРАТЬ
	|	УстановкаЦенНоменклатуры.Ссылка           КАК ДокументНапоминания,
	|	УстановкаЦенНоменклатуры.КомуНапомнить    КАК Пользователь,
	|	&ПустойКонтрагент       КАК Контрагент,
	|	&ПустоеКонтактноеЛицо  КАК КонтактноеЛицо,
	|	УстановкаЦенНоменклатуры.ВремяНапоминания КАК ДатаНапоминания
	|ИЗ
	|	Документ.УстановкаЦенНоменклатуры КАК УстановкаЦенНоменклатуры
	|	
	|ГДЕ 
	|	УстановкаЦенНоменклатуры.НапомнитьОСобытии = Истина
	|	И УстановкаЦенНоменклатуры.ВремяНапоминания <= &ДатаСреза
	|	И УстановкаЦенНоменклатуры.КомуНапомнить = &ВыбПользователь
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ДокументНапоминания <> Неопределено И НЕ Выборка.ДокументНапоминания.Пустая() Тогда
				...
				ИначеЕсли ТипЗнч(Док) = Тип("ДокументСсылка.ИзменениеСостоянияОС") Тогда
					СтрокаРезТаблицы.Тема = Док.Метаданные().Синоним;
				//m.ionov@a-prof.ru 05.09.2014
				ИначеЕсли ТипЗнч(Док) = Тип("ДокументСсылка.УстановкаЦенНоменклатуры") Тогда
					СтрокаРезТаблицы.Тема = Док.Метаданные().Синоним;
					СтрокаРезТаблицы.ТекстНапоминания = "Согласования запроса на установку цен";
				//----m.ionov@a-prof.ru---
				КонецЕсли;
