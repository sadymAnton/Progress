...
Процедура ПроверитьЗначенияРеквизитов(ЭтотОбъект, Отказ, ТекстСообщенияОбОшибке) Экспорт
	...
	Если РежимНабораПерсонала = Перечисления.ВидыОрганизационнойСтруктурыПредприятия.ПоСтруктуреЮридическихЛиц Тогда
		Если Организация.Пустая() Тогда
			Отказ = Истина;
			ТекстСообщенияОбОшибке = ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("Не указана организация!");
		КонецЕсли;
		Если Подразделение = Неопределено ИЛИ Подразделение.Пустая() Тогда
			Отказ = Истина;
			ТекстСообщенияОбОшибке = ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("Не указано подразделение организации!");
		КонецЕсли;
		Если Отказ Тогда
			Возврат;
		КонецЕсли;
		Если Организация <> Подразделение.Владелец Тогда
			Отказ = Истина;
			ТекстСообщенияОбОшибке = ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("Указано подразделение, принадлежащее другой организации!");
		КонецЕсли;
	Иначе
		Если Подразделение = Неопределено ИЛИ Подразделение.Пустая() Тогда
			Отказ = Истина;
			ТекстСообщенияОбОшибке = ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("Не указано подразделение!");
		КонецЕсли;
	КонецЕсли;
