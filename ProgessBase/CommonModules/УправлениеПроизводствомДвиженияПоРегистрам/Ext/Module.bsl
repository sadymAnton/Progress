...
//
//+ДС 10.01.2014 Добавлен 3й и 4й параметры "Выборка" и "РезультатЗапроса" для определения субконто (доделан неработающий типовой механизм)
Функция ПолучитьСчетМежд(
	...
	Если СчетМежд = Неопределено Тогда
		...
		Если Выборка = Неопределено Тогда
			...
			СчетМежд = МеждународныйУчет.ПреобразоватьСчетаБУвСчетМСФО(СчетБух, , , , СтруктураШапкиДокумента.ПериодКонец,, Приход).Счет;
			// is ЯннуровВФ кон 20141016
		Иначе
			...
			Субконто = Новый Структура("Субконто1, Субконто2, Субконто3");
			// is ЯннуровВФ кон 20140708

			// is ЯннуровВФ нач 20141011
			//Соответствие = Новый Соответствие;
			// is ЯннуровВФ кон 20141011
			Для каждого стр Из СчетБух.ВидыСубконто Цикл
				// is ЯннуровВФ нач 20141003
				//// is ЯннуровВФ нач 20140708
				//лОписаниеТипов = Новый ОписаниеТипов(стр.ВидСубконто.ТипЗначения.Типы());
				//Субконто["Субконто"+стр.НомерСтроки] = лОписаниеТипов.ПривестиЗначение(Субконто["Субконто"+стр.НомерСтроки]);
				//// is ЯннуровВФ кон 20140708
				...
				Для каждого Колонка из РезультатЗапроса.Колонки Цикл
					...
					// is ЯннуровВФ кон 20141011
						// is ЯннуровВФ нач 20141011
						//Соответствие.Вставить("Субконто" + стр.НомерСтроки, Колонка.Имя);
						// is ЯннуровВФ кон 20141011
						// is ЯннуровВФ нач 20140708
						//Выполнить("Субконто" + стр.НомерСтроки + " = Выборка[Колонка.Имя]");
						Субконто["Субконто"+стр.НомерСтроки] = Выборка[Колонка.Имя];
						// is ЯннуровВФ кон 20140708
					КонецЕсли;					
				КонецЦикла;	
			КонецЦикла;	
			...
			//+ДС 10.01.2014
			// is ЯннуровВФ нач 20140708
			//СтруктураСчета = МеждународныйУчет.ПреобразоватьСчетаБУвСчетМСФО(СчетБух, Субконто1, Субконто2, Субконто3, СтруктураШапкиДокумента.ПериодКонец);
			СтруктураСчета = МеждународныйУчет.ПреобразоватьСчетаБУвСчетМСФО(СчетБух, Субконто.Субконто1, Субконто.Субконто2, Субконто.Субконто3, СтруктураШапкиДокумента.ПериодКонец);
			// is ЯннуровВФ кон 20140708
			
			// is ЯннуровВФ нач 20141011
			Соответствие = Новый Соответствие;
			Если ЗначениеЗаполнено(СтруктураСчета.Счет) Тогда 
				Для каждого стр Из СтруктураСчета.Счет.ВидыСубконто Цикл
					ТипСубконто = стр.ВидСубконто.ТипЗначения.Типы()[0];
					Для каждого Колонка из РезультатЗапроса.Колонки Цикл
						Если ТипЗнч(Выборка[Колонка.Имя]) = ТипСубконто И ЗначениеЗаполнено(Выборка[Колонка.Имя]) Тогда 
							Соответствие.Вставить("Субконто" + стр.НомерСтроки, Колонка.Имя);
			...
			СтруктураСчета.Вставить("Соответствие", Соответствие);
				
			//-ДС
			СчетМежд = СтруктураСчета.Счет;	 	                           
...
Функция СформироватьЗапросПоРегиструНезавершенноеПроизводство(
	...
	|	И Не НезавершенноеПроизводство.Регистратор ССЫЛКА Документ.КорректировкаСтоимостиСписанияТоваров
	...
	//КодыОпераций.Добавить(Перечисления.КодыОперацийНезавершенноеПроизводство.СписаниеПартийВПроизводствоОперативно);
	КодыОпераций.Добавить(Перечисления.КодыОперацийНезавершенноеПроизводство.ПустаяСсылка());
	// is ЯннуровВФ нач 20150918; значение уже есть в массиве 
	//КодыОпераций.Добавить(Перечисления.КодыОперацийНезавершенноеПроизводство.СписаниеПрочихЗатратВПроизводство);
	// is ЯннуровВФ кон 20150918 
...
Процедура ПеренестиДвиженияПоРегиструНезавершенноеПроизводствоМежд(
	...
	СчетаУчета.Добавить(ПланыСчетов.Международный._ОсновноеПроизводство); // 20.01
	СчетаУчета.Добавить(ПланыСчетов.Международный.ВспомогательныеПроизводства); // 23
	СчетаУчета.Добавить(ПланыСчетов.Международный.ОбслуживающиеПроизводства); // 29
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВидЗатрат = Перечисления.ВидыЗатрат.Материальные 
		   ...
		   И Выборка.КодОперации <> Перечисления.КодыОперацийНезавершенноеПроизводство.ВнутренниеУслугиНаПроизводство
		   // is ЯннуровВФ кон 20141017
		   // is ЯннуровВФ нач 20150918
		   И Выборка.КодОперации <> Перечисления.КодыОперацийНезавершенноеПроизводство.СписаниеПрочихЗатратВПроизводство
		   // is ЯннуровВФ кон 20150918
		Тогда
			Продолжить;
		КонецЕсли;
		...
		СтруктСчета = ПолучитьСчетМежд(
			СтруктураШапкиДокумента,
			Выборка.СчетУчета,
			Выборка, 
			РезультатЗапроса,
			// is ЯннуровВФ нач 20141016
			(Выборка.ВидДвижения=ВидДвиженияНакопления.Приход)
			// is ЯннуровВФ кон 20141016
		);
		СчетУчетаМежд = СтруктСчета.Счет;
		
		Соответствие = СтруктСчета.Соответствие;
		...
		Для каждого СтрокаСоотв Из Соответствие Цикл
			Если Не ЗначениеЗаполнено(СтруктСчета[СтрокаСоотв.Ключ]) Тогда
				Продолжить;
			КонецЕсли;	
			СтрокаЗамены[СтрокаСоотв.Значение] = СтруктСчета[СтрокаСоотв.Ключ];
		КонецЦикла;
		...
		Если ЗначениеЗаполнено(СчетУчетаМежд) 
		 И СчетаУчета.Найти(СчетУчетаМежд) <> Неопределено 
		// is ЯннуровВФ кон 20141001
		// is ЯннуровВФ нач 20141010
		 И СтрокаЗамены.СтатьяЗатрат.ВидЗатрат <> Перечисления.ВидыЗатрат.Амортизация Тогда
		// is ЯннуровВФ кон 20141010
...
Функция СформироватьЗапросПоРегиструЗатраты(
	...
	//>>290315 Степанов 50744 убрать из выборки перенос по сторно требований-накладных
	//ТекстЗапроса = "
	//|ВЫБРАТЬ
	//|	*
	//|ИЗ
	...
	//// is ЯннуровВФ нач 20141026
	//|	И Не Затраты.Регистратор ССЫЛКА Документ.КорректировкаСтоимостиСписанияТоваров
	//// is ЯннуровВФ кон 20141026
	//|
	//|УПОРЯДОЧИТЬ ПО
	//|	Затраты.Регистратор,
	...
	//>>310315 Степанов убрать из выборки перенос по сторно требований-накладных, которые переносят на затраты 43 и 21 счета
	ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	КорректировкаЗаписейРегистровЗаполнениеДвижений.Документ,
	|	КорректировкаЗаписейРегистровЗаполнениеДвижений.Ссылка
	|ПОМЕСТИТЬ ВТ_ОтмененныеТН
	|ИЗ
	|	Документ.КорректировкаЗаписейРегистров.ЗаполнениеДвижений КАК КорректировкаЗаписейРегистровЗаполнениеДвижений
	|ГДЕ
	|	КорректировкаЗаписейРегистровЗаполнениеДвижений.Документ ССЫЛКА Документ.ТребованиеНакладная
	|	И КорректировкаЗаписейРегистровЗаполнениеДвижений.Ссылка.Дата МЕЖДУ &ПериодНачало И &ПериодКонец
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТребованиеНакладнаяМатериалы.Счет,
	|	ВТ_ОтмененныеТН.Документ,
	|	ВТ_ОтмененныеТН.Ссылка
	|ПОМЕСТИТЬ ВТ_ДокиСоСчетами
	|ИЗ
	|	ВТ_ОтмененныеТН КАК ВТ_ОтмененныеТН
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ТребованиеНакладная.Материалы КАК ТребованиеНакладнаяМатериалы
	|		ПО ВТ_ОтмененныеТН.Документ = ТребованиеНакладнаяМатериалы.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ДокиСоСчетами.Ссылка
	|ПОМЕСТИТЬ ВТ_КорректировкиСторноДляИсключения
	|ИЗ
	|	ВТ_ДокиСоСчетами КАК ВТ_ДокиСоСчетами
	|ГДЕ
	|	(ВТ_ДокиСоСчетами.Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ГотоваяПродукция)
	|			ИЛИ ВТ_ДокиСоСчетами.Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Полуфабрикаты))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Затраты.КодОперации,
	|	Затраты.ВидРегламентнойОперации
	|ИЗ
	|	РегистрНакопления.ЗатратыБухгалтерскийУчет КАК Затраты
	|ГДЕ
	|	Затраты.Организация = &Организация
	...
	|				(ВЫБРАТЬ
	|					ВТ_КорректировкиСторноДляИсключения.Ссылка
	|				ИЗ
	|					ВТ_КорректировкиСторноДляИсключения)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Затраты.Регистратор,
	|	Затраты.НомерСтроки";
...
Процедура ПеренестиДвиженияПоРегиструЗатратыМежд(
	...
	// is ЯннуровВФ нач 20141001
	СчетаУчета = Новый Массив;
	СчетаУчета.Добавить(ПланыСчетов.Международный.ОбщепроизводственныеРасходы); // 25
	СчетаУчета.Добавить(ПланыСчетов.Международный.АдминистративныеРасходы); // 26
	СчетаУчета.Добавить(ПланыСчетов.Международный._ИздержкиОбращения); // 44.01
	СчетаУчета.Добавить(ПланыСчетов.Международный._КоммерческиеРасходы); // 44.02
	// is ЯннуровВФ кон 20141001
...
Функция СформироватьЗапросПоРегиструБракВПроизводстве(
	...
	// is ЯннуровВФ нач 20141026
	|	И Не БракВПроизводстве.Регистратор ССЫЛКА Документ.КорректировкаСтоимостиСписанияТоваров
	// is ЯннуровВФ кон 20141026
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура ПеренестиДвиженияПоРегиструБракВПроизводствеМежд(
	...
	// is ЯннуровВФ нач 20141001
	СчетаУчета = Новый Массив;
	СчетаУчета.Добавить(ПланыСчетов.Международный.БракВПроизводстве); // 28
	// is ЯннуровВФ кон 20141001
...
Функция СформироватьТекстЗапросаДляДокументаОтчетПроизводстваЗаСмену(ИмяТабличнойЧасти)
	...
	|	Док.Номенклатура.ВидНоменклатуры  Как ВидНоменклатурыЗатраты,
...
Процедура СформироватьДвиженияПоРегистрамЗатратыНаВыпускПродукции(
	...
	//начало изменений БП 14 
	//ОтражатьВУправленческомУчете  = Не ПараметрыСеанса.НеведетсяУПРУчетВЧастиЗатратИОС и СтруктураШапкиДокумента.ОтражатьВУправленческомУчете;
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
		...
		//Если СтрокаВыборки.Этовода Тогда
		Если СтрокаВыборки.Владелец().Колонки.Найти("Этовода") = Неопределено 
		 Или СтрокаВыборки.Этовода Тогда
...
Процедура СформироватьДвиженияПоРаспределениюМатериалов(
	...
	//начало изменений БП 14 
	ТаблицаРаспределения.Колонки.Добавить("ЭтоВода",Новый ОписаниеТипов("Булево"));
	Если ТаблицаРаспределения.Колонки.Найти("ВидНоменклатурыЗатраты") = Неопределено или УчетнаяПолитикаБух = Неопределено  Тогда
		ТаблицаРаспределения.ЗаполнитьЗначения(Ложь,"ЭтоВода")
	Иначе
		ВидНоменклатурыВода = Неопределено;
		Если Не УчетнаяПолитикаБух.Свойство("ВидНоменклатурыВода",ВидНоменклатурыВода) Тогда
			 УчетнаяПолитикаБух.Вставить("ВидНоменклатурыВода",Константы.ПРГ_ВидНоменклатурыВода.Получить());
			 ВидНоменклатурыВода = УчетнаяПолитикаБух.ВидНоменклатурыВода;
		КонецЕсли;	
		Если ЗначениеЗаполнено(ВидНоменклатурыВода) Тогда
			Для Каждого _ТекСтрока Из ТаблицаРаспределения Цикл
				Если _ТекСтрока.ВидНоменклатурыЗатраты = ВидНоменклатурыВода  Тогда
					_ТекСтрока.ЭтоВода = истина;
				Иначе
					_ТекСтрока.ЭтоВода = Ложь;
				КонецЕсли;	
			КонецЦикла;	
		Иначе
			ТаблицаРаспределения.ЗаполнитьЗначения(Ложь,"ЭтоВода")
		КонецЕсли;	
	КонецЕсли;
	Если ТипЗнч(СтруктураШапкиДокумента.Ссылка) 
		= Тип("ДокументСсылка.ОтчетПроизводстваЗаСмену") Тогда
		Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийОтчетаПроизводстваЗаСмену.РаспределениеМатериаловНаВыпуск Тогда
			ТаблицаРаспределения.ЗаполнитьЗначения(Неопределено,"ДокументВыпуска");
		КонецЕсли;
	КонецЕслИ;	
...
Функция СформироватьТекстЗапросаТаблицыСтоимостиЗатрат(
	...
	//начало изменений БП 14
	ИначеЕсли (ИмяДокумента = "ОтчетПроизводстваЗаСмену") 
		и (СтруктураШапкиДокумента.ВидСписанияМПЗ = Перечисления.ВидыСписанияМПЗ.СписыватьСоСклада) Тогда
		ТекстЗапроса = "
				|ВЫБРАТЬ
				...
				|	//ДляРеглУчета ЕСТЬNULL(Сумма(НезавершенноеПроизводствоНУ.Стоимость), 0) КАК СтоимостьНУОстаток,
				|	//ДляРеглУчета ЕСТЬNULL(Сумма(НезавершенноеПроизводствоНУ.Количество), 0) КАК КоличествоНУОстаток,
				|	Сумма(НезавершенноеПроизводство.Стоимость) Как СтоимостьОстаток,
				|	Сумма(НезавершенноеПроизводство.Количество) как КоличествоОстаток
				|ПОМЕСТИТЬ ТаблицаСтоимостиЗатрат%СуффиксРегл%
				|ИЗ
				...
				|		ПО  
				|		НезавершенноеПроизводство.Регистратор 				  	 = &Ссылка 
				|		И НезавершенноеПроизводство.СтатьяЗатрат   				 = ТаблицаДокумента.СтатьяЗатрат
				|		И НезавершенноеПроизводство.НоменклатурнаяГруппа		 = ТаблицаДокумента.НоменклатурнаяГруппа
				|		И НезавершенноеПроизводство.Затрата                 	 = ТаблицаДокумента.Номенклатура
				|		И НезавершенноеПроизводство.ХарактеристикаЗатраты   	 = ТаблицаДокумента.ХарактеристикаНоменклатуры
				|		И НезавершенноеПроизводство.СерияЗатраты            	 = ТаблицаДокумента.СерияНоменклатуры
				|		
				|	//ДляРеглУчета И НезавершенноеПроизводство.СчетУчета     = ТаблицаДокумента.СчетУчета
				|	//ДляРеглУчета И НезавершенноеПроизводство.Организация   = ТаблицаДокумента.Организация
				|	//ДляРеглУчета И НезавершенноеПроизводство.Подразделение = ТаблицаДокумента.ПодразделениеОрганизацииНЗП 				
				|	//ДляРеглУчета И НезавершенноеПроизводство.Заказ  	     = ТаблицаДокумента.ЗаказНЗПРегл

				|	//ДляУпрУчета  И НезавершенноеПроизводство.Заказ		   = ТаблицаДокумента.ЗаказНЗП
				|	//ДляУпрУчета  И НезавершенноеПроизводство.Подразделение  = ТаблицаДокумента.ПодразделениеНЗП 
				|	
				|		
				|	//ДляРеглУчета ЛЕВОЕ СОЕДИНЕНИЕ
				|	//ДляРеглУчета 		РегистрНакопления.НезавершенноеПроизводствоНалоговыйУчет КАК НезавершенноеПроизводствоНУ
				|	//ДляРеглУчета ПО
				...
				|	//ДляРеглУчета      И НезавершенноеПроизводствоНУ.Регистратор = &Ссылка 
				|   Сгруппировать ПО
				|	НезавершенноеПроизводство.Подразделение,
				|	НезавершенноеПроизводство.Затрата,
				|	НезавершенноеПроизводство.ХарактеристикаЗатраты,
...
//начало изменений БП 14
Функция СформироватьТекстЗапросаДляРеквизитаЗаказВыпускПродукции()
	
	ТекстЗапроса = "
	|		ВЫБОР КОГДА
	|				 ТабличнаяЧасть.Заказ ССЫЛКА Документ.ЗаказПокупателя 
	|				И ТабличнаяЧасть.Заказ <> ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка) ТОГДА
	|			ТабличнаяЧасть.Заказ
	|		ИНАЧЕ
	|			Неопределено
...
Функция СформироватьТекстЗапросаПоПродукцииВыпускПродукции()
	...
	|		) КАК Количество
	|	ИЗ	
	|		Документ.ВыпускПродукции.Продукция КАК ТабличнаяЧасть
	|		
	|	ГДЕ
	|		ТабличнаяЧасть.Ссылка = &Ссылка
	|		
	...
	ТекстЗапросаСКомментариями = СформироватьТекстЗапросаДляРеквизитаЗаказВыпускПродукции();
	ТекстЗапросаДляРеквизитаЗаказ = ЗаменитьКомментарииВТекстеЗапросаПоОтражениюВыпуска(
		ТекстЗапросаСКомментариями,
		Перечисления.ВидыОтраженияВУчете.ОтражатьВУправленческомУчете
	);
...
//начало изменений БП 14
//СформироватьТекстЗапросаПоВыпускуПродукцииНаСкладВыпускПродукции();
Функция СформироватьТекстЗапросаПоВыпускуПродукцииНаСкладВыпускПродукции()
	...
	ТекстЗапроса = "
		...
		|	ТабличнаяЧасть.ЗаказРезерв КАК ЗаказПолучательРегл,
		|	ТабличнаяЧасть.ЗаказРезерв.ВидОперации КАК ВидОперации,
		|	ЕСТЬNULL(ТабличнаяЧасть.ЗаказРезерв.ДоговорКонтрагента.ОбособленныйУчетТоваровПоЗаказамПокупателей, ЛОЖЬ) КАК ОбособленныйУчетТоваровПоЗаказамПокупателей,
		|	ТабличнаяЧасть.Качество,
		|	Значение(Перечисление.СтатусыПартийТоваров.Продукция) Как СтатусПартии,
		...
		|	&ВидВыпускаВыпуск КАК ВидВыпуска,
		|	ВЫБОР
		|		КОГДА ТабличнаяЧасть.Заказ ССЫЛКА Документ.ЗаказПокупателя
		|				И ТабличнаяЧасть.Заказ.ВидОперации = &ВидОперацииПереработка
		|			ТОГДА &ПродукцияИзДавальческогоСырья
		|		ИНАЧЕ &СобственнаяПродукция
		|	КОНЕЦ КАК ВариантВыпускаПродукции,
		|	
		|	ТабличнаяЧасть.Количество * ВЫБОР
		|		КОГДА НЕ ТабличнаяЧасть.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент ЕСТЬ NULL 
		|				И ТабличнаяЧасть.Коэффициент <> 0
		|			ТОГДА ТабличнаяЧасть.Коэффициент / ТабличнаяЧасть.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК Количество,
		|	
		|	0 КАК КоличествоДоделка,
		//m_ionov@mail.ru 01.09.2016
		//Добавляем показатели для записи в регистр выпуск продукции
		|	УП_МДЖ КАК УП_МДЖ,
		|	УП_МДБ КАК УП_МДБ,
		|	УП_МДСВ КАК УП_МДСВ
		//------- m_ionov@mail.ru -------
		...
		Если (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетПроизводстваЗаСмену"))
		  ...
		  ИЛИ ((ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПоступлениеТоваровИзПереработки"))
			...
			Если (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетПроизводстваЗаСмену")) Тогда
				Если ДокументСсылка.ВидСписанияМПЗ = Перечисления.ВидыСписанияМПЗ.СписыватьСоСклада Тогда
					// сделали движения при списании товаров, поэтому ничего не перезаписываем
					СтруктураПараметров.ИзмененыДвиженияПартииТоваровНаСкладахБух = Ложь;
					СтруктураПараметров.ИзмененыДвиженияПартииТоваровНаСкладахНал = Ложь;
					СтруктураПараметров.ИзмененыДвиженияНезавершенноеПроизводствоБух = Ложь;
					СтруктураПараметров.ИзмененыДвиженияНезавершенноеПроизводствоНАЛ = Ложь;
					СтруктураПараметров.ИзмененыДвиженияХозрасчетный 				 = Ложь;
					СтруктураПараметров.ИзмененыДвиженияНДСПартииТоваров			 = Ложь;
					СтруктураПараметров.ИзмененыДвиженияНалоговый					 = Ложь;
				КонецЕсли;	
			КонецЕсли;	
			//конец изменений БП 14 
...
Процедура СформироватьДвиженияПоВыпускуПродукцииИНаправлениямВыпуска(
	...
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ВыпускПродукции") Тогда
		ТекстЗапросаПоТабличнойЧастиПродукция = СформироватьТекстЗапросаПоПродукцииВыпускПродукции();
	    ТекстЗапросаПоВыпускуПродукцииНаСклад = СформироватьТекстЗапросаПоВыпускуПродукцииНаСкладВыпускПродукции();
	//конец изменений БП 14 
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ПоступлениеТоваровИзПереработки") Тогда
