...
Функция ПолучитьТаблицуДействийСНачислениями(ДанныеДокумента, ДокументСсылка, ДокументДата, Организация, ИмяДатыДействия, КоэффициентИндексацииЗаработка = 1, ПоДаннымТрудовогоДоговора = Ложь, ПолучатьПлановыеНачисления = Истина, ОбособленноеПодразделение = Неопределено) Экспорт
	...
	Если ТипЗнч(ДанныеДокумента) = Тип("ТаблицаЗначений") Тогда
		...
		// Измайлова Полина 28.07.2011 13:23 
		|	ДанныеДокумента.AVTШтатноеРасписание КАК ШтатнаяЕдиница,
		////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
		...
		///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 27.12.2011 17:34 
		Если ДанныеДокумента.Колонки.Найти("AVTШтатноеРасписание") = Неопределено Тогда 
			ДанныеДокумента.Колонки.Добавить("AVTШтатноеРасписание", Новый ОписаниеТипов("СправочникСсылка.AVT_ШтатноеРасписание"));
		КонецЕсли;
		////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
	...
	Иначе	
		...
		|	&Организация,
		///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 28.07.2011 13:24 
		|	NULL КАК ШтатнаяЕдиница,
		////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
		|	&ДатаДокумента
		|ПОМЕСТИТЬ ДанныеДокумента
		|ИЗ
	...
	// ЗАКОММЕНТИРОВАНО: Измайлова Полина 29.08.2011 11:31   
	...
	///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 29.08.2011 11:39 
	// ОПИСАНИЕ: упрощен исходных запрос таким образом, чтобы при приеме на работу и перемещении брался 
	//           только вид расчета из трудового договора, при перемещении прежние начисления прекращать.                                            
	ТекстЗапроса = ТекстЗапроса +  "
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеДокумента.Сотрудник,
	...
	|		КОГДА РаботникиОрганизаций.ПериодЗавершения <= РаботникиСрезПоследних.Период
	|				И РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|			ТОГДА РаботникиОрганизаций.AVTШтатноеРасписаниеЗавершения
	|		ИНАЧЕ РаботникиОрганизаций.AVTШтатноеРасписание
	|	КОНЕЦ КАК ШтатнаяЕдиница,                                          
	|	ВЫБОР
	|		КОГДА РаботникиОрганизаций.ПериодЗавершения <= РаботникиСрезПоследних.Период
	|				И РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|			ТОГДА РаботникиОрганизаций.ОбособленноеПодразделениеЗавершения
	...
	|	СотрудникиОрганизаций.ВалютаТарифнойСтавки, 
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.ШтатнаяЕдиница ЕСТЬ NULL 
	|				ИЛИ ДанныеДокумента.ШтатнаяЕдиница = ЗНАЧЕНИЕ(Справочник.AVT_ШтатноеРасписание.ПустаяСсылка)
	|			ТОГДА РаботникиСрезПоследних.ШтатнаяЕдиница
	|		ИНАЧЕ ДанныеДокумента.ШтатнаяЕдиница
	|	КОНЕЦ КАК ШтатнаяЕдиница,
	|	РаботникиСрезПоследних.ШтатнаяЕдиница КАК СтараяШтатнаяЕдиница,                                          
	|	СотрудникиОрганизаций.ТарифныйРазряд
	|ПОМЕСТИТЬ ДанныеСотрудников
	|ИЗ
	|	ДанныеДокумента КАК ДанныеДокумента
	...
	|	ДанныеСотрудников.ДатаДействия,
	|	ДанныеСотрудников.Подразделение,
	|	ДанныеСотрудников.ШтатнаяЕдиница,
	|	ДанныеСотрудников.Должность КАК Должность
	|ПОМЕСТИТЬ ШтатноеРасписаниеНоваяПозицияКрайниеДаты
	|ИЗ
	|	РегистрСведений.ШтатноеРасписаниеОрганизаций КАК ШтатноеРасписание
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеСотрудников КАК ДанныеСотрудников
	|		ПО ШтатноеРасписание.Период <= ДанныеСотрудников.ДатаДействия
	|			И ШтатноеРасписание.AVTШтатноеРасписание = ДанныеСотрудников.ШтатнаяЕдиница
	...
	|	ОсновныеНачисленияСотрудников.ВидРасчета,
	|	ОсновныеНачисленияСотрудников.ДатаДействия,
	|	СтавкаШтатнойЕдиницы.МаксимальнаяТарифнаяСтавка КАК ТарифнаяСтавка,
	|	СтавкаШтатнойЕдиницы.ВалютаТарифнойСтавки
	|ПОМЕСТИТЬ ВТНоваяСтавкаОснВидаРасчета
	|ИЗ
	|	(ВЫБРАТЬ
	|		ШтатноеРасписаниеНоваяПозицияКрайниеДаты.Сотрудник КАК Сотрудник,
	|		ШтатноеРасписаниеНоваяПозицияКрайниеДаты.ШтатнаяЕдиница КАК ШтатнаяЕдиница,
	...
	|ИЗ
	|		ШтатноеРасписаниеНоваяПозицияКрайниеДаты КАК ШтатноеРасписаниеНоваяПозицияКрайниеДаты
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ШтатноеРасписаниеОрганизаций КАК ШтатноеРасписаниеОрганизаций
	|			ПО ШтатноеРасписаниеНоваяПозицияКрайниеДаты.ШтатнаяЕдиница = ШтатноеРасписаниеОрганизаций.AVTШтатноеРасписание
	|				И ШтатноеРасписаниеНоваяПозицияКрайниеДаты.Период = ШтатноеРасписаниеОрганизаций.Период 
	|               И ШтатноеРасписаниеНоваяПозицияКрайниеДаты.Должность = ШтатноеРасписаниеОрганизаций.Должность
	|               И (ШтатноеРасписаниеОрганизаций.КоличествоСтавок <> 0)) КАК СтавкаШтатнойЕдиницы  //Blik 271014 //Blik 200416 51601
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ " + ?(ПоДаннымТрудовогоДоговора, "ДанныеСотрудников" , "ОсновныеНачисленияСотрудников") + " КАК ОсновныеНачисленияСотрудников
	|		ПО СтавкаШтатнойЕдиницы.Сотрудник = ОсновныеНачисленияСотрудников.Сотрудник
	|;                                          
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ВТНоваяСтавкаОснВидаРасчета.ВидРасчета ЕСТЬ NULL 
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыДействияСНачислением.Прекратить)
	|		КОГДА ВТНоваяСтавкаОснВидаРасчета.ТарифнаяСтавка = НачисленияСотрудника.Показатель1
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыДействияСНачислением.НеИзменять)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыДействияСНачислением.Изменить)
	|	КОНЕЦ КАК Действие,
	|	НачисленияСотрудника.Сотрудник,
	...
	|	НачисленияСотрудника.ВидРасчета,
	|	ЕСТЬNULL(ВТНоваяСтавкаОснВидаРасчета.ТарифнаяСтавка, НачисленияСотрудника.Показатель1) КАК Показатель1,
	|	НачисленияСотрудника.Показатель2,
	|	НачисленияСотрудника.Показатель3,
	|	НачисленияСотрудника.Показатель4,
	|	НачисленияСотрудника.Показатель5,
	...
	|	НачисленияСотрудника.ТарифныйРазряд5,
	|	НачисленияСотрудника.ТарифныйРазряд6,
	|	ЕСТЬNULL(ВТНоваяСтавкаОснВидаРасчета.ВалютаТарифнойСтавки, НачисленияСотрудника.Валюта1) КАК Валюта1,
	|	НачисленияСотрудника.Валюта2,
	|	НачисленияСотрудника.Валюта3,
	|	НачисленияСотрудника.Валюта4,
	|	НачисленияСотрудника.Валюта5,
	...
	|ИЗ
	|	НачисленияСотрудника КАК НачисленияСотрудника
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНоваяСтавкаОснВидаРасчета КАК ВТНоваяСтавкаОснВидаРасчета
	|		ПО НачисленияСотрудника.Сотрудник = ВТНоваяСтавкаОснВидаРасчета.Сотрудник
	|			И НачисленияСотрудника.ВидРасчета = ВТНоваяСтавкаОснВидаРасчета.ВидРасчета ";
 	Если ПоДаннымТрудовогоДоговора Тогда
		ТекстЗапроса = ТекстЗапроса + "
		| 
		|ОБЪЕДИНИТЬ ВСЕ
		...
		|ВЫБРАТЬ
		|	ЗНАЧЕНИЕ(Перечисление.ВидыДействияСНачислением.Начать),
		|	НоваяСтавкаОснВидаРасчета.Сотрудник,
		|	НоваяСтавкаОснВидаРасчета.ВидРасчета,
		|	НоваяСтавкаОснВидаРасчета.ТарифнаяСтавка,
		|	NULL,
		|	NULL,
		|	NULL,
		|	NULL,
		...
		|	NULL,
		|	NULL,
		|	НоваяСтавкаОснВидаРасчета.ВалютаТарифнойСтавки,
		|	NULL,
		|	NULL,
		|	NULL,
		|	NULL,
		|	NULL,
		|	НоваяСтавкаОснВидаРасчета.ДатаДействия
		|ИЗ
		|	ВТНоваяСтавкаОснВидаРасчета КАК НоваяСтавкаОснВидаРасчета";
	КонецЕсли;
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}
...
///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
// Измайлова Полина 20.12.2011 17:00 
//
// Параметры:
// 	AVT_Сотрудник  - [СправочникСсылка.Сотрудники] - сотрудник
//
////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
Функция КоличествоДнейОтпускаПоКалендарнымДням(ДатаНачала, ДатаОкончания, ВключатьПраздники = Ложь, AVT_Сотрудник)
	...
	Если НЕ ВключатьПраздники Тогда
		...
		///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 23.06.2011 11:49 
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций.СрезПоследних(&ДатаНачала, Сотрудник = &Сотрудник) КАК РаботникиОрганизаций
		|		ПО (ВЫБОР
		|				КОГДА РаботникиОрганизаций.ПериодЗавершения <= &ДатаНачала
		...
		|			КОНЕЦ = РегламентированныйПроизводственныйКалендарь.AVT_ПроизводственныйКалендарь)
		////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}
		...
		Запрос.УстановитьПараметр("ДатаОкончания", ДатаОкончания);
					///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 20.12.2011 17:02 
		Запрос.УстановитьПараметр("Сотрудник", AVT_Сотрудник);
		////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА} 
...
Функция ДатаНачалаОтпускаПоКалендарнымДням(ДатаОкончания, КоличествоДней, ВключатьПраздники = Ложь, AVT_Сотрудник)
	...
	Если НЕ ВключатьПраздники Тогда
		...
		|	
			///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 23.06.2011 11:49 
		|			И ДатыКалендаря.AVT_ПроизводственныйКалендарь = Календарь.AVT_ПроизводственныйКалендарь
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций.СрезПоследних(&ДатаОкончания, Сотрудник = &Сотрудник) КАК РаботникиОрганизаций
		|		ПО (ВЫБОР
		|				КОГДА РаботникиОрганизаций.ПериодЗавершения <= &ДатаОкончания
		|						И РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1)
		|					ТОГДА РаботникиОрганизаций.ГрафикРаботыЗавершения.AVT_ПроизводственныйКалендарь
		|				ИНАЧЕ РаботникиОрганизаций.ГрафикРаботы.AVT_ПроизводственныйКалендарь
		|			КОНЕЦ = ДатыКалендаря.AVT_ПроизводственныйКалендарь)
		////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}
		|	СГРУППИРОВАТЬ ПО
...
///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
// Измайлова Полина 20.12.2011 17:05 
//
// Параметры:
// 	AVT_Сотрудник  - [СправочникСсылка.Сотрудники] - сотрудник
//
////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
Функция ДатаОкончанияОтпускаПоКалендарнымДням(ДатаНачала, КоличествоДней, ВключатьПраздники = Ложь, AVT_Сотрудник)	
	Если НЕ ВключатьПраздники Тогда
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		...
		Запрос.УстановитьПараметр("КоличествоДней", КоличествоДней);
			///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 20.12.2011 17:17 
		Запрос.УстановитьПараметр("Сотрудник", AVT_Сотрудник);
		////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}  
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
...
//
////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
Процедура ЗаполнитьДатыОтпуска(ДатаНачала, ДатаОкончания, КоличествоДней, ИмяИзмененногоПараметра = "", ВидОтпуска = Неопределено, ЗапрашиватьПользователя = Истина, ИмяИзменяемогоПараметра = "", AVT_Сотрудник) Экспорт
	...
	Если ЗначениеЗаполнено(ДатаНачала) Тогда
		Если ЗначениеЗаполнено(ДатаОкончания) И ЗначениеЗаполнено(КоличествоДней) И ЗапрашиватьПользователя Тогда
			Если Найти(ИмяИзмененногоПараметра, "ДатаОкончания") > 0 Тогда
				// ИЗМЕНЕНИЕ: Измайлова Полина 20.12.2011 17:04
				...
				ИзменитьПериодОтпускаПоЗапросу(КоличествоДней, КоличествоДнейОтпускаПоКалендарнымДням(ДатаНачала, ДатаОкончания, ВключатьПраздникиВПериодОтпуска(ВидОтпуска), AVT_Сотрудник), "количество дней отпуска");
			Иначе
				// ИЗМЕНЕНИЕ: Измайлова Полина 20.12.2011 17:04
				...
				ИзменитьПериодОтпускаПоЗапросу(ДатаОкончания, ДатаОкончанияОтпускаПоКалендарнымДням(ДатаНачала, КоличествоДней, ВключатьПраздникиВПериодОтпуска(ВидОтпуска), AVT_Сотрудник), "дату окончания отпуска");
			КонецЕсли;
		Иначе
			Если Найти(ИмяИзменяемогоПараметра, "ДатаНачала") > 0 Тогда
				Если ЗначениеЗаполнено(КоличествоДней) Тогда
					...
					ДатаНачала = ДатаНачалаОтпускаПоКалендарнымДням(ДатаОкончания, КоличествоДней, ВключатьПраздникиВПериодОтпуска(ВидОтпуска), AVT_Сотрудник);
				КонецЕсли;
			Иначе
				Если ЗначениеЗаполнено(ДатаОкончания) Тогда
					...
					КоличествоДней = КоличествоДнейОтпускаПоКалендарнымДням(ДатаНачала, ДатаОкончания, ВключатьПраздникиВПериодОтпуска(ВидОтпуска), AVT_Сотрудник);
				ИначеЕсли ЗначениеЗаполнено(КоличествоДней) Тогда
					...
					ДатаОкончания = ДатаОкончанияОтпускаПоКалендарнымДням(ДатаНачала, КоличествоДней, ВключатьПраздникиВПериодОтпуска(ВидОтпуска), AVT_Сотрудник);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	Иначе
...
Функция СформироватьЗапросСведенийОВидахРасчета(ВидыРасчета)
	...
	//	|				И (НЕ ПоказателиВидаРасчета.Показатель В (ЗНАЧЕНИЕ(Справочник.ПоказателиСхемМотивации.ТарифнаяСтавкаМесячная), ЗНАЧЕНИЕ(Справочник.ПоказателиСхемМотивации.ТарифнаяСтавкаДневная), ЗНАЧЕНИЕ(Справочник.ПоказателиСхемМотивации.ТарифнаяСтавкаЧасовая))) 
 	|				И (НЕ ПоказателиВидаРасчета.Показатель В (ЗНАЧЕНИЕ(Справочник.ПоказателиСхемМотивации.ТарифнаяСтавкаМесячная), ЗНАЧЕНИЕ(Справочник.ПоказателиСхемМотивации.ТарифнаяСтавкаДневная), ЗНАЧЕНИЕ(Справочник.ПоказателиСхемМотивации.ТарифнаяСтавкаЧасовая), ЗНАЧЕНИЕ(Справочник.ПоказателиСхемМотивации.AVT_Переработка))) 
 	////ВадимКонец
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
