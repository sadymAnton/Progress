...
Функция УсловиеОтбораЗЛпоОПСизСотрудников() Экспорт
...
Процедура РасшифроватьОтчетПФР(Отчет, Форма, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Форма.ЭлементыФормы.ОсновнаяПанель.ТекущаяСтраница.Имя = "Раздел23" Тогда
		...
		Если НомерСтроки = "10" или НомерСтроки = "20" или НомерСтроки = "30" или НомерСтроки = "40" тогда
			ГоловнаяОрганизация = ОбщегоНазначенияЗК.ГоловнаяОрганизация(Отчет.Организация);
			СписокОбособленныхПодразделенийОрганизации = ОбщегоНазначенияЗК.ПолучитьСписокОбособленныхПодразделенийОрганизации(ГоловнаяОрганизация);
