...
#КонецЕсли		
	    //начало изменений Ожиганов 31.07.2015 Обмен МСФО
		//СоединенияИБ - серверный модуль, пэтому передаем не определено
		//Возврат СоединенияИБ.КоличествоСеансовИнформационнойБазы(Ложь, КлиентскиеСообщения) <= 1;
		Возврат СоединенияИБ.КоличествоСеансовИнформационнойБазы(Ложь, Неопределено) <= 1;
		//конец изменений 
	Исключение
		ЗаписатьСобытие(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), "Ошибка");
		Возврат Ложь;
