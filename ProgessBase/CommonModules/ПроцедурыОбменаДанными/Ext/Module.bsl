...
Процедура ВыполнитьОбменПоНастройкеАвтоматическогоОбмена(НастройкаОбменаДанными, РучнойЗапускОбменов = Истина, 
	...
	Для Каждого СтрокаТабЧасти Из ОбъектНастройки.НастройкиОбмена Цикл
		...
		Иначе
			...
			//начало изменений Ожиганов 30.07.2015 обмен МСФО
			ИначеЕсли ВыполняемоеДействие = Перечисления.ДействиеПриОбмене.ЗапускПриложения Тогда
				
				Если Не ПустаяСтрока(ОбъектНастройки.ПРГ_СтрокаЗапускаПриложения) Тогда
					ЗапуститьПриложение(СокрЛП(ОбъектНастройки.ПРГ_СтрокаЗапускаПриложения),,Истина);
				КонецЕсли;
				
				продолжить;
				
			ИначеЕсли ВыполняемоеДействие = Перечисления.ДействиеПриОбмене.ЗавершитьРаботуПрограммы Тогда
			...
			//начало изменений Ожиганов 01.08.2015 обмен С МСФО
			#Если Клиент Тогда
			ПРГ_КонфигурацияПриОбменеРИБИзменена = Ложь;
			СтруктураНастроекОбмена.Свойство("КонфигурацияПриОбменеРИБИзменена", ПРГ_КонфигурацияПриОбменеРИБИзменена);
			Если ПРГ_КонфигурацияПриОбменеРИБИзменена <> Неопределено 
				и ПРГ_КонфигурацияПриОбменеРИБИзменена = Истина Тогда
				ЗавершитьРаботуСистемы(Ложь);
				//начало изменений Ожиганов 06.08.2015 
				// т.к. запуск осуществляется в обработчике ожидание, ЗавершитьРаботуСистемы не срабатывает сразу.
				возврат;
				//конец изменений 
			КонецЕсли;	
			//конец изменений 
