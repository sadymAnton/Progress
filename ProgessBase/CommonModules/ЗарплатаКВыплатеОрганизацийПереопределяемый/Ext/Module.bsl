...
Процедура ПровестиВзаиморасчеты(Документ, Отказ, Заголовок, РегистрироватьТолькоНДФЛ = Ложь) Экспорт
	...
	Если Не РегистрироватьТолькоНДФЛ Тогда
		...
		Для Каждого СтрокаЗарплаты Из Документ.Зарплата Цикл
			...
			/////Вадим 01.02.2013 13:09:17
			//СтрокаДвижения.Период       		= Документ.Дата;
			Если ТипЗнч(Документ)=тип("ДокументОбъект.ЗарплатаКВыплатеОрганизаций") тогда
				СтрокаДвижения.Период       		= Документ.ПериодРегистрации;
			иначе
				СтрокаДвижения.Период       		= Документ.Дата;
			Конецесли;
			////ВадимКонец
	...
	СтруктураДокумента.Вставить("ХарактерВыплаты", Документ.ХарактерВыплаты); //Blik 200216 49388
...
Процедура ПроверитьФизлицаДляАвтозаполнения(ДокументОбъект, Физлица, Отказ, Заголовок) Экспорт
		...
		если ДокументОбъект.СпособВыплаты=Перечисления.СпособыВыплатыЗарплаты.ЧерезКассу и ДокументОбъект.ХарактерВыплаты=Перечисления.ХарактерВыплатыЗарплаты.ПлановыйАванс тогда
		Запрос = Новый Запрос;
		Запрос.Текст ="ВЫБРАТЬ
					  ...
					  |) КАК ВложенныйЗапрос
		              |ГДЕ
		              |	ВложенныйЗапрос.НомерЛицевогоСчета ЕСТЬ NULL ";	
		Запрос.УстановитьПараметр("дата", НачалоДня(ДокументОбъект.Дата)-1);
	...
	иначеЕсли ДокументОбъект.СпособВыплаты = Перечисления.СпособыВыплатыЗарплаты.ЧерезБанк Тогда
	
		Запрос = Новый Запрос;
				///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Выбор всех физлиц с личными счетами, если банк не указан
		//вадим
		...
		|			ПО ЛицевыеСчетаРаботниковОрганизации.ФизЛицо = РаботникиОрганизацийСрезПоследних.Сотрудник.Физлицо
		|	ГДЕ
		|		ВЫБОР
		|				КОГДА &Банк = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
		|					ТОГДА ЛицевыеСчетаРаботниковОрганизации.AVT_ЛичныеРеквизиты
		|				ИНАЧЕ ЛицевыеСчетаРаботниковОрганизации.Банк = &Банк
		|			КОНЕЦ
		|		И (&ПоВсемФизлицам
		|				ИЛИ ЛицевыеСчетаРаботниковОрганизации.ФизЛицо В (&Физлица))
		|	И"+?(не ЗначениеЗаполнено(ДокументОбъект.Банк),""," не ")+" ЛицевыеСчетаРаботниковОрганизации.AVT_ЛичныеРеквизиты
		|) КАК ВложенныйЗапрос
		|ГДЕ
...
///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Не найдены лицевые счета сотрудников" + ?(ЗначениеЗаполнено(ДокументОбъект.Банк)," в банке " +"«"+ СокрЛП(ДокументОбъект.Банк) +"»","")+ "!", Отказ, Заголовок);
////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}
		КонецЕсли;
...
Функция ТипОснованияПоХарактеруВыплаты(ХарактерВыплаты) Экспорт
	...
	ИначеЕсли ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.ОплатаПразднИВых Тогда //Blik 120713 			
		ТипОснования = Тип("ДокументСсылка.ОплатаПраздничныхИВыходныхДнейОрганизаций") 
	Иначе 
...
///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
// Не проверять заполнение реквизита "Банк"	
	// Банк
