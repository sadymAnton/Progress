...
Функция ПодготовитьСтруктуруСчетовУчета()
	...
	//начало изменений
	СтруктураСчетовУчета.Вставить("СубконтоДох");
	СтруктураСчетовУчета.Вставить("СубконтоРасх");
	//конец изменений
...
Функция ПолучитьСчетаУчетаНоменклатурыИзРегистраСведений(Организация, Номенклатура, Склад = Неопределено)
	...
	//начало изменеий
	|	СчетаУчетаНоменклатуры.СубконтоДох КАК СубконтоДох,
	|	СчетаУчетаНоменклатуры.СубконтоРасх КАК СубконтоРасх,
	//конец изменений
	...
	Если ТаблицаЗапроса.Количество() > 0 Тогда
		...
		//начало изменений 
		СтруктураВозврата.СубконтоДох 	= СведенияОСчетах.СубконтоДох;
		СтруктураВозврата.СубконтоРасх	= СведенияОСчетах.СубконтоРасх;
		//конец изменений
...
Процедура УстановитьСчетаУчета(ЗаполняемыйОбъект, ТаблицаРеквизитов, СтруктураСчетовУчета, ХозяйственнаяСитуация, ДанныеСклада = Неопределено, РозничныйСклад=Неопределено, ЗаполнятьБУ=Истина, ЗаполнятьНУ=Истина, Перезаполнять=Ложь, ЗаполнятьКлоныКолонок=Истина)
	...
	Для Каждого ДанныеРеквизита Из СтрокиРеквизитов Цикл
		...
		//начало изменений
		Если Реквизит = "СубконтоБУРасходов" Тогда
			Если ТаблицаРеквизитов.Найти("СчетРасходовБУ", "Реквизит")<>Неопределено Тогда
				Субконто.Вставить(Реквизит, Новый Структура("Счет,РеквизитНоменклатура","СчетРасходовБУ",ДанныеРеквизита.Номенклатура));
			КонецЕсли;
		КонецЕсли;
		
		Если Реквизит = "СубконтоНУРасходов" Тогда
			Если ТаблицаРеквизитов.Найти("СчетРасходовНУ", "Реквизит")<>Неопределено Тогда
				Субконто.Вставить(Реквизит, Новый Структура("Счет,РеквизитНоменклатура","СчетРасходовНУ",ДанныеРеквизита.Номенклатура));
			КонецЕсли;
		КонецЕсли;
		//конец изменений 
	...
	Для Каждого ЭлементСубконто Из Субконто Цикл
		...
		Если ВидыСубконто.Количество() > 0 Тогда
			...
			ИначеЕсли ТипЗнч(ЗначениеСубконто) = Тип("СправочникСсылка.ПрочиеДоходыИРасходы") Тогда
				Если ЭлементСубконто.Ключ = "СубконтоБУ" или ЭлементСубконто.Ключ = "СубконтоНУ" Тогда
					ЗначениеСубконто = СтруктураСчетовУчета.СубконтоДох;
				иначеЕсли ЭлементСубконто.Ключ = "СубконтоБУРасходов" или ЭлементСубконто.Ключ = "СубконтоНУРасходов" Тогда
					ЗначениеСубконто = СтруктураСчетовУчета.СубконтоРасх;
				КонецЕсли;	
			//конец изменений
			КонецЕсли;
...
Функция ОпределитьХозяйственнуюСитуациюПоДокументу(ИмяТабличнойЧасти, ИмяДокумента, СтруктураШапкиДокумента)
	...
	ИначеЕсли ИмяДокумента = "ТребованиеНакладная" Тогда
		//начало изменений 
		//Если СтруктураШапкиДокумента.СчетаЗатратВШапке и Не СтруктураШапкиДокумента.ВидОперацииПодбораПодразделений = Перечисления.ПРГ_ВидыПодбораПодразделений.ПодразделенияВТабличнойЧасти Тогда
			Возврат "Другое";
		//Иначе
		//	возврат "";
		//КонецЕсли;	
	...
	ИначеЕсли (ИмяДокумента = "ОтчетПроизводстваЗаСмену") и 
		(ИмяТабличнойЧасти = "Материалы" или ИмяТабличнойЧасти = "РаспределениеМатериалов") Тогда
