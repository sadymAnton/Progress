...
Процедура ПроверитьЗаполнениеСтрокиДохода(ВыборкаПоДоходам, Отказ, Заголовок = "") Экспорт
	...
	Если Не ЗначениеЗаполнено(ВыборкаПоДоходам.КодДоходаЕСН) Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "Не указан вид дохода по страховым взносам!", Отказ, Заголовок);
	КонецЕсли;
...
Функция ПолучитьТекстЗапросаОтражениеСторноЗаписей()
	...
	|			ЕСТЬNULL(СоответствиеКодов.ВидПоСтраховымВзносам, ЕСНОсновныеНачисления.КодДоходаЕСН) КАК КодДоходаЕСН,
...
Функция ПолучитьДанныеУчета2010(ВыборкаПоШапкеДокумента, ВыделятьЕНВД, СписокНачислений, ОрганичениеНаСотрудников, ЭтоОсновныеНачисления, ЭтоПервичныеНачисления)
	...
	Если ЭтоОсновныеНачисления Тогда
		...
		|			ТОГДА 1
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ОсновныеНачисления.МесячноеПособие > (ВЫРАЗИТЬ(МаксимумПособияПоУходуЗаРебенкомДоПолутораЛет.РазмерФСС * 0.4 / 12 КАК ЧИСЛО(15, 2)))
		|					ТОГДА ВЫРАЗИТЬ(МаксимумПособияПоУходуЗаРебенкомДоПолутораЛет.РазмерФСС * 0.4 / 12 КАК ЧИСЛО(15, 2))
		...
		|	ЕСТЬNULL(СоответствиеКодов.ВидПоСтраховымВзносам, ЕСТЬNULL(База.КодДоходаЕСНРазрез, БазаСреднегоЗаработка.КодДоходаЕСНРазрез)) КАК КодДоходаЕСНРазрез,
    ...
    Иначе
		...
		Если КоличествоСторноЗаписей <> 0 Тогда
			...
			|			ЕСТЬNULL(СоответствиеКодов.ВидПоСтраховымВзносам, ЕСНДополнительныеНачисления.КодДоходаЕСН) КАК КодДоходаЕСН,
	...
	Пока ПродолжатьОбработку Цикл
		...
		ИначеЕсли ДанныеУчета.ЭтоПособияФСС Тогда
			...
			Иначе 
				...
				СтрокаУчетаПособий.СуммаВсего = СтрокаДанных.Результат;
				СтрокаУчетаПособий.СуммаСверхНорм = ДанныеУчета.ДополнительныйРезультат;
				Если СтрокаУчетаПособий.ВидПособияСоциальногоСтрахования = Перечисления.ВидыПособийСоциальногоСтрахования.ДополнительныеВыходныеДниПоУходуЗаДетьмиИнвалидами Тогда
					СтрокаУчетаПособий.ОплаченныеДни = ДанныеУчета.НормаДней
				Иначе
					СтрокаУчетаПособий.ОплаченныеДни = ?(ДанныеУчета.Сторно,-ДанныеУчета.ВсегоДнейНетрудоспособности,ДанныеУчета.ВсегоДнейНетрудоспособности);
				КонецЕсли;
				СтрокаУчетаПособий.ДатаСтраховогоСлучая = ДанныеУчета.ДатаНачалаСобытия;
...
Функция ПолучитьДанныеУчета2011(ВыборкаПоШапкеДокумента, ВыделятьЕНВД, СписокНачислений, ОрганичениеНаСотрудников, ЭтоОсновныеНачисления, ЭтоПервичныеНачисления)
	...
	Если ЭтоОсновныеНачисления Тогда
		...
		|				ИЛИ ОсновныеНачисления.ВариантОбработкиЗаписиПриОтраженииВРеглУчете В (ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйТравмаНаПроизводстве), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйТравмаНаПроизводствеНовыйСтраховойСлучай), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйОбщееЗаболевание), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйОбщееЗаболеваниеНовыйСтраховойСлучай), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйПрочий), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйПрочийНовыйСтраховойСлучай), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.ОтпускПоБеременностиИРодамПоУходуЗаРебенком), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.ОтпускПоБеременностиИРодамПоУходуЗаРебенкомНовыйСтраховойСлучай)))
		|			И (ВЫБОР
		|				КОГДА ОсновныеНачисления.ВариантОбработкиЗаписиПриОтраженииВРеглУчете В (ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйТравмаНаПроизводстве), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйТравмаНаПроизводствеНовыйСтраховойСлучай), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйОбщееЗаболевание), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйОбщееЗаболеваниеНовыйСтраховойСлучай), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйПрочий), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйПрочийНовыйСтраховойСлучай), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.ОтпускПоБеременностиИРодамПоУходуЗаРебенком), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.ОтпускПоБеременностиИРодамПоУходуЗаРебенкомНовыйСтраховойСлучай), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйЗаСчетРаботодателя), ЗНАЧЕНИЕ(Перечисление.ВариантыОбработкиЗаписиПриОтраженииВРеглУчете.БольничныйЗаСчетРаботодателяНовыйСтраховойСлучай))
		|					ТОГДА ОсновныеНачисления.ПериодРегистрации
		|				КОГДА ОсновныеНачисления.ВидРасчета.СтратегияОтраженияВУчете = ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВРеглУчетеНачислений.КакЗаданоНаНачалоСобытия)
		|					ТОГДА ВЫБОР
		|							КОГДА НАЧАЛОПЕРИОДА(ОсновныеНачисления.ДатаНачалаСобытия, МЕСЯЦ) > ОсновныеНачисления.ПериодРегистрации
		...
		|						И ОсновныеНачисления.МесячноеПособие > (ВЫРАЗИТЬ(МаксимумПособияПоУходуЗаРебенкомДоПолутораЛет.РазмерФСС * 0.4 / 12 КАК ЧИСЛО(15, 2)))
		|					ТОГДА ВЫРАЗИТЬ(МаксимумПособияПоУходуЗаРебенкомДоПолутораЛет.РазмерФСС * 0.4 / 12 КАК ЧИСЛО(15, 2))
		|				ИНАЧЕ ОсновныеНачисления.МесячноеПособие
		|			КОНЕЦ * ВЫБОР
		|				КОГДА ОсновныеНачисления.ПериодДействияНачало = ОсновныеНачисления.ПериодДействия
...
//////////////////////////////////////////////////////////////////////////////////
// алгоритмы с 2013 года

Функция ПолучитьДанныеУчета2013(ВыборкаПоШапкеДокумента, ВыделятьЕНВД, СписокНачислений, ОрганичениеНаСотрудников, ЭтоОсновныеНачисления, ЭтоПервичныеНачисления, УчитыватьСуществующиеДанные)
	...
	МассивАвторскихДоходов.Добавить(Справочники.ДоходыПоСтраховымВзносам.АвторскиеОткрытия);
	МассивАвторскихДоходов.Добавить(Справочники.ДоходыПоСтраховымВзносам.АвторскиеСкульптуры);
	МассивАвторскихДоходов.Добавить(Справочники.ДоходыПоСтраховымВзносам.АвторскиеАудиовизуальныеПроизведенияОблагаемыеФСС_НС);
	МассивАвторскихДоходов.Добавить(Справочники.ДоходыПоСтраховымВзносам.АвторскиеГрафическиеПроизведенияОблагаемыеФСС_НС);
	МассивАвторскихДоходов.Добавить(Справочники.ДоходыПоСтраховымВзносам.АвторскиеДругиеМузыкальныеПроизведенияОблагаемыеФСС_НС);
	МассивАвторскихДоходов.Добавить(Справочники.ДоходыПоСтраховымВзносам.АвторскиеИсполнениеПроизведенийОблагаемыеФСС_НС);
	МассивАвторскихДоходов.Добавить(Справочники.ДоходыПоСтраховымВзносам.АвторскиеЛитературныеПроизведенияОблагаемыеФСС_НС);
	МассивАвторскихДоходов.Добавить(Справочники.ДоходыПоСтраховымВзносам.АвторскиеМузыкальноСценическиеПроизведениеОблагаемыеФСС_НС);
	МассивАвторскихДоходов.Добавить(Справочники.ДоходыПоСтраховымВзносам.АвторскиеНаучныеТрудыОблагаемыеФСС_НС);
	МассивАвторскихДоходов.Добавить(Справочники.ДоходыПоСтраховымВзносам.АвторскиеОткрытияОблагаемыеФСС_НС);
	МассивАвторскихДоходов.Добавить(Справочники.ДоходыПоСтраховымВзносам.АвторскиеСкульптурыОблагаемыеФСС_НС);
	Запрос.УстановитьПараметр("ДоходыАвторские", МассивАвторскихДоходов);
	...
	Если ЭтоОсновныеНачисления Тогда
		...
		|	ОсновныеНачисления.Результат,
		|	ВЫБОР
		|		КОГДА ОсновныеНачисления.ПодразделениеОрганизации.КодПоОКТМО <> """"
		|				ИЛИ ОсновныеНачисления.ПодразделениеОрганизации.КодПоОКАТО <> """"
		|			ТОГДА ВЫБОР
		|					КОГДА ОсновныеНачисления.ПодразделениеОрганизации.РайонныйКоэффициентРФ > 1
		|						ТОГДА ОсновныеНачисления.ПодразделениеОрганизации.РайонныйКоэффициентРФ
		|					ИНАЧЕ 1
		...
		|			КОНЕЦ * ВЫБОР
		|				КОГДА ОсновныеНачисления.ПериодДействияНачало = ОсновныеНачисления.ПериодДействия
		|						И КОНЕЦПЕРИОДА(ОсновныеНачисления.ПериодДействияКонец, ДЕНЬ) = КОНЕЦПЕРИОДА(ОсновныеНачисления.ПериодДействия, МЕСЯЦ)
		|					ТОГДА 1
		|				ИНАЧЕ РАЗНОСТЬДАТ(ОсновныеНачисления.ПериодДействияНачало, ДОБАВИТЬКДАТЕ(ОсновныеНачисления.ПериодДействияКонец, СЕКУНДА, 1), ДЕНЬ) / РАЗНОСТЬДАТ(ОсновныеНачисления.ПериодДействия, ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(ОсновныеНачисления.ПериодДействия, МЕСЯЦ), СЕКУНДА, 1), ДЕНЬ)
		|			КОНЕЦ * ВЫБОР
		|				КОГДА ОсновныеНачисления.Сторно
		...
		Если Не УчитыватьСуществующиеДанные Тогда
			ЗапросПерерасчета = Новый Запрос;
			ЗапросПерерасчета.МенеджерВременныхТаблиц = Запрос.МенеджерВременныхТаблиц;
			ЗапросПерерасчета.Текст = 
        	"ВЫБРАТЬ
        	|	НЕОПРЕДЕЛЕНО КАК Сотрудник,
        	|	НЕОПРЕДЕЛЕНО КАК ВидРасчета,
        	|	НЕОПРЕДЕЛЕНО КАК ДокументОснование,
        	|	ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0) КАК ПериодРегистрации,
        	|	ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0) КАК ПериодДействияНачало,
        	|	ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0) КАК ПериодДействияКонец
        	|ПОМЕСТИТЬ ВТЕСНОсновныеНачисления
        	|ГДЕ
        	|	ЛОЖЬ";
			ЗапросПерерасчета.Выполнить();
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РегистрРасчета.ЕСНОсновныеНачисления", "ВТЕСНОсновныеНачисления")	
		КонецЕсли;
	...
	Иначе
		...
		Если Не УчитыватьСуществующиеДанные Тогда
        	...
        	|	НЕОПРЕДЕЛЕНО КАК ДокументОснование,
        	|	ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0) КАК ПериодРегистрации
        	|ПОМЕСТИТЬ ВТЕСНДополнительныеНачисления
        	|ГДЕ
        	|	ЛОЖЬ";
			ЗапросПерерасчета.Выполнить();
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РегистрРасчета.ЕСНДополнительныеНачисления", "ВТЕСНДополнительныеНачисления")	
		КонецЕсли;
...
Функция ПолучитьДанныеУчета(ВыборкаПоШапкеДокумента, ВыделятьЕНВД, СписокНачислений, ОрганичениеНаСотрудников, ЭтоОсновныеНачисления, ЭтоПервичныеНачисления, УчитыватьСуществующиеДанные)
	...
	ИначеЕсли ВыборкаПоШапкеДокумента.ПериодРегистрации < ДобавитьМесяц(ПроведениеРасчетов.ДатаПереходаНаКодыОКТМО(), -12) Тогда
	...
	Иначе
		Возврат ПолучитьДанныеУчета2013(ВыборкаПоШапкеДокумента, ВыделятьЕНВД, СписокНачислений, ОрганичениеНаСотрудников, ЭтоОсновныеНачисления, ЭтоПервичныеНачисления, УчитыватьСуществующиеДанные);
	КонецЕсли;
...
Функция СоздатьСтрокуДвижений(ВыборкаПоШапкеДокумента, ДанныеУчета, НаборЗаписей, ЭтоОсновныеНачисления)
	...
	Иначе
		...
		СтрокаДанных.ДатаНачисления			= ДанныеУчета.ПериодРегистрации;
	КонецЕсли;
...
//
Функция АвтозаполнениеНаборовЗаписейОДоходах(ДокументОбъект, ВыборкаПоШапкеДокумента, НаборЗаписейОсновной, НаборЗаписейДополнительный, ОрганичениеНаСотрудников, ТаблицаОшибок, УчитыватьСуществующиеДанные = Истина) Экспорт
	...
	Для Счетчик = 0 По ПорядкиРасчета.ВГраница() Цикл
		...
		Если СписокНачисленийДляОтбора.Количество() > 0 Тогда
			...
			ДанныеУчета = ПолучитьДанныеУчета(ВыборкаПоШапкеДокумента, ЕНВД, СписокНачисленийДляОтбора, ОрганичениеНаСотрудников, Истина, ?(Счетчик = 0, Истина, Ложь), УчитыватьСуществующиеДанные);
		...
		Если СписокНачисленийДляОтбора.Количество() > 0 Тогда
			//получим и обработаем данные учета
			ДанныеУчета = ПолучитьДанныеУчета(ВыборкаПоШапкеДокумента, ЕНВД, СписокНачисленийДляОтбора, ОрганичениеНаСотрудников, Ложь, ?(Счетчик = 0, Истина, Ложь), УчитыватьСуществующиеДанные);
