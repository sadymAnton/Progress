...
//начало изменений БП 14
 ...
 Функция СформироватьТекстЗапросаБазаНормыГаз()
	...
	//-ДС		
	|	//ДляНалУчета СУММА(ВыпускПродукции.Количество * НормыПотребления.Газ 
	|	//ДляНалУчета ) КАК БазаНУ,
	|	//ДляНалУчета СУММА(ВыпускПродукции.Количество * НормыПотребления.Газ 
	|	//ДляНалУчета 	) КАК БазаПриходНУ,
	|	//ДляНалУчета 0 КАК БазаОстатокНЗПНУ,
	|
	|	СУММА(
	|		ВыпускПродукции.Количество * НормыПотребления.Газ 
    |		) КАК База,
	|	СУММА(
	|		ВыпускПродукции.Количество * НормыПотребления.Газ 
	|		) КАК БазаПриход,
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ПРГ_НормыПотребленияПоЛиниям.СрезПоследних(&КонДата) КАК НормыПотребления // заменяется на временную таблицу Продажи
	|	ПО ВыпускПродукции.Организация = НормыПотребления.Организация
	|		И ВыпускПродукции.Подразделение = НормыПотребления.Подразделение
	|		И ВыпускПродукции.НоменклатурнаяГруппа = НормыПотребления.НоменклатурнаяГруппа	
	|
	|{ГДЕ
	|	//ДляУпрУчета ВыпускПродукции.Подразделение.* КАК Подразделение,
	|	//ДляРеглУчета ВыпускПродукции.Подразделение.* КАК ПодразделениеОрганизации,
...
КонецФункции // СформироватьТекстЗапросаБазаФактическаяЦенаРеализации()
 ...
 Функция СформироватьТекстЗапросаБазаНормыЭлектроэнергия()
	...
	|	//ДляМеждУчета 0 КАК БазаОстатокНЗПНУ,
	//-ДС 14.03.14		
	|	//ДляНалУчета СУММА(ВыпускПродукции.Количество * НормыПотребления.Электричество 
	|	//ДляНалУчета ) КАК БазаНУ,
	|	//ДляНалУчета СУММА(ВыпускПродукции.Количество * НормыПотребления.Электричество 
	|	//ДляНалУчета 	) КАК БазаПриходНУ,
	|	//ДляНалУчета 0 КАК БазаОстатокНЗПНУ,
	|
	|	СУММА(
	|		ВыпускПродукции.Количество * НормыПотребления.Электричество 
    |		) КАК База,
	|	СУММА(
	|		ВыпускПродукции.Количество * НормыПотребления.Электричество 
	|		) КАК БазаПриход,
...
КонецФункции // СформироватьТекстЗапросаБазаФактическаяЦенаРеализации()
 ...
 Функция СформироватьТекстЗапросаБазаНормыВода()
	...
	|	//ДляМеждУчета 0 КАК БазаОстатокНЗПНУ,
	//-ДС	
	|	//ДляНалУчета СУММА(ВыпускПродукции.Количество * НормыПотребления.Вода 
	|	//ДляНалУчета ) КАК БазаНУ,
	|	//ДляНалУчета СУММА(ВыпускПродукции.Количество * НормыПотребления.Вода 
	|	//ДляНалУчета 	) КАК БазаПриходНУ,
	|	//ДляНалУчета 0 КАК БазаОстатокНЗПНУ,
	|
	|	СУММА(
	|		ВыпускПродукции.Количество * НормыПотребления.Вода 
    |		) КАК База,
	|	СУММА(
	|		ВыпускПродукции.Количество * НормыПотребления.Вода 
	|		) КАК БазаПриход,
...
Функция СформироватьБазуРаспрКосвенныхНаша()
	ТекстЗапроса = "ВЫБРАТЬ
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.СпособРаспределенияЗатрат,
	               |//ДляРеглУчета	БазаРаспределенияЗатратБухгалтерскийУчет.Организация,
	               |//ДляРеглУчета	БазаРаспределенияЗатратБухгалтерскийУчет.СчетУчета,
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.Подразделение Как ПодразделениеНЗП,
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.НоменклатурнаяГруппа Как НоменклатурнаяГруппаНЗП,
				   ...
				   // is ЯннуровВФ нач 20140611 Ошибка при налоговом учете
				   ...
				   |	//ДляМеждУчета 0 КАК БазаОстатокНЗПНУ,
				   //-ДС	
				   |	//ДляНалУчета СУММА(БазаРаспределенияЗатратБухгалтерскийУчет.База
				   |	//ДляНалУчета ) КАК БазаНУ,
				   |	//ДляНалУчета СУММА(БазаРаспределенияЗатратБухгалтерскийУчет.БазаПриход
				   |	//ДляНалУчета 	) КАК БазаПриходНУ,
				   |	//ДляНалУчета 0 КАК БазаОстатокНЗПНУ,
	               | 	СУММА(БазаРаспределенияЗатратБухгалтерскийУчет.БазаПриход) КАК БазаПриход,
	               | 	СУММА(БазаРаспределенияЗатратБухгалтерскийУчет.База) КАК База,
	               |    СУММА(0) КАК БазаОстатокНЗП,
				   //начало изменений исправление косяков Инфосьюта
				   //|  ЗНАЧЕНИЕ(Перечисление.ВидыВыпуска.Выпуск) КАК ВидВыпуска
				   //конец изменений 
				    //+ДС 14.03.14
					// is ЯннуровВФ нач 20141114
				   //| //ДляМеждУчета	СУММА(БазаРаспределенияЗатратБухгалтерскийУчет.БазаПриход) КАК БазаПриход,
				   //| //ДляМеждУчета	СУММА(БазаРаспределенияЗатратБухгалтерскийУчет.База) КАК База,
				   //| //ДляМеждУчета	СУММА(0) КАК БазаОстатокНЗП,
				   ...
				   |//ДляБухУчета	РегистрСведений.БазаРаспределенияЗатратБухгалтерскийУчет КАК БазаРаспределенияЗатратБухгалтерскийУчет
				   //начало изменений исправление косяков Инфосьюта
                   |//ДляНалУчета	РегистрСведений.БазаРаспределенияЗатратНалоговыйУчет КАК БазаРаспределенияЗатратБухгалтерскийУчет
                   |//ДляМеждУчета	РегистрСведений.БазаРаспределенияЗатратМеждународныйУчет КАК БазаРаспределенияЗатратБухгалтерскийУчет
				   //-ДС
	               |ГДЕ
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.Организация = &Организация
	               ...
	               |	И БазаРаспределенияЗатратБухгалтерскийУчет.Период МЕЖДУ &НачДата И &КонДата
	               |{ГДЕ
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.Подразделение.*,
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.Подразделение.* КАК ПодразделениеОрганизации,
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.НоменклатурнаяГруппа.*}
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.СпособРаспределенияЗатрат,
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.Организация,
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.Подразделение,
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.СчетУчета,
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.НоменклатурнаяГруппа,
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.Заказ,
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.Продукция,
	               |	БазаРаспределенияЗатратБухгалтерскийУчет.ХарактеристикаПродукции,
...
Функция ПолучитьПостроительЗапросаДляСпособаРаспределенияЗатрат(
	...
	Если Не СтруктураПостроителиЗапроса.Свойство(ИмяПостроителя, ПостроительЗапроса) Тогда
		...
		ТекстЗапросаСКомментариями = "";
		//начало изменений 
		Если Константы.ПРГ_СпособРаспределнияДляПрямыхЗатрат.Получить() =  СтруктураПараметров.СпособРаспределенияЗатрат Тогда
			    ТекстЗапросаСКомментариями  = СформироватьБазуРаспрКосвенныхНаша();
		ИначеЕсли СтруктураПараметров.БазаРаспределенияЗатрат = Перечисления.БазыРаспределенияЗатрат.ПоСтоимостиЗатрат Тогда
			Если СтруктураПараметров.ИспользоватьРасширеннуюАналитику Тогда
		...
		ИначеЕсли СтруктураПараметров.БазаРаспределенияЗатрат = Перечисления.БазыРаспределенияЗатрат.ПоНормативам Тогда
			...
			КонецЕсли;
		//начало изменений БП 14
		ИначеЕсли СтруктураПараметров.БазаРаспределенияЗатрат = Перечисления.БазыРаспределенияЗатрат.НормыГаз  Тогда
			Если СтруктураПараметров.ИспользоватьРасширеннуюАналитику Тогда
			Иначе
				ТекстЗапросаСКомментариями = СформироватьТекстЗапросаБазаНормыГаз();
			КонецЕсли;	
		ИначеЕсли СтруктураПараметров.БазаРаспределенияЗатрат = Перечисления.БазыРаспределенияЗатрат.НормыВоды Тогда
			Если СтруктураПараметров.ИспользоватьРасширеннуюАналитику Тогда
			Иначе
				ТекстЗапросаСКомментариями = СформироватьТекстЗапросаБазаНормыВода();
			КонецЕсли;	
		ИначеЕсли СтруктураПараметров.БазаРаспределенияЗатрат = Перечисления.БазыРаспределенияЗатрат.НормыЭлетроэнергия Тогда
			Если СтруктураПараметров.ИспользоватьРасширеннуюАналитику Тогда
			Иначе
				ТекстЗапросаСКомментариями = СформироватьТекстЗапросаБазаНормыЭлектроэнергия();
			КонецЕсли;	
		//конец изменений БП 14
...
Процедура ЗаполнитьОтборПостоителяЗапроса(
	...
	Если Не СтруктураПараметров.ОтражатьВМеждународномУчете Или
	// is Зайцева нач 20140711 
	 (СтруктураПараметров.Свойство("ПодразделениеОрганизации") и ЗначениеЗаполнено(СтруктураПараметров.ПодразделениеОрганизации)) Тогда
	// Или ЗначениеЗаполнено(СтруктураПараметров.ПодразделениеОрганизации) Тогда  
...
Функция СформироватьВременнуюТаблицуБазаРаспределенияЗатрат(
	...
	БазаРассчитана = Ложь;
	//начало изменений
	//для косвенных считается в другом месте
	Если Не РаспределениеКосвенныхЗатрат и  
		Константы.ПРГ_СпособРаспределнияДляПрямыхЗатрат.Получить() =  СтруктураПараметров.СпособРаспределенияЗатрат Тогда
		возврат Ложь;
	КонецЕсли;	
	//конец изменений 
...
Процедура РассчитатьБазуРаспределения(
	...
	Если БазаРассчитана Тогда
		...
		Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете или СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			ЗапросНеучПодр = Новый Запрос("ВЫБРАТЬ
			...
			ЗапросНеучПодр.УстановитьПараметр("Организация",СтруктураШапкиДокумента.Организация);					  
			ВыборкаНеучПодр = ЗапросНеучПодр.Выполнить().Выбрать();
			Пока ВыборкаНеучПодр.Следующий() Цикл
				 МассивПодр.Добавить(ВыборкаНеучПодр.Подразделение);
			КонецЦикла;	
		КонецЕсли;	
		//конец изменений 	
		...
		Пока Выборка.Следующий() Цикл
			...
			//начало изменений 
			Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете или СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
				Если МассивПодр.Найти(Выборка.ПодразделениеНЗП) <> Неопределено Тогда
					  продолжить;
				КонецЕсли;	
			КонецЕсли;	
			//конец изменений 
...
Функция СформироватьЗапросПоИспользуемымСпособамРаспределения(
	...
	|//ДляРеглУчета И (&РаспределятьКосвенные)
   	|//ДляРеглУчета 
	...
	Запрос.УстановитьПараметр("РаспределятьКосвенные",РаспределениеКосвенныхЗатрат);
...
Функция СформироватьПустуюТаблицуБазыРаспределения()
	...
	ТаблицаРезультата.Колонки.Добавить("СчетЗатратМУ",					Новый ОписаниеТипов("ПланСчетовСсылка.Международный"));
	...
	// is ЯннуровВФ нач 20140708
	ТаблицаРезультата.Колонки.Добавить("ОтражатьВМеждународномУчете", 		Новый ОписаниеТипов("Булево"));
	// is ЯннуровВФ кон 20140708
	...
	// is ЯннуровВФ нач 20140708
	ТаблицаРезультата.Колонки.Добавить("БазаМежд",Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
	ТаблицаРезультата.Колонки.Добавить("БазаПриходМежд",Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
	ТаблицаРезультата.Колонки.Добавить("БазаОстатокНЗПМежд",Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
	// is ЯннуровВФ кон 20140708
...
Функция СформироватьТекстЗапросаПоБазамРаспределения()
	...
	// is ЯннуровВФ нач 20140708
	|	БазаРаспределенияЗатрат.СчетЗатратМУ,
	// is ЯннуровВФ кон 20140708
	|	
	// is ЯннуровВФ нач 20140708
	//|	ВЫБОР КОГДА БазаРаспределенияЗатрат.ИндексУчета = 3 ТОГДА
	|	ВЫБОР КОГДА БазаРаспределенияЗатрат.ИндексУчета = 4 ТОГДА
	// is ЯннуровВФ кон 20140708
	|		БазаРаспределенияЗатрат.База
	|	ИНАЧЕ
	|		0
	...
	|	ИНАЧЕ
	|		0
	|	КОНЕЦ КАК БазаПриходМежд,
	|	ВЫБОР КОГДА БазаРаспределенияЗатрат.ИндексУчета = 1 ТОГДА
	|		БазаРаспределенияЗатрат.БазаОстатокНЗП
	|	ИНАЧЕ
	|		0
	|	КОНЕЦ КАК БазаОстатокНЗПМежд
	// is ЯннуровВФ кон 20140708
 	|
	|ИЗ (
	// is ЯннуровВФ нач 20140708
	...
	|		Неопределено КАК БазаПриход,
	|		Неопределено КАК БазаОстатокНЗП
	|
	|	//ДляУпрУчета ОБЪЕДИНИТЬ ВСЕ
	|
	|	//ДляУпрУчета ВЫБРАТЬ
	|	//ДляУпрУчета 	4 КАК ИндексУчета,
	|	//ДляУпрУчета 	Подразделение КАК Подразделение,
	|	//ДляУпрУчета 	Неопределено КАК ПодразделениеОрганизации,
	|	//ДляУпрУчета 	Неопределено КАК СчетЗатрат,
	|	//ДляУпрУчета 	Неопределено КАК СчетЗатратНУ,
	|	//ДляУпрУчета 	Неопределено КАК СчетЗатратМУ,
	|	//ДляУпрУчета 	Организация,
	|	//ДляУпрУчета 	Продукция,
	|	//ДляУпрУчета 	ХарактеристикаПродукции,
	|	//ДляУпрУчета 	СерияПродукции,
	|	//ДляУпрУчета 	Заказ,
	|	//ДляУпрУчета 	НоменклатурнаяГруппа,
	|	//ДляУпрУчета 	Спецификация,
	|	//ДляУпрУчета 	ВариантВыпускаПродукции,
	|	//ДляУпрУчета 	ВидВыпуска,
	|	//ДляУпрУчета 	База,
	|	//ДляУпрУчета 	БазаПриход,
	|	//ДляУпрУчета 	БазаОстатокНЗП
	|   //ДляУпрУчета ИЗ
	|	//ДляУпрУчета 	БазаРаспределения
	|
	...
	|
	|	//ДляБухУчета ВЫБРАТЬ
	|	//ДляБухУчета 	3 КАК ИндексУчета,
	|	//ДляБухУчета 	Неопределено КАК Подразделение,
	|	//ДляБухУчета 	Подразделение КАК ПодразделениеОрганизации,
	|	//ДляБухУчета 	СчетУчета КАК СчетЗатрат,
	|	//ДляБухУчета 	Неопределено КАК СчетЗатратНУ,
	|	//ДляБухУчета 	Неопределено КАК СчетЗатратМУ,
	|	//ДляБухУчета 	Организация,
	|	//ДляБухУчета 	Продукция,
	|	//ДляБухУчета 	ХарактеристикаПродукции,
	|	//ДляБухУчета 	СерияПродукции,
	|	//ДляБухУчета 	Заказ,
	|	//ДляБухУчета 	НоменклатурнаяГруппа,
	|	//ДляБухУчета 	Спецификация,
	|	//ДляБухУчета 	ВариантВыпускаПродукции,
	|	//ДляБухУчета 	ВидВыпуска,
	|	//ДляБухУчета 	База,
	|	//ДляБухУчета 	БазаПриход,
	|	//ДляБухУчета 	БазаОстатокНЗП
	|   //ДляБухУчета ИЗ
	|	//ДляБухУчета 	БазаРаспределенияБухгалтерскийУчет
	|
	|	//ДляНалУчета ОБЪЕДИНИТЬ ВСЕ
	|
	|	//ДляНалУчета ВЫБРАТЬ
	|	//ДляНалУчета 	2 КАК ИндексУчета,
	|	//ДляНалУчета 	Неопределено КАК Подразделение,
	|	//ДляНалУчета 	Подразделение КАК ПодразделениеОрганизации,
	|	//ДляНалУчета 	Неопределено КАК СчетЗатрат,
	|	//ДляНалУчета 	СчетУчета КАК СчетЗатратНУ,
	|	//ДляНалУчета 	Неопределено КАК СчетЗатратМУ,
	|	//ДляНалУчета 	Организация,
	|	//ДляНалУчета 	Продукция,
	|	//ДляНалУчета 	ХарактеристикаПродукции,
	|	//ДляНалУчета 	СерияПродукции,
	|	//ДляНалУчета 	Заказ,
	|	//ДляНалУчета 	НоменклатурнаяГруппа,
	|	//ДляНалУчета 	Спецификация,
	|	//ДляНалУчета 	ВариантВыпускаПродукции,
	|	//ДляНалУчета 	ВидВыпуска,
	|	//ДляНалУчета 	База,
	|	//ДляНалУчета 	БазаПриход,
	|	//ДляНалУчета 	БазаОстатокНЗП
	|   //ДляНалУчета ИЗ
	|	//ДляНалУчета 	БазаРаспределенияНалоговыйУчет
	|
	|	//ДляМеждУчета ОБЪЕДИНИТЬ ВСЕ
	|
	|	//ДляМеждУчета ВЫБРАТЬ
	|	//ДляМеждУчета 	1 КАК ИндексУчета,
	|	//ДляМеждУчета 	Неопределено КАК Подразделение,
	|	//ДляМеждУчета 	Подразделение КАК ПодразделениеОрганизации,
	|	//ДляМеждУчета 	Неопределено КАК СчетЗатрат,
	|	//ДляМеждУчета 	Неопределено КАК СчетЗатратНУ,
	|	//ДляМеждУчета 	СчетУчета КАК СчетЗатратМУ,
	|	//ДляМеждУчета 	Организация,
	|	//ДляМеждУчета 	Продукция,
	|	//ДляМеждУчета 	ХарактеристикаПродукции,
	|	//ДляМеждУчета 	СерияПродукции,
	|	//ДляМеждУчета 	Заказ,
	|	//ДляМеждУчета 	НоменклатурнаяГруппа,
	|	//ДляМеждУчета 	Спецификация,
	|	//ДляМеждУчета 	ВариантВыпускаПродукции,
	|	//ДляМеждУчета 	ВидВыпуска,
	|	//ДляМеждУчета 	База,
	|	//ДляМеждУчета 	БазаПриход,
	|	//ДляМеждУчета 	БазаОстатокНЗП
	|   //ДляМеждУчета ИЗ
	|	//ДляМеждУчета 	БазаРаспределенияМеждународныйУчет
	// is ЯннуровВФ кон 20140708
	...
	// is ЯннуровВФ нач 20140708
	|ГДЕ
	|	БазаРаспределенияЗатрат.ИндексУчета > 0
	// is ЯннуровВФ кон 20140708
	|{ГДЕ
	...
	// is ЯннуровВФ нач 20140708
	|	МАКСИМУМ(БазаМежд), МАКСИМУМ(БазаПриходМежд), МАКСИМУМ(БазаОстатокНЗПМежд)
	// is ЯннуровВФ кон 20140708
	|ПО
	|	БазаРаспределенияЗатрат.НоменклатурнаяГруппа,
	|	БазаРаспределенияЗатрат.Заказ,
						...
						Пока ОбходПоСпецификации.Следующий() Цикл
							...
							Пока ОбходПоВидуВыпуска.Следующий() Цикл
								...
								Если ОбходПоВидуВыпуска.База = 0
								   ...
								   // is ЯннуровВФ нач 20140709
								   И ОбходПоВидуВыпуска.БазаМежд = 0
								   И ОбходПоВидуВыпуска.БазаПриходМежд = 0
								   // is ЯннуровВФ кон 20140709
								Тогда
									Продолжить;
								КонецЕсли;
								...
								// is ЯннуровВФ нач 20140709
								НоваяСтрока.БазаМежд = 0;
								НоваяСтрока.БазаПриходМежд = 0;
								НоваяСтрока.БазаОстатокНЗПМежд = 0;
								// is ЯннуровВФ нач 20140709
								...
								Пока ОбходПоПодразделению.Следующий() Цикл
									...
									Пока ОбходПоПодразделениюОрганизации.Следующий() Цикл
										...
										Пока ОбходПоСчетуЗатрат.Следующий() Цикл
											...
											// is ЯннуровВФ нач 20140709
												ОбходПоСчетуЗатратМУ = ОбходПоСчетуЗатратНУ.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
												Пока ОбходПоСчетуЗатратМУ.Следующий() Цикл
												// is ЯннуровВФ кон 20140709
													...
													Если ЗначениеЗаполнено(НоваяСтрока.СчетЗатрат) И НоваяСтрока.СчетЗатрат <> ОбходПоСчетуЗатратМУ.СчетЗатрат
													 ИЛИ ЗначениеЗаполнено(НоваяСтрока.СчетЗатратНУ) И НоваяСтрока.СчетЗатратНУ <> ОбходПоСчетуЗатратМУ.СчетЗатратНУ
													 ИЛИ ЗначениеЗаполнено(НоваяСтрока.СчетЗатратМУ) И НоваяСтрока.СчетЗатратМУ <> ОбходПоСчетуЗатратМУ.СчетЗатратМУ
													 ИЛИ ЗначениеЗаполнено(НоваяСтрока.Подразделение) И НоваяСтрока.Подразделение <> ОбходПоСчетуЗатратМУ.Подразделение
													 ИЛИ ЗначениеЗаполнено(НоваяСтрока.ПодразделениеОрганизации) И НоваяСтрока.ПодразделениеОрганизации <> ОбходПоСчетуЗатратМУ.ПодразделениеОрганизации Тогда
													// is ЯннуровВФ кон 20140709
													 
														НоваяСтрока = ТаблицаРезультата.Добавить();
														// is ЯннуровВФ нач 20140709
														//ЗаполнитьЗначенияСвойств(НоваяСтрока, ОбходПоСчетуЗатратНУ);
														ЗаполнитьЗначенияСвойств(НоваяСтрока, ОбходПоСчетуЗатратМУ);
														// is ЯннуровВФ нач 20140709
													...
													Обход = ОбходПоСчетуЗатратМУ.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
													// is ЯннуровВФ нач 20140709
													Пока Обход.Следующий() Цикл
														...
														Если Обход.ИндексУчета = 4 Тогда
														// is ЯннуровВФ кон 20140709
														...
														ИначеЕсли Обход.ИндексУчета = 3 Тогда
														// is ЯннуровВФ кон 20140709
														...
														ИначеЕсли Обход.ИндексУчета = 1 Тогда
															...
															НоваяСтрока.ВариантВыпускаПродукции = Обход.ВариантВыпускаПродукции;
															НоваяСтрока.ВидВыпуска = Обход.ВидВыпуска;
															НоваяСтрока.СчетЗатратМУ = Обход.СчетЗатратМУ;
															НоваяСтрока.БазаМежд = Обход.БазаМежд;
															НоваяСтрока.БазаПриходМежд = Обход.БазаПриходМежд;
															НоваяСтрока.БазаОстатокНЗПМежд = Обход.БазаОстатокНЗПМежд;
														// is ЯннуровВФ кон 20140709
...
Функция СформироватьТаблицуБазыРаспределенияЗатрат(
	...
	Если Не СтруктураШапкиДокумента.ОтражатьВУправленческомУчете 
	   ...
	   // is ЯннуровВФ нач 20140708
	   И (Не СтруктураШапкиДокумента.Свойство("ОтражатьВМеждународномУчете") Или Не СтруктураШапкиДокумента.ОтражатьВМеждународномУчете) Тогда
	   // is ЯннуровВФ кон 20140708
	   Возврат ТаблицаРезультата;
	КонецЕсли;
	...
	Если Не СтруктураШапкиДокумента.Свойство("ОтражатьВМеждународномУчете") Тогда 
	// is ЯннуровВФ кон 20140708
		СтруктураШапкиДокумента.Вставить("ОтражатьВМеждународномУчете", Ложь);
	КонецЕсли;
	...
	// is ЯннуровВФ нач 20140708
	Если СтруктураШапкиДокумента.ОтражатьВМеждународномУчете Тогда 
		ЕстьПодразделениеВДокументе = ИсходнаяСтруктураПараметров.Свойство("Подразделение") И ЗначениеЗаполнено(ИсходнаяСтруктураПараметров.Подразделение);
	Иначе
	// is ЯннуровВФ кон 20140708
	...
	ТаблицаРезультата.ЗаполнитьЗначения(СтруктураПараметров.ОтражатьВМеждународномУчете, 	"ОтражатьВМеждународномУчете");
