...
Функция СформироватьТаблицуРаспределенияЗатратПоПродукции(СтруктураШапкиДокумента)
	...
	// is ЯннуровВФ нач 20150915 Правка данных
	Если СтруктураШапкиДокумента.ОтражатьВМеждународномУчете 
	 И НачалоМесяца(СтруктураШапкиДокумента.Дата) = '2014-07-01' Тогда
		Отбор = Новый Структура("ВозвратИзНЗП", Истина);
		СтрокиТаб = ТабПеределов.НайтиСтроки(Отбор);
		Для Каждого СтрокаТаб Из СтрокиТаб Цикл 
			СтрокаТаб.ВозвратИзНЗП = Ложь;
		КонецЦикла;
	КонецЕсли;
	// is ЯннуровВФ кон 20150915
	
	//начало изменений БП 04 
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете или СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда //исключим воду
		Запрос = Новый Запрос("ВЫБРАТЬ
		                      |	ПРГ_РаспределениеВоды.Номенклатура,
		                      |	ПРГ_РаспределениеВоды.Организация,
		                      |	ПРГ_РаспределениеВоды.Подразделение,
		                      |	ПРГ_РаспределениеВоды.СчетУчета
		                      |ИЗ
		...
		Отбор  = Новый Структура("Продукция,Подразделение");
		Пока Выборка.Следующий() Цикл
			 Отбор.Продукция = Выборка.Номенклатура;
			 Отбор.Подразделение = Выборка.Подразделение;
			 найдСтроки = ТабПеределов.НайтиСтроки(Отбор); 
			 для каждого ТекСтрока  Из НайдСтроки Цикл
				 ТабПеределов.Удалить(ТекСтрока);
			 КонецЦикла; 
		КонецЦикла;	
	// is ЯннуровВФ нач 20150914
	ИначеЕсли СтруктураШапкиДокумента.ОтражатьВМеждународномУчете Тогда //исключим воду
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
...
Процедура РаспределениеПродукцииИЗатратПоПеределам(СтруктураШапкиДокумента) Экспорт
	...
	ИначеЕсли СтруктураШапкиДокумента.ОтражатьВМеждународномУчете Тогда
	    ДвиженияРаспределениеПродукцииПоПеределам 	= ДокументОбъект.Движения.ДС_РаспределениеПродукцииПоПеределамОрганизацийМСФО;
		ДвиженияРаспределениеЗатратПоПеределам 		= ДокументОбъект.Движения.ДС_РаспределениеЗатратПоПеределамОрганизацийМСФО;
    //-ДС
	Иначе	
