...
//
Функция ПолучитьТекстУсловияТаблицыОстатков(КлючУсловия, ИмяДокумента, ИмяТаблицы, СтруктураПараметров, КонтрольПоДокументу = Ложь,ИспользоватьГруппировки=Ложь)
	...
	Результат = Результат + ?(КлючУсловия = "ВременнаяТаблица", "",
		...
		+ ?(СтруктураПараметров.ЕстьСоставНабора И (КлючУсловия = "ВременнаяТаблица" ИЛИ НЕ СтруктураПараметров.ИспользоватьВременнуюТаблицу),"
			...
			|" 
			//{28.07.2015 Островерхий заявка №б/н 
			+?(ИспользоватьГруппировки И НЕ КлючУсловия = "ВременнаяТаблица",
				"	СГРУППИРОВАТЬ ПО "
				+ ?(СтруктураПараметров.ПолеСклад = "", " ", " Док." + СтруктураПараметров.ПолеСклад+",")
				+" Док.Номенклатура " ,"")
			//28.07.2015 Островерхий} 
...
//начало изменений
Функция ПолучитьТекстУсловияТаблицыОстатковКорретировкаРеализации(КлючУсловия, ИмяДокумента, ИмяТаблицы, СтруктураПараметров, КонтрольПоДокументу = Ложь)
...
Функция ПолучитьТекстУсловияТаблицыОстатковРеализациияТоваровУслуг(КлючУсловия, ИмяДокумента, ИмяТаблицы, СтруктураПараметров, КонтрольПоДокументу = Ложь)
...
Функция ПолучитьТекстУсловияТаблицыОстатковПеремещениеТоваров(КлючУсловия, ИмяДокумента, ИмяТаблицы, СтруктураПараметров, КонтрольПоДокументу = Ложь)
	...
	|	0									 					КАК КПередачеБезСерииКоличество,
	|	0														КАК КПолучению,
	|	0														КАК КПолучениюПоДокументуКоличество,
	|	%ПОЛЕ_Док_Количество%									КАК РезервыПоДокументуКоличество,
	|	%ПОЛЕ_Док_Количество%									КАК РезервыПоДокументуБезСерииКоличество,
...
КонецФункции // ПолучитьТекстУсловияТаблицыОстатковПеремещениеТоваров()

Функция ПолучитьТекстУсловияТаблицыОстатковТребованиеНакладная(КлючУсловия, ИмяДокумента, ИмяТаблицы, СтруктураПараметров, КонтрольПоДокументу = Ложь)
...
//
//		
Функция ПолучитьСтруктуруУсловийТаблицыОстатков(ИмяДокумента, ИмяТаблицы, УсловияТаблицыОстатков, СтруктураПараметров = Неопределено, КонтрольПоДокументу = Ложь, ИспользоватьГруппировки=Ложь) //m.ionov@a-prof.ru 05.02.2015
	...
	Если УсловияТаблицыОстатков.Свойство("Полное") Тогда
		УсловияТаблицыОстатков.Полное= ПолучитьТекстУсловияТаблицыОстатков("Полное", ИмяДокумента, ИмяТаблицы, СтруктураПараметровКопия, КонтрольПоДокументу, ИспользоватьГруппировки); //m.ionov@a-prof.ru 05.02.2015
	КонецЕсли; 
	
	//Не используются поле Качество
	Если УсловияТаблицыОстатков.Свойство("БезКачества") Тогда
		УсловияТаблицыОстатков.БезКачества = ПолучитьТекстУсловияТаблицыОстатков("БезКачества", ИмяДокумента, ИмяТаблицы, СтруктураПараметровКопия, , ИспользоватьГруппировки);
	КонецЕсли; 
	
	//Используется поле ДокументРезерва. Не используются поле Качество
	Если УсловияТаблицыОстатков.Свойство("Резерв") Тогда
		УсловияТаблицыОстатков.Резерв = ПолучитьТекстУсловияТаблицыОстатков("Резерв", ИмяДокумента, ИмяТаблицы, СтруктураПараметровКопия, , ИспользоватьГруппировки);
	КонецЕсли; 
	...
	Если СтруктураПараметровКопия.ИспользоватьВременнуюТаблицу Тогда
		Если ИмяДокумента = "КорректировкаРеализации" Тогда
			УсловияТаблицыОстатков.Вставить("ВременнаяТаблица", ПолучитьТекстУсловияТаблицыОстатковКорретировкаРеализации("ВременнаяТаблица", ИмяДокумента, ИмяТаблицы, СтруктураПараметровКопия));
		//{27.07.2015 Островерхий заявка №б/н 
		ИначеЕсли ИмяДокумента = "РеализацияТоваровУслуг" Тогда 
			УсловияТаблицыОстатков.Вставить("ВременнаяТаблица", ПолучитьТекстУсловияТаблицыОстатковРеализациияТоваровУслуг("ВременнаяТаблица", ИмяДокумента, ИмяТаблицы, СтруктураПараметровКопия));
		ИначеЕсли ИмяДокумента = "ПеремещениеТоваров"  Тогда 
			УсловияТаблицыОстатков.Вставить("ВременнаяТаблица", ПолучитьТекстУсловияТаблицыОстатковПеремещениеТоваров("ВременнаяТаблица", ИмяДокумента, ИмяТаблицы, СтруктураПараметровКопия));
		ИначеЕсли ИмяДокумента = "ТребованиеНакладная" Тогда	
		 	УсловияТаблицыОстатков.Вставить("ВременнаяТаблица", ПолучитьТекстУсловияТаблицыОстатковТребованиеНакладная("ВременнаяТаблица", ИмяДокумента, ИмяТаблицы, СтруктураПараметровКопия));
		//27.07.2015 Островерхий} 	
		Иначе	
...
Процедура ТоварыНаСкладахКонтрольОстатков_КорректировкаРеализации(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу = Ложь,РежимПроведения)
	...
	УсловияТаблицыОстатков = ПолучитьСтруктуруУсловийТаблицыОстатков(
			...
			ПараметрыУсловий, КонтрольПоДокументу);
	...
	Если ЕстьСоставНабора Тогда
		ТекстЗапросаРеквизитыДокумента = "
			...
			|		%ПОЛЕ_ЗаказПокупателя% КАК ЗаказПокупателя, 
			|		//Количество
			/// Кунов О.В., 25.05.2017 - 65876
			//|		Количество - КоличествоДоИзменения Как Количество
			|		СУММА(Количество - КоличествоДоИзменения) Как Количество
			///
			|		ИЗ
			...
			/// Кунов О.В., 25.05.2017 - 65876
			//|			И Количество > КоличествоДоИзменения
			|		СГРУППИРОВАТЬ ПО Ссылка, Номенклатура, %ПОЛЕ_Склад%, %ПОЛЕ_СпособСписанияОстаткаТоваров%, %ПОЛЕ_Качество%, ХарактеристикаНоменклатуры, СерияНоменклатуры, Коэффициент
			|		ИМЕЮЩИЕ СУММА(Количество - КоличествоДоИзменения) > 0
			///
			|
			//строки ТЧ СоставНабора 
			|		ОБЪЕДИНИТЬ ВСЕ
			...
			|				ДокНаб.ЕдиницаИзмерения.Коэффициент КАК Коэффициент,
			|				%ПОЛЕ_Набор_ЗаказПокупателя% КАК ЗаказПокупателя,
			|				ДокНаб.Количество * (ДокТов.Количество - ДокТов.КоличествоДоИзменения)КАК Количество
			|			ИЗ
			...
			|					ПО ДокТов.КлючСтроки = ДокНаб.КлючСтроки
			|					 И ДокТов.Ссылка     = &ДокументСсылка
			|  					 И ДокТов.Количество  > ДокТов.КоличествоДоИзменения
			|			ГДЕ
			|				ДокНаб.Ссылка = &ДокументСсылка
			|			) КАК Набор
			|
	...
	ПРГ_КонтрольНадату = Истина;
	Если КонтрольПоДокументу = Ложь или РежимПроведения = РежимПроведенияДокумента.Оперативный Тогда
		ПРГ_КонтрольНадату = Ложь;
	Иначе
		Запрос.УстановитьПараметр("ДатаОстатков", Новый Граница(Новый МоментВремени(СтруктураШапкиДокумента.Дата,СтруктураШапкиДокумента.Ссылка),ВидГраницы.Исключая));
	КонецЕсли;
	//----m.ionov@a-prof.ru---
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыНаСкладах.Остатки("+?(ПРГ_КонтрольНадату,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК Остатки
	|ПО 
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыНаСкладах.Остатки("+?(ПРГ_КонтрольНадату,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК ОстаткиБезСерии
	|ПО 
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки("+?(ПРГ_КонтрольНадату,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.БезКачества + ") КАК Резервы
	|ПО 
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки("+?(ПРГ_КонтрольНадату,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.БезКачества + ") КАК РезервыПоСерии
	|ПО 
	...
	|		) КАК ТЧ
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки("+?(ПРГ_КонтрольНадату,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.БезКачества + ") КАК ВремРезервы
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыКПередачеСоСкладов.Остатки("+?(ПРГ_КонтрольНадату,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК ТоварыКПередаче
	|ПО 
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыКПередачеСоСкладов.Остатки("+?(ПРГ_КонтрольНадату,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК ТоварыКПередачеБезСерии
	|ПО 
	...
	ТоварыНаСкладахОбработатьРезультатЗапроса(Выборка,истина,ТекстЗапросаРезервыПоДокументуБезСерии<>"",СтруктураПараметров.ИспользоватьУказаниеСерийНоменклатурыПриРезервировании,Заголовок,Отказ,КонтрольПоДокументу);
...
//
Процедура ТоварыНаСкладахКонтрольОстатков(ИмяТабличнойЧасти, СтруктураШапкиДокумента, ОтказПроведения, ЗаголовокСообщения, РежимПроведения, КонтрольПоДокументу = Ложь) Экспорт
	
	//m.ionov@a-prof.ru 05.02.2015
	//КонтрольПоДокументу - если передана ссылка на документ, значит:
	//1. Контролируем вне зависимости оперативное или нет проведение
	//2. Контролируем только по остаткам
	//----m.ionov@a-prof.ru---
	
	Если РежимПроведения <> РежимПроведенияДокумента.Оперативный И КонтрольПоДокументу = Ложь Тогда //m.ionov@a-prof.ru 05.02.2015
		Возврат;
	КонецЕсли;
	...
	Если глЗначениеПеременной("ИспользоватьРегистрСвободныеОстатки") И КонтрольПоДокументу = Ложь Тогда //m.ionov@a-prof.ru 05.02.2015
		СвободныеОстаткиКонтрольОстатков(ИмяТабличнойЧасти, СтруктураШапкиДокумента, ОтказПроведения, ЗаголовокСообщения);
		Возврат;
	КонецЕсли; 
	
	//начало изменений
	Если Не СтруктураШапкиДокумента.Свойство("РежимПроведения") Тогда
	...
	Если СтруктураПараметров.ИмяДокумента = "ПеремещениеТоваров" Тогда
		...
		//{27.07.2015 Островерхий заявка №б/н 
		Если ЗначениеЗаполнено(СтруктураШапкиДокумента.СП_ЗаданиеНаПодбор) Тогда
			ТоварыНаСкладахКонтрольОстатков_ПеремещениеТоваровСЗаданиямиНаПодбор(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу); //m.ionov@a-prof.ru 05.02.2015
		Иначе	
			ТоварыНаСкладахКонтрольОстатков_ПеремещениеТоваров(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу); //m.ionov@a-prof.ru 05.02.2015
		КонецЕсли; 
		//27.07.2015 Островерхий} 
	...
	ИначеЕсли СтруктураПараметров.ИмяДокумента = "РеализацияТоваровУслуг" Тогда
		...
		Иначе	
			//{24.07.2015 Островерхий заявка №б/н 
			Если ЗначениеЗаполнено(СтруктураШапкиДокумента.СП_ЗаданиеНаПодбор) Тогда
				ТоварыНаСкладахКонтрольОстатков_Реализация(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу);
			Иначе	
				ТоварыНаСкладахКонтрольОстатков_Реализация_ОтчетОРознПродажах_ЧекККМ(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу); //m.ionov@a-prof.ru 05.02.2015
			КонецЕсли; 
			//24.07.2015 Островерхий} 
	...
	//начало изменений
	ИначеЕсли СтруктураПараметров.ИмяДокумента = "КорректировкаРеализации" Тогда
	
		ТоварыНаСкладахКонтрольОстатков_КорректировкаРеализации(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу,РежимПроведения);
	//конец изменений
	...
	ИначеЕсли СтруктураПараметров.ИмяДокумента = "ТребованиеНакладная"
		...
		ИЛИ СтруктураПараметров.ИмяДокумента = "СписаниеТоваров" 
		//начало изменений БП 14 
		ИЛИ СтруктураПараметров.ИмяДокумента = "ОтчетПроизводстваЗаСмену" 
		//конец изменений БП 14 
		Тогда
		
		//{28.07.2015 Островерхий заявка №б/н 
		Если ЗначениеЗаполнено(СтруктураШапкиДокумента.СП_ЗаданиеНаПодбор) Тогда
			ТоварыНаСкладахКонтрольОстатков_ТребованиеНакладная(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу); //m.ionov@a-prof.ru 05.02.2015
		Иначе	
			ТоварыНаСкладахКонтрольОстатков_ЗаказВТабЧасти(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу); //m.ionov@a-prof.ru 05.02.2015
		КонецЕсли;
		//28.07.2015 Островерхий} 
...
//
Процедура ТоварыНаСкладахКонтрольОстатков_ПеремещениеТоваров(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу = Ложь) //m.ionov@a-prof.ru 05.02.2015
	...
	УсловияТаблицыОстатков = ПолучитьСтруктуруУсловийТаблицыОстатков(
			...
			СтруктураПараметров.ИмяТаблицы,
			Новый Структура("Полное,БезКачества"),
			ПараметрыУсловий, КонтрольПоДокументу); //m.ionov@a-prof.ru 05.02.2015
	...
	//m.ionov@a-prof.ru 05.02.2015
	ПРГ_КонтрольПоДокументу = КонтрольПоДокументу;
	Если КонтрольПоДокументу = Ложь Тогда
	Иначе
		
		РежПров = РежимПроведенияДокумента.Неоперативный;
		СтруктураШапкиДокумента.Свойство("РежимПроведения",РежПров);
		Если РежПров = Неопределено Тогда
			РежПров = РежимПроведенияДокумента.Неоперативный;
		КонецЕсли;	
		
		Если РежПров = РежимПроведенияДокумента.Оперативный Тогда
			ПРГ_КонтрольПоДокументу = Ложь;
		Иначе
			ПРГ_КонтрольПоДокументу = Истина;
			Запрос.УстановитьПараметр("ДатаОстатков", Новый Граница(Новый МоментВремени(СтруктураШапкиДокумента.Дата,СтруктураШапкиДокумента.Ссылка),ВидГраницы.Исключая));
		КонецЕслИ;	
...
//	|	Док.ДокументРезерва                                    	КАК ДокументРезерва,
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления." + СтруктураПараметров.РегистрОстаткиТоваров + ".Остатки("+?(ПРГ_КонтрольПоДокументу,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК Остатки
	|ПО 
	|	Остатки.Склад = &Склад
	|	И Остатки.Номенклатура = Док.Номенклатура
	|	И Остатки.Качество = %СОЕДИНЕНИЕ_Качество_Остатки%
	...
	//таблица товаров в резерве на складе без учета серий номенклатуры
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки("+?(ПРГ_КонтрольПоДокументу,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.БезКачества + ") КАК Резервы
	|ПО 
	...
	|		) КАК ТЧ
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки("+?(ПРГ_КонтрольПоДокументу,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.БезКачества + ") КАК ВремРезервы
	|	ПО
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыКПередачеСоСкладов.Остатки("+?(ПРГ_КонтрольПоДокументу,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК ТоварыКПередаче
	|ПО 
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыКПолучениюНаСклады.Остатки("+?(ПРГ_КонтрольПоДокументу,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + " 
...
//	|	 Док.ДокументРезерва 
	...
	Если ЕстьСерия Тогда
		ТекстЗапросаОстаткиБезСерии = "
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	РегистрНакопления." + СтруктураПараметров.РегистрОстаткиТоваров + ".Остатки("+?(ПРГ_КонтрольПоДокументу,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК ОстаткиБезСерии
			|ПО 
		...
		ТекстЗапросаКПередачеБезСерии = "
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	РегистрНакопления.ТоварыКПередачеСоСкладов.Остатки("+?(ПРГ_КонтрольПоДокументу,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК ТоварыКПередачеБезСерии
			|ПО 
		...
		Если СтруктураПараметров.ИспользоватьУказаниеСерийНоменклатурыПриРезервировании Тогда
			...
			ТекстЗапросаРезервыПоСерии = "
				|ЛЕВОЕ СОЕДИНЕНИЕ
				|	РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки("+?(ПРГ_КонтрольПоДокументу,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.БезКачества + ") КАК РезервыПоСерии
				|ПО 
	...
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	ТоварыНаСкладахОбработатьРезультатЗапроса(Выборка,ЕстьСерия,ТекстЗапросаРезервыПоДокументуБезСерии<>"",СтруктураПараметров.ИспользоватьУказаниеСерийНоменклатурыПриРезервировании,Заголовок,Отказ, КонтрольПоДокументу); //m.ionov@a-prof.ru 05.02.2015
...
//2) разные критерии определения количества, списываемого из резерва, в зависимости от того указан ли в шапке внутренний заказ
//
Процедура ТоварыНаСкладахКонтрольОстатков_ПеремещениеТоваровСЗаданиямиНаПодбор(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу = Ложь) //m.ionov@a-prof.ru 05.02.2015
	...
	|   ЕстьNull(Остатки.КоличествоОстаток,0)					КАК ОстатокБезСерииКоличество,
	|	ЕстьNull(Остатки.КоличествоОстаток,0)         			КАК ОстатокКоличество,
	|	0					      							    КАК РезервыКоличество,
	|	0														КАК РезервыПоСерииКоличество,
	|	0 														КАК КПередачеКоличество,
	|	0									 					КАК КПередачеБезСерииКоличество,
	|	0														КАК КПолучению,
	|	0														КАК КПолучениюПоДокументуКоличество,
	|	Док.ДокументКоличество									КАК РезервыПоДокументуКоличество,
	|	Док.ДокументКоличество									КАК РезервыПоДокументуБезСерииКоличество,
	|	0 														КАК КПередачеПоДокументуКоличество,
	|	0              											КАК КПередачеПоДокументуБезСерииКоличество
	|
	|ИЗ 
	...
	//таблица остатков на складах с учетом серий номенклатуры
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления." + СтруктураПараметров.РегистрОстаткиТоваров + ".Остатки("+?(ПРГ_КонтрольПоДокументу,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.БезКачества + ") КАК Остатки
	|ПО 
	|	Остатки.Склад = &Склад
	|	И Остатки.Номенклатура = Док.Номенклатура
	|	И Остатки.Качество = %СОЕДИНЕНИЕ_Качество_Остатки%
	...
	Если глЗначениеПеременной("ИспользоватьБлокировкуДанных")  Тогда
		...
		ЗапросДляБлокировки.Текст = 
			...
			|ИЗ Документ." + СтруктураПараметров.ИмяТаблицы + "
			|ГДЕ
			|	Ссылка  =  &ДокументСсылка";
			
		ЗапросДляБлокировки.УстановитьПараметр("ДокументСсылка",СтруктураШапкиДокумента.Ссылка);
		ЗапросДляБлокировки.УстановитьПараметр("Склад",СтруктураШапкиДокумента.Склад);	
		РезультатЗапросаДляБлокировки = ЗапросДляБлокировки.Выполнить();
...
//
Процедура ТоварыНаСкладахКонтрольОстатков_Реализация_ОтчетОРознПродажах_ЧекККМ(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу = Ложь)
	...
	//m.ionov@a-prof.ru 05.02.2015
	//начало изменений ниже не учтен способ проведения оперативный
	ПРГ_КонтрольПоДокументу = КонтрольПоДокументу;
	Если КонтрольПоДокументу = Ложь Тогда
	Иначе
		РежПров = РежимПроведенияДокумента.Неоперативный;
	...
	//таблица остатков товаров с учетом серий
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыНаСкладах.Остатки("+?(НЕ ПРГ_КонтрольПоДокументу = Ложь,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК Остатки
	|ПО 
	|	Остатки.Склад = %СОЕДИНЕНИЕ_Склад_Остатки%
	|	И Остатки.Номенклатура = Док.Номенклатура
	|	И Остатки.ХарактеристикаНоменклатуры = %СОЕДИНЕНИЕ_Характеристика_Остатки%
	...
	//таблица остатков товаров без учета серий
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыНаСкладах.Остатки("+?(НЕ ПРГ_КонтрольПоДокументу = Ложь,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК ОстаткиБезСерии
	|ПО 
	|	Док.Номенклатура = ОстаткиБезСерии.Номенклатура
	|	" + ?(СтруктураПараметров.ИспользоватьХарактеристикиНоменклатуры, "И Док.ХарактеристикаНоменклатуры = ОстаткиБезСерии.ХарактеристикаНоменклатуры", "") + "
	| //СОЕДИНЕНИЕ_Качество_ОстаткиБезСерии
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки("+?(НЕ ПРГ_КонтрольПоДокументу = Ложь,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.БезКачества + ") КАК Резервы
	|ПО 
	|	Док.Номенклатура = Резервы.Номенклатура
	|	" + ?(СтруктураПараметров.ИспользоватьХарактеристикиНоменклатуры, "И Док.ХарактеристикаНоменклатуры = Резервы.ХарактеристикаНоменклатуры", "") + "
	| //СОЕДИНЕНИЕ_Склад_Резервы
	...
	|"+?(СтруктураПараметров.ИспользоватьУказаниеСерийНоменклатурыПриРезервировании,"
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки("+?(НЕ ПРГ_КонтрольПоДокументу = Ложь,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.БезКачества + ") КАК РезервыПоСерии
	|ПО 
	|	Док.Номенклатура = РезервыПоСерии.Номенклатура
	|	" + ?(СтруктураПараметров.ИспользоватьХарактеристикиНоменклатуры, "И Док.ХарактеристикаНоменклатуры 	= РезервыПоСерии.ХарактеристикаНоменклатуры", "") + "
	|	" + ?(СтруктураПараметров.ИспользоватьСерииНоменклатуры, "И Док.СерияНоменклатуры = РезервыПоСерии.СерияНоменклатуры", "") + "
	...
	|		) КАК ТЧ
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки("+?(НЕ ПРГ_КонтрольПоДокументу = Ложь,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.БезКачества + ") КАК ВремРезервы
	|	ПО ТЧ.Номенклатура = ВремРезервы.Номенклатура 
	|		И ТЧ.Склад = ВремРезервы.Склад
	|		И ТЧ.ЗаказПокупателя = ВремРезервы.ДокументРезерва
	|		" + ?(СтруктураПараметров.ИспользоватьХарактеристикиНоменклатуры, "И ТЧ.ХарактеристикаНоменклатуры = ВремРезервы.ХарактеристикаНоменклатуры", "") + "
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыКПередачеСоСкладов.Остатки("+?(НЕ ПРГ_КонтрольПоДокументу = Ложь,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК ТоварыКПередаче
	|ПО 
	|	Док.Номенклатура = ТоварыКПередаче.Номенклатура
	|	" + ?(СтруктураПараметров.ИспользоватьХарактеристикиНоменклатуры, "И Док.ХарактеристикаНоменклатуры = ТоварыКПередаче.ХарактеристикаНоменклатуры", "") + "
	|	" + ?(СтруктураПараметров.ИспользоватьСерииНоменклатуры, "И Док.СерияНоменклатуры = ТоварыКПередаче.СерияНоменклатуры", "") + "
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыКПередачеСоСкладов.Остатки("+?(НЕ ПРГ_КонтрольПоДокументу = Ложь,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК ТоварыКПередачеБезСерии
	|ПО 
	|	Док.Номенклатура = ТоварыКПередачеБезСерии.Номенклатура
	|	" + ?(СтруктураПараметров.ИспользоватьХарактеристикиНоменклатуры, "И Док.ХарактеристикаНоменклатуры = ТоварыКПередачеБезСерии.ХарактеристикаНоменклатуры", "") + "
	|	//СОЕДИНЕНИЕ_Склад_ТоварыКПередачеБезСерии
...
// Процедура контролирует остати для документов РеализацияТоваровИУслуг (таб части Товары и СоставНабора)
//
Процедура ТоварыНаСкладахКонтрольОстатков_Реализация(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу = Ложь)
	
	//{24.07.2015 Островерхий заявка №б/н 
	//процедура создана для замены ТоварыНаСкладахКонтрольОстатков_Реализация_ОтчетОРознПродажах_ЧекККМ(), вызывается при проведении в документов: РеализацияТоваровИУслуг	
	//24.07.2015 Островерхий} 
	...
	ЕстьСпособСписания = МетаданныеТабЧасти.Реквизиты.Найти("СпособСписанияОстаткаТоваров")	<> Неопределено;
    ЕстьСкладВТабЧасти = МетаданныеТабЧасти.Реквизиты.Найти("Склад") <> Неопределено;
    ИспользоватьГруппировкиВПараметрахВитруальныхТаблиц = Истина;
	...
	УсловияТаблицыОстатков = ПолучитьСтруктуруУсловийТаблицыОстатков(
			...
			СтруктураПараметров.ИмяТаблицы,
			Новый Структура("Полное,БезКачества"),
			ПараметрыУсловий, КонтрольПоДокументу,ИспользоватьГруппировкиВПараметрахВитруальныхТаблиц);

	// Текст вложенного запроса, ограничивающего номенклатуру при получении остатков
	Если ЕстьСоставНабора Тогда
		ТекстЗапросаРеквизитыДокумента = УсловияТаблицыОстатков.ВременнаяТаблица;
	Иначе
        ТекстЗапросаРеквизитыДокумента = "
			|	Документ." + СтруктураПараметров.ИмяТаблицы + " 
			|";
	...
	|	0     														КАК КПередачеКоличество,
	|	0 															КАК КПередачеБезСерииКоличество,
	|	Док.Количество												КАК РезервыПоДокументуКоличество,
	|	Док.Количество												КАК РезервыПоДокументуБезСерииКоличество,
	|	0 															КАК КПолучению,
	|	0                 											КАК КПолучениюПоДокументуКоличество,
	|	0 															КАК КПередачеПоДокументуКоличество,
	|	0              												КАК КПередачеПоДокументуБезСерииКоличество
	...
	//таблица остатков товаров с учетом серий
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыНаСкладах.Остатки("+?(НЕ ПРГ_КонтрольПоДокументу = Ложь,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.БезКачества + ") КАК Остатки
	|ПО 
	|	Остатки.Склад = %СОЕДИНЕНИЕ_Склад_Остатки%
	|	И Остатки.Номенклатура = Док.Номенклатура
	|	И Остатки.ХарактеристикаНоменклатуры = %СОЕДИНЕНИЕ_Характеристика_Остатки%
...
//
Процедура ТоварыНаСкладахКонтрольОстатков_ЗаказВТабЧасти(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу = Ложь) //m.ionov@a-prof.ru 05.02.2015
	...
	//начало изменений БП 14
	ИначеЕсли СтруктураПараметров.ИмяДокумента = "ОтчетПроизводстваЗаСмену" Тогда
		ИмяРеквизитаЗаказ = "Заказ";
	//конец изменений БП 14 
    КонецЕсли;
	...
	Иначе
		//начало изменений БП 14 
		//Запрос.УстановитьПараметр("Склад", СтруктураШапкиДокумента.Склад);
		Если  СтруктураПараметров.ИмяДокумента = "ОтчетПроизводстваЗаСмену" Тогда
			Запрос.УстановитьПараметр("Склад", СтруктураШапкиДокумента.СкладМатериалов);
		Иначе	
			Запрос.УстановитьПараметр("Склад", СтруктураШапкиДокумента.Склад);
		КонецЕсли;	
		//конец изменений БП 14 
	...
	//таблица остатков товаров с учетом серий
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыНаСкладах.Остатки("+?(ПРГ_КонтрольПоДокументу,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК Остатки
	|ПО 
	|	Остатки.Склад = %СОЕДИНЕНИЕ_Склад_Остатки%
	|	И Остатки.Номенклатура = Док.Номенклатура
	|	И Остатки.ХарактеристикаНоменклатуры = %СОЕДИНЕНИЕ_Характеристика_Остатки%
	...
	Если ЕстьСерия Тогда
		ТекстЗапросаОстаткиБезСерии = "
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрНакопления.ТоварыНаСкладах.Остатки("+?(ПРГ_КонтрольПоДокументу,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.Полное + ") КАК ОстаткиБезСерии
		|ПО 
		|	Док.Номенклатура = ОстаткиБезСерии.Номенклатура
	...
	ТоварыНаСкладахОбработатьРезультатЗапроса(Выборка,ЕстьСерия,ТекстЗапросаРезервыПоДокументуБезСерии<>"",СтруктураПараметров.ИспользоватьУказаниеСерийНоменклатурыПриРезервировании,Заголовок,Отказ,КонтрольПоДокументу);
...
//	с виртуальными таблицами по документу резерва
//
Процедура ТоварыНаСкладахКонтрольОстатков_ТребованиеНакладная(СтруктураШапкиДокумента, СтруктураПараметров, МетаданныеТабЧасти, Заголовок, Отказ, КонтрольПоДокументу = Ложь) //m.ionov@a-prof.ru 05.02.2015
	...
	////таблица остатков товаров с учетом серий
	//|ЛЕВОЕ СОЕДИНЕНИЕ
	//|	РегистрНакопления.ТоварыНаСкладах.Остатки("+?(ПРГ_КонтрольПоДокументу,"&ДатаОстатков","")+", " + УсловияТаблицыОстатков.БезКачества + ") КАК Остатки
	//|ПО 
	//|	Остатки.Склад = %СОЕДИНЕНИЕ_Склад_Остатки%
	//|	И Остатки.Номенклатура = Док.Номенклатура
	//|	И Остатки.ХарактеристикаНоменклатуры = %СОЕДИНЕНИЕ_Характеристика_Остатки%
...
//
Процедура ТоварыНаСкладахОбработатьРезультатЗапроса(Выборка,ЕстьСерия,ЕстьРезервыПоДокументуБезСерии,ИспользоватьУказаниеСерийНоменклатурыПриРезервировании,Заголовок,Отказ,КонтрольПоДокументу = Ложь) //m.ionov@a-prof.ru 05.02.2015
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЕстьСерия Тогда
			...
			Если КоличествоБезРезерва < ДокументКоличество Тогда
				...
				, ?(КонтрольПоДокументу И Выборка.Номенклатура.НСИ_РазрешитьОтгрузкуВМинус,Ложь,Отказ), Заголовок
				, СокрЛП(Выборка.Склад)                                   // Склад (представление)
				,                                                         // Заказ поставщику (представление)
				, Выборка.НоменклатураПредставление + " (Код: " + СокрЛП(Выборка.Номенклатура.Код) + ")"                       // Номенклатура (представление)
		...
		//m.ionov@a-prof.ru 05.02.2015
		Если Не КонтрольПоДокументу = Ложь Тогда
			Если КоличествоНаСкладе < ДокументКоличество Тогда
				
				УправлениеЗапасами.СообщитьОНедостаткеТовара(ДокументКоличество                  // Требуемое количество товара
				, КоличествоНаСкладе                 // Свободное количество товара
...
Процедура ТоварыВРезервеНаСкладахКонтрольОстатков(ИмяТабличнойЧасти, СтруктураШапкиДокумента, ОтказПроведения, ЗаголовокСообщения, РежимПроведения) Экспорт
	...
	Если РежимПроведения <> РежимПроведенияДокумента.Оперативный и ТипЗнч(СтруктураШапкиДокумента.Ссылка) <> Тип("ДокументСсылка.РезервированиеТоваров") Тогда
		Возврат;
	КонецЕсли;      
	...
	ИначеЕсли СтруктураПараметров.ИмяДокумента = "РезервированиеТоваров" Тогда
		...
		ТоварыВРезервеНаСкладахКонтрольОстатков_СкладВТабЧасти_ЗаказВШапке_Резерв(СтруктураШапкиДокумента, СтруктураПараметров, Заголовок, Отказ);
...
Процедура ТоварыВРезервеНаСкладахКонтрольОстатков_СкладВТабЧасти_ЗаказВТабЧасти(СтруктураШапкиДокумента, СтруктураПараметров, Заголовок, Отказ)
	...
	// Установим параметры запроса
	//начало изменений БП 14 
	Запрос.УстановитьПараметр("ВидНоменклатурыВода",      ВидНоменклатурыВода);
	//конец изменений БП 14 
	...
	|	И Док.Ссылка  =  &ДокументСсылка
	//начало изменений БП 14
	|	"+?(НеУчитыватьВоду,"И Док.Номенклатура.ВидНоменклатуры <> &ВидНоменклатурыВода","")+"
	//конец изменений БП 14 
	|	" + ?(СтруктураПараметров.ИмяДокумента = "АвансовыйОтчет"," И " + ИмяДокументРезерва + ".БезПраваПродажи","И НЕ Док." + ИмяДокументРезерва + " В (&МассивПустыхСсылок)") + "
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ТоварыВРезервеНаСкладахКонтрольОстатков_СкладВТабЧасти_ЗаказВШапке(СтруктураШапкиДокумента, СтруктураПараметров, Заголовок, Отказ)
	...
	Если СтруктураПараметров.ИмяДокумента = "РезервированиеТоваров" Тогда
		Запрос.УстановитьПараметр("ДокументРезерва", 		?(ЗначениеЗаполнено(ДокументРезерва),ДокументРезерва,Неопределено));
		Запрос.УстановитьПараметр("АП_НазначениеРезерва", 		СтруктураШапкиДокумента.АП_НазначениеРезерва);
		ДопУсловиеПоНазначениюРезерва = " И АП_НазначениеРезерва = &АП_НазначениеРезерва";
	Иначе
		Запрос.УстановитьПараметр("АП_НазначениеРезерва", 		Справочники.АП_НазначениеРезервов.ПустаяСсылка());
		ДопУсловиеПоНазначениюРезерва = " И АП_НазначениеРезерва = &АП_НазначениеРезерва";
	КонецЕсли;
	//----m.ionov@a-prof.ru---
    Запрос.УстановитьПараметр("ПустаяХарактеристика",  	Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка());
	...
	|	РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки " + ?(СтруктураПараметров.ИмяТабличнойЧасти = "ВозвратнаяТара", "", "(, ДокументРезерва = &ДокументРезерва И " + УсловияТаблицыОстатков.Резерв + ДопУсловиеПоНазначениюРезерва +")") + " КАК Резервы
	|ПО 
	...
	Пока Выборка.Следующий() Цикл
		...
		Если типЗнч(Выборка.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда		
			мКодНоменклатуры = "'" + СокрЛП(Выборка.Номенклатура.Код) + "'";
		Иначе
			мКодНоменклатуры = "";
		КонецЕсли; 
		...
		//++ Spl_Апроф 06.03.2015 (k.russkih@a-prof.ru)
		//УправлениеЗапасами.ПредставлениеНоменклатуры(Выборка.НоменклатураПредставление, 
		УправлениеЗапасами.ПредставлениеНоменклатуры(мКодНоменклатуры + Выборка.НоменклатураПредставление,
								  ?(СтруктураПараметров.ИмяТабличнойЧасти <> "ВозвратнаяТара", " "+Выборка.ХарактеристикаНоменклатурыПредставление, ""),
								  ?(УчитыватьСерии, " "+Выборка.СерияНоменклатурыПредставление, "")) +
		" на складе """ + СокрЛП(Выборка.Склад) + """ по документу """ + СокрЛП(Выборка.ДокументРезерва) + 
		""" недостаточно.";
...
Процедура ТоварыПолученныеКонтрольОстатков(ИмяТабличнойЧасти, СтруктураШапкиДокумента, СтатусПолучения, ОтказПроведения, ЗаголовокСообщения, РежимПроведения) Экспорт
	...
	//Для тары не ведем сделку - поэтому добавим доп условие на пустую сделку
	...
	//m.ionov@a-prof.ru 30.04.2014
	//|		И Сделка             = &Сделка
	|		И (Сделка             = &Сделка" + ?(ИмяТаблицы = "ВозвратнаяТара" И СтруктураШапкиДокумента.ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомитентом," ИЛИ Сделка = &ПустаяСделка",")") + " 
...
Процедура СвободныеОстаткиКонтрольОстатков(ИмяТабличнойЧасти, СтруктураШапкиДокумента, ОтказПроведения, ЗаголовокСообщения)
	...
	|"+?(СтруктураПараметров.СУчетомКПолучениюИФиксРезерв,"
	| МАКСИМУМ(ВложенныйЗапрос.КоличествоОстаток)+ МАКСИМУМ(КПолучКоличествоОстаток) - МАКСИМУМ(ФиксРезКоличествоОстаток) как СвободныйОстаток," 
	...
	//
	//начало изменений Ожиганов 08.04.2015 37889 
	//|		ЕСТЬNULL(СвободныеОстатки.КоличествоОстаток, 0) КАК КоличествоОстаток,
	|"+?(СтруктураПараметров.СУчетомКПолучениюИФиксРезерв,
	"  ЕСТЬNULL(СвободныеОстатки.КоличествоОстаток, 0) КАК КоличествоОстаток,
	|  ЕСТЬNULL(ТоварыКПолучению.КоличествоОстаток, 0) Как КПолучКоличествоОстаток,
	|  ЕСТЬNULL(ФиксированныйРезерв.Количество, 0) 	   Как ФиксРезКоличествоОстаток,
	|
	|",
	"ЕСТЬNULL(СвободныеОстатки.КоличествоОстаток, 0) КАК КоличествоОстаток, ")+"
	//конец изменений 
...
Функция ПолучитьСтруктуруПараметров(СтруктураШапкиДокумента, ИмяТабличнойЧасти, ЗаголовокСообщения)
	...
	//начало изменений Ожиганов 08.04.2015 37889 
	СУчетомКПолучениюИФиксРезерв = Ложь;
	КонтрольНаДвеДаты 		     = Ложь;
	//конец изменений 
	...
	ИначеЕсли ИмяДокумента = "РезервированиеТоваров" Тогда
		...
		//начало изменений Ожиганов 08.04.2015 37889 
		СУчетомКПолучениюИФиксРезерв = Истина;
		КонтрольНаДвеДаты 		     = СтруктураШапкиДокумента.РежимПроведения = РежимПроведенияДокумента.Неоперативный; 
		//конец изменений 
	...
	//начало изменений Ожиганов 08.04.2015 37889 
	СтруктураПараметров.Вставить("СУчетомКПолучениюИФиксРезерв",		СУчетомКПолучениюИФиксРезерв);
	СтруктураПараметров.Вставить("КонтрольНаДвеДаты",					КонтрольНаДвеДаты);
	//конец изменений 
...
Функция ПолучитьТекстЗапросаСвободныеОстатки(СтруктураПараметров)
	...
	КонецЕсли;
	//начало изменений Ожиганов 08.04.2015 37889 
	Если СтруктураПараметров.СУчетомКПолучениюИФиксРезерв Тогда
		ТекстЗапроса = ТекстЗапроса + "
		| ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыКПолучениюНаСклады.Остатки(, 
		| (ДокументПолучения ССЫЛКА Документ.СП_ЗаданиеНаСмену или 
 		|ДокументПолучения ССЫЛКА Документ.ПоступлениеТоваровУслуг)
    	|И
		|				(Склад, Номенклатура, Качество" + ?(СтруктураПараметров.ЕстьХарактеристика, ", ХарактеристикаНоменклатуры", "") + ")
		|				 В	(ВЫБРАТЬ
		|						Док.Склад, Док.Номенклатура, Док.Качество"
		...
		|	И ТоварыКПолучению.Склад = Док.Склад
		|	И ТоварыКПолучению.Качество = Док.Качество
		|	И ТоварыКПолучению.ХарактеристикаНоменклатуры = " + ?(СтруктураПараметров.ЕстьХарактеристика, "Док.ХарактеристикаНоменклатуры", "ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)") + "
		|	" + ?(СтруктураПараметров.ЕстьСерия, "И ТоварыКПолучению.СерияНоменклатуры = Док.СерияНоменклатуры", "");
		
		ТекстЗапроса = ТекстЗапроса + "		
		|ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|	ФиксРезерв.Склад КАК Склад,
		|"+?(СтруктураПараметров.ЕстьХарактеристика, ", ФиксРезерв.ХарактеристикаНоменклатуры", "") + "
		|	ФиксРезерв.Номенклатура КАК Номенклатура,
		|   Сумма(ФиксРезерв.Количество) Как Количество
		|ИЗ
		|	РегистрСведений.КП_ФиксированныеРезервыПоНазначениюРезервов.СрезПоследних(
		|			,
		|			(Склад, Номенклатура "+ ?(СтруктураПараметров.ЕстьХарактеристика, ", ХарактеристикаНоменклатуры", "") + ") В
		|				(ВЫБРАТЬ
		|					Док.Склад,
		|					Док.Номенклатура
		|" + ?(СтруктураПараметров.ЕстьХарактеристика, ", Док.ХарактеристикаНоменклатуры", "") + "		
		|				ИЗ
		|					ВременнаяТаблицаДокумента КАК Док)) КАК ФиксРезерв
		|"+?(Не СтруктураПараметров.ЕстьХарактеристика, " ГДЕ ФиксРезерв.ХарактеристикаНоменклатуры = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) ", "")+" 
		|СГРУППИРОВАТЬ ПО
		|	ФиксРезерв.Склад,
		|	ФиксРезерв.Номенклатура
		|"+?(СтруктураПараметров.ЕстьХарактеристика, " , ФиксРезерв.ХарактеристикаНоменклатуры", "")+" 
		|) КАК ФиксированныйРезерв
		|ПО   Док.Склад = ФиксированныйРезерв.Склад
		|	И Док.Номенклатура = ФиксированныйРезерв.Номенклатура
		|"+?(СтруктураПараметров.ЕстьХарактеристика, "	И Док.ХарактеристикаНоменклатуры = ФиксированныйРезерв.ХарактеристикаНоменклатуры", "");
		
			
		//
		//ТекстЗапроса = ТекстЗапроса + "
		//| ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КП_ФиксированныеРезервыПоНазначениюРезервов.СрезПоследних(, 
		//|				(Склад, Номенклатура " + ?(СтруктураПараметров.ЕстьХарактеристика, ", ХарактеристикаНоменклатуры", "") + ")
		//|				 В	(ВЫБРАТЬ
		//|						Док.Склад, Док.Номенклатура "
		//						+ ?(СтруктураПараметров.ЕстьХарактеристика, ", Док.ХарактеристикаНоменклатуры", "") + "
		//|					 ИЗ
		//|						ВременнаяТаблицаДокумента КАК Док)
		//|															)КАК ФиксированыйРезерв
		//|	ПО ФиксированыйРезерв.Номенклатура = Док.Номенклатура
		//|	И ФиксированыйРезерв.Склад = Док.Склад
		//|	И ФиксированыйРезерв.ХарактеристикаНоменклатуры = " + ?(СтруктураПараметров.ЕстьХарактеристика, "Док.ХарактеристикаНоменклатуры", "ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)");		
...
//  Заголовок					- строка, заголовок сообщения об ошибке проведения.
//
Процедура ТоварыПолученныеКонтрольОстатков_Перемещение(ИмяТабличнойЧасти, СтруктураШапкиДокумента, СтатусПолучения, ОтказПроведения, ЗаголовокСообщения, РежимПроведения) Экспорт
	...
	Запрос.УстановитьПараметр("ВидНоменклатурыТара",          Константы.УП_ВидНоменклатурыВозвратнаяТара.Получить());
	...
	|ПО 
	|	Док.Номенклатура                = Остатки.Номенклатура
	|	И (Док.СкладОтправитель                = Остатки.УП_Склад ИЛИ Остатки.УП_Склад = Значение(Справочник.Склады.ПустаяСсылка))
	|   // СОЕДИНЕНИЕ_Характеристика_Остатки
	|   // СОЕДИНЕНИЕ_Серия_Остатки
	|
	|ГДЕ
	|	Док.Ссылка  =  &ДокументСсылка
	|	И Док.Номенклатура.ВидНоменклатуры  =  &ВидНоменклатурыТара
	|
	|СГРУППИРОВАТЬ ПО
	|	Док.Номенклатура,
	|   %ПОЛЕ_Док_Характеристика%,
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаСообщения = "Остатка " + 
			УправлениеЗапасами.ПредставлениеНоменклатуры(Выборка.НоменклатураПредставление, 
									  ?(ЕстьХарактеристика, Выборка.ХарактеристикаНоменклатурыПредставление, ""),
									  ?(ЕстьСерия, Выборка.СерияНоменклатурыПредставление, "")) +
			" полученного по договору """ + СокрЛП(СтруктураШапкиДокумента.ДоговорКонтрагента) + 
			""" недостаточно.";

		ВрОтказ = ОтказПроведения; 
		УправлениеЗапасами.ОшибкаНетОстатка(СтрокаСообщения, Выборка.ОстатокКоличество, Выборка.ДокументКоличество,
			Выборка.ЕдиницаХраненияОстатковПредставление, ВрОтказ, ЗаголовокСообщения);
			
		Если Не РольДоступна("ПолныеПрава") Тогда
			ОтказПроведения = ВрОтказ;
		КонецЕсли;
...
//начало изменений Ожиганов 37889  
Процедура ТоварыВРезервеНаСкладахКонтрольОстатков_СкладВТабЧасти_ЗаказВШапке_Резерв(СтруктураШапкиДокумента, СтруктураПараметров, Заголовок, Отказ)
	
	//начало изменений Ожиганов 18.04.2015 37867  
	ЗнакКоличества = "";
	ДопУсловияТабличнойЧасти = "";
	...
	КонтролироватьНаДату = 	СтруктураШапкиДокумента.РежимПроведения <> РежимПроведенияДокумента.Оперативный;
	Запрос.УстановитьПараметр("ДатаОст",Новый Граница(Новый МоментВремени(СтруктураШапкиДокумента.Дата,СтруктураШапкиДокумента.Ссылка),ВидГраницы.Исключая));
    ...
    ТекстЗапроса = 
	   ...
	   |	Док.ИсходноеРазмещение КАК Склад,
	   |	%ПОЛЕ_Док_Характеристика%   Как ХарактеристикаНоменклатуры,
	   |	%ПОЛЕ_Док_Серия% 			Как СерияНоменклатуры,
	   |	%ПОЛЕ_Док_Количество%       КАК ДокументКоличество,
	   |	ВЫБОР
	   |		КОГДА Док.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРезервированиеТоваров.АП_ОбщийРезерв)
	   |			ТОГДА Док.Ссылка.АП_НазначениеРезерва
	   |		ИНАЧЕ Док.АП_НазначениеРезерва
	   |	КОНЕЦ КАК АП_НазначениеРезерва,
	   |	ВЫБОР
	   |		КОГДА Док.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРезервированиеТоваров.АП_ОбщийРезерв)
	   |			ТОГДА НЕОПРЕДЕЛЕНО
	   |		КОГДА Док.АП_НазначениеРезерва <> ЗНАЧЕНИЕ(Справочник.АП_НазначениеРезервов.ПустаяСсылка)
	   |			ТОГДА НЕОПРЕДЕЛЕНО
	   |		ИНАЧЕ &ДокументРезерва
	   ...
	   |ГДЕ
	   |	Док.Ссылка = &ДокументСсылка
	   |	И Док.ИсходноеРазмещение ССЫЛКА Справочник.Склады
	   |	И Док.ИсходноеРазмещение <> ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	   |	И Не Док.Номенклатура.Комплект
	   |
	   |СГРУППИРОВАТЬ ПО
	   |	Док.Номенклатура,
	   |	Док.ИсходноеРазмещение,
	...
	|   Док.ДокументРезерва                             		КАК ДокументРезерва,
	|   Док.Склад               								КАК Склад,"
	+?(КонтролироватьНаДату,"
	|	ЕСТЬNULL(МАКСИМУМ(РезервыНаДату.КоличествоОстаток), 0) 		КАК РезервыНаДатуКоличество,
	|","") +"	
	|	ЕСТЬNULL(МАКСИМУМ(Резервы.КоличествоОстаток), 0) 		КАК РезервыКоличество
	|ИЗ 
	|	ТаблДок КАК Док
	...
	|	// В остальных случаях необходимо указывать условия
	|	РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки(,  
	|  (Номенклатура,ХарактеристикаНоменклатуры,СерияНоменклатуры,Склад,АП_НазначениеРезерва,ДокументРезерва) в
	|	(ВЫБРАТЬ
	|		ТаблДок.Номенклатура,
	|		ТаблДок.ХарактеристикаНоменклатуры,
	|		ТаблДок.СерияНоменклатуры,
	|		ТаблДок.Склад,
	|		ТаблДок.АП_НазначениеРезерва,
	|		ТаблДок.ДокументРезерва
	|	ИЗ
	|		ТаблДок КАК ТаблДок) 	) КАК Резервы
	|ПО 
	|	Резервы.Номенклатура = Док.Номенклатура
	|   И Резервы.ХарактеристикаНоменклатуры = Док.ХарактеристикаНоменклатуры
	|	И Резервы.Склад = Док.Склад
	|	И Резервы.СерияНоменклатуры 		 = Док.СерияНоменклатуры
	|	И Резервы.ДокументРезерва			 = Док.ДокументРезерва
	|	И Резервы.АП_НазначениеРезерва 		 = Док.АП_НазначениеРезерва
	|"	
	//m.ionov@a-prof.ru 22.10.2014
	+?(КонтролироватьНаДату," 
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки(&ДатаОст,  
	|  (Номенклатура,ХарактеристикаНоменклатуры,СерияНоменклатуры,Склад,АП_НазначениеРезерва,ДокументРезерва) в
	|	(ВЫБРАТЬ
	|		ТаблДок.Номенклатура,
	|		ТаблДок.ХарактеристикаНоменклатуры,
	...
	|		ТаблДок.ДокументРезерва
	|	ИЗ
	|		ТаблДок КАК ТаблДок) 	) КАК РезервыНаДату
	|ПО 
	|	РезервыНаДату.Номенклатура 			   = Док.Номенклатура
	|   И РезервыНаДату.ХарактеристикаНоменклатуры = Док.ХарактеристикаНоменклатуры
	|	И РезервыНаДату.Склад 					   = Док.Склад
	|	И РезервыНаДату.СерияНоменклатуры 		   = Док.СерияНоменклатуры
	|	И РезервыНаДату.ДокументРезерва			   = Док.ДокументРезерва
	|	И РезервыНаДату.АП_НазначениеРезерва 	   = Док.АП_НазначениеРезерва
	|","")+"
	|СГРУППИРОВАТЬ ПО
	|
	|	Док.Номенклатура,
	...
	|	Док.Склад,
	|	Док.ДокументРезерва
	|ИМЕЮЩИЕ ЕСТЬNULL(МАКСИМУМ(Резервы.КоличествоОстаток), 0) < МАКСИМУМ(Док.ДокументКоличество)
	|"+?(КонтролироватьНаДату," ИЛИ ЕСТЬNULL(МАКСИМУМ(РезервыНаДату.КоличествоОстаток), 0)  < МАКСИМУМ(Док.ДокументКоличество) ","")+"
	|ДЛЯ ИЗМЕНЕНИЯ РегистрНакопления.ТоварыВРезервеНаСкладах.Остатки // Блокирующие чтение таблицы остатков регистра для разрешения коллизий многопользовательской работы
	|";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если КонтролироватьНаДату и Выборка.РезервыНаДатуКоличество < Выборка.ДокументКоличество  Тогда
			...
			СтрокаСообщения = "Остатка резерва  "+
				...
				УправлениеЗапасами.ОшибкаНетОстатка(СтрокаСообщения, Выборка.РезервыНаДатуКоличество, Выборка.ДокументКоличество,
				Выборка.ЕдиницаХраненияОстатковПредставление, Отказ, Заголовок);
		...
		СтрокаСообщения = "Остатка резерва " +?(КонтролироватьНаДату," на текущую дату ","")+
