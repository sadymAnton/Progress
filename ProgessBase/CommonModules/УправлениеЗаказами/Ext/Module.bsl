	...
	Если УказаниеЗаказов Тогда
		Если ВидОперации = "Реализация" Тогда
			...
			ИначеЕсли (ДокументВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПродажаКомиссия)
			  ...
			  ИЛИ (ДокументВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.АктВыполненныхРабот) Тогда
...
Процедура СформироватьОтчетАнализЗаказа(ЗаказСсылка,флПроверятьЗаписьЗаказа=ложь, флСообщатьОбОшибках) Экспорт
	...
	//БП4 
		ТипЗнч(ЗаказСсылка) <> Тип("ДокументСсылка.ОтчетПроизводстваЗаСмену") И
		//%%%%%%%%%%%%%%%%%%%%%%%
	...
	Если не УправлениеЗаказами.ПравоДоступаКОтчету("АнализЗаказа", флСообщатьОбОшибках) Тогда
		Возврат;
	КонецЕсли;
	//БП4 Для внутреннего заказа с видом операции Заявка на СИМ - свой отчет
	Если ТипЗнч(ЗаказСсылка) = Тип("ДокументСсылка.ВнутреннийЗаказ") И 
		ЗаказСсылка.ПРГ_ВидОперации	= Перечисления.ПРГ_ВидыОперацийВнутреннегоЗаказа.ЗаявканаСИМ Тогда
		
		АнализЗаказа = Отчеты.ОтчетПоВнутреннемуЗаказу.Создать();
	ИначеЕсли ТипЗнч(ЗаказСсылка) = Тип("ДокументСсылка.ОтчетПроизводстваЗаСмену") Тогда
		АнализЗаказа = Отчеты.ОтчетПоВнутреннемуЗаказу.Создать();
	Иначе
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
//		МоментВремени - момент времени документа для определения остатков
//		СписокЗаказов - массив заказов для отбора
Функция ПодготовитьТаблицуДляЗакрытияЗаказовПокупателейИзЗаданияНаПодбор(ДокСсылка,МоментВремени,СписокЗаказов, Знак = "") Экспорт
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДокументСсылка",            ДокСсылка);
	Запрос.УстановитьПараметр("МоментДокумента",           МоментВремени);
	Запрос.УстановитьПараметр("СписокЗаказов",             СписокЗаказов);
...
Функция ПодготовитьТаблицуДляЗакрытияЗаказовПоставщикам(ДокСсылка,МоментВремени,СписокЗаказов, Знак = "") Экспорт
	...
	//Бирюков добавил ДатуПоставки для реализации списания по ФИФО
	//|	 ДатаПоставки,
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Функция ОстаткиТоваровПоЗаказуПоставщику( Заказ, Договор, КонДата, ДопПараметры = Неопределено) Экспорт
	...
	//Бирюков добавим заполнение ДатыПоставки
	ТекстЗапроса =
		//"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		//|	1 КАК Источник,
		//|	РегЗаказы.Номенклатура КАК Номенклатура,
		...
		|	РегЗаказы.ХарактеристикаНоменклатуры КАК ХарактеристикаНоменклатуры,
		//{24.11.2014 Островерхий заявка №б/н 
		|	РегЗаказы.УдалитьДатаПоставки КАК ДатаПоставки 
		//24.11.2014 Островерхий} ,
		|	РегЗаказы.Цена,
		|	РегЗаказы.СтавкаНДС,
		|	РегЗаказы.ЕдиницаИзмерения,
		...
		Пока ОбходХарактеристика.Следующий() Цикл
			...
			Пока Обход.Следующий() Цикл
				...
				Если Обход.Источник = 1 Тогда // Это строка с остатком. Добавляем в документ
					...
					//{24.11.2014 Островерхий заявка №б/н 
					////Бирюков добавим заполнение ДатыПоставки
					Если ТЗ.Колонки.Найти("ДатаПоставки")<>неопределено Тогда
						НоваяСтрока.ДатаПоставки = Обход.ДатаПоставки;
					КонецЕсли;
					////%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
					//24.11.2014 Островерхий} 
...
Процедура ПроверитьРеквизитыЗаказов(ДокументОбъект, ИмяТабличнойЧасти, ИмяРевизитаЗаказ, 
	...
	//начало изменений Ожиганов 03.08.2015 б/н уберем получение данных по всему заказу 
	КэшЗаказов = Новый ТаблицаЗначений;
	КэшЗаказов.Колонки.Добавить("Заказ");
	КэшЗаказов.Колонки.Добавить("Реквизиты");
	КэшЗаказов.Индексы.Добавить("Заказ");
	//конец изменений  
	...
	Для Каждого СтрокаТаблицы Из ДокументОбъект[ИмяТабличнойЧасти] Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы[ИмяРевизитаЗаказ]) И 
			...
			//начало изменений Ожиганов 03.08.2015 б/н уберем получение данных по всему заказу 
			НайдЗнч = КэшЗаказов.Найти(СтрокаТаблицы[ИмяРевизитаЗаказ],"Заказ");
			Если НайдЗнч <> Неопределено Тогда
				ДокЗаказ = НайдЗнч.Реквизиты;
			Иначе
				НайдЗнч = КэшЗаказов.Добавить();
				НайдЗнч.Заказ = СтрокаТаблицы[ИмяРевизитаЗаказ];
				НайдЗнч.Реквизиты = ПРГ_ДопФункцииКлиентСервер.ПолучитьСтруктРевизитаЗаказа(СтрокаТаблицы[ИмяРевизитаЗаказ],Новый Структура("Организация,Контрагент,ДоговорКонтрагента,Дата"));
				ДокЗаказ =  НайдЗнч.Реквизиты;
			КонецЕсли;	
