...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, БанковскийСчет = Неопределено) Экспорт
	...
	//Бирюков добавление адреса погрузки для печатных форм
//	Сведения = Новый Структура("Представление, ПолноеНаименование, НаименованиеСокращенное, КодПоОКПО, ИНН, КПП, Телефоны, ЮридическийАдрес, Банк, БИК, КоррСчет, НомерСчета, АдресБанка");
	Сведения = Новый Структура("Представление, ПолноеНаименование, НаименованиеСокращенное, КодПоОКПО, ИНН, КПП, Телефоны, ЮридическийАдрес, Банк, БИК, КоррСчет, НомерСчета, АдресБанка, АдресПогрузки");
	...
	ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Организации") Тогда
		...
		|					ТОГДА  """"
		|				ИНАЧЕ
		|					ВЫБОР КОГДА КонтактнаяИнформация.Вид = &ВидАдресПогрузки 
		|						ТОГДА ВЫРАЗИТЬ(КонтактнаяИнформация.Представление КАК Строка(1000))
		|	  					ИНАЧЕ """"
		|					КОНЕЦ
		...
		Если ЮрФизЛицо.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда 
			ТекстУсловияОбъектКонтактнойИнформации = ТекстУсловияОбъектКонтактнойИнформации +"
			|	И КонтактнаяИнформация.Вид В (&ВидЮридическийАдресФизЛицо, &ВидФактическийАдресФизЛицо, &ВидЮридическийАдрес, &ВидФактическийАдрес, &ВидТелефона)
			|	И КонтактнаяИнформация.Тип В (&ТипАдрес, &ТипТелефон)";
		Иначе
			ТекстУсловияОбъектКонтактнойИнформации = ТекстУсловияОбъектКонтактнойИнформации +"
			|	И КонтактнаяИнформация.Вид В (&ВидЮридическийАдресФизЛицо, &ВидФактическийАдресФизЛицо, &ВидЮридическийАдрес, &ВидФактическийАдрес, &ВидАдресПогрузки, &ВидТелефона)
			|	И КонтактнаяИнформация.Тип В (&ТипАдрес, &ТипТелефон)";
		КонецЕсли;
		...
		Запрос.УстановитьПараметр("ВидФактическийАдрес",        Справочники.ВидыКонтактнойИнформации.ФактАдресОрганизации);
		Запрос.УстановитьПараметр("ВидЮридическийАдрес",        Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации);
		Запрос.УстановитьПараметр("ВидАдресПогрузки", 			Справочники.ВидыКонтактнойИнформации.АдресПогрузкиОрганизации);
		//Бирюков конец изменений
	...
	ИначеЕсли ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		|	, МАКСИМУМ (ВЫБОР КОГДА КонтактнаяИнформация.Вид = &ВидФактическийАдрес ТОГДА ВЫРАЗИТЬ(КонтактнаяИнформация.Представление КАК Строка(1000))
		|	  	ИНАЧЕ """" КОНЕЦ) КАК ФактическийАдрес
		|	, МАКСИМУМ (ВЫБОР КОГДА КонтактнаяИнформация.Вид = &ВидАдресПогрузки ТОГДА ВЫРАЗИТЬ(КонтактнаяИнформация.Представление КАК Строка(1000))
		|	  	ИНАЧЕ """" КОНЕЦ) КАК АдресПогрузки";
		
		ТекстУсловияОбъектКонтактнойИнформации = 
		"КонтактнаяИнформация.Объект = Организации.Ссылка
		|	И КонтактнаяИнформация.Вид В (&ВидЮридическийАдрес, &ВидФактическийАдрес, &ВидАдресПогрузки, &ВидТелефона)
		|	И КонтактнаяИнформация.Тип В (&ТипАдрес, &ТипТелефон)";
		...
		Запрос.УстановитьПараметр("ВидЮридическийАдрес", Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента);
		Запрос.УстановитьПараметр("ВидФактическийАдрес", Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента);
		Запрос.УстановитьПараметр("ВидАдресПогрузки",    Справочники.ВидыКонтактнойИнформации.АдресПогрузкиКонтрагента);
		//Бирюков конец изменений
	...
	Если ЮрФизЛицо.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
		...
		//Бирюков добавление адреса погрузки для печатных форм
		Сведения.Вставить("АдресПогрузки", Данные.АдресПогрузки);
		//Бирюков конец изменений
	Иначе
...
Функция ПолучитьАдресИзКонтактнойИнформации(Объект, ТипАдреса = "") Экспорт
	...
	Если ТипЗнч(Объект) = Тип("СправочникСсылка.АдресаПоставки") Тогда
		ВидАдреса = Справочники.ВидыКонтактнойИнформации.АП_ФактАдресАдресаПоставки;
	Иначе
