...
Функция РазузловатьНоменклатуру(Источник, Результат = Неопределено, Параметры = Неопределено,РазузловыватьНоменклатуруСоСпецификацией = Ложь,РассчитыватьПотребность=Ложь) Экспорт
		...
		Для каждого СтрокаТекущегоУровня из СтрокиТекущегоУровня Цикл
			...
			Если СтрокаТекущегоУровня.Спецификация <> Неопределено И НЕ СтрокаТекущегоУровня.Спецификация.Пустая() Тогда
				...
				Пока ИсходноеКомплектующее.Следующий() Цикл
					...
					//начало изменений БП 14 
					ДобавитьВПолноеРазулование(ИсходноеКомплектующее, СтрокаТекущегоУровня, КоличествоВыпусков, КоэффициентОсновногоСырья, Параметры, Уровень, Результат, МассивОшибок,,РазузловыватьНоменклатуруСоСпецификацией,РассчитыватьПотребность);
					//конец изменений БП 14 
					...
					Если Параметры.ДоПервогоПФ Тогда
						Если Не ИсходноеКомплектующее.ЭтоКупаж
							И ИсходноеКомплектующее.ВидВоспроизводства = Перечисления.ВидыВоспроизводстваНоменклатуры.Производство Тогда
							Параметры.КоличествоУровнейРазузлования = Уровень;
						КонецЕсли;
					КонецЕсли;
					//------- m_ionov@mail.ru -------					
				...
				//{22.10.2014 Островерхий заявка №32204 
				Если Параметры.ИспользоватьВозвратныеОтходы Тогда
...
//начало изменений БП 14 
Функция ДобавитьВПолноеРазулование(СтрокаСпецификации, ИсходнаяСтрока, КоличествоВыпусков, КоэффициентОсновногоСырья, Параметры, Уровень, Результат, МассивОшибок, ПоследовательностьУзлов = Неопределено, РазузловыватьНоменклатуруСоСпецификацией = Ложь, РассчитыватьПотребность=Ложь)
//конец изменений БП 14 	
	...
	ЭтоУзел = РазузловыватьНоменклатуруСоСпецификацией и ЗначениеЗаполнено(СтрокаСпецификации.Спецификация) и СтрокаСпецификации.ВидНорматива = Перечисления.ВидыНормативовНоменклатуры.Номенклатура;
	
	
	Если ЭтоУзел тогда
		Если ДобавитьСпецификациюВПоследовательность(СтрокаСпецификации.Спецификация, ПоследовательностьУзлов) Тогда
			...
			ИсходноеКомплектующее = ПолучитьИсходныеКомплектующие(СтрокаСпецификации.Спецификация, ИсходнаяСтрока, Новый Структура("НомерОперацииМаршрута",СтрокаСпецификации.НомерОперацииМаршрута));
			...
			Пока ИсходноеКомплектующее.Следующий() Цикл
				...
				КолвоВыхИзделия = ПолучитьКоличествоВыхИзделияПоСпецификации(СтрокаСпецификации.Номенклатура,СтрокаСпецификации.Спецификация,МассивОшибок);
				Если КолвоВыхИзделия = 0 Тогда
					ВызватьИсключение "Спецификация не может быть полной."+СтрокаСпецификации.Спецификация+" Ошибка разузлования!!!";
				КонецЕсли;	
				...
				КоличествоВыпусковУзла = УправлениеПланированием.ПолучитьДробь(ПараметрыИсходногоКомплектующего.Количество * КоличествоВыпусков.Числитель, КоличествоВыпусков.Знаменатель * КолвоВыхИзделия);
		...
		КонецЕсли;
	//конец изменений БП 04 	
	ИначеЕсли СтрокаСпецификации.ВидНорматива = Перечисления.ВидыНормативовНоменклатуры.Номенклатура Тогда
		...
		//начало изменений БП 04
		НоваяСтрока.ВариантРасчетаСВ = СтрокаСпецификации.ВариантРасчетаСВ;
		//конец  изменений БП 04
		
		// <- Шевченков 20150807 №42531
		//Если СтрокаСпецификации.ЭтоКупаж Тогда
			НоваяСтрока.КодКупажа = ИсходнаяСтрока.Спецификация.Код;
		//КонецЕсли;
		// ->
		//m_ionov@mail.ru 13.10.2016
		НоваяСтрока.ЭтоКупаж = НоваяСтрока.Номенклатура.ВидНоменклатуры = Константы.ПРГ_ВидНоменклатурыКупаж.Получить();
		//------- m_ionov@mail.ru -------
		...
		Если СтрокаСпецификации.УказаниеНорматива = Перечисления.ВидыУказанияНорматива.НаКоличествоОсновногоСырья Тогда
			//m.ionov@a-prof.ru 10/12/2013- обработаем с учетом купажа
			//НоваяСтрока.Количество = НоваяСтрока.Количество * КоэффициентОсновногоСырья;
			//начало изменений БП 04 изменения связаны с тем что потребность и фактический расход рассчитываются по разному
			Если РассчитыватьПотребность и СтрокаСпецификации.ЭтоКупаж //НоваяСтрока.Номенклатура.ВидНоменклатуры = Константы.ПРГ_ВидНоменклатурыКупаж.Получить()
			//конец изменений БП 04 	
				И ИсходнаяСтрока.ЕдиницаИзмерения.ПРГ_Нетто > 0 Тогда
				
				НоваяСтрока.Количество = ИсходнаяСтрока.ЕдиницаИзмерения.ПРГ_Нетто * КоэффициентОсновногоСырья;
		...
		Иначе
			...
			Если РассчитыватьПотребность и СтрокаСпецификации.ЭтоКупаж //НоваяСтрока.Номенклатура.ВидНоменклатуры = Константы.ПРГ_ВидНоменклатурыКупаж.Получить()   
				И ИсходнаяСтрока.ЕдиницаИзмерения.ПРГ_Нетто > 0 Тогда
			//конец изменений БП 04 	
				НоваяСтрока.Количество = ИсходнаяСтрока.ЕдиницаИзмерения.ПРГ_Нетто * КоличествоВыпусков.Числитель / КоличествоВыпусков.Знаменатель;
								
			Иначе
				//m_ionov@mail.ru 13.10.2016
				//НоваяСтрока.Количество = НоваяСтрока.Количество * КоличествоВыпусков.Числитель / КоличествоВыпусков.Знаменатель;
				Если ИсходнаяСтрока.Спецификация.ВидСпецификации = Перечисления.ВидыСпецификаций.Полная
					И Не РассчитыватьПотребность Тогда
					//Если вид спецификации Полный и это не для расчета потребностей - то мы должны получить количество в соответсвии с долей стоимости
					НоваяСтрока.Количество = НоваяСтрока.Количество * (КоличествоВыпусков.Числитель / КоличествоВыпусков.Знаменатель) * ИсходнаяСтрока.ДоляСтоимости;
				Иначе
					НоваяСтрока.Количество = НоваяСтрока.Количество * КоличествоВыпусков.Числитель / КоличествоВыпусков.Знаменатель;
				КонецЕсли;				
		...
		//m.ionov@a-prof.ru учтем ПроцентПотерь
		//начало изменений БП 04 изменения связаны с тем что потребность и фактический расход рассчитываются по разному
		Если РассчитыватьПотребность и Не СтрокаСпецификации.Владелец().Колонки.Найти("ПроцентПотерь") = Неопределено Тогда
		//конец изменений БП 04	
			НоваяСтрока.Количество = НоваяСтрока.Количество*(1 + СтрокаСпецификации.ПроцентПотерь/100);	
		КонецЕсли;
		//---- m.ionov@a-prof.ru
		...
		УстановитьВидВоспроизводства(НоваяСтрока);
		//начало изменений БП 04
		//очередные правки :)) разузловываем если не вид воспроизводство производство
		//СтрокаСпецификации
		Попытка
			Если СтрокаСпецификации.ВидВоспроизводства = Перечисления.ВидыВоспроизводстваНоменклатуры.Производство Тогда
	...
	ИначеЕсли СтрокаСпецификации.ВидНорматива = Перечисления.ВидыНормативовНоменклатуры.Узел Тогда
		...
		Если ДобавитьСпецификациюВПоследовательность(СтрокаСпецификации.Номенклатура, ПоследовательностьУзлов) Тогда
			...
			Пока ИсходноеКомплектующее.Следующий() Цикл
				...
				//начало изменений БП 14
				КоэффициентУзла = ?(ИсходноеКомплектующее.КоэффициентУзла<=0,1,ИсходноеКомплектующее.КоэффициентУзла);
				//конец изменений БП 14 
				РассчитатьКоличествоПоФормуле(ИсходнаяСтрока, СтрокаСпецификации, ПараметрыИсходногоКомплектующего, КоэффициентОсновногоСырья, Параметры.ПараметрыВыпуска, МассивОшибок);
				
				//начало изменений БП 14 
				//КоличествоВыпусковУзла = УправлениеПланированием.ПолучитьДробь(ПараметрыИсходногоКомплектующего.Количество * КоличествоВыпусков.Числитель, КоличествоВыпусков.Знаменатель);
				КоличествоВыпусковУзла = УправлениеПланированием.ПолучитьДробь(ПараметрыИсходногоКомплектующего.Количество * КоличествоВыпусков.Числитель, КоличествоВыпусков.Знаменатель*КоэффициентУзла);
				//конец изменений БП 14 
...
Процедура ДобавитьВСопутствующиеИзделия(Источник, Результат)
	...
	Пока ВыходныеИзделия.Следующий() Цикл
		...
		//m_ionov@mail.ru 11.10.2016
		//Приводим кол-во сопутсвующего товара к кол-ву выпуска
		Если КолВоОсновногоПродукта > 0 И ВыходныеИзделия.СопутствующееИзделие Тогда
			НоваяСтрока.Количество = Источник.Количество*(НоваяСтрока.Количество/КолВоОсновногоПродукта);	
		КонецЕсли;
		
		КолВоОсновногоПродукта = НоваяСтрока.Количество;
		//------- m_ionov@mail.ru -------
...
Процедура ОбновитьСопутствующиеИзделия(НоваяСтрока, ИсходнаяСтрока, Результат)
	...
	КонецЕсли;
	//начало изменений БП 04
	//пока без округлений в большую сторону
	возврат;
	//конец изменений БП 04 
...
Функция РазместитьВСопутствующихИзделиях(Источник, Параметры, Результат)
	...
	//m_ionov@mail.ru 13.10.2016
	//Принято решение что распределение исходных комплектующих между несколькими выходными продуктами будет по СВ
	//Поэтому корректировать колличество на основании данных сопуствующих изделий не нужно
	Возврат НеразмещенноеКоличество;
	//------- m_ionov@mail.ru -------
	...
	Для каждого СтрокаСопутствующегоИзделия из СтрокиСопутствующихИзделий Цикл
		...
		Если СтрокаСопутствующегоИзделия.Количество > 0 Тогда
			...
			//m_ionov@mail.ru 25.09.2016
			//Если вид спецификации полная, то при выпуске основного продукта мы уже получили какое то количество сопуствующего товара,
			//поэтому если нужно считать потребности - то только на ту часть сопуствующего которого не выпустили
			//------- m_ionov@mail.ru -------			
...
Функция ПолучитьИсходныеКомплектующие(Спецификация, Источник, НаследуемыеСвойства = Неопределено)
	...
	|	ИсходныеКомплектующие.Ссылка.ПРГ_КоэффициентДляУзла КАК КоэффициентУзла,
	...
	|	ИсходныеКомплектующие.Номенклатура.ЕдиницаХраненияОстатков.ПРГ_Нетто КАК ПРГ_Нетто,
	|	ИсходныеКомплектующие.ПроцентПотерь КАК ПроцентПотерь,
	|	ВЫБОР
	|		КОГДА ИсходныеКомплектующие.ПРГ_ВариантРасчетаСВ = ЗНАЧЕНИЕ(Перечисление.ПРГ_ВариантыРасчетаСВ.СкладыватьФактНаБазис)
	|			ТОГДА 2
	|		КОГДА ИсходныеКомплектующие.ПРГ_ВариантРасчетаСВ = ЗНАЧЕНИЕ(Перечисление.ПРГ_ВариантыРасчетаСВ.НеУчитыватьКонцентрацию)
	|			ТОГДА 3
	|		КОГДА ИсходныеКомплектующие.Ссылка.ПРГ_ПересчитыватьНаБазис
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ВариантРасчетаСВ,
	|	ВЫБОР
	...
	|	ИсходныеКомплектующие.СтатьяЗатрат КАК СтатьяЗатратКомпл,
	...
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(ИсходныеКомплектующие.Номенклатура КАК Справочник.Номенклатура).ВидНоменклатуры = &ВидНоменклатурыКупаж
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыВоспроизводстваНоменклатуры.Производство)
	|		КОГДА ИсходныеКомплектующие.Ссылка.ИспользоватьВидВоспроизводства = ИСТИНА
	|			ТОГДА ВЫБОР
	|					КОГДА ИсходныеКомплектующие.ВидВоспроизводства = ЗНАЧЕНИЕ(Перечисление.ВидыВоспроизводстваНоменклатуры.ПустаяСсылка)
	|						ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыВоспроизводстваНоменклатуры.Закупка)
	|					ИНАЧЕ ИсходныеКомплектующие.ВидВоспроизводства
	|				КОНЕЦ
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыВоспроизводстваНоменклатуры.Закупка)
	...
	|	ВЫРАЗИТЬ(ИсходныеКомплектующие.Номенклатура КАК Справочник.Номенклатура).ВидНоменклатуры КАК ВидНоменклатуры,
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(ИсходныеКомплектующие.Номенклатура КАК Справочник.Номенклатура).ВидНоменклатуры = &ВидНоменклатурыКупаж
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	Запрос.УстановитьПараметр("ВидНоменклатурыКупаж", Константы.ПРГ_ВидНоменклатурыКупаж.Получить());
...
Функция ПолучитьСпецификацию(ИсходнаяСтрока, Параметры)
	...
	//m.ionov@a-prof.ru 09/12/2013 - попросили все равно проводить разузлование, даже если вид вопроизводства не производство.
	...
	Если Спецификация = Неопределено Тогда
		...
		Спецификация = ПРГ_ДопФункцииКлиентСервер.ОпределитьСпецификацию(ИсходнаяСтрока.Номенклатура, ИсходнаяСтрока.ХарактеристикаНоменклатуры, Параметры.ДатаСпецификации, Подразделение)
...
Процедура УстановитьПараметрыВыходногоИзделия(НоваяСтрока)
	...
	//m_ionov@mail.ru 13.10.2016
	//Добавил заполнение доли стоимости
	//------- m_ionov@mail.ru -------
	...
	Иначе
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка, "НомерОперацииМаршрута, МинимальнаяПартия, Кратность, ТочкаМаршрута, Подразделение, РабочийЦентр, ДоляСтоимости");
...
Процедура УстановитьРазузлован(НоваяСтрока)
	...
	Иначе
		...
		//m.ionov@a-prof.ru 09/12/2013 - разузловываем без учета вида вопроизводства
		Если ЗначениеЗаполнено(НоваяСтрока.Спецификация) Тогда
			НоваяСтрока.Разузлован = Ложь;
			НоваяСтрока.ВидВоспроизводства = Перечисления.ВидыВоспроизводстваНоменклатуры.Производство;
		Иначе
		//---- m.ionov@a-prof.ru
			 НоваяСтрока.Разузлован = Истина;
		КонецЕсли;
...
Процедура ИнициализацияРезультатаРазузлования(Результат, Параметры)
	...
	Если Результат.Свойство("ПолноеРазузлование") И Результат.ПолноеРазузлование = Неопределено Тогда
		...
		//m.ionov@a-prof.ru 09/12/2013 - Точность количества увеличиваем до того же как и в спецификации
		//Результат.ПолноеРазузлование.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)));
		Результат.ПолноеРазузлование.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 8)));
		//---- m.ionov@a-prof.ru
		...
		//Увеличиваем точность распреления по спецификации между полуфабрикатами
		//Результат.ПолноеРазузлование.Колонки.Добавить("ДоляСтоимости", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
		Результат.ПолноеРазузлование.Колонки.Добавить("ДоляСтоимости", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 4)));
		//------- m_ionov@mail.ru -------
		...
		//начало изменений БП 04
		//0 не  пересчитывать на базис 1 пересчитывать на базис 2 складывать факт на базис 3 Не учитывать концентрацию сырья
		Результат.ПолноеРазузлование.Колонки.Добавить("ВариантРасчетаСВ",Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(10,0)));
		//конец изменений БП 04 
		Результат.ПолноеРазузлование.Колонки.Добавить("КодКупажа", Новый ОписаниеТипов("Строка")); // Шевченков 20150807 №42531
		//m_ionov@mail.ru 13.10.2016
		Результат.ПолноеРазузлование.Колонки.Добавить("ЭтоКупаж", Новый ОписаниеТипов("Булево"));
		//------- m_ionov@mail.ru -------
	...
	Если Результат.Свойство("ВыходныеИзделия") И Результат.ВыходныеИзделия = Неопределено Тогда
		...
		Результат.ВыходныеИзделия.Колонки.Добавить("ДоляСтоимости", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 4)));
		//------- m_ionov@mail.ru -------
	...
	Если Результат.Свойство("ИсходныеКомплектующие") И Результат.ИсходныеКомплектующие = Неопределено Тогда
		...
		Результат.ИсходныеКомплектующие.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 8)));
		//---- m.ionov@a-prof.ru		
		...
		//начало изменений БП 04
		//0 не  пересчитывать на базис 1 пересчитывать на базис 2 складывать факт на базис 3 Не учитывать концентрацию сырья
		Результат.ИсходныеКомплектующие.Колонки.Добавить("ВариантРасчетаСВ",Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(10,0)));
		//конец изменений БП 04 
		Результат.ИсходныеКомплектующие.Колонки.Добавить("КодКупажа", Новый ОписаниеТипов("Строка")); // Шевченков 20150807 №42531
		//m_ionov@mail.ru 13.10.2016
		Результат.ИсходныеКомплектующие.Колонки.Добавить("ЭтоКупаж", Новый ОписаниеТипов("Булево"));
		//------- m_ionov@mail.ru -------
	...
	Если (Параметры.РазмещатьВСопутствующихИзделиях И (НЕ Результат.Свойство("СопутствующиеИзделия") ИЛИ Результат.Свойство("СопутствующиеИзделия") И Результат.СопутствующиеИзделия = Неопределено)) ИЛИ (Результат.Свойство("СопутствующиеИзделия") И Результат.СопутствующиеИзделия = Неопределено) Тогда
		...
		Результат.СопутствующиеИзделия.Колонки.Добавить("ДоляСтоимости", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 4)));
		//------- m_ionov@mail.ru -------
...
Процедура ИнициализацияПараметров(Параметры)
	...
	//{22.10.2014 Островерхий заявка №32204 
	Если НЕ Параметры.Свойство("ИспользоватьВозвратныеОтходы") Тогда
		
		Параметры.Вставить("ИспользоватьВозвратныеОтходы", Константы.ПРГИспользоватьВозвратныеОтходы.Получить());
		
	КонецЕсли; 
	//22.10.2014 Островерхий}
	
	//m_ionov@mail.ru 13.10.2016
	Если НЕ Параметры.Свойство("ДоПервогоПФ") Тогда
		//Не не задано ограничение по уровню разузлования, то можно задать что разузловывать до первого ПФ
		//Данная потребность возникла для корректного заполнения отчета производства материалами при производстве белой основы
		Параметры.Вставить("ДоПервогоПФ", Ложь);
...
//начало изменений БП 14 
Функция ПолучитьКоличествоВыхИзделияПоСпецификации(Номенклатура,Спецификация,МассивОшибок)
	// проверм сколько выходных изделий
	Запрос = Новый Запрос;
	...
	Если Выборка.Следующий() Тогда
		Если Выборка.Количество <> 1 Тогда
			ДобавитьВОшибки(СтатусСообщения.ОченьВажное, НСтр("ru='В спецификации обнаружено несколько выходных измделий. Допускается одно !!!'"), "Ошибка", Спецификация, Неопределено,Неопределено, МассивОшибок);
			возврат 0;
		КонецЕсли;	
	Иначе
		 ДобавитьВОшибки(СтатусСообщения.ОченьВажное, 
		 НСтр("ru='Не найдена ошибка !!!'"),
		 "Ошибка", Спецификация, Неопределено,Неопределено, МассивОшибок);
		 возврат 0 ;
	КонецЕсли;	
	 
	Запрос.Текст  = "ВЫБРАТЬ
	                |	СпецификацииНоменклатурыВыходныеИзделия.Номенклатура,
	                |	СпецификацииНоменклатурыВыходныеИзделия.ХарактеристикаНоменклатуры,
	                |	СпецификацииНоменклатурыВыходныеИзделия.Количество * ЕСТЬNULL(СпецификацииНоменклатурыВыходныеИзделия.ЕдиницаИзмерения.Коэффициент, 1) КАК Количество,
	                |	СпецификацииНоменклатурыВыходныеИзделия.МинимальнаяПартия
	                |ИЗ
	                |	Справочник.СпецификацииНоменклатуры.ВыходныеИзделия КАК СпецификацииНоменклатурыВыходныеИзделия
	                |ГДЕ
	                |	СпецификацииНоменклатурыВыходныеИзделия.Ссылка = &Спецификация
	...
	Выборка  = Запрос.Выполнить().Выбрать();				
	Если Выборка.Следующий() Тогда
		 возврат ?(Выборка.Количество>0, Выборка.Количество,1);
	 Иначе
		 ДобавитьВОшибки(СтатусСообщения.ОченьВажное, 
		 НСтр("ru='Не найдено спецификация для !!!'"),
		 "Ошибка", Спецификация, Неопределено,Неопределено, МассивОшибок);
		 
		МассивОшибок.Добавить("Не найдено спецификация для "+Номенклатура);
	КонецЕсли;	
