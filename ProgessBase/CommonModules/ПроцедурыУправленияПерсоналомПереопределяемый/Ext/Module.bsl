...
// Измайлова Полина 23.06.2011 10:37 
//  AVT_Календарь - ссылка на проиводственный календарь
////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
Функция РегламентированныйКалендарьЗаполнен(ДатаНач, ДатаКон, ВыводитьСообщения = Ложь, AVT_Календарь=Неопределено) Экспорт 

	Сообщение = "";
	КалендарьЗаполнен = ПроцедурыУправленияПерсоналом.РегламентированныйКалендарьЗаполнен(ДатаНач, ДатаКон, Сообщение, AVT_Календарь); 
...
//
Функция СведенияОПозицииШтатногоРасписания(Период, Подразделение, Должность, ШтатноеРасписание) Экспорт

	///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Петрухин Андрей 23.06.2011 00:02 
	// ОПИСАНИЕ: Добавили в запрос условие  "ШтатноеРасписание"
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}
	Запрос = Новый Запрос;
	...
	|И AVTШтатноеРасписание = &ШтатноеРасписание) КАК ШтатноеРасписаниеОрганизацийСрезПоследних";
...
Функция СформироватьТекстЗапросаПоПроверкеСоответствияШтатномуРасписанию() Экспорт 
	...
	|		ТЧНачисления.Должность КАК Должность,
		///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 25.10.2011 15:33 
	|		ТЧНачисления.AVTШтатноеРасписание КАК AVTШтатноеРасписание,
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА} 
	...
	|			и ТЧНачисления.AVTШтатноеРасписание = ШтатноеРасписаниеОрганизаций.AVTШтатноеРасписание
	...
	|		ТЧНачисления.Должность,
			///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 25.10.2011 15:37 
...
//
///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
// Измайлова Полина 21.06.2011 16:55 
//	(12) AVT_Календарь	- ссылка на справочник AVT_ПроизводственныеКалендари, используется для отбора графиков работы
////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
Процедура ОткрытьФормуВыбораСотрудникаОрганизации(Владелец, Ключ, ЗакрыватьПриВыборе, ДатаСреза, ГоловнаяОрганизация = Неопределено, Режим = 0, СтандартнаяОбработка = Неопределено, ТекущееЗначение = Неопределено, НеВыбиратьГруппы = Истина, ВыборФизлица = Ложь, ПараметрыПодбора = Неопределено, AVT_Календарь = Неопределено) Экспорт
	...
	///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 21.06.2011 16:57 
	//Если Не AVT_Календарь = Неопределено Тогда
	//	Форма.Отбор.ГрафикРаботы.ВидСравнения  = ВидСравнения.ВСписке;
	//	Форма.Отбор.ГрафикРаботы.Значение      = AVT_ОбщегоНазначения.ПолучитьГрафикиРаботыПоКалендарю(AVT_Календарь);
	//	Форма.Отбор.ГрафикРаботы.Использование = Истина;	
	//КонецЕсли;
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          	
