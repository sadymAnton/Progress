	...
	Для каждого СтрокаОбъекта Из ДеревоПолейЗапросаПоШапке.Строки Цикл
		...
		Если Объект = "Сделка" ИЛИ Объект = "РасчетныйДокумент" Тогда
			...
			Для Каждого СтрокаПоля Из СтрокаОбъекта.Строки Цикл
				//начало изменений Ожиганов 30.05.2015 использование даты и номера
				Если Нрег(СтрокаПоля.Поле) = "дата" или Нрег(СтрокаПоля.Поле) = "номер" Тогда
					ИмяОбъекта = " Док."+Объект+ "." + СокрЛП(СтрокаПоля.Поле);
				ИначеЕсли ОбъектМетаданные = Неопределено 
				//конец изменений 	
		...
		ИначеЕсли Объект = "ВЫРАЖЕНИЕ" Тогда
			//Особым образом обрабатывается значение NULL
			//	Значение NULL используется, если некоторому значению в структуре шапки требуется 
			//	явно присвоить значение  NULL
			Для Каждого СтрокаПоля Из СтрокаОбъекта.Строки Цикл
				СтрокиЗапроса = СтрокиЗапроса + ",
				|	"+СокрЛП(СтрокаПоля.Поле)+?(НЕ ЗначениеЗаполнено(СтрокаПоля.Псевдоним), "", " КАК " + СокрЛП(СтрокаПоля.Псевдоним));
			КонецЦикла
		//конец изменений 
		Иначе
...
Процедура ПроверитьЧтоНетУслуг(ДокументОбъект, ИмяТабличнойЧасти, ТаблицаЗначений = Неопределено, 
	...
	Если (НЕ ТаблицаЗначений = Неопределено) И (ТаблицаЗначений.Количество() > 0) Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
			...
			//m.ionov@a-prof.ru 23.04.2014
			ИначеЕсли ИмяТабличнойЧасти = "ВозвратнаяТара" 
			И (ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПоступлениеТоваровУслуг") ИЛИ ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ВозвратТоваровПоставщику"))
			И ЗначениеЗаполнено(ВидНоменклатурыТара) Тогда
			
				Если ДокументОбъект.ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком 
					И СтрокаТаблицы.Номенклатура.ВидНоменклатуры = ВидНоменклатурыТара Тогда
					
					ТекстОшибки = НСтр("ru = 'В строке номер ""%НомерСтроки%"" табличной части ""%ПредставлениеТабличнойЧасти%"": содержится возвратная тара (счет 002). Возвратной тары по договору с поставщиком здесь быть не должно!'");
					...
					ВрОтказ = Отказ;
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, ДокументОбъект, ТекстПоля,, ВрОтказ);
					Если Не РольДоступна("ПолныеПрава") Тогда
						Отказ = ВрОтказ;
					КонецЕсли;
					
				ИначеЕсли ДокументОбъект.ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомитентом 
					И Не СтрокаТаблицы.Номенклатура.ВидНоменклатуры = ВидНоменклатурыТара Тогда
					
					ТекстОшибки = НСтр("ru = 'В строке номер ""%НомерСтроки%"" табличной части ""%ПредставлениеТабличнойЧасти%"": содержится тара. По договору ответсвенного хранения может указываться только возвратная тара!'");
...
//конец изменений 										   
	...
	//начало изменений Ожиганов проведение с 0 количествоv
	ИначеЕсли ПРГ_УчитыватьКоличество Тогда
	     Запрос.Текст = СтрЗаменить(Запрос.Текст, "ГДЕ Док.Ссылка = &ДокументСсылка", "ГДЕ Док.Ссылка = &ДокументСсылка и Док.Количество > 0");
	//конец изменений 
	КонецЕсли;
...
Процедура СообщитьОНедостаткеТовара(Требуется, Свободно = 0, ВРезерве = 0, КПередаче = 0, КПолучению = 0, Отказ, Заголовок="", Склад = "", ЗаказПоставщику = "", Номенклатура = "", Характеристика = "", Серия = "", Качество = "", ЕИ = "", СтрокаСообщения = "") Экспорт
	...
	//++ Spl_Апроф 03.03.2015 (k.russkih@a-prof.ru) Добавим вывод кода номенклатуры, чтобы было понятно
	Если ТипЗнч(Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
		СтрокаСообщения = СтрокаСообщения + "'" + СокрЛП(Номенклатура.Код) + "' " + Номенклатура + Характеристика + Серия;
	Иначе	
