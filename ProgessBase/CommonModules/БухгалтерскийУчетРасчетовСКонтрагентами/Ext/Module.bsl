...
Функция ПодготовкаСтруктурыПараметровДляДвиженияДенег(Ссылка, ВалютаРегламентированногоУчета, Заголовок = Неопределено,КоррСчет=Неопределено, ТаблицаДокумента = Неопределено, СтруктураШапкиДокумента = Неопределено) Экспорт
    ...
    // <- Шевченков
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручениеВходящее")
		И Ссылка.ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратДенежныхСредствПоставщиком Тогда
		СтруктураТаблицы.Вставить("Претензия");
	КонецЕсли;
	// ->
	//Blik 020414 26538 н 
	//Бирюков 03_07_14 м.б. и взаимозачеты и переносы долга
	//Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.КорректировкаДолга")
	//	И Ссылка.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.СписаниеЗадолженности Тогда
...
//начало изменений
Функция ПодготовитьТаблицуДляРегистраРасчетовПоПриобретению(СтруктураПараметровВзаиморасчетов, СтруктураШапкиДокумента,ТаблСторноУслуг=Неопределено) Экспорт
//конец изменений 
	...
	Если ЕстьНесколькоСФ = Истина И СтруктураШапкиДокумента.УЗ_НесколькоСФ Тогда
	...
	//31_01_14 Бирюков нужно учитывать во взаиморасчетах сумму по табчасти "Тара"
			Если ИмяТаблицы = "ВозвратнаяТара" Тогда
				СтрокаДвижения.СуммаВзаиморасчетов	= СтрокаТаблицы.СуммаВзаиморасчетов;
				СтрокаДвижения.СуммаСНДС			= СтрокаТаблицы.СуммаВзаиморасчетов;
			Иначе
	...
	//начало изменений БП
	Если ТаблСторноУслуг <> Неопределено Тогда
		
		Для каждого СтрокаТаблицы Из ТаблСторноУслуг Цикл
			СтрокаДвижения = ТаблицаДвижений.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаДвижения, СтрокаТаблицы);
			
			СтрокаДвижения.ТекущаяТаблица = "Услуги";
			СтрокаДвижения.СчетУчетаНУ = СтрокаТаблицы.СчетЗатратНУ;
			
			СтрокаДвижения.СуммаВзаиморасчетов	= -1*СтрокаТаблицы .СуммаВзаиморасчетов;
			СтрокаДвижения.СуммаСНДС			= -1*СтрокаТаблицы .СуммаВзаиморасчетов;			
	...
	Для Каждого Движение Из ТаблицаДвижений Цикл
		...
		Для каждого ВидСубконто из КоррСчет.ВидыСубконто Цикл
			...
			Если Движение.ТекущаяТаблица = "Услуги" И ЗначениеЗаполнено(Движение.СтатьяЗатрат) 
...
//
Процедура БухгалтерскийУчетРасчетыСКонтрагентами_Оплата(СтруктураПараметров, СтруктураШапкиДокумента, СтруктураДвижений, Отказ, Заголовок, ПринадлежностьПоследовательностям = Неопределено, ВызванРегламентнойПроцедурой = ложь, УдаляемыеВидыРегОпераций = Неопределено, КэшВидовСубконтоПоСчетам = Неопределено) Экспорт
	...
	Для каждого СтрокаПлатежа Из ТаблицаПлатежей Цикл
		...
		Для каждого СтрокаОстатка Из СтрокиОстатка Цикл
			...
			//Курс получим на дату документа долга
			Если СтрокаПлатежа.ВестиПоДокументамРасчетовСКонтрагентом Тогда
				//m.ionov@a-prof.ru 12.05.2014
				Если ТипЗнч(СтрокаОстатка.Документ) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
					КурсОстатка = СтрокаОстатка.Документ.КурсВзаиморасчетов;
				Иначе
				//----m.ionov@a-prof.ru---
					ДанныеВалютыВзаиморасчетов        = МодульВалютногоУчета.ПолучитьКурсВалюты(СтрокаПлатежа.ВалютаВзаиморасчетов, СтрокаОстатка.Документ.Дата);	
					КурсОстатка = ДанныеВалютыВзаиморасчетов.Курс; 
				КонецЕсли;
			КонецЕсли;
			//m.ionov@a-prof.ru
	...
	//начало изменений 
	Если ТаблицаПлатежей.Колонки.Найти("Претензия") = Неопределено Тогда
		ТаблицаПлатежей.Колонки.Добавить("Претензия",Новый ОписаниеТипов("СправочникСсылка.ПРГ_Претензии"));
		ЗаполнитьПретензииВТаблицеПроводок(СтруктураШапкиДокумента.Ссылка, ТаблицаПлатежей);                                      // Шевченков 20170726 #68683
	КонецЕсли;	
	
	//Сч7621 = ПланыСчетов.Хозрасчетный.НайтиПоКоду("76.02.1");
	СпСчетовПретен = Новый Массив;
	СпСчетовПретен.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("76.02.1"));
	СпСчетовПретен.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("76.02.2"));
	СпСчетовПретен.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("76.02.3"));
	СпСчетовПретен.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоПретензиямУЕ);
	СпСчетовПретен.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоПретензиямВал);
	...
	Для каждого СтрокаПлатежа из ТаблицаПлатежей Цикл
		...
		Если СтрокаПлатежа.ДолгРег > 0 Тогда 
			...
			//Blik 020414 26538 н 
			//Если ТипЗнч(СтрокаПлатежа.Ссылка) = Тип("ДокументСсылка.КорректировкаДолга")
			//	И СтрокаПлатежа.СчетОплаты = ПланыСчетов.Хозрасчетный.НайтиПоКоду("76.02.2")Тогда
			//	БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + СуфиксСчетаРасчетов], Проводка["Субконто" + СуфиксСчетаРасчетов], "ПРГ_Претензии", СтрокаПлатежа.Претензия, , , БухгалтерскийУчет.ОпределитьВидСубконтоПоСчету(Проводка["Счет" + СуфиксСчетаРасчетов], КэшВидовСубконтоПоСчетам));
			//КонецЕсли;
			//Blik 020414 26538 к	
			//начало изменений
			Если СпСчетовПретен.Найти(СтрокаПлатежа.СчетОплаты) <> Неопределено Тогда
				Если ЗначениеЗаполнено(СтрокаПлатежа.Претензия) Тогда
					БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + СуфиксСчетаРасчетов], Проводка["Субконто" + СуфиксСчетаРасчетов], "ПРГ_Претензии", СтрокаПлатежа.Претензия, , , БухгалтерскийУчет.ОпределитьВидСубконтоПоСчету(Проводка["Счет" + СуфиксСчетаРасчетов], КэшВидовСубконтоПоСчетам));
				// <- Шевченков	
				//ИначеЕсли ТипЗнч(СтрокаПлатежа.Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручениеИсходящее")      // Шевченков, старый алгоритм, убрал по задаче №35887
				//	И СтрокаПлатежа.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ОплатаПоставщику
				//	И СтрокаПлатежа.СчетАванса = ПланыСчетов.Хозрасчетный.НайтиПоКоду("76.02.3")Тогда
					
			ИначеЕсли ТипЗнч(СтрокаПлатежа.Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручениеИсходящее") Тогда
					Если СпСчетовПретен.Найти(СтрокаПлатежа.СчетАванса)<> Неопределено И ЗначениеЗаполнено(СтрокаПлатежа.Документ.РасшифровкаПлатежа[СтрокаПлатежа.НомерСтрокиДокумента-1].ПРГ_Претензия) Тогда
						БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + СуфиксСчетаРасчетов], Проводка["Субконто" + СуфиксСчетаРасчетов], "ПРГ_Претензии", СтрокаПлатежа.Документ.РасшифровкаПлатежа[СтрокаПлатежа.НомерСтрокиДокумента-1].ПРГ_Претензия, , , БухгалтерскийУчет.ОпределитьВидСубконтоПоСчету(Проводка["Счет" + СуфиксСчетаРасчетов], КэшВидовСубконтоПоСчетам));
					КонецЕсли;
				// ->
				КонецЕсли;	
			КонецЕсли;	
		...
		Если СтрокаПлатежа.АвансРег <> 0 Тогда
			...
			БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + СуфиксСчетаРасчетов], Проводка["Субконто" + СуфиксСчетаРасчетов], "Договоры", СтрокаПлатежа.ДоговорКонтрагента, , , БухгалтерскийУчет.ОпределитьВидСубконтоПоСчету(Проводка["Счет" + СуфиксСчетаРасчетов], КэшВидовСубконтоПоСчетам));
			// <- Шевченков
			//Если ТипЗнч(СтрокаПлатежа.Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручениеИсходящее")
			//	И СтрокаПлатежа.ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ВозвратДенежныхСредствПоставщиком 
			//	И СтрокаПлатежа.СчетАванса = ПланыСчетов.Хозрасчетный.НайтиПоКоду("76.02.1")Тогда
			//	БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + СуфиксСчетаРасчетов], Проводка["Субконто" + СуфиксСчетаРасчетов], "ПРГ_Претензии", СтрокаПлатежа.Претензия, , , БухгалтерскийУчет.ОпределитьВидСубконтоПоСчету(Проводка["Счет" + СуфиксСчетаРасчетов], КэшВидовСубконтоПоСчетам));
			//КонецЕсли;
			// -> 	
			//начало изменений 
			Если СпСчетовПретен.Найти(СтрокаПлатежа.СчетАванса) <> Неопределено Тогда
				Если ЗначениеЗаполнено(СтрокаПлатежа.Претензия) Тогда
					БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + СуфиксСчетаРасчетов], Проводка["Субконто" + СуфиксСчетаРасчетов], "ПРГ_Претензии", СтрокаПлатежа.Претензия, , , БухгалтерскийУчет.ОпределитьВидСубконтоПоСчету(Проводка["Счет" + СуфиксСчетаРасчетов], КэшВидовСубконтоПоСчетам));
				// <- Шевченков	
				КонецЕсли;				
				Если ТипЗнч(СтрокаПлатежа.Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручениеИсходящее") 
					// И СтрокаПлатежа.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ОплатаПоставщику Тогда // Шевченков 20161129 59706
					И (СтрокаПлатежа.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ОплатаПоставщику ИЛИ СтрокаПлатежа.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ВозвратДенежныхСредствПокупателю) Тогда // Шевченков 20161129 59706
					//И СтрокаПлатежа.СчетАванса = ПланыСчетов.Хозрасчетный.НайтиПоКоду("76.02.3")Тогда
					БухгалтерскийУчет.УстановитьСубконто(Проводка["Счет" + СуфиксСчетаРасчетов], Проводка["Субконто" + СуфиксСчетаРасчетов], "ПРГ_Претензии", СтрокаПлатежа.Документ.РасшифровкаПлатежа[СтрокаПлатежа.НомерСтрокиДокумента-1].ПРГ_Претензия, , , БухгалтерскийУчет.ОпределитьВидСубконтоПоСчету(Проводка["Счет" + СуфиксСчетаРасчетов], КэшВидовСубконтоПоСчетам));
				// ->
				КонецЕсли;	
...
Процедура РаспределениеСуммовойРазницы(СуммоваяРазницаПоДокументам, СтруктураПараметров, СтруктураШапкиДокумента, СтруктураДвижений, Отказ, Заголовок, УдаляемыеВидыРегОпераций = Неопределено, КэшВидовСубконтоПоСчетам = Неопределено)
	...
	Для каждого Строка Из ТаблицаРаспределенияСР Цикл 
		...
		Иначе
			Если Строка.СуммаСНДС < 0 Тогда
				Строка.СчетУчетаНУ = ПланыСчетов.Налоговый.ВнереализационныеДоходы;
				//начало изменений Ожиганов 23.05.2016 52767 исправление проводок при возникновении суммовой разницы пост оплата покупателем 
				Строка.КоррСчет    = ПланыСчетов.Хозрасчетный.ПрочиеРасходыНеОблагаемыеЕНВД;
				//конец изменений 
			ИначеЕсли Строка.СуммаСНДС > 0 Тогда
				Строка.СчетУчетаНУ = ПланыСчетов.Налоговый.ВнереализационныеРасходы;
	...
	Для Каждого ТекКорректировка из ТаблицаРаспределенияСР Цикл
		...
		Попытка //Blik 040414 н 27171
			Подр = ПРГ_Регламентый.ПолучитьЗатратноеПодразделениеОрганизации(СтруктураШапкиДокумента.подразделение);
			Если Подр = Неопределено Тогда
				Подразделение = ТекКорректировка.КоррСубконто2;
			Иначе
				Подразделение = Подр;
			КонецЕсли;
		исключение
			Подразделение = ТекКорректировка.КоррСубконто2;
		КонецПопытки;   //Blik 040414 к 27171
		//Суммовая разница
		...
		//Если ДатаДока < '20070101' Тогда
			...
			Если Покупки И ТекКорректировка.СуммаСНДС < 0 Тогда
				...
				БухгалтерскийУчет.УстановитьСубконто(ТекКорректировка.КоррСчет,Проводка["Субконто"+СуфиксСчетаРасчетов],2,Подразделение);//ТекКорректировка.КоррСубконто2); //Blik 040414 н 27171
			...
			ИначеЕсли Продажи Тогда
			 		АСуфиксСчетаЗатрат =  ?(ТекКорректировка.СуммаСНДС<0,"Дт","Кт");
					АСуфиксСчетаРасчетов =  ?(ТекКорректировка.СуммаСНДС<0,"Кт","Дт");
					
				Проводка["Счет"+АСуфиксСчетаЗатрат] = ТекКорректировка.КоррСчет;
				БухгалтерскийУчет.УстановитьСубконто(ТекКорректировка.КоррСчет,Проводка["Субконто"+АСуфиксСчетаЗатрат],1,ТекКорректировка.КоррСубконто1);
				БухгалтерскийУчет.УстановитьСубконто(ТекКорректировка.КоррСчет,Проводка["Субконто"+АСуфиксСчетаЗатрат],2,Подразделение);//ТекКорректировка.КоррСубконто2);  //Blik 040414 н 27171
				БухгалтерскийУчет.УстановитьСубконто(ТекКорректировка.КоррСчет,Проводка["Субконто"+АСуфиксСчетаЗатрат],3,ТекКорректировка.КоррСубконто3);
				
				Проводка["Счет"+АСуфиксСчетаРасчетов] = СчетОплаты;
				Проводка["Валюта"+АСуфиксСчетаРасчетов] = ВалютаВзаиморасчетов;
				БухгалтерскийУчет.УстановитьСубконто(СчетОплаты,Проводка["Субконто"+АСуфиксСчетаРасчетов], "Контрагенты", Контрагент);
				БухгалтерскийУчет.УстановитьСубконто(СчетОплаты,Проводка["Субконто"+АСуфиксСчетаРасчетов], "Договоры", Договор);
				Проводка.Сумма = ?(ТекКорректировка.СуммаСНДС<0,-1,1) * ТекКорректировка.Проводка_Сумма;
			...
			Иначе
				...
				БухгалтерскийУчет.УстановитьСубконто(ТекКорректировка.КоррСчет,Проводка["Субконто"+СуфиксСчетаЗатрат],2,Подразделение);//ТекКорректировка.КоррСубконто2);  //Blik 040414 н 27171
...
Функция ЗафиксироватьПриобретениеРеализациюВРегистреРасчетов(Объект, СтруктураПараметров, ВалютаРегламентированногоУчета, ТаблицаДанныхДокумента)
	...
	Для каждого Реквизит из СтруктураПараметров Цикл
		...
		//m.ionov@a-prof.ru 19.02.2014
		Если Реквизит.Ключ = "ДоговорКонтрагента" Тогда	
			ЕстьНесколькоСФ = СтруктураПараметров.Свойство("УЗ_НесколькоСФ");
			Если ЕстьНесколькоСФ = Истина И СтруктураПараметров.УЗ_НесколькоСФ Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;	
		//----m.ionov@a-prof.ru---
...
Функция РасчетыВУсловныхЕдиницахПриобретениеРеализация(СтруктураПараметров,ВалютаРегламентированногоУчета,РежимПроведения=Неопределено,ПроводкиБУ,Объект = Null,Отказ=Ложь,УдаляемыеВидыРегОпераций=Неопределено, ПолучатьПроводкиПоСсылке = Ложь) экспорт
	...
	//Если не поступление с несколькими договорам - то приводим к типовой процедуре
	ЕстьНесколькоСФ = Ложь;
	СтруктураПараметров.Свойство("УЗ_НесколькоСФ",ЕстьНесколькоСФ);
	Если Не ТаблицаРегистрации.Колонки.Найти("ДоговорКонтрагента") = Неопределено
		И (ЕстьНесколькоСФ = Ложь ИЛИ ЕстьНесколькоСФ = Неопределено) Тогда
		ТаблицаРегистрации.Колонки.Удалить("ДоговорКонтрагента");
	КонецЕсли;
	//----m.ionov@a-prof.ru---
	...
	Если Не ТаблицаСделокДокумента.Колонки.Найти("ДоговорКонтрагента") = Неопределено Тогда
		ТаблицаСделокДокумента.Свернуть("Сделка, ДоговорКонтрагента", "СуммаВзаиморасчетов, СуммаСНДС, НДСВал, СуммаНДС");
	Иначе
		ТаблицаСделокДокумента.Свернуть("Сделка", "СуммаВзаиморасчетов, СуммаСНДС, НДСВал, СуммаНДС");
	КонецЕсли;
	//----m.ionov@a-prof.ru---
	...
	Если ЕстьТаблицаПоВзаиморасчетам И УчитыватьАванс Тогда
		...
		Если Не ТаблицаСделокДокумента.Колонки.Найти("ДоговорКонтрагента") = Неопределено Тогда
			ТаблицаПоВзаиморасчетам.Свернуть("Сделка, Документ, ДатаОплаты, ДоговорКонтрагента", "СуммаВзаиморасчетов, СуммаРег");
		Иначе
			ТаблицаПоВзаиморасчетам.Свернуть("Сделка, Документ, ДатаОплаты", "СуммаВзаиморасчетов, СуммаРег");
		КонецЕсли;
		//----m.ionov@a-prof.ru---
	...
	Иначе
		...
		Если НЕ ТаблицаСделокДокумента.Колонки.Найти("ДоговорКонтрагента") = Неопределено Тогда
			Запрос.УстановитьПараметр("Договор", ТаблицаСделокДокумента.ВыгрузитьКолонку("ДоговорКонтрагента"));	
		Иначе
			Запрос.УстановитьПараметр("Договор", ДоговорКонтрагента);	
		КонецЕсли;
		//----m.ionov@a-prof.ru---
		...
		|	РасчетыВУЕ.Документ КАК Документ,
		|	РасчетыВУЕ.ДоговорКонтрагента КАК ДоговорКонтрагента, //m.ionov@a-prof.ru 21.02.2014
		|	РасчетыВУЕ.ДатаОплаты КАК ДатаОплаты,
		...
		+ ?(НЕ ТаблицаСделокДокумента.Колонки.Найти("ДоговорКонтрагента") = Неопределено,"
		...
		|	РасчетыВУЕ.Документ.Дата,
		|	Документ,
		|	ДоговорКонтрагента //m.ionov@a-prof.ru 21.02.2014
		|ИТОГИ ПО
		|	Сделка";
	...
	Для каждого СтрокаТаблицыСделок Из ТаблицаСделокДокумента Цикл
		...
		Если ЕстьТаблицаПоВзаиморасчетам И НЕ ВызванРегламентнойПроцедурой Тогда
			...
			Если Не ТаблицаСделокДокумента.Колонки.Найти("ДоговорКонтрагента") = Неопределено 
				И Не ТаблицаПоВзаиморасчетам.Колонки.Найти("ДоговорКонтрагента") = Неопределено Тогда
				ОстаткиАвансов = ТаблицаПоВзаиморасчетам.НайтиСтроки(Новый Структура("Сделка, ДоговорКонтрагента", СтрокаТаблицыСделок.Сделка, СтрокаТаблицыСделок.ДоговорКонтрагента));
			Иначе
				ОстаткиАвансов = ТаблицаПоВзаиморасчетам.НайтиСтроки(Новый Структура("Сделка", СтрокаТаблицыСделок.Сделка));
			КонецЕсли;
			//----m.ionov@a-prof.ru---
		...
		Иначе
			...
			//КонецЕсли;
			Если Не ТаблицаСделокДокумента.Колонки.Найти("ДоговорКонтрагента") = Неопределено 
				И Не ОстатокАвансаПоСделкам.Колонки.Найти("ДоговорКонтрагента") = Неопределено Тогда
				СтрокиАвансаПоСделке = ОстатокАвансаПоСделкам.Строки.НайтиСтроки(Новый Структура("Сделка, ДоговорКонтрагента", СтрокаТаблицыСделок.Сделка, СтрокаТаблицыСделок.ДоговорКонтрагента), Истина);
				ЕстьОстаткиАвансовПоСделке = СтрокиАвансаПоСделке.Количество() > 0;
				Если ЕстьОстаткиАвансовПоСделке Тогда
					ОстаткиАвансов = СтрокиАвансаПоСделке;
				КонецЕсли;
			Иначе
				СтрокиАвансаПоСделке = ОстатокАвансаПоСделкам.Строки.Найти(СтрокаТаблицыСделок.Сделка, "Сделка", Ложь);
				ЕстьОстаткиАвансовПоСделке = СтрокиАвансаПоСделке <> Неопределено;
			...
			Для каждого ОстатокАванса Из ОбщиеОстаткиАвансов Цикл
				...
				Если ВызванРегламентнойПроцедурой И ЕстьТаблицаПоВзаиморасчетам Тогда
					...
					Если Не ТаблицаСделокДокумента.Колонки.Найти("ДоговорКонтрагента") = Неопределено 
						И Не ТаблицаПоВзаиморасчетам.Колонки.Найти("ДоговорКонтрагента") = Неопределено Тогда
						ОстаткиПоТаблицеВзаиморасчетов = ТаблицаПоВзаиморасчетам.НайтиСтроки(
						Новый Структура("Сделка, Документ, ДоговорКонтрагента", СтрокаТаблицыСделок.Сделка, ОстатокАванса.Документ, СтрокаТаблицыСделок.ДоговорКонтрагента));
					Иначе	
						ОстаткиПоТаблицеВзаиморасчетов = ТаблицаПоВзаиморасчетам.НайтиСтроки(
						Новый Структура("Сделка, Документ", СтрокаТаблицыСделок.Сделка, ОстатокАванса.Документ));
					КонецЕсли;
				...
				//m.ionov@a-prof.ru 19.02.2014
				Если НЕ ТаблицаСделокДокумента.Колонки.Найти("ДоговорКонтрагента") = Неопределено Тогда
					Движение.ДоговорКонтрагента		         = СтрокаТаблицыСделок.ДоговорКонтрагента;
				КонецЕсли;
				//----m.ionov@a-prof.ru---	
				...
				//m.ionov@a-prof.ru 19.02.2014
				Если НЕ ТаблицаСделокДокумента.Колонки.Найти("ДоговорКонтрагента") = Неопределено Тогда
					НоваяСтрока.ДоговорКонтрагента		         = СтрокаТаблицыСделок.ДоговорКонтрагента;
				КонецЕсли;
				//----m.ionov@a-prof.ru---	
		...
		Если Не ТаблицаСделокДокумента.Колонки.Найти("ДоговорКонтрагента") = Неопределено Тогда
			...
			Если НужнаПроводкаПоАвансу Тогда
				...
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Контрагенты"	,Контрагент);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Договоры"		,СтрокаТаблицыСделок.ДоговорКонтрагента);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Контрагенты"	,Контрагент);
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Договоры"		,СтрокаТаблицыСделок.ДоговорКонтрагента);
	...
	//m.ionov@a-prof.ru 24.02.2014
	Если ТаблицаСделокДокумента.Колонки.Найти("ДоговорКонтрагента") = Неопределено Тогда
...
//
Процедура ПодготовкаТаблицыЗначенийДляЦелейПриобретенияИРеализации(ТаблицаЗначений, СтруктураШапкиДокумента, ВключитьНДСВОсновнуюСумму, ВалютаРегламентированногоУчета = "", ВРублях = Ложь,ПРГ_СуммаВВалютеСНДС=-1) Экспорт  //m.ionov@a-prof.ru 16.02.2014 - передаем параметр в рублях
	...
	//начало изменений 
	СтруктураОбязательныхКолонок.Вставить("ПРГ_СуммаСучетомНДС");
	СтруктураОбязательныхКолонок.Вставить("ПРГ_СуммаСучетомНДСВал");
	//конец изменений
			...
			Иначе
				...
				Если ВидРасчетовПоДоговору = Перечисления.ВидыРасчетовПоДоговорам.РасчетыВУсловныхЕдиницах тогда
					...
					Если (СтруктураШапкиДокумента.ВалютаДокумента = ВалютаРегламентированногоУчета) Тогда
  						...
  						//СтруктураОбязательныхКолонок.Вставить("ПРГ_СуммаСучетомНДС");
						//СтруктураОбязательныхКолонок.Вставить("ПРГ_СуммаСучетомНДСВал");
						Для каждого ПРГ_ТекСтрока Из ТаблицаЗначений Цикл
							//ПРГ_ТекСтрока.ПРГ_СуммаСучетомНДС =  ПРГ_ТекСтрока.Сумма + ?(ЕстьНДС,?(СтруктураШапкиДокумента.СуммаВключаетНДС,0,ПРГ_ТекСтрока.НДС),0);
							ПРГ_ТекСтрока.ПРГ_СуммаСучетомНДС =  ПРГ_ТекСтрока.Сумма + ?(ЕстьНДС,ПРГ_ТекСтрока.НДС,0);
						КонецЦикла;	
						
						Если ПРГ_СуммаВВалютеСНДС > -1 Тогда
							РаспределениеВал = ОбщегоНазначения.РаспределитьПропорционально(ПРГ_СуммаВВалютеСНДС,ТаблицаЗначений.ВыгрузитьКолонку("ПРГ_СуммаСучетомНДС"));
						Иначе
							СуммаВал = Окр(ТаблицаЗначений.Итог("ПРГ_СуммаСучетомНДС")* КоэффициентПересчета,2);
							РаспределениеВал = ОбщегоНазначения.РаспределитьПропорционально(СуммаВал,ТаблицаЗначений.ВыгрузитьКолонку("ПРГ_СуммаСучетомНДС"));
						КонецЕсли;	
						
						Если Не РаспределениеВал = Неопределено Тогда
							ТаблицаЗначений.ЗагрузитьКолонку(РаспределениеВал,"ПРГ_СуммаСучетомНДСВал");
						КонецЕсли; 
						
						Для каждого ПРГ_ТекСтрока Из ТаблицаЗначений Цикл
							ПРГ_ТекСтрока.СуммаВал =  ПРГ_ТекСтрока.ПРГ_СуммаСучетомНДСВал;
						КонецЦикла;	
						...
						Если ЕстьНДС тогда
								...
								Для каждого ПРГ_ТекСтрока Из ТаблицаЗначений Цикл
									 //ПРГ_ТекСтрока.СуммаВал =  ПРГ_ТекСтрока.ПРГ_СуммаСучетомНДСВал - ?(СтруктураШапкиДокумента.СуммаВключаетНДС,0,ПРГ_ТекСтрока.НДСВал);
									 ПРГ_ТекСтрока.СуммаВал =  ПРГ_ТекСтрока.ПРГ_СуммаСучетомНДСВал - ПРГ_ТекСтрока.НДСВал;
								КонецЦикла;	
...
Процедура ЗаполнитьПретензииВТаблицеПроводок(мСсылка, ТаблицаПлатежей)
	
	мСписокТипов = Новый Массив;
	мСписокТипов.Добавить(Тип("ДокументСсылка.ПлатежноеПоручениеИсходящее"));
	мСписокТипов.Добавить(Тип("ДокументСсылка.ПлатежноеПоручениеВходящее"));
	
	Если мСписокТипов.Найти(ТипЗнч(мСсылка)) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	ТаблицаДокумента.ПРГ_Претензия КАК Претензия
		|ИЗ
		|	Документ.ПлатежноеПоручениеИсходящее.РасшифровкаПлатежа КАК ТаблицаДокумента
		|ГДЕ
		|	ТаблицаДокумента.Ссылка = &Ссылка
		|	И НЕ ТаблицаДокумента.ПРГ_Претензия = ЗНАЧЕНИЕ(Справочник.ПРГ_Претензии.ПустаяСсылка)
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки";
	...
	Если ТипЗнч(мСсылка) = Тип("ДокументСсылка.ПлатежноеПоручениеВходящее") Тогда
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПлатежноеПоручениеИсходящее", "ПлатежноеПоручениеВходящее");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПРГ_Претензия", "Претензия");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ТаблицаПлатежей[ВыборкаДетальныеЗаписи.НомерСтроки - 1].Претензия = ВыборкаДетальныеЗаписи.Претензия;
	КонецЦикла;
