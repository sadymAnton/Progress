...
Процедура ПодготовитьТаблицуПериодов(Запрос, ТаблицаПериодов, ИспользоватьФизлицо = Ложь, ЕстьРегистратор = Ложь)
	...
	Если Не ИспользоватьФизлицо Тогда
		...
		// Измайлова Полина 01.07.2011 13:32 
		|	ТаблицаПериодов.Сотрудник КАК СотрудникДляГрафика,
		////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}  
		|	ТаблицаПериодов.ДатаНачала,
		|	ТаблицаПериодов.ДатаОкончания
	...
	Иначе
		...
		|	ТаблицаПериодов.Сотрудник,
					///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 01.07.2011 13:33 
		|	ТаблицаПериодов.Сотрудник КАК СотрудникДляГрафика,
		////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}  
		|	ТаблицаПериодов.ДатаНачала,
		|	ТаблицаПериодов.ДатаОкончания
...
Функция ПодготовитьДанныеПоУправленческимОтпускам(
	...
	Если ВключатьЗапланированныеОтпуска Тогда
		...
		|	ПланированиеОтпускаРаботники.Сотрудник КАК Сотрудник,
			///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 01.07.2011 15:19 
		|	ПланированиеОтпускаРаботники.СотрудникДляГрафика,
		////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА} 
		...
		|;
	|////////////////////////////////////////////////////////////////////////////////
		// ЗАКОММЕНТИРОВАНО: Измайлова Полина 01.07.2011 15:10  
		//|ВЫБРАТЬ
		...
		//|	Физлицо";
		///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 01.07.2011 15:12                                           
		|ВЫБРАТЬ
		|	ПланированиеОтпускаРаботники.НомерСтроки КАК НомерСтроки,
		|	ПланированиеОтпускаРаботники.Сотрудник КАК Физлицо,
		|	ТаблицаПериодов.ДатаНачала КАК ДатаНачала,
		|	ТаблицаПериодов.ДатаОкончания,
		|	ТаблицаПериодов.СотрудникДляГрафика КАК СотрудникДляГрафика
		|ПОМЕСТИТЬ ВТПланированиеОтпуска
		|ИЗ
		|	ВТТаблицаПериодовПлан КАК ПланированиеОтпускаРаботники
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТТаблицаПериодовПлан КАК ТаблицаПериодов
		|		ПО ПланированиеОтпускаРаботники.Сотрудник = ТаблицаПериодов.Сотрудник
		...
		|	ТаблицаПериодов.СотрудникДляГрафика
		|ИЗ
		|	РегистрСведений.СобытийныйПланЗанятостиФизлиц КАК СобытийныйПланЗанятостиФизлиц
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТТаблицаПериодов КАК ТаблицаПериодов
		|		ПО СобытийныйПланЗанятостиФизлиц.ФизЛицо = ТаблицаПериодов.Сотрудник
		...
		|;
		|ВЫБРАТЬ
		|	ПоследнееКадрИзменение.ДатаНачала,
		|	ПоследнееКадрИзменение.ДатаОкончания,
		|	ПоследнееКадрИзменение.Физлицо КАК Физлицо,
		|	ПоследнееКадрИзменение.НомерСтроки КАК НомерСтроки,
		|	ВЫБОР
		|		КОГДА РаботникиОрганизаций.ПериодЗавершения <= ПоследнееКадрИзменение.ДатаНачала
		...
		|	КОНЕЦ КАК Календарь
		|ПОМЕСТИТЬ ВТТаблицаПериодовСКалендарями
		|ИЗ
		|	(ВЫБРАТЬ
		...
		|		ПланированиеОтпуска.Физлицо КАК Физлицо,
		|		ПланированиеОтпуска.НомерСтроки КАК НомерСтроки,
		|		ПланированиеОтпуска.СотрудникДляГрафика КАК СотрудникДляГрафика,
		|		МАКСИМУМ(РаботникиОрганизаций.Период) КАК Период
		|	ИЗ
		|		ВТПланированиеОтпуска КАК ПланированиеОтпуска
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
		|			ПО ПланированиеОтпуска.СотрудникДляГрафика = РаботникиОрганизаций.Сотрудник
		|				И ПланированиеОтпуска.ДатаНачала >= РаботникиОрганизаций.Период
		|	
		|	СГРУППИРОВАТЬ ПО
		|		ПланированиеОтпуска.ДатаНачала,
		|		ПланированиеОтпуска.ДатаОкончания,
		|		ПланированиеОтпуска.Физлицо,
		|		ПланированиеОтпуска.НомерСтроки,
		|		ПланированиеОтпуска.СотрудникДляГрафика) КАК ПоследнееКадрИзменение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
		|		ПО ПоследнееКадрИзменение.СотрудникДляГрафика = РаботникиОрганизаций.Сотрудник
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТТаблицаПериодовСКалендарями.НомерСтроки КАК НомерСтроки,
		|	ВТТаблицаПериодовСКалендарями.Физлицо КАК Физлицо,
		|	СУММА(РегламентированныйПроизводственныйКалендарь.Пятидневка) КАК ДнейЗапланированногоОтпуска
		|ПОМЕСТИТЬ ВТЗапланированныеОтпуска
		|ИЗ
		|	ВТТаблицаПериодовСКалендарями КАК ВТТаблицаПериодовСКалендарями
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РегламентированныйПроизводственныйКалендарь КАК РегламентированныйПроизводственныйКалендарь
		|		ПО ВТТаблицаПериодовСКалендарями.Календарь = РегламентированныйПроизводственныйКалендарь.AVT_ПроизводственныйКалендарь
		|			И (РегламентированныйПроизводственныйКалендарь.ДатаКалендаря МЕЖДУ ВТТаблицаПериодовСКалендарями.ДатаНачала И ВТТаблицаПериодовСКалендарями.ДатаОкончания)
        |
		|СГРУППИРОВАТЬ ПО
		|	ВТТаблицаПериодовСКалендарями.НомерСтроки,
		|	ВТТаблицаПериодовСКалендарями.Физлицо
		|	
		|ИНДЕКСИРОВАТЬ ПО
		|	НомерСтроки,
		|	Физлицо";		
	...
	|			ТаблицаПериодов.ДатаОкончания КАК ДатаОкончания
	|		ИЗ
	// ЗАКОММЕНТИРОВАНО: Измайлова Полина 04.07.2011 12:38  
	//|			ВТТаблицаПериодов КАК ТаблицаПериодов
	///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 04.07.2011 12:38 
	|	(ВЫБРАТЬ
	|		ПоследнееКадрИзменение.ДатаНачала,
	|		ПоследнееКадрИзменение.ДатаОкончания,
	|		ПоследнееКадрИзменение.Сотрудник,
	|		ПоследнееКадрИзменение.НомерСтроки,
	|		ВЫБОР
	|			КОГДА РаботникиОрганизаций.ПериодЗавершения <= ПоследнееКадрИзменение.ДатаНачала
	|				И РаботникиОрганизаций.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1)
	|				ТОГДА РаботникиОрганизаций.ГрафикРаботыЗавершения.AVT_ПроизводственныйКалендарь
	...
	|	ИЗ
	|		(ВЫБРАТЬ
	|			ВТТаблицаПериодов.ДатаНачала,
	|			ВТТаблицаПериодов.ДатаОкончания,
	|			ВТТаблицаПериодов.Сотрудник,
	|			ВТТаблицаПериодов.НомерСтроки,
	|			ВТТаблицаПериодов.СотрудникДляГрафика,
	|			МАКСИМУМ(РаботникиОрганизаций.Период) КАК Период
	|		ИЗ
	|			ВТТаблицаПериодов КАК ВТТаблицаПериодов
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
	|				ПО ВТТаблицаПериодов.СотрудникДляГрафика = РаботникиОрганизаций.Сотрудник
	|					И ВТТаблицаПериодов.ДатаНачала >= РаботникиОрганизаций.Период
	|	
	|		СГРУППИРОВАТЬ ПО
	|			ВТТаблицаПериодов.ДатаНачала,
	|			ВТТаблицаПериодов.ДатаОкончания,
	|			ВТТаблицаПериодов.Сотрудник,
	|			ВТТаблицаПериодов.НомерСтроки,
	|			ВТТаблицаПериодов.СотрудникДляГрафика) КАК ПоследнееКадрИзменение
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
	|			ПО ПоследнееКадрИзменение.СотрудникДляГрафика = РаботникиОрганизаций.Сотрудник
	|				И ПоследнееКадрИзменение.Период = РаботникиОрганизаций.Период) КАК ТаблицаПериодов
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
	|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РегламентированныйПроизводственныйКалендарь КАК РегламентированныйПроизводственныйКалендарь
	|				ПО (РегламентированныйПроизводственныйКалендарь.ДатаКалендаря МЕЖДУ ТаблицаПериодов.ДатаНачала И ТаблицаПериодов.ДатаОкончания)
		///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 04.07.2011 12:42 
	|					И РегламентированныйПроизводственныйКалендарь.AVT_ПроизводственныйКалендарь = ТаблицаПериодов.Календарь
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА} 
	|		
	|		СГРУППИРОВАТЬ ПО
	|			ТаблицаПериодов.НомерСтроки,
