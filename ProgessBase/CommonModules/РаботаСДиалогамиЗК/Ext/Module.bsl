...
Процедура ОткрытьФормуВыбораОсновныеНачисленияОрганизаций(Владелец, Ключ, СтруктураПараметровОтбора, СтандартнаяОбработка, ТекущееЗначение = Неопределено, СписокПредопределенныхВР = Неопределено) Экспорт
	...
	Для Каждого ЭлементСтруктуры Из СтруктураПараметровОтбора Цикл
		...
		Иначе
			...
			// Измайлова Полина 20.09.2011 10:33 
			Если ТипЗнч(ЭлементСтруктуры.Значение) = Тип("Булево") Тогда
				ФормаВыбора.Отбор[ИмяОтбора].Значение      = ЭлементСтруктуры.Значение;
			...
			ФормаВыбора.Отбор[ИмяОтбора].Использование = Истина;
			// Измайлова Полина 20.09.2011 10:34 
			КонецЕсли;
			////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
...
Процедура УстановитьДоступностьФормыДляРедактирования(ДокументОбъект, ФормаДокумента) Экспорт
	...
	// <- Шевченков	
	РазрешитьРедактированиеКадровыхДокументов = УправлениеДопПравамиПользователей.ПолучитьЗначениеПраваПользователя(ПланыВидовХарактеристик.ПраваПользователей.НеПроверятьДатуЗапретаПриРедактированииКадровыхДокументов, Ложь);
	Если ПРГ_Обработки.ПроверкаПроведенияДокументаНаДатуЗапрета(ДокументОбъект.Метаданные().Имя) И РазрешитьРедактированиеКадровыхДокументов[0] Тогда 
...
Процедура ДополнитьСписокВидовДокументов(МетаданныеЖурнала, СписокВидов) Экспорт
	...
	Для Каждого МетаданныеДокумента Из МетаданныеЖурнала.РегистрируемыеДокументы Цикл
		//Если МетаданныеДокумента.Имя = "РеестрДанныхЭЛНЗаполняемыхРаботодателем" И Не Константы.ИспользуетсяОбменДаннымиЭЛН.Получить() Тогда
		//	Продолжить;
		//КонецЕсли;
