...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, ЭтотОбъект) Экспорт
	...
	Если ИмяМакета = "Печать" Тогда
		...
		// получаем данные для печати
		// ИЗМЕНЕНИЕ: Измайлова Полина 24.08.2011 10:41  
		//Выборка = ЭтотОбъект.СформироватьЗапросДляПечатиТрудовогоДоговора().Выбрать();
		
		///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 24.08.2011 10:41 
		РезультатЗапроса = ЭтотОбъект.СформироватьЗапросДляПечатиТрудовогоДоговора();
		...
		Если Выборка.Следующий() Тогда
			...
			Макет.Параметры.ДокументКемВыдан = СокрЛП(Макет.Параметры.ДокументКемВыдан);
				...
				// ИЗМЕНЕНИЕ: Измайлова Полина 24.08.2011 10:42
			//ВыборкаПерсональныхНадбавок = Выборка.ПерсональныеНадбавки.Выбрать();
			ВыборкаПерсональныхНадбавок = РезультатЗапроса[1].Выбрать();
...
Процедура ЗаполнитьРеквизитыПоШтатномуРасписанию(Форма) Экспорт
	...
	// ИЗМЕНЕНИЕ: Петрухин Андрей 22.06.2011 23:49  
	// Добавлено в вызов "штатное расписание"
	ДанныеШР = ПроцедурыУправленияПерсоналомПереопределяемый.СведенияОПозицииШтатногоРасписания(Форма.ДатаДоговора, Форма.ПодразделениеОрганизации, Форма.Должность, Форма.AVTШтатноеРасписание);
	НадбавкиПоШР = Форма.ЭтотОбъект.СведенияОНадбавкахПозицииШтатногоРасписания(Форма.ДатаДоговора, Форма.ПодразделениеОрганизации, Форма.Должность, Форма.AVTШтатноеРасписание);
	Если ДанныеШР.Количество() > 0 Тогда
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ДанныеШР[0].ГрафикРаботы) Тогда
		...
		КонецЕсли;
	КонецЕсли;
	// ЗАКОММЕНТИРОВАНО: Измайлова Полина 14.09.2011 13:58  
	...
	//КонецЕсли;
	///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 14.09.2011 13:58 
	// ОПИСАНИЕ: если объект новый, то переносим надбавки во временную таблицу, т.к. ссылка еще не создана, иначе в регистр.
	Если НадбавкиПоШР.Количество() > 0 и РегистрыСведений.AVT_ПерсональныеНадбавкиСотрудников.СрезПоследних(, Новый Структура("Сотрудник", Форма.ЭтотОбъект.Ссылка)).Количество() = 0 Тогда
		Сегодня = ТекущаяДата();
		Если Форма.ЭтотОбъект.ЭтоНовый() Тогда
			Форма.AVT_ТЗНадбавки = НадбавкиПоШР;
		Иначе
			НаборЗаписей = РегистрыСведений.AVT_ПерсональныеНадбавкиСотрудников.СоздатьНаборЗаписей();
			Для каждого НадбавкаПоШР Из НадбавкиПоШР Цикл
				Запись = НаборЗаписей.Добавить();
				Запись.Период    = Сегодня;
				Запись.Сотрудник = Форма.Объект.Ссылка;
				ЗаполнитьЗначенияСвойств(Запись, НадбавкаПоШР);
			КонецЦикла;
	...
	Если ВидДоговора = Перечисления.ВидыДоговоровСФизЛицами.ТрудовойДоговор Тогда
		...
		Если Принят И Не Уволен Тогда
			...
			ДобавитьКнопкуВПодменю(МассивКнопокПодменю, "УвольнениеИзОрганизаций", ТипКнопкиКоманднойПанели.Действие, "Увольнение из организаций", СтруктураСоответствияДействий.ДействиеВводНаОснованииУвольнениеИзОрганизаций);
				///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
			// Измайлова Полина 01.02.2012 17:49 
			ДобавитьКнопкуВПодменю(МассивКнопокПодменю, "Разделитель31", ТипКнопкиКоманднойПанели.Разделитель);
			ДобавитьКнопкуВПодменю(МассивКнопокПодменю, "Авт_ИзменениеГрафикаРаботы", ТипКнопкиКоманднойПанели.Действие, "Изменение графика работы", СтруктураСоответствияДействий.ДействиеВводНаОснованииИзменениеГрафикаРаботы);
			////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА
