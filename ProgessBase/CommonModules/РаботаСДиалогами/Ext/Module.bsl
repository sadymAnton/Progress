...
Процедура НачалоВыбораЗначенияДоговораКонтрагента(ДокументОбъект, ФормаДокумента, ЭлементФормы, Контрагент, ДоговорКонтрагента,
	...
	Если ВидыДоговораВзаиморасчетов<>Неопределено Тогда
		//Blik 290414 н 27542
		ТипЗначения = ТипЗнч(ДокументОбъект);
		ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗначения);
		Если Метаданные.Документы.Содержит(ОбъектМетаданных) Тогда
			Если ДокументОбъект.ссылка.Метаданные().Имя = "ПлатежныйОрдерПоступлениеДенежныхСредств" или ДокументОбъект.ссылка.Метаданные().Имя = "ПлатежныйОрдерСписаниеДенежныхСредств" Тогда
				Если ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеБезналичныхДенежныхСредств.ПриобретениеИностраннойВалюты 
					или ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийСписаниеБезналичныхДенежныхСредств.ПрочиеРасчетыСКонтрагентами Тогда
					ВидыДоговораВзаиморасчетов.Добавить(Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура НачалоВыбораЗначенияСпецификации(Номенклатура, ЭлементФормы, СтандартнаяОбработка, ТолькоАктивные = Истина, Подразделение= Неопределено) Экспорт
	...
	Если   Подразделение <> Неопределено Тогда
		 ...
		 Если ИспользуемПодразделение Тогда
		  ...
		  Иначе
		     Запрос = Новый Запрос("ВЫБРАТЬ
		                           ...
		                           |	//ИЛИ СпецификацииНоменклатурыВыходныеИзделия.ТочкаМаршрута.Подразделение В (&Подразделение))
		                           |	И СпецификацииНоменклатурыВыходныеИзделия.Ссылка.ВидСпецификации В(&ВидыСпецификации)
		                           |	И СпецификацииНоменклатурыВыходныеИзделия.Номенклатура = &Номенклатура" + ?(ТолькоАктивные," И СпецификацииНоменклатурыВыходныеИзделия.Ссылка.Активная ","")+"");
		...
		ФормаВыбораСпецификации.Отбор.ссылка.ВидСравнения  = ВидСравнения.ВСписке;
		ФормаВыбораСпецификации.Отбор.ссылка.Значение      = ?(СписокСсылок.Количество()>0, СписокСсылок, СписокСсылокПустойОтбор);
		ФормаВыбораСпецификации.Отбор.ссылка.Использование = Истина;
		...
		ФормаВыбораСпецификации.ПараметрВыборГруппИЭлементов = ИспользованиеГруппИЭлементов.Элементы;
		ФормаВыбораСпецификации.ЭлементыФормы.СправочникСписок.ИерархическийПросмотр  = Ложь;
		// <- Шевченков (32622)
		Если ЗначениеЗаполнено(ЭлементФормы.Значение) Тогда
			ФормаВыбораСпецификации.ПараметрТекущаяСтрока = ЭлементФормы.Значение;
			ФормаВыбораСпецификации.НачальноеЗначениеВыбора = ЭлементФормы.Значение;
		КонецЕсли;
		// ->
	   ...
	   И (Не ДокументОбъект.ПометкаУдаления) Тогда
		...
		Если ТипЗнч(Отбор) = Тип("Структура") И Отбор.Свойство("ВидСчетаФактуры") Тогда
			Если ТипЗнч(Отбор.ВидСчетаФактуры) = Тип("Массив") 
				...
				Если ВидСчетаФактуры = "СчетФактураВыданный" И ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
					ТекВидОперации = ПРГ_ДопФункцииКлиентСервер.ПолучитьРеквизитыОбъекта(ДокументОбъект,"ВидОперации");			
					Если ТекВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение Тогда
...
Процедура ПоказатьКодАртикул(КолонкиТабПоля, ЯчейкиСтрокиТабПоля, Номенклатура, ПРГКод=Неопределено,ПРГАртикул=Неопределено,ПРГСтарыйКод = Неопределено) Экспорт
//конец изменений 
	...
	Если КолонкиТабПоля.Код.Видимость Тогда
		Если Номенклатура <> Неопределено Тогда
			...
			ЯчейкиСтрокиТабПоля.Код.УстановитьТекст(?(ПРГКод=Неопределено,Номенклатура.Код,ПРГКод));
			//конец изменений 
		Иначе
	...
	Если КолонкиТабПоля.Артикул.Видимость Тогда
		Если ТипЗнч(Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
			...
			ЯчейкиСтрокиТабПоля.Артикул.УстановитьТекст(?(ПРГАртикул = Неопределено,Номенклатура.Артикул,ПРГАртикул));
			//конец изменений 
		Иначе
	...
	/////Вадим 05.01.2014 8:22:30  
 	старыйкод=КолонкиТабПоля.найти("старыйкод");
	если не старыйкод=Неопределено тогда
		Если КолонкиТабПоля.старыйкод.Видимость Тогда
			Если ТипЗнч(Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
				//начало изменений Ожиганов 20.04.2015 37867  
				//ЯчейкиСтрокиТабПоля.старыйкод.УстановитьТекст(Номенклатура.СтарыйКод);
				ЯчейкиСтрокиТабПоля.старыйкод.УстановитьТекст(?(ПРГСтарыйКод = Неопределено,Номенклатура.СтарыйКод,ПРГСтарыйКод));
				//конец изменений 
			Иначе
				ЯчейкиСтрокиТабПоля.старыйкод.УстановитьТекст("");
			КонецЕсли;
		КонецЕсли;
	Конецесли;
 	////ВадимКонец
...
Функция ПолучитьМассивНедоступныхВидовОпераций()
	...
	//начало изменений Ожиганов 25.09.2017 убрать изменения. изменения временные
	//МассивНедоступныхВидовОпераций.Добавить(Метаданные.Перечисления.СП_ВидыОперацииЗаданияНаПодбор.ЗначенияПеречисления.ПеремещениеККопекеру);
	//МассивНедоступныхВидовОпераций.Добавить(Метаданные.Перечисления.СП_ВидыОперацииЗаданияНаПодбор.ЗначенияПеречисления.ПеремещениеОтКопекера);
	//МассивНедоступныхВидовОпераций.Добавить(Метаданные.Перечисления.ПРГ_ВидыОперацийВнутреннегоЗаказа.ЗначенияПеречисления.ПеремещениеККопекеру);
	//МассивНедоступныхВидовОпераций.Добавить(Метаданные.Перечисления.ПРГ_ВидыОперацийВнутреннегоЗаказа.ЗначенияПеречисления.ПеремещениеОтКопекера);
