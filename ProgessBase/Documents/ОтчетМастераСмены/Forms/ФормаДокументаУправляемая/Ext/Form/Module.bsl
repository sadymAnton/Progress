...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ПодключаемоеОборудование"
		...
		Если ИмяСобытия = "ScanData" Тогда
			...
			Если Элементы.Страницы.ТекущаяСтраница = Элементы.ГруппаВыпуск Тогда
				ИмяТабличнойЧасти = "Выпуск";
			ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.ГруппаРасходМатериалов Тогда
				ИмяТабличнойЧасти = "РасходМатериалов";
			КонецЕсли;
			
			ПолученыШтрихкоды(Данные, ИмяТабличнойЧасти);
...
&НаКлиенте
Процедура ПолученыШтрихкоды(ДанныеШтрихкодов, ИмяТабличнойЧасти)
	...
	Если СведенияПоШтрихКоду <> Неопределено Тогда
		ОбработкаПодбора(ИмяТабличнойЧасти, СведенияПоШтрихКоду);		
	КонецЕсли; 
...
&НаКлиенте
Процедура ВыпускПоискПоШтрихкоду(Команда)
	
	ОчиститьСообщения();
	ТекШтрихкод = "";
	Если ШтрихКодыНоменклатурыКлиент.ВвестиШтрихкод(ТекШтрихкод) Тогда
		ПолученыШтрихкоды(ШтрихКодыНоменклатурыКлиентСервер.ПолучитьСтруктуруДанныхШтрихкода(ТекШтрихкод, 1), "Выпуск");
	КонецЕсли;
...
&НаКлиенте
Процедура РасходМатериаловПоискПоШтрихкоду(Команда)
	
	ОчиститьСообщения();
	ТекШтрихкод = "";
	Если ШтрихКодыНоменклатурыКлиент.ВвестиШтрихкод(ТекШтрихкод) Тогда
		ПолученыШтрихкоды(ШтрихКодыНоменклатурыКлиентСервер.ПолучитьСтруктуруДанныхШтрихкода(ТекШтрихкод, 1), "РасходМатериалов");
	КонецЕсли;
