...
//начало изменений
Перем НеКонтролироватьОстатки Экспорт;
//конец изменений
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	//m.ionov@a-prof.ru 17.10.2014
	//СтруктураОбязательныхПолей = 
	//Новый Структура("ВидОперации, Заказ");
	
	СтруктураОбязательныхПолей = 
	Новый Структура("ВидОперации, КП_Отделение");
	
	Если ВидОперации = Перечисления.ВидыОперацийРезервированиеТоваров.АП_ОбщийРезерв Тогда
		СтруктураОбязательныхПолей.Вставить("АП_НазначениеРезерва");
		
		Если ЗначениеЗаполнено(АП_НазначениеРезерва) И Не АП_НазначениеРезерва.Отделение = КП_Отделение Тогда
			ОбщегоНазначения.СообщитьОбОшибке("Отделение в назначении резерва не совпадает с отделением в шапке", Отказ);
		КонецЕсли;
	КонецЕсли;
	//----m.ionov@a-prof.ru---
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(ТаблицаПоТоварам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРезервированиеТоваров.АП_ОбщийРезерв И АП_НазначениеРезерва.ФиксированныйРезерв Тогда
		СтруктураОбязательныхПолей.Вставить("НовоеРазмещение");	
	Иначе
	...
	Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийРезервированиеТоваров.АП_ОбщийРезерв 
		//и Не 
	Тогда
			 ПроверятьТолькоНовеРазмещение = АП_НазначениеРезерва.ФиксированныйРезерв;
		//    СтруктураПолей.Вставить("ИсходноеРазмещениеОтделение" , "ИсходноеРазмещение.НСИ_Отделение");
		//	СтруктураПолей.Вставить("НовоеРазмещениеОтделение"    , "НовоеРазмещение.НСИ_Отделение");
			Для Каждого СтрокаТЧ Из ТаблицаПоТоварам Цикл
				Если Не ПроверятьТолькоНовеРазмещение и ЗначениеЗаполнено(СтрокаТЧ.ИсходноеРазмещение) Тогда
					Если СтрокаТЧ.ИсходноеРазмещениеОтделение <> СтруктураШапкиДокумента.КП_Отделение Тогда
						ОбщегоНазначения.СообщитьОбОшибке("В строке № " + СтрокаТЧ.НомерСтроки + " табличной части товары 
							| не совпадает отделение заказа с отделением склада
							| отделение склада "+СтрокаТЧ.ИсходноеРазмещениеОтделение+"
							| отделение заказа "+СтруктураШапкиДокумента.КП_Отделение,Отказ);
					КонецЕсли;	
				КонецЕсли;
				Если ЗначениеЗаполнено(СтрокаТЧ.НовоеРазмещение) Тогда
					Если СтрокаТЧ.НовоеРазмещениеОтделение <> СтруктураШапкиДокумента.КП_Отделение Тогда
						ОбщегоНазначения.СообщитьОбОшибке("В строке № " + СтрокаТЧ.НомерСтроки + " табличной части товары 
							| не совпадает отделение заказа с отделением склада
							| отделение склада "+СтрокаТЧ.НовоеРазмещениеОтделение+"
							| отделение заказа "+СтруктураШапкиДокумента.КП_Отделение,Отказ);
					КонецЕсли;	
				КонецЕсли;
			КонецЦикла	
...
Процедура ПроверкаПревышенияОбъемаЗаказа(РежимПроведения, СтруктураШапкиДокумента,ТаблицаПоТоварам, ТаблицаПоТаре, Заголовок)
	...
	//начало изменений Ожиганов 12.04.2015 изменение контроля остатков при вызове из обработки резервирования 
	//Если ТипЗнч(СтруктураШапкиДокумента.Заказ)=Тип("ДокументСсылка.ЗаказПокупателя")  Тогда
	Если ТипЗнч(СтруктураШапкиДокумента.Заказ)=Тип("ДокументСсылка.ЗаказПокупателя") и НЕ НеКонтролироватьОстатки Тогда
	//конец изменений 	
	...
	ИначеЕсли ТипЗнч(СтруктураШапкиДокумента.Заказ)=Тип("ДокументСсылка.ЗаказНаПроизводство") и НЕ НеКонтролироватьОстатки Тогда
	//конец изменений 
	 ...
	 ИначеЕсли ТипЗнч(СтруктураШапкиДокумента.Заказ)=Тип("ДокументСсылка.ВнутреннийЗаказ") и Не НеКонтролироватьОстатки Тогда
	 //конец изменений 
...
Функция СоздатьТаблицуРезервированияПодЗаказ(ТаблицаПоТоварам)
	...
	// <- Шевченков 37851 закомментировал
	//Для каждого СтрокаТаблицы Из ТаблицаПоТоварамРезервирование Цикл
	//	СтрокаТаблицы.Количество = - СтрокаТаблицы.МинусКоличество;
	//КонецЦикла;
	// ->

	//{26.03.2015 Островерхий заявка №б/н 
	Для каждого СтрокаТаблицы Из ТаблицаПоТоварамРезервирование Цикл
		СтрокаТаблицы.Количество = СтрокаТаблицы.МинусКоличество;
	КонецЦикла; 
	//26.03.2015 Островерхий} 
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	ТаблицаПоТоварамСнятиеРазмещения = СоздатьТаблицуРазмещенияЗаказовТовары(ТаблицаПоТоварам);
	//----m.ionov@a-prof.ru---	
	...
	ТаблицаПоТоварамРазмещение = СоздатьТаблицуСНовымРазмещениемЗаказаТовары(ТаблицаПоТоварам);
	//----m.ionov@a-prof.ru---	
	...
	ТаблицаПоТоварамСнятиеРезерва = СоздатьТаблицуРезервированияПодЗаказТовары(ТаблицаПоТоварам);
	
	ТаблицаПоТоварамСнятиеНазначенияРезерва = СоздатьТаблицуРезервированияПодЗаказТовары(ТаблицаПоТоварам, Истина);
	//----m.ionov@a-prof.ru---
	...
	ТаблицаПоТоварамРезервирование = СоздатьТаблицуСНовымРезервированиемПодЗаказТовары(ТаблицаПоТоварам);
	
	ТаблицаПоТоварамРезервированиеНазначенияРезерва = СоздатьТаблицуСНовымРезервированиемПодЗаказТовары(ТаблицаПоТоварам, Истина);
	//----m.ionov@a-prof.ru---
	...
	Если ТаблицаПоТоварамСнятиеРезерва.Количество() > 0 ИЛИ ТаблицаПоТареСнятиеРезерва.Количество() > 0 
		...
		//{06.04.2015 Островерхий заявка №б/н 
		//Если Товары.Количество() <> 0 Тогда
		Если Товары.Количество() <> 0 И НЕ НеКонтролироватьОстатки Тогда
		...
		КонецЕсли;
		//{06.04.2015 Островерхий заявка №б/н 
		//Если ВозвратнаяТара.Количество() <> Тогда 
		Если ВозвратнаяТара.Количество() <> 0 И НЕ НеКонтролироватьОстатки Тогда
		//06.04.2015 Островерхий} 
	...
	Если не Отказ Тогда
		Если ТаблицаПоТоварамСнятиеРезерва.Количество() > 0 ИЛИ ТаблицаПоТареСнятиеРезерва.Количество() > 0 Тогда
			...
			//ОбщегоНазначения.ЗаписатьТаблицыДокументаВРегистр(НаборДвижений, ВидДвиженияНакопления.Приход, ТаблицыДанныхДокумента, Дата);
			ОбщегоНазначения.ЗаписатьТаблицыДокументаВРегистр(НаборДвижений, ВидДвиженияНакопления.Расход, ТаблицыДанныхДокумента, Дата); // Шевченков 37851
		...
		//m.ionov@a-prof.ru 24.10.2014
		Если ТаблицаПоТоварамСнятиеНазначенияРезерва.Количество() > 0 Тогда
		
			НаборДвижений = Движения.ТоварыВРезервеНаСкладах;

			СтруктТаблицДокумента = Новый Структура;
			СтруктТаблицДокумента.Вставить("ТаблицаПоТоварам", ТаблицаПоТоварамСнятиеНазначенияРезерва);
		...
		Если ТаблицаПоТоварамРезервированиеНазначенияРезерва.Количество() > 0 Тогда

			НаборДвижений = Движения.ТоварыВРезервеНаСкладах;
			
			СтруктТаблицДокумента = Новый Структура;
			СтруктТаблицДокумента.Вставить("Товары", ТаблицаПоТоварамРезервированиеНазначенияРезерва);
   ...
   КонецЕсли;
	...
	Если Не Отказ Тогда
		...
		Если Не РазрешитьЗапись Тогда
			Отказ = Не МожноМенятьРезервирование();
		КонецЕсли;
	КонецЕсли;
	//----m.ionov@a-prof.ru---
	//начало изменений Ожиганов  запретим проводить общие резервы не текущей датов
	Если Не Отказ 
	  и  ВидОперации = Перечисления.ВидыОперацийРезервированиеТоваров.АП_ОбщийРезерв
	  и  НачалоДня(ТекущаяДата()) > Дата тогда
	  Сообщить("Запрещено менять заказ не текущой датой !!!");
	  Если Не РольДоступна("ПолныеПрава") и Не РольДоступна("АП_Администратор") тогда
		  Отказа = Истина;
	  КонецЕсли  
	КонецЕсли; 
	//конец изменений 
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	НеКонтролироватьОстатки = ?(НеКонтролироватьОстатки=Неопределено,Ложь,НеКонтролироватьОстатки);
	//конец изменений 
	
	Если мУдалятьДвижения Тогда
		//начало изменений Ожиганов 37889  
		Если НеКонтролироватьОстатки Тогда
			ОбщегоНазначения.УдалитьДвиженияРегистратора(ЭтотОбъект, Отказ, Истина, РежимПроведения);
		Иначе	
			//нужно для удаления движений
			ОбщегоНазначения.УдалитьДвиженияРегистратора(ЭтотОбъект, Отказ, Истина, РежимПроведенияДокумента.Оперативный);
		КонецЕсли;	
		//конец изменений 
		//ОбщегоНазначения.УдалитьДвиженияРегистратора(ЭтотОбъект, Отказ, Истина, РежимПроведения);
	КонецЕсли;
	...
	//начало изменений Ожиганов 37889  
	УправлениеЗапасами.ДобавитьСтрокуВДеревоПолейЗапросаПоШапке(ДеревоПолейЗапросаПоШапке, "АП_НазначениеРезерва", "ФиксированныйРезерв", 			"ФиксированныйРезерв");
	//конец изменений 
	...
	//m.ionov@a-prof.ru 24.10.2014
	СтруктураПолей.Вставить("АП_НазначениеРезерва"            , "АП_НазначениеРезерва");
	//----m.ionov@a-prof.ru---
	//начало изменений Ожиганов 21.04.2015 37907 
    СтруктураПолей.Вставить("ИсходноеРазмещениеОтделение" , "ИсходноеРазмещение.НСИ_Отделение");
	СтруктураПолей.Вставить("НовоеРазмещениеОтделение"    , "НовоеРазмещение.НСИ_Отделение");
	...
	//m.ionov@a-prof.ru 23.10.2014
	Если Не Отказ и ВидОперации = Перечисления.ВидыОперацийРезервированиеТоваров.АП_ОбщийРезерв Тогда
		Отказ = Не ПроверкаДоступностиИспользованияРезерва();
	КонецЕсли;
	//----m.ionov@a-prof.ru---
	...
	Если Не Отказ Тогда
		...
		Если ВидОперации = Перечисления.ВидыОперацийРезервированиеТоваров.АП_ОбщийРезерв Тогда
			ДивжениеПоРегистрамОбщегоРезерва(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре, Отказ, Заголовок);
		Иначе
			ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре, Отказ, Заголовок);
		КонецЕсли;
		//----m.ionov@a-prof.ru---
...
Функция МожноМенятьРезервирование() Экспорт
	...
	Если РольДоступна("АП_Администратор") и Не ВидОперации = Перечисления.ВидыОперацийРезервированиеТоваров.АП_ОбщийРезерв Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Разрешен только просмотр документа", Ложь);
		МожноМенять = Истина;
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРезервированиеТоваров.АП_ОбщийРезерв Тогда
		МожноМенять = Истина;
	Иначе
		ОбщегоНазначения.СообщитьОбОшибке("Разрешен только просмотр документа", Ложь);
		МожноМенять = Ложь;
...
Функция ПроверкаДоступностиИспользованияРезерва()
	
	Если РольДоступна("АП_Администратор") Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(АП_НазначениеРезерва)
		ИЛИ АП_НазначениеРезерва.Пользователи.Количество() = 0 Тогда
		
		Возврат Истина;
	КонецЕсли;
	
	СтрокаПользователя = АП_НазначениеРезерва.Пользователи.Найти(глЗначениеПеременной("глТекущийПользователь"), "Пользователь");
	Если СтрокаПользователя = Неопределено Тогда
		ОбщегоНазначения.СообщитьОбОшибке("У вас нет прав над изменением резерва по выбранному назначению резерва", Истина);
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Процедура ДивжениеПоРегистрамОбщегоРезерва(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре, Отказ, Заголовок)
	
	//начало изменений Ожиганов 37889  
	//Если АП_НазначениеРезерва.ФиксированныйРезерв Тогда
	Если СтруктураШапкиДокумента.ФиксированныйРезерв Тогда
	//конец изменений  	 
		// Таблица товаров с непустым новым размещением - складом
		ТаблицаПоТоварамРезервирование = СоздатьТаблицуСНовымРезервированиемПодЗаказ(ТаблицаПоТоварам);
				
		НаборДвижений = Движения.КП_ФиксированныеРезервыПоНазначениюРезервов;
		ТаблицаДвижений 	= НаборДвижений.Выгрузить();
		
		ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(ТаблицаПоТоварамРезервирование, ТаблицаДвижений);
		ТаблицаДвижений.ЗаполнитьЗначения(АП_НазначениеРезерва, "НазначениеРезерва");
		...
		Если Не Отказ Тогда								
			...
			Движения.КП_ФиксированныеРезервыПоНазначениюРезервов.ВыполнитьДвижения();
	...
	Иначе
		...
		Если не Отказ Тогда
			Если ТаблицаПоТоварамСнятиеРезерва.Количество() > 0 ИЛИ ТаблицаПоТареСнятиеРезерва.Количество() > 0 Тогда
				...
				ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "ДокументРезерва", Документы.ЗаказПокупателя.ПустаяСсылка());
				ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "АП_НазначениеРезерва", АП_НазначениеРезерва);
...
Функция СоздатьТаблицуРазмещенияЗаказовТовары(ТаблицаПоТоварам)
	...
	Пока Сч < ТаблицаПоТоварамРазмещение.Количество() Цикл
		СтрокаТаблицы = ТаблицаПоТоварамРазмещение.Получить(Сч);
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ИсходноеРазмещение) 
		 или ТипЗнч(СтрокаТаблицы.ИсходноеРазмещение) = Тип("СправочникСсылка.Склады") 
		 или ЗначениеЗаполнено(СтрокаТаблицы.АП_НазначениеРезерва) Тогда
			 ТаблицаПоТоварамРазмещение.Удалить(СтрокаТаблицы);
		 Иначе
			Сч = Сч + 1;
		КонецЕсли; 
...
Функция СоздатьТаблицуСНовымРазмещениемЗаказаТовары(ТаблицаПоТоварам)
	...
	Пока Сч < ТаблицаПоТоварамРазмещение.Количество() Цикл
		СтрокаТаблицы = ТаблицаПоТоварамРазмещение.Получить(Сч);
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.НовоеРазмещение) 
		 или ТипЗнч(СтрокаТаблицы.НовоеРазмещение) = Тип("СправочникСсылка.Склады")
		 или ЗначениеЗаполнено(СтрокаТаблицы.АП_НазначениеРезерва) Тогда
			 ТаблицаПоТоварамРазмещение.Удалить(СтрокаТаблицы);
		 Иначе
			Сч = Сч + 1;
...
Функция СоздатьТаблицуРезервированияПодЗаказТовары(ТаблицаПоТоварам, Назначения = Ложь)
	...
	Пока Сч < ТаблицаПоТоварамРезервирование.Количество() Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ИсходноеРазмещение) 
		 или ТипЗнч(СтрокаТаблицы.ИсходноеРазмещение) <> Тип("СправочникСсылка.Склады")
		 или (Не Назначения И ЗначениеЗаполнено(СтрокаТаблицы.АП_НазначениеРезерва)) 
		 или (Назначения И Не ЗначениеЗаполнено(СтрокаТаблицы.АП_НазначениеРезерва)) Тогда
			 ТаблицаПоТоварамРезервирование.Удалить(СтрокаТаблицы);
		Иначе 
			Сч = Сч + 1;
		КонецЕсли; 
	...
	Для каждого СтрокаТаблицы Из ТаблицаПоТоварамРезервирование Цикл
		//СтрокаТаблицы.Количество = - СтрокаТаблицы.МинусКоличество;
		СтрокаТаблицы.Количество = СтрокаТаблицы.МинусКоличество; // Шевченков 37851
	КонецЦикла;
...
Функция СоздатьТаблицуСНовымРезервированиемПодЗаказТовары(ТаблицаПоТоварам, Назначения = Ложь)
