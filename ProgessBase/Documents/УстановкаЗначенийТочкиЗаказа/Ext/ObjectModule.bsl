	...
	Для каждого СтрокаТаблицы Из ТаблицаПоТоварам Цикл
		...
		Иначе
			...
			Если СтрокаТаблицы.СпособОпределенияЗначенияТочкиЗаказа = Перечисления.СпособыОпределенияЗначенияТочкиЗаказа.Фиксированная Тогда
				...
				//{17.06.2015 Островерхий заявка №39845 
				//// ЗначениеТочкиЗаказа.
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//{16.06.2015 Островерхий заявка №39845 
	СтруктураПолей.Вставить("ПРГМаксимальныйЗапас", "ПРГМаксимальныйЗапас");
	СтруктураПолей.Вставить("ПРГСреднедневнаяПотребность", "ПРГСреднедневнаяПотребность");
	
	СтруктураСложныхПолей = Новый Структура;
	СтруктураСложныхПолей.Вставить("ПРГПериодРасчетаНО", "НАЧАЛОПЕРИОДА(Док.Ссылка.ПРГПериодРасчетаНО, МЕСЯЦ)");
