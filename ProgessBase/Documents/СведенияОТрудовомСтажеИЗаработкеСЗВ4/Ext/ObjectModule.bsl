			...
			Иначе// Проверки для формы СЗВ-4-2
				Если Не Отказ Тогда
					Если (ЧислоОсновныхЗаписейОСтаже > 1) ИЛИ (СтрЧислоСтрок(СтрокиСтажРаботникаОрганизации) > 1) Тогда
						ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Сотрудник имеет более одной записи о стаже.
						|				Данные необходимо подавать по форме СЗВ-4-1!", Отказ, Заголовок);
						
					ИначеЕсли РаботникОрганизацииИмеетЛьготныйСтаж тогда	
						ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- В записях о стаже имеется льготный стаж. 
						|				Данные необходимо подавать по форме СЗВ-4-1!", Отказ, Заголовок);
						
					ИначеЕсли ТУРаботникаНеСоответствуютПачке тогда	
						ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Территориальные условия не соответствуют пачке!", Отказ);
		...
		Пока ВыборкаПоРаботникиОрганизации.СледующийПоЗначениюПоля("НомерСтроки")	Цикл
			...
			Иначе
				...
				Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
					ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Предупреждение: Строка №"+ВыборкаПоРаботникиОрганизации.НомерСтроки+": Сотрудник "+ВыборкаПоРаботникиОрганизации.ФизЛицо+ " Адрес" + ТекстОшибки, Ложь);
				КонецЕсли;
			КонецЕсли;
					...
					Пока ВыборкаПоРаботникиОрганизации.СледующийПоЗначениюПоля("НомерОсновнойЗаписи")	Цикл	
						...
						Пока ВыборкаПоРаботникиОрганизации.СледующийПоЗначениюПоля("НомерДополнительнойЗаписи")	Цикл
							...
							Если ВыборкаПоРаботникиОрганизации.ПервыйПараметрИсчисляемогоСтажа > 0 ИЛИ ВыборкаПоРаботникиОрганизации.ВторойПараметрИсчисляемогоСтажа > 0 Тогда
								НаборЗаписейВыработкаКалендарная = НаборЗаписейДопИсчисляемыйСтаж.ВыработкаКалендарная.Значение;
								НаборЗаписейВыработкаКалендарная.ВсеМесяцы = ВыборкаПоРаботникиОрганизации.ПервыйПараметрИсчисляемогоСтажа;
								НаборЗаписейВыработкаКалендарная.ВсеДни = ВыборкаПоРаботникиОрганизации.ВторойПараметрИсчисляемогоСтажа;
							КонецЕсли;
...
Процедура Автозаполнение(Режим = "") Экспорт 
	...
	НовыеАдреса = ПроцедурыПерсонифицированногоУчета.АдресаДляИнформирования(РаботникиОрганизации.Выгрузить(,"НомерСтроки,ФизЛицо"), Организация, Дата, Режим);
...
Процедура ПроверитьПравильностьЗаполненияДокумента(ЕстьОшибки, СообщатьОбУспехе = Истина, ВыборкаПоШапкеДокумента = Неопределено, РезультатЗапросаПоЗаписямСтажа = Неопределено) Экспорт
	...
	ПериодРасчетаВзносов = ?(ТипСведенийСЗВ = Перечисления.ТипыСведенийСЗВ.ИСХОДНАЯ, ВыборкаПоШапкеДокумента.ОтчетныйПериод, ВыборкаПоШапкеДокумента.КорректируемыйПериод);
	ТарифыПФР = ПроцедурыПерсонифицированногоУчета.ТарифыПоКатегориям(ПериодРасчетаВзносов).Получить(ВыборкаПоШапкеДокумента.КатегорияЗастрахованныхЛиц);
	...
	ИначеЕсли ВыборкаПоШапкеДокумента.ОтчетныйПериод < ПроведениеРасчетов.ДатаЗаменыЕСНСтраховымиВзносами() Тогда
		...
		Если ВыборкаПоШапкеДокумента.ВидКорректировки = "" И Не ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ТипСведенийСЗВ) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Тип сведений и вид корректировки не могут быть одновременно пустыми!", Отказ,,,, СообщенияРазделПроверки);
		КонецЕсли; 
	...
	Иначе
		...
		ИначеЕсли ВыборкаПоШапкеДокумента.ТипСведенийСЗВ <> Перечисления.ТипыСведенийСЗВ.ИСХОДНАЯ Тогда
			...
			ИначеЕсли ВыборкаПоШапкеДокумента.КорректируемыйПериод < ПроведениеРасчетов.ДатаЗаменыЕСНСтраховымиВзносами() Тогда
				ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Корректируемый период не может быть ранее 1 полугодия 2010 года! Для корректировки сведений за 2009 год укажите этот год в поле отчетный период и проставьте вид корректировки.", Отказ,,,, СообщенияРазделПроверки);
			ИначеЕсли ВыборкаПоШапкеДокумента.КорректируемыйПериод >= ПроведениеРасчетов.ДатаВводаДополнительногоТарифаЗаРаботыСДосрочнойПенсией() Тогда
				ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Корректируемый период форм СЗВ-6-1(2) не может быть позже 2012 года!", Отказ,,,, СообщенияРазделПроверки);
			КонецЕсли; 
		КонецЕсли;
		...
		Если ВыборкаПоШапкеДокумента.ТипСведенийСЗВ = Перечисления.ТипыСведенийСЗВ.ИСХОДНАЯ И ОтчетныйПериод >= ПроведениеРасчетов.ДатаВводаДополнительногоТарифаЗаРаботыСДосрочнойПенсией() Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Исходные формы СЗВ-6-1(2) представляются за отчетные периоды 2010-2012 годов! Для формирования сведений с 1 квартала 2013 года используйте документ ""Пачка документов СЗВ-6-4"".", Отказ,,,, СообщенияРазделПроверки);
		КонецЕсли;
	...
	Если ТипФормДокументов <> "СЗВ-6-2" И КоличествоДокументов > 200 тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("В документе должно быть не более 200 форм " + ТипФормДокументов +" (сотрудников)!", Отказ, Заголовок,,, СообщенияРазделПроверки);
	ИначеЕсли ТипФормДокументов = "СЗВ-6-2" И КоличествоДокументов > 32000 тогда	
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("В форме СЗВ-6-2 должно быть не более 32000 сотрудников!", Отказ, Заголовок,,, СообщенияРазделПроверки);
	КонецЕсли;
	...
	Пока ВыборкаПоСтрокамДокумента.Следующий() цикл
		...
		Иначе
			...
			Если НЕ ВыборкаПоСтрокамДокумента.ФизЛицоРаботаетВОрганизации тогда
				ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- не зарегистрирован(а) как сотрудник организации " + ВыборкаПоШапкеДокумента.ОрганизацияНаименование, Отказ, Заголовок);
			КонецЕсли;
			...
			Если ВыборкаПоШапкеДокумента.ТипСведенийСЗВ <> Перечисления.ТипыСведенийСЗВ.ОТМЕНЯЮЩАЯ И ВыборкаПоШапкеДокумента.ОтчетныйПериод >= ПроведениеРасчетов.ДатаЗаменыЕСНСтраховымиВзносами() И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.УплаченоНакопительная) И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НачисленоНакопительная) И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.УплаченоСтраховая) И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.НачисленоСтраховая) И Не ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КоличествоЗаписейОСтаже) Тогда
				ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- У сотрудника отсутствуют данные о стаже и суммы взносов!", Отказ, Заголовок,,, СообщенияРазделПроверки);
			КонецЕсли;
			
			Если ВыборкаПоШапкеДокумента.ТипСведенийСЗВ <> Перечисления.ТипыСведенийСЗВ.ОТМЕНЯЮЩАЯ Тогда 
				ПроверяемыйПериод = ?(ТипСведенийСЗВ = Перечисления.ТипыСведенийСЗВ.КОРРЕКТИРУЮЩАЯ, КорректируемыйПериод, ОтчетныйПериод);
				
				Если ПроверяемыйПериод >= ПроведениеРасчетов.ДатаСниженияТарифовСтраховыхВзносов() Тогда
					
					// П. 4.1 Должны отсутствовать  значения в графах «Начислено» и «Упла-чено» графы «Сумма страховых взносов на накопительную часть трудовой пенсии» 
					...
					Если МассивКатегорийИностранцев.Найти(ВыборкаПоШапкеДокумента.КатегорияЗастрахованныхЛиц) <> Неопределено 
						...
						ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Для категории " + КодКатегории + ", данные о начисленных и уплаченных суммах на накопительную часть пенсии должны отсутствовать!", Отказ, Заголовок);	
					КонецЕсли;	
					...
					Если ТарифыПФР <> Неопределено Тогда
						...
						Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаРождения) И ВыборкаПоСтрокамДокумента.ДатаРождения >= '19670101' И 
							...
							ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + " - не заполнена сумма страховых взносов на накопительную часть трудовой пенсии!", Отказ, Заголовок);
						ИначеЕсли ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаРождения) И ВыборкаПоСтрокамДокумента.ДатаРождения >= '19670101' И ТарифыПФР.ПФРНакопительная <> 0 И ТарифыПФР.ПФРСтраховая <> 0 Тогда	
							...
							Если Разность >= 0.10 Тогда
								ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + ТекстСообщения, Отказ, Заголовок);	
							КонецЕсли;		
						КонецЕсли;	
					КонецЕсли;
				КонецЕсли;	
			...
			Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.АдресДляИнформирования) Тогда
				...
				Если ЗаПределамиРФ <> Истина Тогда
					Если  РегламентированнаяОтчетность.АдресСоответствуетТребованиям(АдресДляИнформированияСписком) Тогда
						...
						Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
							ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Адрес для информирования не соответствует адресному классификатору: в классификаторе не найден" + ТекстОшибки + "!", , Заголовок);
						КонецЕсли;
					Иначе
						ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Адрес для информирования заполнен не полностью или неверно или присутствуют латинские или недопустимые символы!", Отказ ,Заголовок);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура РассчитатьВзносы(КомментироватьРасчет = Ложь, Физлицо = Неопределено, ИзменятьДанные = Истина, ВыводитьПолныйКомментарий = Истина) Экспорт
	...
	Иначе	
		...
		ВзносыФизлиц = ПроцедурыПерсонифицированногоУчетаПолныеПрава.ДанныеОВзносахПоКатегориям(Дата, ПериодРасчетаВзносов, ПериодРасчетаВзносов, Организация, МассивФизлиц, КомментироватьРасчет, ДанныеКомментирования);
		ВзносыФизлиц.Индексы.Добавить("Физлицо,КатегорияЗастрахованныхЛиц");
		
		Если КомментироватьРасчет Тогда
			
			Если ДанныеКомментирования.РаспределениеСУчетомПринятыхПФРДанных И ВыводитьПолныйКомментарий Тогда
				...
				Если Итоги.ВсегоУплаченоСтраховая > 0 Тогда
					...
					Если КатегорияЗастрахованныхЛиц = Перечисления.КатегорииЗастрахованныхЛицПФР.ЧЭС
						...
						ОбработкаКомментариев.ДобавитьСообщение("По категории " + КатегорияЗастрахованныхЛиц + " уплаты текущего периода не предусмотрено", Перечисления.ВидыСообщений.Информация, , РазделКоэффициентовУплаты, Ложь);
					Иначе
...
Функция СформироватьЗапросПоРаботникиОрганизации(ВыборкаПоШапкеДокумента)
	...
	|	СведенияОТрудовомСтажеИЗаработкеСЗВ4РаботникиОрганизации.ФизЛицо.ДатаРождения КАК ДатаРождения
