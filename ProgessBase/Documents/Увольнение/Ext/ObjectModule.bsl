...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначенияЗК.СформироватьЗаголовокДокумента(ЭтотОбъект,"Увольнение "));
	...
	Если ТипОснования = Тип("ДокументСсылка.РезультатИспытательногоСрока") Тогда
		...
		Если Выборка.Следующий() Тогда
			Если Выборка.УжеУволен Тогда
				ОбщегоНазначенияЗК.СообщитьОбОшибке("Сотрудник " + Выборка.Сотрудник + " уже уволен документом " + Выборка.ДокументУвольнения);
			Иначе
