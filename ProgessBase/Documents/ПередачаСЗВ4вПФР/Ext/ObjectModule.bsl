...
Функция ПечатьАДВ62(ВариантФормы = "", ОчищатьСообщения = Истина)
	...
	Иначе
		...
		Пока ВыборкаПоПачкам.СледующийПоЗначениюПоля("Таблица") Цикл
			...
			Пока ВыборкаПоПачкам.Следующий() Цикл
				...
				ВыводимаяОбласть.Параметры.КодКатегории = ОбщегоНазначенияЗК.ПолучитьИмяЭлементаПеречисленияПоЗначению(ВыборкаПоПачкам.КатегорияЗастрахованныхЛиц);
	...
	Возврат УниверсальныеМеханизмы.НапечататьДокумент(ДокументРезультат, 1 , Ложь, "Опись сведений, передаваемых страхователем в ПФР (АДВ-6-2); пачка №" + НомерПачки,,ДокументРезультат);
...
Функция СборСведенийОПачке(ДокументСведений)
	...
	Если ДеревоПФР = Неопределено Тогда
		Отказ = Ложь;
		ОбщегоНазначенияЗК.СообщитьОбОшибке("Не удалось разобрать файл с данными!",Отказ, ОбщегоНазначенияЗК.ПредставлениеДокументаПриПроведении(ДокументСведений.Ссылка));
		Возврат ИнформацияОПачке;
	КонецЕсли;
	ДеревоПФР = ДеревоПФР.Строки.Найти("ФайлПФР","Имя");
	Если ДеревоПФР = Неопределено Тогда
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ОтчетныйПериод >= РасчетСтраховыхВзносов.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР() И Не НеСодержитСЗВ Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ОтчетныйПериод < ПроведениеРасчетов.ДатаВводаДополнительногоТарифаЗаРаботыСДосрочнойПенсией() Тогда
		ТабличнаяЧасть = ПачкиДокументов;	
	Иначе
		ТабличнаяЧасть = ПачкиДокументов2013;	
	КонецЕсли;
...
Процедура АвтозаполнениеПачек(АДВ11ПерваяВКомплекте = Ложь) Экспорт
	...
	ОбщегоНазначенияЗК.СообщитьИнформациюПользователю("--------------------------------------------------------------------");
	ОбщегоНазначенияЗК.СообщитьИнформациюПользователю("Генерация документов-пачек СЗВ-4:");
				...
				Пока Выборка.СледующийПоЗначениюПоля("ЯвляетсяЛетчиком") Цикл
					...
					Пока Выборка.Следующий() Цикл
						...
						Если НовыйДок.РаботникиОрганизации.Количество() = 200  Тогда
							...
							НовыйДок.Записать();
							ОбщегоНазначенияЗК.СообщитьИнформациюПользователю("	создан документ: " + НовыйДок);
	...
	Если СтрСписокФизЛицБезСтрахНомеров <> "" Тогда
		...
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Следующие физические лица не включены в пачки документов перс.учета из-за отсутствия страховых номеров ПФР:" + СтрСписокФизЛицБезСтрахНомеров);
	КонецЕсли;
...
Процедура АвтозаполнениеСтрокЗадолженности() Экспорт
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ОтчетныйПериод) Тогда
			ОбщегоНазначенияЗК.СообщитьОбОшибке("Не указан отчетный период!",,"Расчет задолженности перед ПФР");
			Возврат
		КонецЕсли;
...
Процедура ПроверитьПравильностьЗаполненияДокумента(ЕстьОшибки, СообщатьОбУспехе = Истина, ВыборкаПоШапкеДокумента = Неопределено, СообщенияРазделПроверки = Неопределено) Экспорт
	...
	Заголовок =  "Проверка документа Опись сведений АДВ-6-2 (до 2010 года - Ведомость уплаты АДВ-11), пачка № " + ВыборкаПоШапкеДокумента.НомерПачки + " от " + ВыборкаПоШапкеДокумента.Дата;
	...
	Если Не НеСодержитСЗВ Тогда
		...
		Если КоличествоПачек = 0 И ОтчетныйПериод < РасчетСтраховыхВзносов.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР() Тогда 
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Список пачек документов пуст!", Отказ, Заголовок,,,СообщенияРазделПроверки);
		КонецЕсли;	 
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Если НастройкаПравДоступаПереопределяемый.ДоступнаРольРасчетчикаРегл() Тогда
		...
		Иначе
			Возврат Новый Структура("АДВ11,АДВ62,СЗВ4,Записка,Разделитель,ВсеДокументы",?(НеСодержитСЗВ,"АДВ-11","АДВ-6-2 (форма 2011 г., в 2009 г. - АДВ-11)"),"АДВ-6-2 (форма 2010 г.)","СЗВ-6 (в 2009 г. - СЗВ-4)","Пояснительная записка",Неопределено,"Все документы");
		КонецЕсли;
	Иначе 
...
Функция СформироватьЗапросДляФайла()
	...
	|ГДЕ
	|	ПередачаСЗВ4вПФРПачкиДокументов2013.Ссылка = &ДокументСсылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ПачкаДокументовСЗВ_6_4РаботникиОрганизации.Ссылка.НомерПачки,
	|	ПачкаДокументовСЗВ_6_4РаботникиОрганизации.Ссылка.ФорматФайла,
	|	ПачкаДокументовСЗВ_6_4РаботникиОрганизации.Ссылка.Организация.РегистрационныйНомерПФР,
	|	ЛОЖЬ,
	|	СУММА(ПачкаДокументовСЗВ_6_4РаботникиОрганизации.НачисленоСтраховая),
	|	СУММА(ПачкаДокументовСЗВ_6_4РаботникиОрганизации.УплаченоСтраховая),
	|	СУММА(ПачкаДокументовСЗВ_6_4РаботникиОрганизации.НачисленоНакопительная),
	|	СУММА(ПачкаДокументовСЗВ_6_4РаботникиОрганизации.УплаченоНакопительная),
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПачкаДокументовСЗВ_6_4РаботникиОрганизации.ФизЛицо),
	|	СУММА(ПачкаДокументовСЗВ_6_4РаботникиОрганизации.ДоначисленоСтраховая),
	|	СУММА(ПачкаДокументовСЗВ_6_4РаботникиОрганизации.ДоначисленоНакопительная),
	|	СУММА(ПачкаДокументовСЗВ_6_4РаботникиОрганизации.ДоУплаченоНакопительная),
	|	СУММА(ПачкаДокументовСЗВ_6_4РаботникиОрганизации.ДоУплаченоСтраховая)
	|ИЗ
...
Функция ФайлДо2010Года(Отказ, ВыборкаПоШапкеДокумента, Заголовок)
	...
	ИначеЕсли ВыборкаПоШапкеДокумента.ФорматФайла = Перечисления.ФорматФайлаПФР.Версия07 Тогда 
		...
		НаборЗаписейВедомостьУплаты.ДатаВедомости.Значение = ?(РасчетныйПериодГод < 2010, ДатаДействия, ПроцедурыПерсонифицированногоУчета.ОкончаниеОтчетногоПериодаПерсучета(ОтчетныйПериод));
	...
	Пока ВыборкаПоПачкам.СледующийПоЗначениюПоля("Таблица") Цикл
		...
		Пока ВыборкаПоПачкам.СледующийПоЗначениюПоля("КатегорияЗастрахованныхЛиц") Цикл
			...
			Пока ВыборкаПоПачкам.Следующий() Цикл
				...
				НаборЗаписей.КодКатегории.Значение = ОбщегоНазначенияЗК.ПолучитьИмяЭлементаПеречисленияПоЗначению(ВыборкаПоПачкам.КатегорияЗастрахованныхЛиц);
...
Функция СформироватьЗапросПоШапке()
	...
	Если ОтчетныйПериод >= РасчетСтраховыхВзносов.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР() И Не НеСодержитСЗВ Тогда
		Возврат Документы.ПередачаСЗВ4вПФР.СформироватьЗапросПоШапке2014(Ссылка)
	КонецЕсли;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КОЛИЧЕСТВО(ДанныеПачек.КоличествоФизлиц) КАК КоличествоФизлиц,
	...
	|	СУММА(ДанныеПачек.НачисленоСтраховаяИсходных) КАК НачисленоСтраховаяИсходных,
	|	СУММА(ДанныеПачек.НачисленоСтраховаяКорректирующих) КАК НачисленоСтраховаяКорректирующих,
	|	СУММА(ДанныеПачек.НачисленоНакопительнаяИсходных) КАК НачисленоНакопительнаяИсходных,
	|	СУММА(ДанныеПачек.НачисленоНакопительнаяКорректирующих) КАК НачисленоНакопительнаяКорректирующих,
	|	СУММА(ДанныеПачек.УплаченоСтраховая) КАК УплаченоСтраховая,
	|	СУММА(ДанныеПачек.УплаченоНакопительная) КАК УплаченоНакопительная
	|ПОМЕСТИТЬ ВТКоличествоФизлиц
	|ИЗ
	|	(ВЫБРАТЬ
	|		ФормыСЗВ4.ФизЛицо КАК КоличествоФизлиц,
	|		ВЫБОР
	...
	|		ВЫБОР
	|			КОГДА ПачкаДокументовСЗВ_6_4РаботникиОрганизации.Ссылка.ТипСведенийСЗВ = ЗНАЧЕНИЕ(Перечисление.ТипыСведенийСЗВ.ИСХОДНАЯ)
	|				ТОГДА ПачкаДокументовСЗВ_6_4РаботникиОрганизации.НачисленоСтраховая
	|			ИНАЧЕ 0
	|		КОНЕЦ,
	|		ВЫБОР
	|			КОГДА ПачкаДокументовСЗВ_6_4РаботникиОрганизации.Ссылка.ТипСведенийСЗВ = ЗНАЧЕНИЕ(Перечисление.ТипыСведенийСЗВ.КОРРЕКТИРУЮЩАЯ)
	|				ТОГДА ПачкаДокументовСЗВ_6_4РаботникиОрганизации.НачисленоСтраховая
	|			ИНАЧЕ 0
	|		КОНЕЦ,
	|		ВЫБОР
	|			КОГДА ПачкаДокументовСЗВ_6_4РаботникиОрганизации.Ссылка.ТипСведенийСЗВ = ЗНАЧЕНИЕ(Перечисление.ТипыСведенийСЗВ.ИСХОДНАЯ)
	|				ТОГДА ПачкаДокументовСЗВ_6_4РаботникиОрганизации.НачисленоНакопительная
	|			ИНАЧЕ 0
	|		КОНЕЦ,
	|		ВЫБОР
	|			КОГДА ПачкаДокументовСЗВ_6_4РаботникиОрганизации.Ссылка.ТипСведенийСЗВ = ЗНАЧЕНИЕ(Перечисление.ТипыСведенийСЗВ.КОРРЕКТИРУЮЩАЯ)
	...
	|		ВЫБОР
	|			КОГДА ПачкаДокументовСЗВ_6_4РаботникиОрганизации.Ссылка.ТипСведенийСЗВ = ЗНАЧЕНИЕ(Перечисление.ТипыСведенийСЗВ.ИСХОДНАЯ)
	|				ТОГДА ПачкаДокументовСЗВ_6_4РаботникиОрганизации.УплаченоСтраховая
	|			ИНАЧЕ 0
	|		КОНЕЦ,
	|		ВЫБОР
	|			КОГДА ПачкаДокументовСЗВ_6_4РаботникиОрганизации.Ссылка.ТипСведенийСЗВ = ЗНАЧЕНИЕ(Перечисление.ТипыСведенийСЗВ.ИСХОДНАЯ)
	|				ТОГДА ПачкаДокументовСЗВ_6_4РаботникиОрганизации.УплаченоНакопительная
	|			ИНАЧЕ 0
	|		КОНЕЦ
	|	ИЗ
	|		Документ.ПачкаДокументовСЗВ_6_4.РаботникиОрганизации КАК ПачкаДокументовСЗВ_6_4РаботникиОрганизации
	...
	|					Ссылки.ДокументПачка
	|				ИЗ
	|					ВТДокументыСЗВ КАК Ссылки)) КАК ДанныеПачек
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Функция СформироватьЗапросПоПачкам()
	...
	|	Пачки.НомерСтроки,
	|	Пачки.ДокументПачка,
	|	МИНИМУМ(Пачки2.НомерСтроки),
	|	Пачки.ДокументПачка.Организация,
	|	Пачки.ДокументПачка.ФорматФайла,
	|	Пачки.ДокументПачка.НомерПачки,
	|	Пачки.ДокументПачка.ОтчетныйПериод
	|ИЗ
	|	Документ.ПередачаСЗВ4вПФР.ПачкиДокументов2013 КАК Пачки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПередачаСЗВ4вПФР.ПачкиДокументов2013 КАК Пачки2
	|		ПО (Пачки2.Ссылка = &ДокументСсылка)
	|			И Пачки.ДокументПачка = Пачки2.ДокументПачка
	|			И Пачки.НомерСтроки < Пачки2.НомерСтроки
	|ГДЕ
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок, СообщенияРазделПроверки = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ФорматФайла) Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Не указан формат файла!", Отказ, Заголовок,,,СообщенияРазделПроверки);
...
Процедура ПроверитьЗаполнениеПачек(ВыборкаПоПачкам,ВыборкаПоШапкеДокумента, Отказ, Заголовок = "")
	...
	Пока ВыборкаПоПачкам.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаПоПачкам.ДокументПачка) тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Строка таблицы пачек № "+ ВыборкаПоПачкам.НомерСтроки + ": Не указан документ-пачка!", Отказ, Заголовок);
			Возврат;
		КонецЕсли;	 
		
		Если ВыборкаПоПачкам.ОтчетныйПериод <> ВыборкаПоШапкеДокумента.ОтчетныйПериод тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Строка таблицы пачек № "+ ВыборкаПоПачкам.НомерСтроки + ": Отчетный период документа-пачки отличается от указанного!", Отказ, Заголовок);
		КонецЕсли;	 
		
		Если ВыборкаПоПачкам.Организация <> ВыборкаПоШапкеДокумента.Организация тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Строка таблицы пачек № "+ ВыборкаПоПачкам.НомерСтроки + ": " + ОбщегоНазначенияЗК.ПреобразоватьСтрокуИнтерфейса("Документ-пачка оформлен на другую организацию!"), Отказ, Заголовок);
		КонецЕсли;	 
		...
		Если ВыборкаПоШапкеДокумента.ОтчетныйПериод < ПроведениеРасчетов.ДатаЗаменыЕСНСтраховымиВзносами() Тогда
			Если ВыборкаПоПачкам.ФорматФайла <> ВыборкаПоШапкеДокумента.ФорматФайла Тогда
				ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Строка таблицы пачек № "+ ВыборкаПоПачкам.НомерСтроки + ": Документ-пачка имеет формат файла " + ВыборкаПоПачкам.ФорматФайла + "!", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
		
		// Проверка на уникальность номеров пачек СЗВ и АДВ
		Если ВыборкаПоПачкам.НомерПачки = ВыборкаПоШапкеДокумента.НомерПачки Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Строка таблицы пачек № "+ ВыборкаПоПачкам.НомерСтроки + ": Документ-пачка имеет такой же номер как и документ описи, рекомендуется изменить номер пачки в документе описи", Отказ, Заголовок);
		КонецЕсли;
		
		//Контроль дубля пачек документов
		Если ВыборкаПоПачкам.НомерПовторяющийсяСтроки <> NULL тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Строка таблицы пачек № "+ВыборкаПоПачкам.НомерПовторяющийсяСтроки+": Документ-пачка указан дважды!", Отказ, Заголовок);
		КонецЕсли;
...
Процедура ПроверитьСтрокуЗадолженности(ВыборкаПоЗадолженности, ВыборкаПоШапкеДокумента, Отказ, Заголовок = "", СообщенияРазделПроверки = Неопределено)
	...
	Если ВыборкаПоЗадолженности.НомерПовторяющийсяСтроки <> NULL Тогда
		...
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Повторно указан тип строки " +ВыборкаПоЗадолженности.ТипСтроки + " " + ВыборкаПоЗадолженности.Год + " год!", Отказ, Заголовок,,,СообщенияРазделПроверки);
	...
	Иначе
		...
		Если ВыборкаПоЗадолженности.Год = 0 тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Не указан год образования задолженности!", Отказ, Заголовок,,,СообщенияРазделПроверки);
		КонецЕсли;	
		...
		Если ЗначениеЗаполнено(ВыборкаПоЗадолженности.ТипСтроки) И ВыборкаПоЗадолженности.ТипСтроки <> Перечисления.РазделыАДВ11.УплатаЗаПериод И ВыборкаПоЗадолженности.СтраховаяЧасть * ВыборкаПоЗадолженности.НакопительнаяЧасть < 0 Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Суммы страховой и накопительной части пенсии должны иметь один знак!", Отказ, Заголовок,,,СообщенияРазделПроверки);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ВыборкаПоЗадолженности.ТипСтроки)  тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Не указан тип строки!", Отказ, Заголовок);
			
		ИначеЕсли (ВыборкаПоЗадолженности.ТипСтроки = Перечисления.РазделыАДВ11.ЗадолженностьНаНачало Или ВыборкаПоЗадолженности.ТипСтроки = Перечисления.РазделыАДВ11.ЗадолженностьНаКонец) И (ВыборкаПоЗадолженности.СтраховаяЧасть > 0 Или ВыборкаПоЗадолженности.НакопительнаяЧасть > 0) И ВыборкаПоШапкеДокумента.ОтчетныйПериод >= ПроведениеРасчетов.ДатаЗаменыЕСНСтраховымиВзносами() И ВыборкаПоЗадолженности.Год > 2010 Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Год задолженности не должен превышать 2010-го!", Отказ, Заголовок,,,СообщенияРазделПроверки);
			
		ИначеЕсли ВыборкаПоЗадолженности.ТипСтроки = Перечисления.РазделыАДВ11.НачисленияЗаПериод И ВыборкаПоШапкеДокумента.ОтчетныйПериод >= ПроведениеРасчетов.ДатаЗаменыЕСНСтраховымиВзносами() Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- С 2010 года начисленные суммы не указываются!", Отказ, Заголовок,,,СообщенияРазделПроверки);
			
		ИначеЕсли ВыборкаПоЗадолженности.ТипСтроки = Перечисления.РазделыАДВ11.УплатаЗаПериод И ВыборкаПоШапкеДокумента.ОтчетныйПериод >= ПроведениеРасчетов.ДатаЗаменыЕСНСтраховымиВзносами() И (ВыборкаПоЗадолженности.СтраховаяЧасть > 0 Или ВыборкаПоЗадолженности.НакопительнаяЧасть > 0) И ВыборкаПоЗадолженности.Год >= 2010 Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Год, за который уплачены взносы, не должен превышать 2009-го!", Отказ, Заголовок,,,СообщенияРазделПроверки);
		...
		ИначеЕсли ВыборкаПоЗадолженности.ТипСтроки = Перечисления.РазделыАДВ11.ЗадолженностьНаНачало Тогда
			Если ВыборкаПоЗадолженности.СтраховаяЧасть >= 0 И ВыборкаПоЗадолженности.НакопительнаяЧасть >= 0 Тогда
				Если ВыборкаПоЗадолженности.Год >= ВыборкаПоШапкеДокумента.Год тогда
					ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Год задолженности должен быть меньше расчетного периода!", Отказ, Заголовок,,,СообщенияРазделПроверки);
				КонецЕсли;
			...
			ИначеЕсли ВыборкаПоЗадолженности.СтраховаяЧасть <= 0 И ВыборкаПоЗадолженности.НакопительнаяЧасть <= 0 Тогда
				Если ВыборкаПоЗадолженности.Год > ВыборкаПоШапкеДокумента.Год тогда
					ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Год переплаты не должен превышать год расчетного периода!", Отказ, Заголовок,,,СообщенияРазделПроверки);
				КонецЕсли;
		...
		ИначеЕсли ВыборкаПоЗадолженности.ТипСтроки = Перечисления.РазделыАДВ11.НачисленияЗаПериод И ВыборкаПоЗадолженности.Год <> 0 И ВыборкаПоЗадолженности.Год <> ВыборкаПоШапкеДокумента.Год Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Начисленные суммы указываются только для текущего расчетного периода!", Отказ, Заголовок,,,СообщенияРазделПроверки);
		...
		ИначеЕсли ВыборкаПоЗадолженности.ТипСтроки = Перечисления.РазделыАДВ11.УплатаЗаПериод И ВыборкаПоЗадолженности.Год > ВыборкаПоШапкеДокумента.Год Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Год уплаты взносов не должен превышать год расчетного периода!", Отказ, Заголовок,,,СообщенияРазделПроверки);
			
		ИначеЕсли ВыборкаПоЗадолженности.ТипСтроки = Перечисления.РазделыАДВ11.ЗадолженностьНаКонец  Тогда 
			Если ВыборкаПоЗадолженности.СтраховаяЧасть >= 0 И ВыборкаПоЗадолженности.НакопительнаяЧасть >= 0 Тогда
				Если ВыборкаПоЗадолженности.Год > ВыборкаПоШапкеДокумента.Год тогда
					ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Год задолженности не должен превышать год расчетного периода!", Отказ, Заголовок,,,СообщенияРазделПроверки);
				КонецЕсли;
			...
			ИначеЕсли ВыборкаПоЗадолженности.СтраховаяЧасть <= 0 И ВыборкаПоЗадолженности.НакопительнаяЧасть <= 0 Тогда
				Если ВыборкаПоЗадолженности.Год > ВыборкаПоШапкеДокумента.Год + 1 тогда
					ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "- Год переплаты не должен превышать год расчетного периода более чем на 1!", Отказ, Заголовок,,,СообщенияРазделПроверки);
				КонецЕсли;
...
Процедура ПриКопировании(ОбъектКопирования)
	СпециальныйДокумент2010года = Ложь;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	|ВЫБРАТЬ
	|	ПередачаСЗВ4вПФРПачкиДокументов2013.ДокументПачка,
	|	ПередачаСЗВ4вПФРПачкиДокументов2013.ДокументПачка.Представление
	|ИЗ
	|	Документ.ПередачаСЗВ4вПФР.ПачкиДокументов2013 КАК ПередачаСЗВ4вПФРПачкиДокументов2013
	|ГДЕ
	|	НЕ ПередачаСЗВ4вПФРПачкиДокументов2013.ДокументПачка.Проведен
	|	И ПередачаСЗВ4вПФРПачкиДокументов2013.Ссылка = &Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ПередачаСЗВ4вПФРПачкиДокументов2014.ДокументПачка,
	|	ПередачаСЗВ4вПФРПачкиДокументов2014.ДокументПачка.Представление
	|ИЗ
	|	Документ.ПередачаСЗВ4вПФР.ПачкиДокументов2014 КАК ПередачаСЗВ4вПФРПачкиДокументов2014
	|ГДЕ
	|	НЕ ПередачаСЗВ4вПФРПачкиДокументов2014.ДокументПачка.Проведен
	|	И ПередачаСЗВ4вПФРПачкиДокументов2014.Ссылка = &Ссылка");
						  
	Запрос.УстановитьПараметр("Ссылка",	Ссылка);
	Результат = Запрос.Выполнить();
	
	Если Не Результат.Пустой() Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке("Обнаружены непроведенные пачки документов СЗВ:", Отказ, Заголовок);
		Выборка = Результат.Выбрать();
		Пока Выборка.Следующий() Цикл
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(Символы.Таб + Выборка.Представление, Отказ, Заголовок);
		КонецЦикла;
	КонецЕсли;
	...
	Если ОтчетныйПериод >= РасчетСтраховыхВзносов.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР() Тогда
		...
		Документы.ПачкаРазделов6РасчетаРСВ_1.ФормированиеФайлов(ПачкиДокументов2014, Файлы, , ОбщегоНазначенияЗК.ПолучитьЗначениеРеквизита(РасчетРСВ1, "ДатаПодписи"), Ложь);
		Для каждого СтрокаТЗ Из Файлы Цикл
			...
			НаборЗаписей.Отбор.Регистратор.Установить(СтрокаТЗ.Ссылка);
			Запись = НаборЗаписей.Добавить();
			Запись.Регистратор = СтрокаТЗ.Ссылка;
			Запись.Объект = СтрокаТЗ.Ссылка;
			Запись.ОписаниеДанных = "Раздел 6 расчета РСВ-1";
			Запись.Данные = СтрокаТЗ.ТекстФайла; 
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ОтчетныйПериод < РасчетСтраховыхВзносов.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР() Или НеСодержитСЗВ Тогда
		ПроцедурыПерсонифицированногоУчета.ПроставитьНомерПачки(ЭтотОбъект);
	Иначе
		ДанныеРСВ1 = ОбщегоНазначенияЗК.ПолучитьЗначенияРеквизитов(РасчетРСВ1, "НомерПачки, Вид");
		НомерПачки = Формат(ДанныеРСВ1.НомерПачки,"ЧГ=0");
		НомерКорректировки = ДанныеРСВ1.Вид;
	КонецЕсли;
	
	Если НеСодержитСЗВ Тогда 
		ПачкиДокументов.Очистить();
		ПачкиДокументов2013.Очистить();
		ПачкиДокументов2014.Очистить();
	Иначе
	...
	ИначеЕсли ОтчетныйПериод < РасчетСтраховыхВзносов.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР() Тогда
		ТабличнаяЧасть = ПачкиДокументов2013;	
	Иначе
		ТабличнаяЧасть = ПачкиДокументов2014;	
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ТабличнаяЧасть Цикл
		Если ПустаяСтрока(КраткийСоставДокумента) Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТаблицы.ДокументПачка) Тогда
