...
Функция СтруктураОбязательныхПолейШапка()
	...
	СтруктураПолей= Новый Структура("Контрагент,Ответственный,ВалютаДокумента");	
...
Процедура ПроверитьЗаполнениеТЧ(Отказ, Заголовок)
	...
	Для Каждого Платеж Из РасшифровкаПлатежа Цикл
		...
		ПроверитьЗначение(Платеж.ДатаОплаты,Отказ, Заголовок,"Дата оплаты");
		
		Если Не Отказ Тогда
	
			Если ЗначениеЗаполнено(Организация) 
				И ЗначениеЗаполнено(Платеж.ДоговорКонтрагента) 
...
Процедура ДвиженияПоРегистрам(РежимПроведения, Отказ, Заголовок, СтруктураКурсыВалют)
	...
	// По регистру "ГарантийныеПисьма"
	НаборДвиженийГарантийныеПисьма = Движения.ПКК_ГарантийныеПисьма;
	ТаблицаДвиженийГарантийныеПисьма = НаборДвиженийГарантийныеПисьма.ВыгрузитьКолонки();
	...
	Для Каждого Платеж Из РасшифровкаПлатежа Цикл
		...
		СтруктураКурсВзаиморасчетов=МодульВалютногоУчета.ПолучитьКурсВалюты(
			ВалютаВзаиморасчетов,?(Платеж.ДатаОплаты='00010101',Дата,Платеж.ДатаОплаты));
		...
		СтрокаДвиженийГарантийныеПисьма = ТаблицаДвиженийГарантийныеПисьма.Добавить();
		
		ЗаполнитьЗначенияСвойств(СтрокаДвиженийГарантийныеПисьма, Платеж);
		
		СтрокаДвиженийГарантийныеПисьма.СуммаУпр				= СуммаУпр;
		СтрокаДвиженийГарантийныеПисьма.Сумма					= Платеж.СуммаПлатежа;
		СтрокаДвиженийГарантийныеПисьма.ГарантийноеПисьмо		= Ссылка;
		СтрокаДвиженийГарантийныеПисьма.Контрагент				= Контрагент;
		СтрокаДвиженийГарантийныеПисьма.Организация				= Организация;
		
		//{17.04.2015 Островерхий заявка №37762 
		СтрокаДвиженийГарантийныеПисьма.Период = Платеж.ДатаОплаты; 
		//17.04.2015 Островерхий} 
		
	КонецЦикла;
	
	//{17.04.2015 Островерхий заявка №37762 
	//НаборДвиженийГарантийныеПисьма.мПериод			= ?(ДатаВходящегоДокумента = '00010101', Дата, ДатаВходящегоДокумента); 
	//17.04.2015 Островерхий} 
	
	НаборДвиженийГарантийныеПисьма.мТаблицаДвижений	= ТаблицаДвиженийГарантийныеПисьма;
	Движения.ПКК_ГарантийныеПисьма.ВыполнитьПриход();
...
Функция СформироватьСтруктуруКурсыВалютУпр()
	...
	СтруктураКурсыВалют=УправлениеДенежнымиСредствами.ПолучитьКурсыДляГруппыВалют(СтруктураГруппаВалют,?(ДатаВходящегоДокумента='00010101',Дата,ДатаВходящегоДокумента));
