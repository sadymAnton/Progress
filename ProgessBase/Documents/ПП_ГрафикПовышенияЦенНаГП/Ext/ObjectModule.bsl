...
Функция СформироватьЗапросПоТЧ()
	 ...
	 ЗапросПоТЧ.Текст ="ВЫБРАТЬ
	                   ...
	                   |	ПП_ГрафикПовышенияЦенНаГПТабличнаяЧасть.Процент,
	                   |	ПП_ГрафикПовышенияЦенНаГПТабличнаяЧасть.Ссылка.Период как Месяц
	                   |ИЗ
	                   |	Документ.ПП_ГрафикПовышенияЦенНаГП.ТабличнаяЧасть КАК ПП_ГрафикПовышенияЦенНаГПТабличнаяЧасть
	                   |ГДЕ
	                   |	ПП_ГрафикПовышенияЦенНаГПТабличнаяЧасть.Ссылка = &Ссылка" ;
...
// Выполняет движения по регистрам.
 ...
 Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, ТаблицаПоТЧ, Отказ, Заголовок)
	 ...
	 НаборДвиженийТЧ 	= Движения.ППП_ГрафикПовышенияЦенНаГП;
	 ТаблицаДвиженийТЧ 	= НаборДвиженийТЧ.Выгрузить();
	 
	 //Нужно обнулить все записи которые уже есть
	 Запрос = Новый Запрос;
	 Запрос.Текст = "ВЫБРАТЬ
	                |	ППП_ГрафикПовышенияЦенНаГП.КатегорияКлиента,
	                |	ППП_ГрафикПовышенияЦенНаГП.Брендообъем,
	                |	ППП_ГрафикПовышенияЦенНаГП.Месяц
	                |ИЗ
	                |	РегистрСведений.ППП_ГрафикПовышенияЦенНаГП.СрезПоследних(&Дата, Месяц = &Месяц) КАК ППП_ГрафикПовышенияЦенНаГП";
	 ...
	 Пока Сч < Результат.Количество() Цикл
		 ...
		 СтрТаблицы = ТаблицаПоТЧ.НайтиСтроки(Новый Структура("КатегорияКлиента,Брендообъем,Месяц", 
	 ...
	 Если Не Отказ Тогда								
	 	Движения.ППП_ГрафикПовышенияЦенНаГП.ВыполнитьДвижения();
...
Функция МожноМенятьДокумент() экспорт
	...
	Запрос.Текст = 
		...
		|	ППП_ГрафикПовышенияЦенНаГП.Регистратор
		|ИЗ
		|	РегистрСведений.ППП_ГрафикПовышенияЦенНаГП.СрезПервых(&Дата, Месяц = &Месяц и Регистратор <> &Регистратор) КАК ППП_ГрафикПовышенияЦенНаГП ";
