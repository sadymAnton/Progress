...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если Лев(ИмяМакета,2) = "Т7" Тогда
		...
		// ИЗМЕНЕНИЕ: Измайлова Полина 22.06.2011 13:40  
		//Запрос.Текст =
		//"ВЫБРАТЬ
		...
		// ИЗМЕНЕНИЕ: Измайлова Полина 22.06.2011 13:41  
		// Изменен запрос таким образом, чтобы продолжительность отпуска считалась по календарю из графика работы сотрудника.
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ГрафикОтпусковОрганизацииРаботникиОрганизации.Сотрудник КАК Сотрудник,
		|	ГрафикОтпусковОрганизацииРаботникиОрганизации.Сотрудник.Физлицо КАК Физлицо,
		...
		|		ПО РаботникиОрганизацииСрезПоследних.Сотрудник = ВТДанныеДокумента.Сотрудник
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТДанныеОРаботниках.Сотрудник,
		|	ВТДанныеОРаботниках.ПодразделениеОрганизации,
		|	ВТДанныеОРаботниках.Должность,
		|	ВТДанныеОРаботниках.ПодразделениеОрганизацииНаименование,
		|	ВТДанныеОРаботниках.ДолжностьНаименование,
		|	СУММА(ЕСТЬNULL(КалендарьДнейВсего.КалендарныеДни, 0)) КАК Продолжительность
		|ПОМЕСТИТЬ ВТДанныеОРаботникахИДлинеОтпуска
		|ИЗ
		|	ВТДанныеОРаботниках КАК ВТДанныеОРаботниках
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РегламентированныйПроизводственныйКалендарь КАК КалендарьДнейВсего
		|		ПО (КалендарьДнейВсего.AVT_ПроизводственныйКалендарь = ВТДанныеОРаботниках.Календарь)
		|			И (КалендарьДнейВсего.ВидДня <> ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Праздник))
		|			И (КалендарьДнейВсего.ДатаКалендаря МЕЖДУ ВТДанныеОРаботниках.ДатаНачала И ВТДанныеОРаботниках.ДатаОкончания)
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТДанныеОРаботниках.Сотрудник,
		|	ВТДанныеОРаботниках.ПодразделениеОрганизации,
		...
		|	ГрафикОтпусковОрганизацииРаботники.ДатаНачала,
		|	ГрафикОтпусковОрганизацииРаботники.ДатаОкончания,
		|	РаботникиОрганизацииСрезПоследних.Продолжительность,
		|	ГрафикОтпусковОрганизацииРаботники.СотрудникКод КАК ТабельныйНомер,
		|	ОтветственныеЛицаОрганизацийСрезПоследних.Должность КАК ДолжностьРуководителя,
		|	ЕСТЬNULL(ФИОФизЛицСрезПоследних.Фамилия + "" "" + ФИОФизЛицСрезПоследних.Имя + "" "" + ФИОФизЛицСрезПоследних.Отчество, ОтветственныеЛицаОрганизацийСрезПоследних.ФизическоеЛицоНаименование) КАК ФИОРуководителя,
		|	РаботникиОрганизацииСрезПоследних.ПодразделениеОрганизации,
		...
		|ИЗ
		|	ВТДанныеДокумента КАК ГрафикОтпусковОрганизацииРаботники
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДанныеОРаботникахИДлинеОтпуска КАК РаботникиОрганизацииСрезПоследних
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВТДанныеОбОтветственномЛице КАК ОтветственныеЛицаОрганизацийСрезПоследних
		|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(
		|						&ДатаДокумента,
		|						ФизЛицо В
