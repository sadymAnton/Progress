...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//{25.09.2015 Островерхий заявка №б/н 
	Если НЕ РольДоступна(Метаданные.ОсновнаяРоль.Имя) Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Не возможно записать Документ Изменение заказа покупателя тк он не включен ни в один утвержденный бизнес процесс", Отказ);	
	КонецЕсли;  
