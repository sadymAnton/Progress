...
Процедура ПроверитьЗаполнениеСтрокиЗарплаты(СтрокаЗарплаты, Отказ, Заголовок = "")
	...
	Если СтрокаЗарплаты.Сумма = 0 Тогда
		ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указана сумма к выплате. Строка пропущена.");
	КонецЕсли;
	Если СтрокаЗарплаты.Сумма < 0 Тогда
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Возврат УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначенияЗК.СформироватьЗаголовокДокумента(ЭтотОбъект,"Зарплата к выплате "));
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ ПометкаУдаления И РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
		Если ПолучитьДокументыВведенныеНаОсновании().Количество()  > 0 Тогда
			...
			ОбщегоНазначенияЗК.СообщитьОбОшибке("По документу уже созданы платежные документы, отмена проведения невозможна!", Отказ, Заголовок);
