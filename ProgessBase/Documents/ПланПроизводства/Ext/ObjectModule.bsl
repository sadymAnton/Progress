...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//{09.06.2015 Островерхий заявка №39845 
	СтруктураПолей.Вставить("ПРГКратностьПроизводства", "ПРГКратностьПроизводства");
	//09.06.2015 Островерхий} 
...
//m.ionov@a-prof.ru 16/12/2013
Функция УстановитьПериодВПотребностях(СтрокаПотребности) Экспорт
	
	стрПериод = Формат(СтрокаПотребности.Период,"ДФ=dd.MM.yyyy");
	...
	ИначеЕсли ПериодичностьДетализации = Перечисления.Периодичность.Год Тогда
		стрПериод = Формат(СтрокаПотребности.Период,"ДФ=yyyy");
	ИначеЕсли ПериодичностьДетализации = Перечисления.Периодичность.Месяц Тогда
		стрПериод = Формат(СтрокаПотребности.Период,"ДФ=MMMM");//Месяц(СтрокаПотребности.Период);
	ИначеЕсли ПериодичностьДетализации = Перечисления.Периодичность.Полугодие Тогда
		стрПериод = ?(ДеньГода(СтрокаПотребности.Период) > 182,"2","1");
	ИначеЕсли ПериодичностьДетализации = Перечисления.Периодичность.Квартал Тогда
		стрПериод = Формат(НачалоКвартала(СтрокаПотребности.Период),"ДФ=dd.MM.yyyy"); 
	ИначеЕсли ПериодичностьДетализации = Перечисления.Периодичность.Месяц Тогда
		стрПериод = Месяц(СтрокаПотребности.Период);	
	ИначеЕсли ПериодичностьДетализации = Перечисления.Периодичность.Декада Тогда
		ВрПериод = СтрокаПотребности.Период;
		УправлениеПланированием.ВыровнятьПериод(ВрПериод, ВрПериод, ПериодичностьДетализации);
		стрПериод = Формат(ВрПериод,"ДФ=dd.MM.yyyy");
	ИначеЕсли ПериодичностьДетализации = Перечисления.Периодичность.Неделя Тогда
		стрПериод = НеделяГода(СтрокаПотребности.Период);	
	КонецЕсли;
