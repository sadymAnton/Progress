...
Функция Автозаполнение(Запрос = Неопределено, Сотрудник = Неопределено, ТекущаяСтрокаТабличнойЧасти = Неопределено) Экспорт
	...
	Если Не СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ВЦеломЗаПериод Тогда
		ОтборПоГрафикам1 = "
		|	И ВЫБОР
		...
		ОтборПоГрафикам3 = "
		|	И Работники.ГрафикРаботыЗавершения.AVT_ПроизводственныйКалендарь = &Календарь";
	КонецЕсли;
	...
	Если Запрос = Неопределено Тогда 
		...
		Если Сотрудник = Неопределено Тогда
			...
			///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
			// Измайлова Полина 22.06.2011 11:02 
			|" + ОтборПоГрафикам1 + "
			////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
			|
			|ОБЪЕДИНИТЬ
	...
	///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 22.06.2011 11:00 
	...
	|;
	///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 03.10.2011 11:42 
	|ВЫБРАТЬ
	|	AVT_РаботаСверхНормыОбороты.Период,
	|	AVT_РаботаСверхНормыОбороты.Сотрудник,
	|	AVT_РаботаСверхНормыОбороты.ВидРасчета,
	|	AVT_РаботаСверхНормыОбороты.ЧасовОборот КАК Часов
	|ПОМЕСТИТЬ ВТРаботаСверхурочно
	|ИЗ
	|	РегистрНакопления.AVT_РаботаСверхНормы.Обороты(
	|			&НачалоМесяца,
	|			&КонецМесяца,
	|			День,
	|			Сотрудник В
	...
	|				ИЗ
	|					ВТСписокРаботников)) КАК AVT_РаботаСверхНормыОбороты
	|;
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}  
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|		КОГДА Начисления.ВидВремени = ЗНАЧЕНИЕ(Перечисление.ВидыВремени.ОтработанноеСверхНормы)
		///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 05.06.2012 11:35 
	// ОПИСАНИЕ: выбираем ночное время из кадрового документа "Работа в праздники и выходные"
	|		ИЛИ (Начисления.Авт_ВремяСверхурочно И
	|			Начисления.ВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаНочныеЧасы))
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА} 
	|			ТОГДА ВЫБОР
	...
	|							ТОГДА &НачалоМесяца <= ПлановыеНачисления.Период И ПлановыеНачисления.Период <= &КонецМесяца
	...
	|			Начисления.Регистратор КАК Регистратор
		///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 05.06.2012 11:45 
	|			,ЛОЖЬ КАК АВТ_ВремяСверхурочно
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}  
	|		ИЗ
	...
	|					ИЛИ Начисления.ВидРасчета.ОбозначениеВТабелеУчетаРабочегоВремени В (ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.РаботаВечерниеЧасы), ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.РаботаНочныеЧасы)))
	//) КАК Начисления
	///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 03.10.2011 11:47 
	// ОПИСАНИЕ: учет кадровых документов "Работа в праздники и выходные" и "Работа в сверхурочные часы".
	|
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	|			РаботаСверхурочно.Период,
	|			РаботаСверхурочно.Сотрудник,
	|			ВЫБОР
	|				КОГДА ГрафикиРаботПоВидамВремени.Сотрудник ЕСТЬ NULL 
	|					ТОГДА ВЫБОР
	|							КОГДА РаботникиОрганизаций.ПериодЗавершения <= РаботаСверхурочно.Период
	...
	|						КОНЕЦ
	|				ИНАЧЕ РаботаСверхурочно.Сотрудник
	|			КОНЕЦ,
	|			РаботаСверхурочно.ВидРасчета,
	|			ЗНАЧЕНИЕ(Перечисление.ВидыУчетаВремени.ПоЧасам),
	|			РаботаСверхурочно.Период,
	|			КОНЕЦПЕРИОДА(РаботаСверхурочно.Период, ДЕНЬ),
	|			РаботаСверхурочно.ВидРасчета.ВидВремени,
	|			РаботаСверхурочно.Часов,
	|			0,
	|			NULL,
	|			ИСТИНА
	|		ИЗ
	|			ВТРаботаСверхурочно КАК РаботаСверхурочно
	|				ЛЕВОЕ СОЕДИНЕНИЕ ВТНефильтрованныеОсновныеНачисления КАК НефильтрованныеОсновныеНачисления
	|				ПО РаботаСверхурочно.Период = НефильтрованныеОсновныеНачисления.ПериодДействияНачало
	|					И РаботаСверхурочно.Сотрудник = НефильтрованныеОсновныеНачисления.Сотрудник
	|					И РаботаСверхурочно.ВидРасчета = НефильтрованныеОсновныеНачисления.ВидРасчета
	|				ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ РАЗЛИЧНЫЕ
	|					РаботаСверхурочно.Период КАК Период,
	|					РаботаСверхурочно.Сотрудник КАК Сотрудник
	|				ИЗ
	|					ВТРаботаСверхурочно КАК РаботаСверхурочно
	|						ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикиРаботыПоВидамВремени КАК ГрафикиРаботыПоВидамВремени
	|						ПО РаботаСверхурочно.Сотрудник = ГрафикиРаботыПоВидамВремени.ГрафикРаботы
	|							И (НАЧАЛОПЕРИОДА(РаботаСверхурочно.Период, МЕСЯЦ) = ГрафикиРаботыПоВидамВремени.Месяц)) КАК ГрафикиРаботПоВидамВремени
	|				ПО РаботаСверхурочно.Период = ГрафикиРаботПоВидамВремени.Период
	|					И РаботаСверхурочно.Сотрудник = ГрафикиРаботПоВидамВремени.Сотрудник
	|				ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|					РаботаСверхурочно.Период КАК Период,
	|					РаботаСверхурочно.Сотрудник КАК Сотрудник,
	|					МАКСИМУМ(РаботникиОрганизаций.Период) КАК ДатаПоследДвижения
	|				ИЗ
	|					ВТРаботаСверхурочно КАК РаботаСверхурочно
	|						ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
	|						ПО РаботаСверхурочно.Сотрудник = РаботникиОрганизаций.Сотрудник
	|							И РаботаСверхурочно.Период >= РаботникиОрганизаций.Период
	|				
	|				СГРУППИРОВАТЬ ПО
	|					РаботаСверхурочно.Период,
	|					РаботаСверхурочно.Сотрудник) КАК ДатыПоследнихДвиженийРаботников
	|				ПО РаботаСверхурочно.Сотрудник = ДатыПоследнихДвиженийРаботников.Сотрудник
	|					И РаботаСверхурочно.Период = ДатыПоследнихДвиженийРаботников.Период
	|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций КАК РаботникиОрганизаций
	|				ПО (РаботникиОрганизаций.Период = ДатыПоследнихДвиженийРаботников.ДатаПоследДвижения)
	|					И РаботаСверхурочно.Сотрудник = РаботникиОрганизаций.Сотрудник
	|		ГДЕ
	|			НефильтрованныеОсновныеНачисления.Сотрудник ЕСТЬ NULL ) КАК Начисления
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}                                          
	...
	///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 03.07.2012 11:54 
	|		КОГДА Основной.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Авт_ДополнительныеДниНеоплачиваемые)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.КлассификаторИспользованияРабочегоВремени.ДополнительныеВыходныеДниНеОплачиваемые) 
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}
	|	КОНЕЦ КАК ВидВремени,
	...
	Иначе
		...
		///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 05.06.2012 13:32 
		|							ИЛИ ДневныеНеявки.ВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаНочныеЧасы)
		////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}   
		...
		// ИЗМЕНЕНИЕ: ООО "Автоматизация-Л" Измайлова Полина 21.06.2012 14:17  
		// ОПИСАНИЕ: добавлена агрегатная функция СУММА, сгруппировано, чтобы складывались ночные из документов "Работа в праздники и выходные" и из графиков.
		|			СУММА(ВЫБОР
		|				КОГДА (НЕ ДневныеНеявки.ВремяЗаДень ЕСТЬ NULL )
		|						И ДневныеНеявки.ВидРасчета <> ""ПланируемаяЯвка""
		|						И (ДневныеНеявки.ВремяЗаДень > 0)
		...
		|		)
		///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
		// Измайлова Полина 21.06.2012 14:14 
		// ОПИСАНИЕ: сгруппировано, чтобы складывались ночные из документов "Работа в праздники и выходные" и из графиков.
		|СГРУППИРОВАТЬ ПО
		|	ДниСотрудников.Сотрудник,
		|	ДниСотрудников.Дата,
		|	ВЫБОР
		...
		|			ТОГДА ДневныеНеявки.ВидРасчета.ОбозначениеВТабелеУчетаРабочегоВремени.РабочееВремя
		|		КОГДА (НЕ ВТСостояниеСотрудников.ВидВремени ЕСТЬ NULL )
		|			ТОГДА ВТСостояниеСотрудников.ВидВремени.РабочееВремя
		|		КОГДА ЕСТЬNULL(ИндивидуальныеГрафики.ОсновноеЗначение, ОбщиеГрафики.ОсновноеЗначение) > 0
		|			ТОГДА ВЫБОР
		|					КОГДА ЕСТЬNULL(ИндивидуальныеГрафики.ВидУчетаВремени, ОбщиеГрафики.ВидУчетаВремени) = ЗНАЧЕНИЕ(Перечисление.ВидыУчетаВремени.ПоЧасам)
		|						ТОГДА ЕСТЬNULL(ПлановыеНачисленияСотрудников.КлассификаторВидаВремени.РабочееВремя, ИСТИНА)
...
Процедура ДобавитьСтрокуГрафикиРаботыПоВидамВремениЧасы(ВыборкаПоСтрокамДокумента, ВыборкаПоШапкеДокумента, ТаблицаНабора, ЗаполнятьРеквизитДокумент = Истина)
	...
	//	//Движение.ОсновноеЗначениеПолное = 0;
	//	//Движение.ДополнительноеЗначениеПолное = 0;
	Если ВыборкаПоСтрокамДокумента.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.ВыходныеДни 
		/////Вадим 06.11.2012 11:13:55
        и не ПроверкаДняНаЯвку(ВыборкаПоСтрокамДокумента.ДеньМесяца,СтруктураПоиска.ВидУчетаВремени,МассивСтрок.Количество(),ВыборкаПоСтрокамДокумента.сотрудник)
  		////ВадимКонец
		Тогда
		Движение.ОсновноеЗначениеПолное = 0;
		Движение.ДополнительноеЗначениеПолное = 0;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	КонецЕсли;
		///////////////////////////////////////////////////////////////{ООО "Автоматизация-Л" ДОРАБОТКА
	// Измайлова Полина 22.06.2011 11:17 
	Если СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ВЦеломЗаПериод Тогда
		AVT_Календарь = Справочники.AVT_ПроизводственныеКалендари.ПустаяСсылка();
	КонецЕсли;
	////////////////////////////////////////////////////////////////ООО "Автоматизация-Л" ДОРАБОТКА}  
КонецПроцедуры
...
/////Вадим 06.11.2012 11:00:16

Функция ПроверкаДняНаЯвку(День,ВидВремени,КоличествоСтрок,Сотрудник)
	Если КоличествоСтрок=0 тогда возврат ложь; конецесли;
	Если не ВидВремени=Перечисления.ВидыУчетаВремени.ПоЧасам Тогда возврат ложь КонецЕсли;
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя.Сотрудник
		...
		|	И (ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя.ПервыйВидВремени"+строка(день)+" = &Явка
		|			ИЛИ ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя.ВторойВидВремени"+строка(день)+" = &Явка
		|			ИЛИ ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя.ТретийВидВремени"+строка(день)+" = &Явка
		|			ИЛИ ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя.ЧетвертыйВидВремени"+строка(день)+" = &Явка)
		|	И ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя.Сотрудник = &Сотрудник";
