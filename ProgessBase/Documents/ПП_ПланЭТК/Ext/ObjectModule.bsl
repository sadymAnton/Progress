...
Функция СформироватьЗапросПоТЧ()
	 ...
	 ЗапросПоТЧ.Текст ="ВЫБРАТЬ
	                   ...
	                   |	ПП_ПланЭТКТабличнаяЧасть.Показатель,
	                   |	ПП_ПланЭТКТабличнаяЧасть.Группа,
	                   |	ПП_ПланЭТКТабличнаяЧасть.Ссылка.Период как Месяц
	                   |ИЗ
	                   |	Документ.ПП_ПланЭТК.ТабличнаяЧасть КАК ПП_ПланЭТКТабличнаяЧасть
	                   |ГДЕ
	                   |	ПП_ПланЭТКТабличнаяЧасть.Ссылка = &Ссылка" ;
...
// Выполняет движения по регистрам.
 ...
 Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, ТаблицаПоТЧ, Отказ, Заголовок)
	 ...
	 НаборДвиженийТЧ 	= Движения.ППП_ПланЭТК;
	 ТаблицаДвиженийТЧ 	= НаборДвиженийТЧ.Выгрузить();
	 
	 //Нужно обнулить все записи которые уже есть
	 Запрос = Новый Запрос;
	 Запрос.Текст = "ВЫБРАТЬ
	                |	ППП_ПланЭТК.Дивизион,
	                |	ППП_ПланЭТК.КатегорияКлиента,
	                |	ППП_ПланЭТК.Месяц,
					//bolshevykh@a-prof.ru 12.03.2015
					|	ППП_ПланЭТК.Группа,
					//bolshevykh@a-prof.ru 12.03.2015
					|	ППП_ПланЭТК.ВидПоказателя
	                |ИЗ
	                |	РегистрСведений.ППП_ПланЭТК.СрезПоследних(&Дата, Месяц = &Месяц) КАК ППП_ПланЭТК";
	 ...
	 Пока Сч < Результат.Количество() Цикл
		 ...
		 //bolshevykh@a-prof.ru 12.03.2015
		 СтрТаблицы = ТаблицаПоТЧ.НайтиСтроки(Новый Структура("Дивизион,КатегорияКлиента,Месяц,Группа,ВидПоказателя", 
		 //bolshevykh@a-prof.ru 12.03.2015
	 ...
	 Если Не Отказ Тогда								
	 	Движения.ППП_ПланЭТК.ВыполнитьДвижения();
...
Функция МожноМенятьДокумент() экспорт
	...
	Запрос.Текст = 
		...
		|	ППП_ПланЭТК.Регистратор
		|ИЗ
		|	РегистрСведений.ППП_ПланЭТК.СрезПервых(&Дата, Месяц = &Месяц и Регистратор <> &Регистратор) КАК ППП_ПланЭТК ";
