...
/// Кунов О.В., 08.04.2015 - 38108
Перем мБессрочный Экспорт;
///
...
Процедура ПроверкаРеквизитов(Отказ, Заголовок)
	...
	Если НЕ ЗначениеЗаполнено(НомерПоРегистрации) Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Не заполнен Номер сертификата!", Отказ, Заголовок);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ОрганПоСертификации) Тогда 
		ОбщегоНазначения.СообщитьОбОшибке("Не заполнен сертифицирующий орган!", Отказ, Заголовок);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДатаОкончания)
		И Не мБессрочный Тогда 
		ОбщегоНазначения.СообщитьОбОшибке("Не заполнена дата окончания действия сертификата!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ДвиженияПоРегистрам()
	...
	//Движение по регистру ВнешниеСертификаты
	НаборЗаписей = Движения.ПРГ_ВнешниеСертификаты;
	
	Для Каждого СтрокаТЧ ИЗ Товары Цикл
		Движение 				= НаборЗаписей.Добавить();
		Движение.Номенклатура	= СтрокаТЧ.Номенклатура;
		Движение.Период 		= Дата;
		Движение.ВидСертификата = ВидОперации;
...
мБессрочный = Не ЗначениеЗаполнено(ДатаОкончания);