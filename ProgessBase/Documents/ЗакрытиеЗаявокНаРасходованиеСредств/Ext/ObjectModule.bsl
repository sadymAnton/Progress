...
Перем мВалютаУправленческогоУчета; // Шевченков
...
Процедура ДвиженияПоРегистрам(Отказ, Заголовок)
	...
	|	ЗаявкиКЗакрытию.ВключатьВПлатежныйКалендарь КАК ВключатьВПлатежныйКалендарь, ЗаявкиКЗакрытию.ВалютаЗаявка КАК Валюта,
	...
	ТаблицаПРГ_ОстаткиПоБюджетам       = Движения.ПРГ_ОстаткиПоБюджетам.ВыгрузитьКолонки(); // Шевченков
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаДвиженийОстатковПоБюджетам = ТаблицаПРГ_ОстаткиПоБюджетам.Добавить();  // Шевченков
		
		ЗаполнитьЗначенияСвойств(СтрокаДвиженийЗаявки, Выборка);
		ЗаполнитьЗначенияСвойств(СтрокаДвиженийОстатковПоБюджетам, Выборка);  // <- Шевченков
		
		//СтруктураГруппаВалют = Новый Структура;
		//СтруктураГруппаВалют.Вставить("ВалютаУпрУчета",мВалютаУправленческогоУчета.Код);
		//СтруктураГруппаВалют.Вставить("ВалютаДокумента",Выборка.Валюта.Код);	
		//СтруктураКурсыВалют=УправлениеДенежнымиСредствами.ПолучитьКурсыДляГруппыВалют(СтруктураГруппаВалют,?(Выборка.ЗаявкаНаРасходование.ДатаРасхода='00010101',Выборка.ЗаявкаНаРасходование.Дата,КонецДня(Выборка.ЗаявкаНаРасходование.ДатаРасхода)));
		//СтруктураКурсВзаиморасчетов = МодульВалютногоУчета.ПолучитьКурсВалюты(
		//	Выборка.Валюта, ?(Выборка.ЗаявкаНаРасходование.ДатаРасхода = '00010101', Выборка.ЗаявкаНаРасходование.Дата, КонецДня(Выборка.ЗаявкаНаРасходование.ДатаРасхода)));
		//СуммаУпр = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(
		//	Выборка.СуммаУпр,
		//	Выборка.Валюта, Справочники.Валюты.НайтиПоКоду("643"),
		...
		//СтрокаДвиженийОстатковПоБюджетам.Сумма = Выборка.СуммаУпр;
		СтрокаДвиженийОстатковПоБюджетам.СарехОрех     = Выборка.ЗаявкаНаРасходование.СарехОрех;
		СтрокаДвиженийОстатковПоБюджетам.Подразделение = Выборка.ЗаявкаНаРасходование.ЦФО;
		СтрокаДвиженийОстатковПоБюджетам.СтатьяДДС     = Выборка.СтатьяДвиженияДенежныхСредств;
		СтрокаДвиженийОстатковПоБюджетам.Сумма         = Выборка.СуммаУпр;
		// ->
	...
	// <- Шевченков
	Движения.ПРГ_ОстаткиПоБюджетам.мПериод          = Дата;
	Движения.ПРГ_ОстаткиПоБюджетам.мТаблицаДвижений = ТаблицаПРГ_ОстаткиПоБюджетам;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если Не Отказ Тогда
		...
		// <- Шевченков, очистим бонусные акты
		Для Каждого СтрокаЗаявка Из ЭтотОбъект.ЗаявкиНаРасходованиеСредств Цикл		
			Если ЗначениеЗаполнено(СтрокаЗаявка.Заявка.ПКК_АктНачисленияБонусов) Тогда
				ТекЗаявка = СтрокаЗаявка.Заявка.ПолучитьОбъект();
				ТекЗаявка.ПКК_АктНачисленияБонусов = Документы.ПКК_АктНачисленияБонусовУслуг.ПустаяСсылка();
				ТекЗаявка.Записать();			
			КонецЕсли;		         	
		КонецЦикла;
		// ->
...
мВалютаУправленческогоУчета=глЗначениеПеременной("ВалютаУправленческогоУчета"); // Шевченков