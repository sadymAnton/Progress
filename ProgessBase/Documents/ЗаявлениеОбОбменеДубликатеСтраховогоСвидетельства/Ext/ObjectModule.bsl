				...
				Для Каждого СтрокаДереваЗаявление Из СтрокаДереваПачкаВходящихДокументов.Строки Цикл
					Если СтрокаДереваЗаявление.Имя = "ЗАЯВЛЕНИЕ_О_ДУБЛИКАТЕ" Или СтрокаДереваЗаявление.Имя = "ЗАЯВЛЕНИЕ_ОБ_ОБМЕНЕ" Тогда
						...
						Если СтрокаДереваИзменившиесяДанные <> Неопределено Тогда
							...
							КонецЕсли;
							//Blik 280317 63451 н
							 СтрокаДереваФИОИзм = СтрокаДереваИзменившиесяДанные.Строки.Найти("ФИО");
							 Если СтрокаДереваФИОИзм <> Неопределено Тогда
								 ОбластьЗаявление.Параметры.Фамилия 	= ПроцедурыПерсонифицированногоУчета.ИзвлечьДанное(СтрокаДереваФИОИзм, "Фамилия"); 
								 ОбластьЗаявление.Параметры.Имя 		= ПроцедурыПерсонифицированногоУчета.ИзвлечьДанное(СтрокаДереваФИОИзм, "Имя"); 
								 ОбластьЗаявление.Параметры.Отчество 	= ПроцедурыПерсонифицированногоУчета.ИзвлечьДанное(СтрокаДереваФИОИзм, "Отчество"); 
							 КонецЕсли;
							//Blik 280317 63451 к
