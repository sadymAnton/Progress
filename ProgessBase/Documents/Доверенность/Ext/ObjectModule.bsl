...
Функция ПечатьДоверенности(НазваниеМакета)
	...
	Если НазваниеМакета = "М2" тогда
		...
		// <- Шевченков
		Если НЕ ЗначениеЗаполнено(ОбластьМакета.Параметры.ФИОДоверенного) Тогда
		
			//ИспользоватьКонтрагента = Истина;
			ОбластьМакета.Параметры.ФИОДоверенного = Шапка.ФизЛицо;
	...
	//ОбластьМакета.Параметры.ФамилияИмяОтчествоДоверенного = ФамилияИмяОтчествоДоверенного; Шевченков
	Если ТипЗнч(ЭтотОбъект.ФИЗлИЦО) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		ОбластьМакета.Параметры.ФамилияИмяОтчествоДоверенного = ФамилияИмяОтчествоДоверенного; // + ?(ЗначениеЗаполнено(ЭтотОбъект.Машина), "(" + ЭтотОбъект.Машина + ")", "");
	Иначе
		ОбластьМакета.Параметры.ФамилияИмяОтчествоДоверенного = Строка(ЭтотОбъект.ФизЛицо); // + ?(ЗначениеЗаполнено(ЭтотОбъект.Машина), "(" + ЭтотОбъект.Машина + ")", "");
	КонецЕсли;	
	...
	//ОбластьМакета.Параметры.ПеревозчикПредставление       = ?(ЗначениеЗаполнено(ЭтотОбъект.Перевозчик), Строка(ЭтотОбъект.Перевозчик) + " (" +    // Шевченков 41124
	//														ЭтотОбъект.Перевозчик.ИНН + "/" + ЭтотОбъект.Перевозчик.КПП + ")", "");
	
	ОбластьМакета.Параметры.ПеревозчикПредставление       = ?(ЗначениеЗаполнено(ЭтотОбъект.Перевозчик), Строка(ЭтотОбъект.Перевозчик) + " (" + 
															ЭтотОбъект.Перевозчик.ИНН + "/" + ЭтотОбъект.Перевозчик.КПП + "), " + Строка(ЭтотОбъект.Машина), "");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// <- Шевченков
	мСоздатьПрефикс = ЭтоНовый();
	//>>201015 Степанов 45122
	ОтделениеПрогресс = Справочники.НСИ_Отделения.НайтиПоНаименованию("Прогресс");
	Если Не ЭтоНовый() тогда
		Если ЭтотОбъект.НСИ_Отделение<> ЭтотОбъект.Ссылка.НСИ_Отделение тогда
			Если ((ЭтотОбъект.НСИ_Отделение = Справочники.НСИ_Отделения.ПустаяСсылка()) И (ЭтотОбъект.Ссылка.НСИ_Отделение = ОтделениеПрогресс)) 
			ИЛИ  ((ЭтотОбъект.НСИ_Отделение = ОтделениеПрогресс) И (ЭтотОбъект.Ссылка.НСИ_Отделение = Справочники.НСИ_Отделения.ПустаяСсылка())) тогда
			    мСоздатьПрефикс=Ложь;
		    Иначе
			    мСоздатьПрефикс=Истина;
			конецЕсли;
		КонецЕсли;	
	Конецесли;	
	//<<	
	Если НЕ мСоздатьПрефикс Тогда
		Возврат;
	КонецЕсли;
	ЭтотОбъект.Номер = "";
	//>>201015 Степанов 45122
	Если (ЭтотОбъект.НСИ_Отделение=Справочники.НСИ_Отделения.ПустаяСсылка()) или (ЭтотОбъект.НСИ_Отделение=ОтделениеПрогресс) тогда
		Если ТипЗнч(ЭтотОбъект.ФИЗлИЦО) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			Префикс = "П-";
		ИначеЕсли ТипЗнч(ЭтотОбъект.ФИЗлИЦО) = Тип("СправочникСсылка.Контрагенты") Тогда
			Префикс = "НР-";
		Иначе
			Отказ = Истина;
			Возврат
		КонецЕсли;
	Иначе
		ПрефиксОтделения = ПРГ_ДопФункцииКлиентСервер.ПолучитьРеквизитыОбъекта(ЭтотОбъект.НСИ_Отделение,"ПрефиксОтделения"); 
		Если ТипЗнч(ЭтотОбъект.ФИЗлИЦО) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			Префикс = СокрЛП(ПрефиксОтделения)+"-";
		ИначеЕсли ТипЗнч(ЭтотОбъект.ФИЗлИЦО) = Тип("СправочникСсылка.Контрагенты") Тогда
			Префикс = СокрЛП(ПрефиксОтделения)+"НР-";
		Иначе
			Отказ = Истина;
			Возврат
		КонецЕсли;
