...
Процедура ЗаполнитьДанными(МодельВыгрузки, Параметры)
	...
	ЗаписатьОбязательныйАтрибут(МодельВыгрузки, "ИдФайл", Параметры.ИдФайл);
	ЗаписатьОбязательныйАтрибут(МодельВыгрузки, "ВерсПрог", Параметры.ВерсПрог);
	ЗаписатьОбязательныйАтрибут(МодельВыгрузки, "ВерсФорм", "5.01");
	
	МодельВыгрузки.ЗаписатьНачалоЭлемента("Документ");
		
		ЗаписатьОбязательныйАтрибут(МодельВыгрузки, "КНД", "1115103");
...
Процедура ЗаполнитьДаннымиВсего(МодельВыгрузки, Параметры)
	...
	МодельВыгрузки.ЗаписатьНачалоЭлемента("СвКнПрод");
		ЗаписатьОбязательныйАтрибут(МодельВыгрузки, "Период", Параметры.Период);
		ЗаписатьОбязательныйАтрибут(МодельВыгрузки, "ОтчетГод", Параметры.ОтчГод);
		ЗаписатьОбязательныйАтрибут(МодельВыгрузки, "ВсегоВсПродВклНДС", ФорматДеньги(ВсегоПродаж));
		
		МодельВыгрузки.ЗаписатьНачалоЭлемента("Всего");
			МодельВыгрузки.ЗаписатьАтрибут("СтТовУчНалВсего", ФорматДеньги(ВсегоПродаж));
...
Процедура ЗаполнитьДаннымиТабличнойЧасти(МодельВыгрузки, Параметры)
	...
	Для Каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
		...
		ЗаписатьОбязательныйАтрибут(МодельВыгрузки, "ДатаСчФ", СокрЛП(РеквизитыСчетаФактуры[0]));
		ЗаписатьОбязательныйАтрибут(МодельВыгрузки, "НомерСчФ", СокрЛП(РеквизитыСчетаФактуры[1]));
		...
		ЗаписатьНеобязательныйАтрибут(МодельВыгрузки, "НомИспрСчФ", СокрЛП(РеквизитыИсправленияСчетаФактуры[0]));
		ЗаписатьНеобязательныйАтрибут(МодельВыгрузки, "ДатаИспрСчФ", СокрЛП(РеквизитыИсправленияСчетаФактуры[1]));
		...
		ЗаписатьНеобязательныйАтрибут(МодельВыгрузки, "НомерКСчФ", СокрЛП(РеквизитыКорректировочногоСчетаФактуры[0]));
		ЗаписатьНеобязательныйАтрибут(МодельВыгрузки, "ДатаКСчФ", СокрЛП(РеквизитыКорректировочногоСчетаФактуры[1]));
		...
		ЗаписатьНеобязательныйАтрибут(МодельВыгрузки, "НомИспрКСчФ", СокрЛП(РеквизитыИсправленияКорректировочногоСчетаФактуры[0]));
		ЗаписатьНеобязательныйАтрибут(МодельВыгрузки, "ДатаИспрКСчФ", СокрЛП(РеквизитыИсправленияКорректировочногоСчетаФактуры[1]));
		
		ЗаписатьОбязательныйАтрибут(МодельВыгрузки, "НаимПок", СокрЛП(СтрокаТабличнойЧасти.Покупатель));
		ИНН = СтрокаТабличнойЧасти.ПокупательИНН;
		Если СтрДлина(ИНН) = 12 Тогда
			ЗаписатьНеобязательныйАтрибут(МодельВыгрузки, "ИННФЛ", ИНН);
		ИНаче
			ЗаписатьНеобязательныйАтрибут(МодельВыгрузки, "ИННЮЛ", ИНН);
			ЗаписатьНеобязательныйАтрибут(МодельВыгрузки, "КПП", СтрокаТабличнойЧасти.ПокупательКПП);
		КонецЕсли;
		
		ЗаписатьОбязательныйАтрибут(МодельВыгрузки, "СтТовУчНалВсего", ФорматДеньги(СтрокаТабличнойЧасти.ВсегоПродаж));
