...
Функция ПечатьКомплектацияНоменклатуры()
	...
	|	ЕдиницаИзмерения,
	|	Количество,
	|	СП_КоличествоФакт КАК КоличествоФакт, //m.ionov@a-prof.ru 12.11.2014
	|	Ответственный,
	|	Организация
	|ИЗ
	...
	|	Номенклатура,
	|	Количество,
	|	Ссылка.СП_КоличествоФакт КАК КоличествоФакт, //m.ionov@a-prof.ru 12.11.2014
	|	ЕдиницаИзмерения,
	|	НомерСтроки
	|
	|ИЗ
	...
	Если ДоступенСтатусДокумента() Тогда
		Макет        = ПолучитьМакет("СП_Комплектация");
	Иначе
		Макет        = ПолучитьМакет("Комплектация");
	КонецЕсли;
	//----m.ionov@a-prof.ru---
	...
	//m.ionov@a-prof.ru 12.11.2014
	Если ДоступенСтатусДокумента() Тогда
		ОбластьМакета.Параметры.КоличествоНаПечатьФакт = "" + Шапка.КоличествоФакт + " ("+Шапка.ЕдиницаИзмерения+")";
	КонецЕсли;
	//----m.ionov@a-prof.ru---
	ТабДокумент.Вывести(ОбластьМакета);
...
Функция ПодготовитьТаблицуКомплектов(РезультатЗапросаПоКомплектующим, СтруктураШапкиДокумента)
	...
	Если СП_СтатусДокумента = Перечисления.СП_СтатусыКомплектацииНоменклатуры.Закрыто Тогда
		НоваяСтрока.Количество                   = СтруктураШапкиДокумента.СП_КоличествоФакт * СтруктураШапкиДокумента.Коэффициент
											   / СтруктураШапкиДокумента.КоэффициентЕдиницыХранения;
	Иначе
		НоваяСтрока.Количество                   = СтруктураШапкиДокумента.Количество * СтруктураШапкиДокумента.Коэффициент
										   		/ СтруктураШапкиДокумента.КоэффициентЕдиницыХранения;
	...
	НоваяСтрока.Качество					 = ПРГ_Качество;
...
Процедура ПроверитьЗаполнениеТабличнойЧастиКомплектующих(ТаблицаПоКомплектующим, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ДоступенСтатусДокумента() Тогда
		Если Комплектующие.Количество() > 1 Тогда	
			ОбщегоНазначения.СообщитьОбОшибке("Не верно заполнена табличная часть ""Комплектующие"". Должна быть только одна строчка.", Отказ, Заголовок);	
		ИначеЕсли Комплектующие.Количество() = 1 Тогда
			Если Не Комплектующие[0].Количество = Количество Тогда
				ОбщегоНазначения.СообщитьОбОшибке("Не верно заполнена табличная часть ""Комплектующие"". Количество должно быть такое же как в комплекте.", Отказ, Заголовок);	
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	//----m.ionov@a-prof.ru---
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок);
	...
	Если ПРГ_ДопФункцииКлиентСервер.ПРГПроводитьПоПартиям(СтруктураШапкиДокумента) Тогда
		УправлениеЗапасамиПартионныйУчет.ДвижениеПартийТоваров(Ссылка,Движения.СписанныеТовары.Выгрузить(),,,,,Ложь);
	КонецЕслИ;	
...
Процедура СформироватьДвиженияКомплектующиеПриход_ТоварыНаСкладах(РежимПроведения, СтруктураШапкиДокумента,ТаблицаПоКомплектующим, Отказ, Заголовок);
	...
	ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Склад",    Склад);
	//начало изменений
	//Если СтруктураШапкиДокумента.ВидКомплектации  = Перечисления.ВидыОперацийКомплектацияНоменклатуры.
	//ОбщегоНазначения.УстановитьЗначениеВТаблицыДокумента(ТаблицыДанныхДокумента, "Качество", Справочники.Качество.Новый);
	//конец изменений 
...
Процедура ДвиженияПоСписаннымТоварам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоКомплектующим, ТаблицаПоКомплектам, Отказ, Заголовок)
	...
	РаспределятьПоКоличеству = СтруктураШапкиДокумента.Дата >= '20140401';
	...
	Если ВидКомплектации = Перечисления.ВидыКомплектации.Сборка Тогда
		...
		//начало изменений
		ТаблицаПоТоварам.Колонки.Добавить("КачествоНовое");
		//конец изменений 
		...
		Если РаспределятьПоКоличеству Тогда
			ПРГ_Количество =  СтрокаКомплекта.Количество;
			ВсегоКолво 	   = ТаблицаПоТоварам.Итог("Количество");
			
			Для каждого Строка Из ТаблицаПоТоварам Цикл

				//начало изменений  
				Строка.КачествоНовое 	= ПРГ_Качество;
				//конец изменений
				...
				Если ТаблицаПоТоварам.Индекс(Строка) = ТаблицаПоТоварам.Количество() - 1 Тогда
					Строка.КоличествоПоступление = ПРГ_Количество;
				Иначе	
					Строка.КоличествоПоступление = Окр(Строка.Количество / ВсегоКолво * СтрокаКомплекта.Количество,3);
					ПРГ_Количество = ПРГ_Количество-Строка.КоличествоПоступление;
				КонецЕсли;
	...
	ИначеЕсли ВидКомплектации = Перечисления.ВидыКомплектации.Разборка Тогда
		...
		//начало изменений 
		ТаблицаПоТоварам.Колонки.Качество.Имя					= "КачествоНовое";
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если СП_СтатусДокумента = Перечисления.СП_СтатусыКомплектацииНоменклатуры.Закрыто Тогда
		СтруктураПолей.Вставить("Количество"                   , "Ссылка.СП_КоличествоФакт * Коэффициент /Номенклатура.ЕдиницаХраненияОстатков.Коэффициент");
	Иначе
		СтруктураПолей.Вставить("Количество"                   , "Количество * Коэффициент /Номенклатура.ЕдиницаХраненияОстатков.Коэффициент");
	КонецЕсли;
	//----m.ionov@a-prof.ru---
	...
	//m.ionov@a-prof.ru 12.11.2014
	//Если НЕ Отказ Тогда
	Если НЕ Отказ И Не СП_СтатусДокумента = Перечисления.СП_СтатусыКомплектацииНоменклатуры.Отменено Тогда
	//----m.ionov@a-prof.ru---
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ЗначениеЗаполнено(ПРГ_Качество) Тогда
		ПРГ_Качество = ТекКачество;
	КонецЕсли;	
	Для Каждого ТекСтрока Из Комплектующие Цикл
		Если Не ЗначениеЗаполнено(ТекСтрока.Качество) Тогда
			ТекСтрока.Качество = ТекКачество;
		КонецЕсли;	
	КонецЦикла;	
	//конец изменений 
	...
	Если Не ДоступенСтатусДокумента() Тогда
		СП_СтатусДокумента = Перечисления.СП_СтатусыКомплектацииНоменклатуры.ПустаяСсылка();
	КонецЕсли;        
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
		...
		КонецЕсли;
	//m.ionov@a-prof.ru 12.11.2014
	//Добавили ввод на основании Перемещения товаров
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
		...
		НоваяСтрока.ЕдиницаИзмерения = Основание.Товары[0].ЕдиницаИзмерения;
		НоваяСтрока.Коэффициент = Основание.Товары[0].Коэффициент;
		НоваяСтрока.Количество = Основание.Товары[0].Количество;
		НоваяСтрока.ХарактеристикаНоменклатуры = Основание.Товары[0].ХарактеристикаНоменклатуры;
		
		Если НоваяСтрока.Номенклатура <> НоваяСтрока.ЕдиницаИзмерения.Владелец Тогда
			НоваяСтрока.ЕдиницаИзмерения = НоваяСтрока.Номенклатура.ЕдиницаХраненияОстатков;
			НоваяСтрока.Коэффициент      = НоваяСтрока.ЕдиницаИзмерения.Коэффициент;
		КонецЕсли; 
		
		Если НоваяСтрока.Номенклатура <> НоваяСтрока.СерияНоменклатуры.Владелец Тогда
			НоваяСтрока.СерияНоменклатуры = Неопределено;
		КонецЕсли; 
		
		Если НоваяСтрока.Номенклатура <> НоваяСтрока.ХарактеристикаНоменклатуры.Владелец Тогда
			НоваяСтрока.ХарактеристикаНоменклатуры = Неопределено;
		КонецЕсли; 
		
		ОбработкаТабличныхЧастей.ЗаполнитьСпособСписанияОстаткаТоваровТабЧасти(НоваяСтрока, ЭтотОбъект);

		УправлениеЗатратами.ЗаполнитьНоменклатурнуюГруппуИСтатьюЗатратВСтрокеТабличногоПоля(НоваяСтрока,, Ложь);
		ЗаполнитьСчетаУчетаВСтрокеТабЧастиРегл(НоваяСтрока, "Комплектующие", ОтражатьВБухгалтерскомУчете, ОтражатьВНалоговомУчете);
...
Функция МожноИнтрактивноМенятьДокумент() Экспорт
	...
	Если МожноМенять И Не ЭтоНовый() Тогда
		Если ДоступенСтатусДокумента()
			И Не СП_СтатусДокумента = Перечисления.СП_СтатусыКомплектацииНоменклатуры.Открыто 
			И Не СП_СтатусДокумента = Перечисления.СП_СтатусыКомплектацииНоменклатуры.Отменено
...
Функция ДоступенСтатусДокумента() Экспорт
	...
	Если Не ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.КомплектацияНоменклатуры
		ИЛИ Не ВидКомплектации = Перечисления.ВидыКомплектации.Сборка Тогда
		Возврат Доступен;
	КонецЕсли;		
	
	Если Склад.НСИ_ПодключенКSolvo Тогда//ИЛИ (ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПеремещениеТоваров") И ЗначениеЗаполнено(ДокументОснование)) Тогда
		Доступен = Истина;
	КонецЕсли;
...
Функция МожноМенятьСоставСтрок() Экспорт
	...
	Если Не Проведен Тогда
		Возврат Истина;
	ИначеЕсли Не СП_СтатусДокумента = Перечисления.СП_СтатусыКомплектацииНоменклатуры.Открыто 
		И Не СП_СтатусДокумента = Перечисления.СП_СтатусыКомплектацииНоменклатуры.Отменено Тогда
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;	
