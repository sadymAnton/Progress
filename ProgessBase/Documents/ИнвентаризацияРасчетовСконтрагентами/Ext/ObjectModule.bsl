...
Процедура ЗаполнитьАктДляИнв17(ВидЗадолженности = Неопределено) Экспорт
	...
	|	ХозрасчетныйОстатки.Субконто2 КАК ПРГ_ДоговорКонтрагента,
	...
	/// Кунов О.В., 19.02.2015 - 36733
	СчетКонтрагент = Новый Соответствие;
	Для Каждого СчетаРасчетовСтрока Из СчетаРасчетов Цикл
		СчетКонтрагент.Вставить(СчетаРасчетовСтрока.СчетРасчетов, СчетаРасчетовСтрока.Контрагент);
	КонецЦикла;
	
	Пока (Выборка.Следующий()) Цикл
		
		КонтрагентПоСчету = СчетКонтрагент[Выборка.СчетРасчетов];
		Если Не ЗначениеЗаполнено(КонтрагентПоСчету) Тогда
			Условие = Истина;
		ИначеЕсли КонтрагентПоСчету.ЭтоГруппа Тогда
			Условие = Выборка.Контрагент.ПринадлежитЭлементу(КонтрагентПоСчету);
		Иначе
			Условие = (Выборка.Контрагент = КонтрагентПоСчету);
		КонецЕсли;
...
Функция ПечатьИнв17() Экспорт
	...
	|	ЕСТЬNULL(Инвентаризация.ПРГ_ДоговорКонтрагента.Наименование,"""") КАК ДоговорНаименование,
	...
	|	Инвентаризация.ПРГ_ДоговорКонтрагента.Наименование
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Если ВыборкаПоВидуЗадолженности.НайтиСледующий(Перечисления.ВидыЗадолженности.Дебиторская, "ВидЗадолженности") Тогда
		...
		Пока ВыборкаПоКонтрагенту.Следующий() Цикл
			...
			/// Кунов О.В., 19.02.2015 - 36733
			КонтрагентПредставление = ?(ПустаяСтрока(ВыборкаПоКонтрагенту.КонтрагентНаименованиеПолное),
				ВыборкаПоКонтрагенту.КонтрагентНаименование, ВыборкаПоКонтрагенту.КонтрагентНаименованиеПолное);
			КонтрагентПредставление = КонтрагентПредставление + Символы.ПС + ВыборкаПоКонтрагенту.ДоговорНаименование;
			СтрокаТаблицыДт.Параметры.КонтрагентПредставление = КонтрагентПредставление;
			///
	...
	Если ВыборкаПоВидуЗадолженности.НайтиСледующий(Перечисления.ВидыЗадолженности.Кредиторская, "ВидЗадолженности") Тогда
		...
		Пока ВыборкаПоКонтрагенту.Следующий() Цикл
			...
			КонтрагентПредставление = ?(ПустаяСтрока(ВыборкаПоКонтрагенту.КонтрагентНаименованиеПолное),
				ВыборкаПоКонтрагенту.КонтрагентНаименование, ВыборкаПоКонтрагенту.КонтрагентНаименованиеПолное);
			КонтрагентПредставление = КонтрагентПредставление + Символы.ПС + ВыборкаПоКонтрагенту.ДоговорНаименование;
			СтрокаТаблицыКт.Параметры.КонтрагентПредставление = КонтрагентПредставление;
			///
