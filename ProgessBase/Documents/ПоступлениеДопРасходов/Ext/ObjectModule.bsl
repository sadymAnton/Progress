...
Процедура ЗаполнитьТоварыПоПоступлениюТоваров(ДокументОснование, ТабЧасть) Экспорт
	...
	|	" + ?(ИмяДокумента = "ПоступлениеТоваровУслуг" И ТабЧасть = Товары, "Док.ЗаказПоставщику КАК ЗаказПоставщику,",      "") + "
	...
	РезультатЗапроса = Запрос.Выполнить();
  	//начало изменений 
	ТаблОстаков = Неопределено;
	Если ТабЧасть = Товары Тогда
		ТаблОстаков = ПолучитьОстаткиТоваровПоКачеству(ДокументОснование,?(ЭтоНовый(),КонецДня(Дата),Дата)); 
	КонецЕсли;	
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ИмяДокумента = "ПоступлениеТоваровУслуг"
		   И ТабЧасть = Товары Тогда
			...
			//начало изменений БП 04 
			СтрокаТабличнойЧасти.Сделка 		 = Выборка.ЗаказПоставщику;
			//конец изменений БП 04 
			//начало изменений пока без разбивки по количеству
			НайдЗнч = ТаблОстаков.Найти(СтрокаТабличнойЧасти.Номенклатура,"Номенклатура"); 
			Если НайдЗнч <> Неопределено Тогда
				СтрокаТабличнойЧасти.Качество = НайдЗнч.Качество;
			Иначе
...
Процедура ДобавитьРаспределение( ТаблицаПоТоварам, ТаблицаПоОборудованию, СтруктураШапкиДокумента)
	...
	Если ТаблицаПоТоварам.Колонки.Найти("СуммаВзаиморасчетовБезНДСРаспределения") = Неопределено тогда
		ТаблицаПоТоварам.Колонки.Добавить("СуммаВзаиморасчетовБезНДСРаспределения", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	КонецЕсли;
	//----m.ionov@a-prof.ru---
	...
	КонецЕсли;
	//m.ionov@a-prof.ru 24.02.2014
	Если ТаблицаПоОборудованию.Колонки.Найти("СуммаВзаиморасчетовБезНДСРаспределения") = Неопределено тогда
		ТаблицаПоОборудованию.Колонки.Добавить("СуммаВзаиморасчетовБезНДСРаспределения", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	КонецЕсли;
	//----m.ionov@a-prof.ru---
	...
	//m.ionov@a-prof.ru 24.02.2014
	СуммаРасчетовБезНДС = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(
		СтруктураШапкиДокумента.Сумма - ?(СтруктураШапкиДокумента.СуммаВключаетНДС,СтруктураШапкиДокумента.СуммаНДС, 0),
		СтруктураШапкиДокумента.ВалютаДокумента, СтруктураШапкиДокумента.ВалютаВзаиморасчетов,
		ЗаполнениеДокументов.КурсДокумента(ЭтотОбъект, мВалютаРегламентированногоУчета), СтруктураШапкиДокумента.КурсВзаиморасчетов,
		ЗаполнениеДокументов.КратностьДокумента( ЭтотОбъект, мВалютаРегламентированногоУчета), СтруктураШапкиДокумента.КратностьВзаиморасчетов);
	//----m.ionov@a-prof.ru---
	...
	МассивРасчетыБезНДС  = ОбщегоНазначения.РаспределитьПропорционально( СуммаРасчетовБезНДС, МассивКоэф, 2); //m.ionov@a-prof.ru 24.02.2014
	...
	Для К = 0 По СтрокТоваров Цикл
		...
		КонецЕсли;
		//m.ionov@a-prof.ru 24.02.2014
		Если Не МассивРасчетыБезНДС = Неопределено Тогда
			ТаблицаПоТоварам[К].СуммаВзаиморасчетовБезНДСРаспределения = ТаблицаПоТоварам[К].СуммаВзаиморасчетовБезНДСРаспределения + МассивРасчетыБезНДС[К];
		КонецЕсли;
		//----m.ionov@a-prof.ru---
		Если Не МассивСумм = Неопределено Тогда
	...
	Для К = 0 По ТаблицаПоОборудованию.Количество() - 1 Цикл
		...
		//m.ionov@a-prof.ru 24.02.2014
		Если Не МассивРасчетыБезНДС = Неопределено Тогда
			ТаблицаПоОборудованию[К].СуммаВзаиморасчетовБезНДСРаспределения = ТаблицаПоОборудованию[К].СуммаВзаиморасчетовБезНДСРаспределения + МассивРасчетыБезНДС[К + СтрокТоваров + 1];
		КонецЕсли;
		//----m.ionov@a-prof.ru---
		Если Не МассивСумм = Неопределено Тогда
...
Процедура ДвиженияПоРегистрамУпр(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоОборудованию, ТаблицаПоВзаиморасчетам, ТаблицаПоРасчетам, Отказ, Заголовок, РежимПроведения)
	...
	Если ВидОперации <> Перечисления.ВидыОперацийПоступлениеДопРасходов.ВнутреннийРасход Тогда
		...
		Для Каждого СтрокаЗакупки Из ТаблицаПоЗакупкам Цикл
			...
			СтрокаЗакупки.СуммаВзаиморасчетов = СтрокаЗакупки.СуммаВзаиморасчетовРаспределения + СтрокаЗакупки.СуммаВзаиморасчетов; // Шевченков №54944
		...
		// <- Шевченков №54944
		ТаблицаПоЗакупкам.Колонки.СуммаВзаиморасчетов.Имя = "ПРГ_СтоимостьВал";
		ТаблицаПоЗакупкам.Колонки.НДСВал.Имя = "ПРГ_НДСВал";
		Если ТаблицаПоЗакупкам.Колонки.Найти("ПРГ_ВалютаДокумента") = Неопределено Тогда
			ТаблицаПоЗакупкам.Колонки.Добавить("ПРГ_ВалютаДокумента", новый описаниетипов("СправочникСсылка.Валюты"));			
		КонецЕсли;
		// ->
...
Процедура ДвиженияПоРегистрамБухгалтерииРегл(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоОборудованию, ТаблицаПоДокументамРасчетовСКонтрагентом, Отказ, Заголовок, РежимПроведения)
	...
	Если (СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете) 
	   И (ВидОперации = Перечисления.ВидыОперацийПоступлениеДопРасходов.УслугаСтороннейОрганизации) Тогда
		...
		//начало изменений БП 04
		Если ОбщаяТаблицаДокумента.Колонки.Найти("Сделка") = Неопределено  Тогда
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоОборудованию, Отказ = Ложь, Заголовок = "") Экспорт
	...
	СтруктураПолей.Удалить("УЗ_СуммаРегл");
	СтруктураПолей.Удалить("УЗ_НДСРегл");
	СтруктураПолей.Удалить("УЗ_СуммаВал");
	СтруктураПолей.Удалить("УЗ_НДСВал");
	...
	Если УЗ_ПоказыватьСуммыВРублях Тогда
		ДобавитьРаспределениеРуб( ТаблицаПоТоварам, ТаблицаПоОборудованию, СтруктураШапкиДокумента);
	Иначе
	...
	БухгалтерскийУчетРасчетовСКонтрагентами.ПодготовкаТаблицыЗначенийДляЦелейПриобретенияИРеализации(ТаблицаПоТоварам,      СтруктураШапкиДокумента,СтруктураШапкиДокумента.НДСВключенВСтоимость,мВалютаРегламентированногоУчета, УЗ_ПоказыватьСуммыВРублях); //m.ionov@a-prof.ru 16.02.2014
	//m.ionov@a-prof.ru 16.02.2014
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//конец изменений БП 04 
	//начало изменений
	НовКачество = Справочники.Качество.Новый;
	Для каждого ТекСтрока  Из  Товары Цикл
		Если Не ЗначениеЗаполнено(ТекСтрока.Качество) Тогда
			ТекСтрока.Качество = НовКачество;
	    КонецЕсли;
	КонецЦикла; 
	//конец изменений 
	//m.ionov@a-prof.ru 24.02.2014
...
Процедура ПересчитатьСуммуРуб(СтрокаТабличнойЧасти) Экспорт
	...
	Если СтрокаТабличнойЧасти = Неопределено Тогда
		УЗ_СуммаРегл = Сумма*КоэффициентПересчета;
		УЗ_СуммаНДСРегл = СуммаНДС*КоэффициентПересчета;
	Иначе
		
		Если Товары.Индекс(СтрокаТабличнойЧасти) >= 0  Тогда
...
Процедура ПересчитатьТолькоСуммуРуб(СтрокаТабличнойЧасти) Экспорт
	...
	Если СтрокаТабличнойЧасти = Неопределено Тогда
		ИтогоСумма = (Сумма + СуммаНДС)*КоэффициентПересчета;
		
		УЗ_СуммаРегл = ИтогоСумма - УЗ_СуммаНДСРегл; 
	Иначе
		Если Товары.Индекс(СтрокаТабличнойЧасти) >= 0  Тогда
...
// Процедура добавляет в таблицы данные о суммах распределения.
//
Процедура ДобавитьРаспределениеРУБ(ТаблицаПоТоварам, ТаблицаПоОборудованию, СтруктураШапкиДокумента)
	...
	// Сумма к распределению
	Если СтруктураШапкиДокумента.НеВключатьНДСВСтоимостьПартий И НЕ НДСВключенВСтоимость Тогда
		СуммаВсего = УЗ_СуммаРегл - ?(УчитыватьНДС И СуммаВключаетНДС, УЗ_СуммаНДСРегл, 0);
	Иначе
		//СуммаВсего = Сумма - ?(УчитыватьНДС И НЕ СуммаВключаетНДС, 0, СуммаНДС);
		СуммаВсего = УЗ_СуммаРегл - ?(УчитыватьНДС И СуммаВключаетНДС, 0, УЗ_СуммаНДСРегл);
	КонецЕсли;
	
	СуммаВсегоСНДС = УЗ_СуммаРегл + ?(УчитыватьНДС И НЕ СуммаВключаетНДС, УЗ_СуммаНДСРегл, 0);
	...
	СуммаВсегоБезНДСВал = Сумма - ?(УчитыватьНДС И НЕ СуммаВключаетНДС, 0, СуммаНДС);
	СуммаВсегоВал = Сумма;
	СуммаВсегоНДСВал = СуммаНДС;
	...
	МассивСуммБезНДСВал = ОбщегоНазначения.РаспределитьПропорционально( СуммаВсегоБезНДСВал, МассивКоэф, 2);
	МассивСуммВал = ОбщегоНазначения.РаспределитьПропорционально( СуммаВсегоВал, МассивКоэф, 2);
	МассивНДСВал = ОбщегоНазначения.РаспределитьПропорционально( СуммаВсегоНДСВал, МассивКоэф, 2, ЛОЖЬ);
	...
	Если Не МассивСумм = Неопределено Тогда
		...
		Для К = 0 По СтрокТоваров Цикл
			...
			ТаблицаПоТоварам[К].УЗ_СуммаВал   = ТаблицаПоТоварам[К].УЗ_СуммаВал   + МассивСуммВал[К];
			ТаблицаПоТоварам[К].УЗ_СуммаБезНДСВал   = ТаблицаПоТоварам[К].УЗ_СуммаБезНДСВал   + МассивСуммБезНДСВал[К];
			ТаблицаПоТоварам[К].УЗ_НДСВал   = ТаблицаПоТоварам[К].УЗ_НДСВал   + МассивНДСВал[К];
		КонецЦикла;
		Для К = 0 По ТаблицаПоОборудованию.Количество() - 1 Цикл
			ТаблицаПоОборудованию[К].Стоимость              = ТаблицаПоОборудованию[К].Стоимость + ?(СтруктураШапкиДокумента.НеВключатьНДСВСтоимостьПартий И НЕ НДСВключенВСтоимость,МассивСумм[К + СтрокТоваров + 1],МассивСуммСНДС[К + СтрокТоваров + 1]);
			ТаблицаПоОборудованию[К].СуммаУпрРаспределения  = ТаблицаПоОборудованию[К].СуммаУпрРаспределения + МассивСуммСНДС[К + СтрокТоваров + 1];
	...
	//СуммаРегл нужна не только для БУ, но и для взаиморасчетов по документам расчетов
	СуммаРегл = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(
		УЗ_СуммаРегл - ?(СуммаВключаетНДС, УЗ_СуммаНДСРегл, 0),
	...
	МассивСуммРегл = ОбщегоНазначения.РаспределитьПропорционально( СуммаРегл,     МассивКоэф, 2);
	МассивНДСРегл  = ОбщегоНазначения.РаспределитьПропорционально( СуммаНДСРегл,  МассивКоэф, 2);
	МассивСумм     = ОбщегоНазначения.РаспределитьПропорционально( УЗ_СуммаРегл,     МассивКоэф, 2);
	МассивНДС      = ОбщегоНазначения.РаспределитьПропорционально( УЗ_СуммаНДСРегл,  МассивКоэф, 2);
	МассивРасчеты  = ОбщегоНазначения.РаспределитьПропорционально( СуммаРасчетов, МассивКоэф, 2);
	МассивРасчетыБезНДС  = ОбщегоНазначения.РаспределитьПропорционально( СуммаРасчетовБезНДС, МассивКоэф, 2);
...
//----m.ionov@a-prof.ru---
//начало изменений
Функция ПолучитьОстаткиТоваровПоКачеству(ВыбОснование,Период)
	
	ПодчДокументы = ПолныеПрава.ПолучитьВыборкуПоКритериюОтбора("СвязанныеДокументы", ВыбОснование);	
	//ПодчДокументы.ВыбратьСтроку();
	ТаблДоков = Новый ТаблицаЗначений;
	ТаблДоков.Колонки.Добавить("Регистратор",Документы.ТипВсеСсылки());
	
	НоваяСтрока = ТаблДоков.Добавить();
	НоваяСтрока.Регистратор = ВыбОснование;
	
	Для Каждого ТекСтрока Из ПодчДокументы Цикл
		Если ЗначениеЗаполнено(ТекСтрока.Ссылка) Тогда
			 НоваяСтрока = ТаблДоков.Добавить();
			 НоваяСтрока.Регистратор = ТекСтрока.Ссылка;
		КонецЕсли;	
	КонецЦикла;	
	
	//ТаблДоков.ВыбратьСтроку();
	
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	ТаблДоков.Регистратор
	                      |ПОМЕСТИТЬ ТаблДоков
	                      |ИЗ
	                      ...
	                      |		ПО ТаблДоков.Регистратор = ТоварыНаСкладах.Регистратор
	                      |ГДЕ
	...
	Запрос.УстановитьПараметр("ТаблДоков",ТаблДоков);
	Запрос.УстановитьПараметр("Период",Период);
	
	ТаблОст = Запрос.Выполнить().Выгрузить();
	ТаблОст.Индексы.Добавить("Номенклатура");
...
мСтруктураПараметровВзаиморасчетов.Вставить("ИмяЗаказаВТабличныхЧастях","Сделка");
//начало изменений БП 