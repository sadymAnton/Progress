...
Процедура ЗаполнитьСписокРеквизитовЗависимыхОтТиповУчетаРегл()
	...
	мСтрокаРеквизитыБухУчета = "Затраты.ПодразделениеОрганизации, НадписьПодразделениеОрганизации,
...
Процедура ПроверкаРеквизитов(Отказ, Заголовок, СтруктураШапкиДокумента) Экспорт
	...
	/////Вадим 16.10.2013 10:46:52
	//УправлениеЗатратами.ПроверитьПодразделениеОрганизации(ЭтотОбъект, Отказ, Заголовок); 
	Если ОтражатьВБухгалтерскомУчете Тогда
		НайденныеСтроки =Затраты.НайтиСтроки(новый Структура("ПодразделениеОрганизации",Справочники.ПодразделенияОрганизаций.ПустаяСсылка()));
		для стр=0 по НайденныеСтроки.Количество()-1 цикл
			сообщить("В строке "+СокрЛП(НайденныеСтроки[стр].НомерСтроки)+" не выбранно подразделение!");				
			Отказ=Истина;
		КонецЦикла;	
...
Процедура ДополнитьРеквизитыТабличнойЧастиРегл(Реквизиты, СтруктураШапкиДокумента)
	...
	Если СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете Тогда
		Реквизиты = Реквизиты + ?(ПустаяСтрока(Реквизиты),"",", ") + "СчетЗатрат,Счет";
	КонецЕсли;
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
		Реквизиты = Реквизиты + ", СчетЗатратНУ,СчетНУ";
	КонецЕсли;
	...
	Для Каждого СтрокаТабличнойЧасти Из Затраты Цикл
		...
		Если ОтражатьВБухгалтерскомУчете Тогда
			...
			Если ВидОперации = Перечисления.ВидыОперацийПрочиеЗатраты.Отражение Тогда
				...
				УправлениеЗатратами.ЗаполнитьСчетИСубконтоУчетаЗатрат(НоваяПроводка, СтрокаТабличнойЧасти, СтруктураШапкиДокумента,,,истина);
				//ВадимКонец
				НоваяПроводка.СчетКт = СтрокаТабличнойЧасти.Счет;
				БухгалтерскийУчет.УстановитьСубконто(СтрокаТабличнойЧасти.Счет, НоваяПроводка.СубконтоКт, 1, СтрокаТабличнойЧасти.СчетСубконто1);
				БухгалтерскийУчет.УстановитьСубконто(СтрокаТабличнойЧасти.Счет, НоваяПроводка.СубконтоКт, 2, СтрокаТабличнойЧасти.СчетСубконто2);
				БухгалтерскийУчет.УстановитьСубконто(СтрокаТабличнойЧасти.Счет, НоваяПроводка.СубконтоКт, 3, СтрокаТабличнойЧасти.СчетСубконто3);

			Иначе
				НоваяПроводка.СчетДт = СтрокаТабличнойЧасти.Счет;
				БухгалтерскийУчет.УстановитьСубконто(СтрокаТабличнойЧасти.Счет, НоваяПроводка.СубконтоДт, 1, СтрокаТабличнойЧасти.СчетСубконто1);
				БухгалтерскийУчет.УстановитьСубконто(СтрокаТабличнойЧасти.Счет, НоваяПроводка.СубконтоДт, 2, СтрокаТабличнойЧасти.СчетСубконто2);
				БухгалтерскийУчет.УстановитьСубконто(СтрокаТабличнойЧасти.Счет, НоваяПроводка.СубконтоДт, 3, СтрокаТабличнойЧасти.СчетСубконто3);
				//Вадим
				//УправлениеЗатратами.ЗаполнитьСчетИСубконтоУчетаЗатрат(НоваяПроводка, СтрокаТабличнойЧасти, СтруктураШапкиДокумента, , "Кт");
				УправлениеЗатратами.ЗаполнитьСчетИСубконтоУчетаЗатрат(НоваяПроводка, СтрокаТабличнойЧасти, СтруктураШапкиДокумента, , "Кт",истина);
				//ВадимКонец
			КонецЕсли;
		...
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчете Тогда
			...
			Если СтрокаТабличнойЧасти.СуммаНал <> 0 Тогда
				...
				Если ВидОперации = Перечисления.ВидыОперацийПрочиеЗатраты.Отражение Тогда
					...
					УправлениеЗатратами.ЗаполнитьСчетИСубконтоУчетаЗатрат(НоваяПроводкаНУ, СтрокаТабличнойЧасти, СтруктураШапкиДокумента, "Налоговый",,Истина);
					
					НоваяПроводкаНУ.СчетКт = СтрокаТабличнойЧасти.СчетНУ;
					БухгалтерскийУчет.УстановитьСубконто(СтрокаТабличнойЧасти.СчетНУ, НоваяПроводкаНУ.СубконтоКт, 1,	СтрокаТабличнойЧасти.СчетНУСубконтоНУ1);
					БухгалтерскийУчет.УстановитьСубконто(СтрокаТабличнойЧасти.СчетНУ, НоваяПроводкаНУ.СубконтоКт, 2, СтрокаТабличнойЧасти.СчетНУСубконтоНУ2);
					БухгалтерскийУчет.УстановитьСубконто(СтрокаТабличнойЧасти.СчетНУ, НоваяПроводкаНУ.СубконтоКт, 3, СтрокаТабличнойЧасти.СчетНУСубконтоНУ3);
					
				Иначе
					НоваяПроводкаНУ.СчетДт = СтрокаТабличнойЧасти.СчетНУ;
					БухгалтерскийУчет.УстановитьСубконто(СтрокаТабличнойЧасти.СчетНУ, НоваяПроводкаНУ.СубконтоДт, 1, СтрокаТабличнойЧасти.СчетНУСубконтоНУ1);
					БухгалтерскийУчет.УстановитьСубконто(СтрокаТабличнойЧасти.СчетНУ, НоваяПроводкаНУ.СубконтоДт, 2, СтрокаТабличнойЧасти.СчетНУСубконтоНУ2);
					БухгалтерскийУчет.УстановитьСубконто(СтрокаТабличнойЧасти.СчетНУ, НоваяПроводкаНУ.СубконтоДт, 3, СтрокаТабличнойЧасти.СчетНУСубконтоНУ3);
					
					УправлениеЗатратами.ЗаполнитьСчетИСубконтоУчетаЗатрат(НоваяПроводкаНУ, СтрокаТабличнойЧасти, СтруктураШапкиДокумента, "Налоговый", "Кт",Истина);
