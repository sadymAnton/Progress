	...
	ИначеЕсли ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию Тогда 
		...
		Если МассивДокументовРеализации.Количество() > 0 Тогда
			...
			Если НЕ ИспользуетсяРАУЗ Тогда
				...
				//начало изменений Ожиганов А. 05.07.2017 67830 минимизируем обращением к бух регистрам 
				...
				ПРГ_МассивРеализаций = Новый Массив;
				Пока Выборка.Следующий() Цикл
					ПРГ_МассивРеализаций.Добавить(Выборка.Ссылка);
				КонецЦикла;	
				
				Если ПРГ_МассивРеализаций.Количество() = 0 Тогда
					//поскольку данных по бу нет мы не можем определить есть комиссионные товары или нет
					//комиссии у нас нет можно впринципе вообще оставить один запрос
					Запрос.Текст = "ВЫБРАТЬ
						     	 |	0 КАК ЕстьКомиссионныеТоварыИУслуги,
						     	 |	1 КАК ЕстьСобственныеТоварыИУслуги
						     	 |ГДЕ
						     	 |	1 = 0
								 |ИТОГИ
								 ...
								 | ПО
						     	 |	ОБЩИЕ";
				Иначе	            
					Запрос.УстановитьПараметр("ДокументыОснования",ПРГ_МассивРеализаций);	
				//конец изменений 
					Запрос.Текст =
					"ВЫБРАТЬ
					|	ВЫБОР
	...
	Если ДокументОплатыМетаданные.ТабличныеЧасти.Найти("РасшифровкаПлатежа") <> Неопределено Тогда
		...
		Если Не Результат.Пустой() Тогда
			...
			Пока Выборка.Следующий() И Не ДокументЗаполнен Цикл
				...
				//Распределение суммы рег учета
				МассивСумм = ОбщегоНазначения.РаспределитьПропорционально(Выборка.СуммаОплаты, ТаблицаПоСтавкам.ВыгрузитьКолонку("Сумма"));
				Если не МассивСумм = Неопределено Тогда
					ТаблицаПоСтавкам.ЗагрузитьКолонку(МассивСумм,"Сумма");
				Иначе
					МассивСумм = ТаблицаПоСтавкам.ВыгрузитьКолонку("Сумма");
...
Процедура ЗаполнитьНаОснованииИсправления(Основание, СтрокаОснованиеВТабличнойЧасти)
	...
	Если ИспользоватьПараметрыСчетаФактуры И ЗначениеЗаполнено(ИсходныйДокумент) Тогда
		...
		ДанныеИсходногоДокумента = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(
			ИсходныйДокумент, 
			"КодВидаОперации,КодВидаОперацииНаУменьшение,Исправление,НомерИсправления,ВидСчетаФактуры,НомерИсходногоДокумента,ДатаИсходногоДокумента,Дата,
		...
		НомерИсправленияСФ = ДанныеИсходногоДокумента.НомерИсправления;
		
		Если ДанныеИсходногоДокумента.Исправление ИЛИ ДанныеИсходногоДокумента.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.Корректировочный Тогда
			ПараметрыСчетаФактуры.НомерИсходногоДокумента = ДанныеИсходногоДокумента.НомерИсходногоДокумента;
			ПараметрыСчетаФактуры.ДатаИсходногоДокумента = ДанныеИсходногоДокумента.ДатаИсходногоДокумента;
		Иначе
		...
		Если ДанныеИсходногоДокумента.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.Корректировочный Тогда
			ПараметрыСчетаФактуры.УчитыватьИсправлениеИсходногоДокумента = ДанныеИсходногоДокумента.УчитыватьИсправлениеИсходногоДокумента;
			Если ПараметрыСчетаФактуры.УчитыватьИсправлениеИсходногоДокумента Тогда
				ПараметрыСчетаФактуры.НомерИсправленияИсходногоДокумента = ДанныеИсходногоДокумента.НомерИсправленияИсходногоДокумента;
				ПараметрыСчетаФактуры.ДатаИсправленияИсходногоДокумента  = ДанныеИсходногоДокумента.ДатаИсправленияИсходногоДокумента;
			КонецЕсли;
			
			Если ДанныеИсходногоДокумента.Исправление Тогда
				НомерИсправляемогоКорректировочногоДокумента = ДанныеИсходногоДокумента.НомерИсправляемогоКорректировочногоДокумента;
				ДатаИсправляемогоКорректировочногоДокумента  = ДанныеИсходногоДокумента.ДатаИсправляемогоКорректировочногоДокумента;
			ИначеЕсли Исправление Тогда
				НомерИсправляемогоКорректировочногоДокумента = ОбщегоНазначения.ПолучитьНомерНаПечать(ИсходныйДокумент.ПолучитьОбъект());
				ДатаИсправляемогоКорректировочногоДокумента  = ДанныеИсходногоДокумента.Дата;
			КонецЕсли;
			
		Иначе
			Если ДанныеИсходногоДокумента.Исправление И ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.Корректировочный Тогда
				ПараметрыСчетаФактуры.УчитыватьИсправлениеИсходногоДокумента = Истина;
				ПараметрыСчетаФактуры.НомерИсправленияИсходногоДокумента = ДанныеИсходногоДокумента.НомерИсправления;
				ПараметрыСчетаФактуры.ДатаИсправленияИсходногоДокумента  = ДанныеИсходногоДокумента.Дата;
			КонецЕсли;
		КонецЕсли;
		...
		Если ЭтотОбъект.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.Корректировочный Тогда
			Если НЕ СтрокаОснованиеВТабличнойЧасти.УчитыватьИсправлениеИсходногоДокумента Тогда
				Если ЗначениеЗаполнено(ТекущаяСтрока.НомерИсправления) Тогда
					...
					/// Кунов О.В., 09.12.2015 - 46783
					ИсправляемыйСчетФактура = ТекущаяСтрока.Ссылка.ИсправляемыйСчетФактура;
					Если ЗначениеЗаполнено(ИсправляемыйСчетФактура) Тогда
						СтрокаОснованиеВТабличнойЧасти.НомерИсходногоДокумента = ОбщегоНазначения.ПолучитьНомерНаПечать(ИсправляемыйСчетФактура);
						СтрокаОснованиеВТабличнойЧасти.ДатаИсходногоДокумента = ИсправляемыйСчетФактура.Дата;
					КонецЕсли;
					///
				КонецЕсли;
			КонецЕсли;
...
Процедура ПроверитьЗаполнениеТабличнойЧастиПоДокументамОснованиям(ТаблицаПоДокументамОснованиям, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Для Каждого СтрокаТЧ Из ТаблицаПоДокументамОснованиям Цикл
		...
		Если ТипЗнч(СтрокаТЧ.ДокументОснование) = Тип("ДокументСсылка.ПринятиеКУчетуОС") Тогда
			...
			КонецЕсли;
		КонецЕсли;
		//начало изменений Ожиганов 03.08.2015 уберем излишние получение всего документа 
		...
		Если не ТипЗнч(СтрокаТЧ.ДокументОснование) = Тип("ДокументСсылка.ДокументРасчетовСКонтрагентом") и НЕ СтрокаТЧ.ДокументОснованиеПроведен Тогда
		//конец изменений 
...
Процедура ОбработкаЗаполнения(Основание)
	...
	/////Вадим 03.01.2014 15:07:37  бп 9
 	иначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ВозвратМатериаловИзЭксплуатации") Тогда
 		ЗаполнитьНаОснованииВозвратМатериаловИзЭксплуатации(Основание);
 	////ВадимКонец
	...
	ИначеЕсли ЗначениеЗаполнено(Основание) 
		...
		ИначеЕсли ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.Корректировочный Тогда
			...
			// <- Шевченков 20160212
			ВидОперацииОснования = ПРГ_ДопФункцииКлиентСервер.ПолучитьРеквизитыОбъекта(Основание,"ВидОперации");
			Если ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаРеализации") И ВидОперацииОснования = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки Тогда
				ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию;
			ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.КорректировкаРеализации") И ВидОперацииОснования = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение Тогда						
				ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.Корректировочный;
			КонецЕсли;
			// ->
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// <- Шевченков №41587 20150708
	Если НачалоДня(ДатаВыставления) <> НачалоДня(Дата) Тогда
		ДатаВыставления = НачалоДня(Дата);
	КонецЕсли;	
	// ->
...
///Вадим 03.01.2014 15:37:55   бп 9
Процедура ЗаполнитьНаОснованииВозвратМатериаловИзЭксплуатации(Основание)
	ВидСчетаФактуры=Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию;
	нов=ДокументыОснования.Добавить();
	нов.ДокументОснование=Основание;
	ОпределениеПараметровСчетаФактуры();
КонецПроцедуры
//ВадимКонец
