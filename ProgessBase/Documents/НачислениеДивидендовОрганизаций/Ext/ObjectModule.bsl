		...
		Если НЕ Отказ Тогда
			...
			Если НЕ Отказ И УпрощенныйУчетНДФЛ И Движения.НДФЛРасчетыСБюджетом.Количество() > 0 Тогда
				Если ДатаВыплатыДохода >= ПроведениеРасчетов.ДатаИзмененияСроковУплатыНалога() Тогда
					...
					//ПроведениеРасчетов.ВписатьСуммыВыплаченнойЗарплатыВУдержанныеНалоги(Движения.НДФЛРасчетыСБюджетом, Движения.ВзаиморасчетыСРаботникамиОрганизаций, ДатаВыплатыДохода);
				КонецЕсли;
