...
//  Заголовок         - строка, заголовок сообщения об ошибке проведения.
//
Процедура КонтрольОстатков_Барьер(ДокументОбъект, ИмяТабличнойЧасти, СтруктураШапкиДокумента, Отказ, Заголовок, РежимПроведения) Экспорт
	
	//Ожиганов контроль для всех режимов проведения
	Если //РежимПроведения <> РежимПроведенияДокумента.Оперативный ИЛИ
		 ДокументОбъект[ИмяТабличнойЧасти].Количество() = 0 Тогда
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПоступлениеТоваровУслуг") Тогда
		СформироватьВременнуюТаблицуДокумента_Поступление(МенеджерВременныхТаблиц,СтруктураПараметров);
	...
	|ВЫБРАТЬ // Запрос, контролирующий остатки на складах
	|	Док.ЗаказПоставщику 								   КАК ЗаказПоставщику,
	|	Док.ЗаказПоставщику.Дата 							   КАК ЗаказПоставщикуДата,
	|	Док.Номенклатура 									   КАК Номенклатура,
	|	Док.Номенклатура.Представление                         КАК НоменклатураПредставление,
	|	Док.Номенклатура.ЕдиницаХраненияОстатков.Представление КАК ЕдиницаХраненияОстатковПредставление,"
	+ ?(ЕстьХарактеристика, "
	...
	|	Док.Цена 											   КАК Цена,","")+"
	| 	СУММА(Док.ДокументКоличество)                          КАК ДокументКоличество,
	|	ЕСТЬNULL(МАКСИМУМ(Остатки.КоличествоКонечныйОстаток), 0)       КАК ОстатокКоличество,
	|	ЕСТЬNULL(МАКСИМУМ(Остатки.КоличествоПриход), 0)       КАК КоличествоЗаказано
	|ИЗ 
	|	ВременнаяТаблицаДокумента КАК Док
	|
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ЗаказыПоставщикам.ОстаткиИОбороты(,&ДатаКонца,,, 
	|		(ДоговорКонтрагента,ЗаказПоставщику,Номенклатура"+?(ЕстьХарактеристика,",ХарактеристикаНоменклатуры","")+") В (ВЫБРАТЬ ДоговорКонтрагента,ЗаказПоставщику,Номенклатура"+?(ЕстьХарактеристика,",ХарактеристикаНоменклатуры","")+" ИЗ ВременнаяТаблицаДокумента)
	|   И СтатусПартии = &СтатусПартии
	|	) КАК Остатки
	|ПО 
	...
	|	Док.Цена ", "")	+"
	|
	|ИМЕЮЩИЕ ЕСТЬNULL(МАКСИМУМ(Остатки.КоличествоКонечныйОстаток), 0) < СУММА(Док.ДокументКоличество)
	|ДЛЯ ИЗМЕНЕНИЯ РегистрНакопления.ЗаказыПоставщикам.ОстаткиИОбороты // Блокирующие чтение таблицы остатков регистра для разрешения коллизий многопользовательской работы
	|";
	
	 
	Запрос.УстановитьПараметр("ДатаКонца",Новый Граница(ДокументОбъект.МоментВремени(),ВидГраницы.Исключая));
	...
	Барьер = Константы.УЗ_БарьерПриПоступлении.Получить();
	Если Барьер = 0 Тогда
		Барьер = 10;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Выборка.ЗаказПоставщику) Тогда
			Продолжить;
		КонецЕсли;
		
		ДопПоБарьеру = Окр(Выборка.КоличествоЗаказано*Барьер/100,3); 
		
		//начало изменений
		Если ((Выборка.ОстатокКоличество + ДопПоБарьеру) < Выборка.ДокументКоличество) и (Выборка.ЗаказПоставщикуДата >= '20140201') Тогда
		//конец изменений	
			...
			УправлениеЗапасами.ОшибкаНетОстатка(СтрокаСообщения, (Выборка.ОстатокКоличество + ДопПоБарьеру), Выборка.ДокументКоличество,
			Выборка.ЕдиницаХраненияОстатковПредставление, Отказ, Заголовок);
...
КонецПроцедуры // КонтрольОстатков_Барьер()

Процедура СформироватьВременнуюТаблицуДокумента_Поступление(МенеджерВременныхТаблиц, СтруктураПараметров)
	ИмяТаблицы          = СтруктураПараметров.ИмяДокумента + "." + СокрЛП(СтруктураПараметров.ИмяТабличнойЧасти);
	МетаданныеТабЧастиДокумента = СтруктураПараметров.МетаданныеТабЧастиДокумента;
	...
	|ВЫБРАТЬ // Запрос, контролирующий остатки на складах
	|	ЗаказПоставщику 									   КАК ЗаказПоставщику,
	|	Док.Ссылка.ДоговорКонтрагента 								   КАК ДоговорКонтрагента,
	|	Док.Номенклатура 									   КАК Номенклатура,"
	+?(СтруктураПараметров.флКонтролироватьЦену,"
	|	Док.Цена 											   КАК Цена,","")
	+ ?(СтруктураПараметров.ЕстьХарактеристика, "
