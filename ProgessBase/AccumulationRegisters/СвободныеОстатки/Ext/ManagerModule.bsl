...
Процедура КонтрольОстатков(ИмяТабличнойЧасти, СтруктураШапкиДокумента, ОтказПроведения, ЗаголовокСообщения, РежимПроведения) Экспорт
	...
	Если РежимПроведения <> РежимПроведенияДокумента.Оперативный Тогда
		Запрос.УстановитьПараметр("Датаостатков",Новый Граница(Новый МоментВремени(СтруктураШапкиДокумента.Дата, СтруктураПараметров.ДокументСсылка),ВидГраницы.Включая));
	Иначе
...
Функция ПолучитьСтруктуруПараметров(СтруктураШапкиДокумента, ИмяТабличнойЧасти, ЗаголовокСообщения)
	...
	ИначеЕсли ИмяДокумента = "ПеремещениеТоваров" Тогда
		Если СтруктураШапкиДокумента.СкладыВТабличнойЧасти Тогда
	...
	ИначеЕсли ИмяДокумента = "ВнутреннийЗаказ" 
		...
		ДопУсловияТабличнойЧасти = ДопУсловияТабличнойЧасти + Символы.ПС + 
			" И (ВЫРАЗИТЬ(Док.Размещение КАК Справочник.Склады) ССЫЛКА Справочник.Склады И Док.Размещение <> ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))";
	ИначеЕсли ИмяДокумента = "ТребованиеНакладная"
...
Функция СформироватьВременнуюТаблицуПоТаблицеДокумента(МенеджерВременныхТаблиц, СтруктураПараметров)
	...
	ТекстЗапроса = 
		...
		//начало изменений БП 14
		+?(НеУчитыватьВоду,"
		| И Док.Номенклатура.ВидНоменклатуры <> &ВидНоменклатурыВода ","")+"
		//конец изменений БП 14 
		|";
...
Процедура ПроверитьОстатки(Выборка, Отказ, Заголовок, СтруктураПараметров)
	...
	Пока Выборка.Следующий() Цикл
		ПредставлениеНоменклатуры = УправлениеЗапасами.ПредставлениеНоменклатуры(СокрЛП(Выборка.Номенклатура) + " (Код: " + СокрЛП(Выборка.Номенклатура.Код) + ")", 
		...
		//Для некоторых номенклатур разрешаем проводить в минус
		//ОбщегоНазначения.СообщитьОбОшибке(ТекстСообщения , Отказ, Заголовок);
		Если Выборка.Склад.НСИ_ПодключенКSolvo И (ТипЗнч(СтруктураПараметров.ДокументСсылка) = Тип("ДокументСсылка.ПеремещениеТоваров")
			 ИЛИ ТипЗнч(СтруктураПараметров.ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
			 ИЛИ ТипЗнч(СтруктураПараметров.ДокументСсылка) = Тип("ДокументСсылка.ТребованиеНакладная")) Тогда
			 
			Если ЗначениеЗаполнено(СтруктураПараметров.ДокументСсылка.СП_ЗаданиеНаПодбор) и Выборка.Номенклатура.НСИ_РазрешитьОтгрузкуВМинус Тогда
