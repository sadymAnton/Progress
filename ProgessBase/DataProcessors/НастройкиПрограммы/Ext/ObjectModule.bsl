...
Процедура ПолучитьОбщиеДанныеИЗаполнитьФорму(ФормаОбработки) Экспорт
	...
	Если ФормаОбработки.ЭлементыФормы.ПанельРазделов.Страницы.РегламентныеЗадания.Видимость Тогда
		ПрочитатьНастройкиРегламентногоЗадания(ФормаОбработки, "РасчетОстатковОтпусков");
...
Процедура ФормаОбработкиПередОткрытием(ФормаОбработки, Отказ) Экспорт
	...
	Если Не Отказ Тогда
		...
		Если ПравоДоступа("Администрирование", Метаданные) Тогда
			...
			КонецЕсли;
			ФормаОбработки.ЭлементыФормы.ПанельРазделов.Страницы.СтраницаЗащитаПерсональныхДанных.Видимость = Истина;			
		Иначе
			ФормаОбработки.ЭлементыФормы.ПанельРазделов.Страницы.СтраницаЗащитаПерсональныхДанных.Видимость = Ложь;
		КонецЕсли;
...
Функция ЗаписатьНастройкиЗащитыПерсональныхДанных(ФормаОбработки) Экспорт
	...
	Если ПравоДоступа("Администрирование", Метаданные) И ПанельЗащитаПерсДанных.ТекущаяСтраница = ПанельЗащитаПерсДанных.Страницы.РежимДоступен Тогда
		...
		Если ЗначениеЗаполнено(ОписаниеОшибок) Тогда
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ОписаниеОшибок);
			Возврат Ложь;
		КонецЕсли;
		...
		Исключение
			Если ТранзакцияАктивна() Тогда
				ОтменитьТранзакцию();
			КонецЕсли;
			ОбщегоНазначенияЗК.ВывестиИнформациюОбОшибке(ОбщегоНазначения.ПолучитьПричинуОшибки(ИнформацияОбОшибке()).Описание);
			Возврат Ложь;
		КонецПопытки;
