...
Процедура ЗаполнитьПостроительОтчетаПоЦенамНоменклатуры() Экспорт
	...
	//Ставки НДС
	СписокСтавок18 = Новый СписокЗначений;
	СписокСтавок18.Добавить(Перечисления.СтавкиНДС.НДС18);
	СписокСтавок18.Добавить(Перечисления.СтавкиНДС.НДС18_118);		
	ПостроительОтчета.Параметры.Вставить("НДС18", СписокСтавок18);
	
	СписокСтавок10 = Новый СписокЗначений;
	СписокСтавок10.Добавить(Перечисления.СтавкиНДС.НДС10);
	СписокСтавок10.Добавить(Перечисления.СтавкиНДС.НДС10_110);
	ПостроительОтчета.Параметры.Вставить("НДС10", СписокСтавок10);
	
	СписокСтавок20 = Новый СписокЗначений;
	СписокСтавок20.Добавить(Перечисления.СтавкиНДС.НДС20);
	СписокСтавок20.Добавить(Перечисления.СтавкиНДС.НДС20_120);
	ПостроительОтчета.Параметры.Вставить("НДС20", СписокСтавок20);
	//----m.ionov@a-prof.ru---
	...
	|	ЗапросПрайс.Брендообъем,
	|	ЗапросПрайс.Брендообъем.Представление КАК БрендообъемПредставление, 
	...
	//|		ЗапросПрайс.Цена
	Если ПЦ_ЦеныБезНДС = 0 Тогда
		ТекстЗапроса = ТекстЗапроса + "
	|		ЗапросПрайс.Цена";
	Иначе
		ТекстЗапроса = ТекстЗапроса + 
	?(ПЦ_ЦеныБезНДС = 1,"
	|               ВЫБОР КОГДА ЗапросПрайс.ТипЦен.ЦенаВключаетНДС ТОГДА
			|               ВЫБОР КОГДА ЗапросПрайс.Номенклатура.СтавкаНДС В (&НДС18) ТОГДА
			|               ЗапросПрайс.Цена - ВЫРАЗИТЬ(ЗапросПрайс.Цена*18/118 КАК ЧИСЛО(15,2))
			|               ИНАЧЕ ВЫБОР КОГДА ЗапросПрайс.Номенклатура.СтавкаНДС В (&НДС10) ТОГДА
			|               ЗапросПрайс.Цена - ВЫРАЗИТЬ(ЗапросПрайс.Цена*10/110 КАК ЧИСЛО(15,2))
			|               ИНАЧЕ ВЫБОР КОГДА ЗапросПрайс.Номенклатура.СтавкаНДС В (&НДС20) ТОГДА
			|               ЗапросПрайс.Цена - ВЫРАЗИТЬ(ЗапросПрайс.Цена*20/120 КАК ЧИСЛО(15,2))
			|               ИНАЧЕ
			|               ЗапросПрайс.Цена
			|               КОНЕЦ
			|               КОНЕЦ
			...
			|               КОНЕЦ","
			|               ВЫБОР КОГДА Не ЗапросПрайс.ТипЦен.ЦенаВключаетНДС ТОГДА
			|               ВЫБОР КОГДА ЗапросПрайс.Номенклатура.СтавкаНДС В (&НДС18) ТОГДА
			|               ЗапросПрайс.Цена + ВЫРАЗИТЬ(ЗапросПрайс.Цена*18/100 КАК ЧИСЛО(15,2))
			|               ИНАЧЕ ВЫБОР КОГДА ЗапросПрайс.Номенклатура.СтавкаНДС В (&НДС10) ТОГДА
			|               ЗапросПрайс.Цена + ВЫРАЗИТЬ(ЗапросПрайс.Цена*10/100 КАК ЧИСЛО(15,2))
			|               ИНАЧЕ ВЫБОР КОГДА ЗапросПрайс.Номенклатура.СтавкаНДС В (&НДС20) ТОГДА
			|               ЗапросПрайс.Цена + ВЫРАЗИТЬ(ЗапросПрайс.Цена*20/100 КАК ЧИСЛО(15,2))
			|               ИНАЧЕ
			|               ЗапросПрайс.Цена
			|               КОНЕЦ
			|               КОНЕЦ
	...
	|               ВЫБОР КОГДА ЗапросПрайс.ТипЦен.ЦенаВключаетНДС ТОГДА
		...
		КонецЕсли;	
			ТекстЗапроса = ТекстЗапроса + "
	|* ВЫБОР КОГДА КурсыВалютСрезПоследних.Курс = 0 ТОГДА
	|										1
	|									ИНАЧЕ
	...
	//{23.03.2016 Островерхий заявка №50518 
	|	ЦеныБазовые.Брендообъем, 
	//23.03.2016 Островерхий} 
	...
	|					0
	|				ИНАЧЕ
						//{23.11.2015 Островерхий заявка №б/н 
	//|					ЦеныРасчетные.Цена * (1 + ЦеныБазовые.ПроцентСкидкиНаценки / 100) 
	|					Выразить(ЦеныРасчетные.Цена * (1 + ЦеныБазовые.ПроцентСкидкиНаценки / 100) КАК Число(15,2))					
						//23.11.2015 Островерхий} 
	|				КОНЕЦ
	|			ИНАЧЕ
	...
	//{23.03.2016 Островерхий заявка №50518 
	|	СписокНоменклатуры.Номенклатура.НСИ_Брендообъем КАК Брендообъем, 
	//23.03.2016 Островерхий} 
	...
	ТекстЗапроса = ТекстЗапроса + "
	//{29.03.2016 Островерхий заявка №50518 
	|{ГДЕ СписокНоменклатуры.Номенклатура.НСИ_Брендообъем.* КАК Брендообъем}
	...
	"ТипЦен,    Номенклатура,   ХарактеристикаНоменклатуры ,   ЕдиницаИзмерения, Остаток,  ЦеноваяГруппа,    ШтрихКод, Брендообъем",
	"Тип цен", "Номенклатура", "Характеристика номенклатуры", "Ед. измерения", "Остаток", "Ценовая группа", "Штрихкод", "Брендообъем");
	...
	//{23.03.2016 Островерхий заявка №50518 
	СоответствиеДоступныхПолей.Вставить("Брендообъем", 0); 
	//23.03.2016 Островерхий} 
    ...
    //{23.03.2016 Островерхий заявка №50518 
	СоответствиеДоступныхОтборов.Вставить("Брендообъем", 0); 
	//23.03.2016 Островерхий} 
	...
	ПостроительОтчета.ИзмеренияСтроки.Добавить("Брендообъем", ,ТипИзмеренияПостроителяОтчета.Иерархия);
	//23.03.2016 Островерхий} 
	...
	ПостроительОтчета.Порядок.Добавить("Брендообъем");
	//23.03.2016 Островерхий} 
	...
	МассивОтбора.Добавить("Брендообъем");
	//23.03.2016 Островерхий} 
...
Процедура ПечатьПрайсЛиста(ДокументРезультат) Экспорт
	...
	//m.ionov@a-prof.ru 27.10.2014
	Если ПЦ_ЦеныБезНДС = 0 Тогда
		Секция.Параметры.НДС = "Вывод НДС задан в типе цен";
	ИначеЕсли ПЦ_ЦеныБезНДС = 1 Тогда
		Секция.Параметры.НДС = "Цены без НДС";
	Иначе
		Секция.Параметры.НДС = "Цены с НДС";
	КонецЕсли;		
	//----m.ionov@a-prof.ru---
