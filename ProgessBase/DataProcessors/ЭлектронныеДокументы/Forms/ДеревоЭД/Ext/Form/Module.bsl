	...
	Если СоответствиеВладельцевИЭД.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(Элемент.Значение) Тогда
				Если НЕ ЗначениеЗаполнено(НастройкиОбмена) Тогда
					НастройкиОбмена = ЭлектронныеДокументыСлужебный.ОпределитьНастройкиОбменаЭДПоИсточнику(
						ОбъектСсылка, Ложь, , Элемент.Значение);
				КонецЕсли;
		...
		Если ЗначениеЗаполнено(АктуальныйЭД) Тогда
			ПараметрыАктуальногоЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(АктуальныйЭД,
				"СтатусЭД, ВерсияРегламентаЭДО, ВидЭД, НаправлениеЭД");
			Если ПараметрыАктуальногоЭД.СтатусЭД = Перечисления.СтатусыЭД.ОтклоненПолучателем Тогда
...
Процедура ПредварительноеЗаполнениеДерева(ЭД, ДеревоОбъект, ЭтоАктуальныйЭД, КорневойЭлемент = Неопределено)
	...
	Если ЗначениеЗаполнено(НастройкиОбмена) И НастройкиОбмена.Свойство("СоглашениеЭД") Тогда
		ОбменЧерезОператора = НастройкиОбмена.СоглашениеЭД.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском;
		...
		ИначеЕсли ЭД.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Продавец
			...
			Если ОбменЧерезОператора Тогда
				Если ЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий Тогда
					...
					Если ЭД.ВерсияРегламентаЭДО <> Перечисления.ВерсииРегламентаОбмена1С.Версия20 Тогда
...
Функция ПолучитьОжидаемоеДействиеКонтрагента(Строка)
	...
	Если Строка.ВидЭД = Перечисления.ВидыЭД.ИзвещениеОПолучении
		...
		Если Строка.ВидЭД = Перечисления.ВидыЭД.Подтверждение И НЕ ЗначениеЗаполнено(Строка.Ссылка)
			...
			ВозвращаемоеЗначение = "Подтверждения оператора ЭДО";
		ИначеЕсли (Строка.ТипЭД = Перечисления.ТипыЭлементовВерсииЭД.ИПУУЭСФ
			...
			ВозвращаемоеЗначение = "Извещения о получении";
		КонецЕсли;
		
	Иначе
		Если Строка.СтатусЭД = Перечисления.СтатусыЭД.ПереданОператору Тогда
			
			ВозвращаемоеЗначение = "Подтверждения отправки";
		...
		ИначеЕсли Строка.СтатусЭД = Перечисления.СтатусыЭД.Отправлен Тогда
			...
			ВозвращаемоеЗначение = "Подтверждения о доставке";
		КонецЕсли;
	КонецЕсли;
...
Процедура ВывестиПодчиненныеДокументы(ТекущийДокумент, ДеревоРодитель, АктуальныйЭД = Неопределено)
	...
	|	ПрисоединенныеФайлы.ДатаДокументаОтправителя КАК ДатаВладельца,
	|	ПрисоединенныеФайлы.НомерДокументаОтправителя КАК НомерВладельца,
	|	ПрисоединенныеФайлы.ВидЭД,
	|	ПрисоединенныеФайлы.ТипЭлементаВерсииЭД КАК ТипЭД,
	|	ВЫБОР
	|		КОГДА ПрисоединенныеФайлы.ВидЭД В (&СписокВидовЭД)
	|			ТОГДА ВЫБОР
	|					КОГДА ПрисоединенныеФайлы.НаправлениеЭД = ЗНАЧЕНИЕ(Перечисление.НаправленияЭД.Исходящий)
	|						ТОГДА ПрисоединенныеФайлы.ДатаСоздания
	|					ИНАЧЕ ПрисоединенныеФайлы.ДатаФормированияЭДОтправителем
	|				КОНЕЦ
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Версия
	|ИЗ
	...
	|			И (НЕ ПодчиненныеПрисоединенныеФайлы.ВидЭД В (&ВидыЭДисключения))
	|ГДЕ
		...
		Если Поле.Имя = "ДеревоПодчиненныеЭДОжидаемоеДействие" И Элемент.ТекущиеДанные.СтрокаДоступна Тогда
			...
			Если ВыполняемоеДействие = ПредопределенноеЗначение("Перечисление.ОжидаемоеДействиеЭД.Сформировать") Тогда
				...
				Если Элемент.ТекущиеДанные.ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.ИзвещениеОПолучении")
					...
					ЭлектронныеДокументыСлужебныйКлиент.СформироватьПодписатьИОтправитьСлужебныеЭД(МассивЭДДляИзвещений,
			...
			ИначеЕсли ВыполняемоеДействие = ПредопределенноеЗначение("Перечисление.ОжидаемоеДействиеЭД.Отправить") Тогда
				...
				Если МассивСтруктурПакетовЭД.Количество() > 0 Тогда
					...
					ТекСоглашение = Соглашение(МассивСтруктурПакетовЭД[0].ПЭД);
					МассивСоглашений.Добавить(ТекСоглашение);
					СоотвСоглашенийИСтруктурСертификатов = ЭлектронныеДокументыСлужебныйКлиент.СоотвСоглашенийИСтруктурСертификатовДляАвторизации(МассивСоглашений);
					
					ЭлектронныеДокументыСлужебныйКлиент.ОтправитьМассивПакетовЭД(МассивПЭД, СоотвСоглашенийИСтруктурСертификатов);
				КонецЕсли;
...
Функция Соглашение(ПакетЭД)
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПакетЭД, "СоглашениеЭД");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(ОбъектСсылка) = Тип("СправочникСсылка.СоглашенияОбИспользованииЭД") И ОбъектСсылка.ЭтоТиповое Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'По типовым соглашениям не предусмотрено ведение электронных документов!'"));
	Иначе
