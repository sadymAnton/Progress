	...
	Если НЕ ОтборТипЦен.Использование
	 ИЛИ ОтборТипЦен.ВидСравнения <> ВидСравненияКомпоновкиДанных.Равно Тогда
		...
		Если ОтборТипЦен.Использование Тогда
			ОбщегоНазначения.Сообщение("Возможен отбор только по одному типу цен. Информация о ценах номенклатуры не заполнена.", СтатусСообщения.Важное);
		КонецЕсли;
...
Функция ПодготовитьСтруктуруДанных(СтруктураНастроек) Экспорт
	...
	// Схема компоновки.
	СхемаКомпоновкиДанных = Обработки.ПодборНоменклатурыПоОтбору.ПолучитьМакет(СтруктураНастроек.ИмяМакетаСхемыКомпоновкиДанных);
