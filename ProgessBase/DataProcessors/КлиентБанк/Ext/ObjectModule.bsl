...
// <- Шевченков №37608 20150319
Перем ТаблИнтерв;
Перем Дата1;
Перем Дата2;
Перем МассивВидовДоговоров;
Перем МассивДоков;
Перем Контрагент Экспорт;
Перем СформированныеДокументы Экспорт;
Перем ТаблицаПДЗ Экспорт;
// ->

#Если Клиент Тогда
 //Blik 27543 проверим установлен ли префикс н
 Функция ПроверитьПрефиксыВДоПрегистре()
	Запрос       = Новый Запрос;
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПРГ_НумерацияОбъектов.Нумератор.Префикс КАК Префикс
	|ИЗ
	|	РегистрСведений.ПРГ_НумерацияОбъектов КАК ПРГ_НумерацияОбъектов
	|ГДЕ
	|	ПРГ_НумерацияОбъектов.Нумератор.Наименование ПОДОБНО ""Платежные поручения""";
...
КонецФункции
//Blik 27543 проверим установлен ли префикс к
...
Функция ПолучитьНомерВФорматеОбмена(НомерДокумента, Знач ПрефиксОрганизации)
	...
	КонецЕсли;
     ...
     //Blik если установлен префикс через регистр 27543 н
	 ЕстьПрефикс = ПроверитьПрефиксыВДоПрегистре();
	 если не ЕстьПрефикс = Неопределено тогда
		 Номер = Сред(Номер, СтрДлина(ЕстьПрефикс));
	 КонецЕсли;
     //Blik если установлен префикс через регистр 27543 к
...
Процедура ПроверитьЗаполнениеНалоговыхРеквизитов(СтрокаЭкспорта)
	...
	Если СтрокаЭкспорта.ВидДокумента = "Платежное поручение исходящее" Тогда
...
Процедура ЗаписатьОбъект(ОбъектДляЗаписи,СтрокаСекции)
	...
	// <- Шевченков
	//Если ОбъектДляЗаписи.РасшифровкаПлатежа.Количество() > 1 Тогда
		Проводить = Истина;
	//КонецЕсли;	
	// ->
...
//Организация, ДАТА, НОМЕР 
	...
	Если ОбъектДокумента.ЭтоНовый() Тогда
		...
		ПРГ_ДопФункцииКлиентСервер.ПрисвоитьНомер(ОбъектДокумента,Организация.Префикс,"",истина);  //blik 180414 27543
...
Процедура ЗаполнитьРеквизитПлатежногоПорученияВходящего(ОбъектДокумента, ИсходныеДанные, ТаблицаРасшифровкиПлатежа = Неопределено)
...
//Счета учета расчетов с контрагентом			
	...
	// <- Шевченков
	Если ЗначениеЗаполнено(ТаблицаРасшифровкиПлатежа) Тогда
		ОбъектДокумента.РасшифровкаПлатежа.Очистить();
		Для Каждого ТекСтрока Из ТаблицаРасшифровкиПлатежа Цикл			
			СтрокаРасшифровки = ОбъектДокумента.РасшифровкаПлатежа.Добавить();
			//ОднаСтрокаВРасшифровкеПлатежа = ОбъектДокумента.РасшифровкаПлатежа.Количество() = 1;
			УстановитьСвойство(СтрокаРасшифровки, "ДоговорКонтрагента", ?(ТекСтрока.Договор="Не найден",неопределено,ТекСтрока.Договор));
			УстановитьСвойство(СтрокаРасшифровки, "СтатьяДвиженияДенежныхСредств", ТекСтрока.СтатьяДДС);			
			СтрокаРасшифровки.СуммаПлатежа = ТекСтрока.СуммаПоступило;
			РассчитатьКурсИСуммуВзаиморасчетов(СтрокаРасшифровки, ПолучитьВалютуДоговора(СтрокаРасшифровки.ДоговорКонтрагента), ОбъектДокумента.Дата, ОбъектДокумента);
		...
		СформированныеДокументы.Добавить(ТекСтрока.НомерСтроки);
	    УправлениеДенежнымиСредствами.ЗаполнитьСчетаУчетаПлатежейБУ(ОбъектДокумента,глЗначениеПеременной("глТекущийПользователь"),ЛОЖЬ);
	КонецЕсли;	
	// ->
...
Процедура ЗаполнитьРеквизитПлатежногоДокумента(ОбъектДокумента,ТипДокумента, СтрокаСекции, ТабилцаРасшифровкиПлатежа = Неопределено)
	
	Если ТипДокумента = "ПлатежноеПоручениеИсходящее" Тогда
		ЗаполнитьРеквизитПлатежногоПорученияИсходящего(ОбъектДокумента, СтрокаСекции);
		
	ИначеЕсли ТипДокумента = "ПлатежноеПоручениеВходящее" Тогда		
		ЗаполнитьРеквизитПлатежногоПорученияВходящего(ОбъектДокумента, СтрокаСекции, ТабилцаРасшифровкиПлатежа);
...
Процедура Загрузить(ДокументыКИмпорту) Экспорт
	...
	СформированныеДокументы = Новый Массив; // Шевченков
	...
	Для каждого СтрокаСекции из ДокументыКИмпорту Цикл
		Если СформированныеДокументы.Найти(СформированныеДокументы) <> Неопределено Тогда Продолжить КонецЕсли; // Шевченков
		Если СтрокаСекции.Загружать Тогда
			...
			Если ПустаяСтрока(РезультатПроверки) Тогда
				...
				ТабилцаРасшифровкиПлатежа = ПолучитьДанныеПоДоговорамФормированиеПлатежки(ДокументыКИмпорту, СтрокаСекции.НомерДок, СтрокаСекции.ДатаДок, СтрокаСекции.Контрагент); // Шевченков
				...
				ЗаполнитьРеквизитПлатежногоДокумента(ОбъектДокумента,ОбъектДокумента.Метаданные().Имя, СтрокаСекции, ТабилцаРасшифровкиПлатежа);
...
Процедура КнопкаВыполнитьНажатие(Кнопка)
	...
	//
	//Возврат;
	РезЗапрос = ПросчитатьЗадолженность(Контрагент);
...
Функция ПросчитатьЗадолженность(ТекКонтрагент)
	
	ВыводитьВремя = Ложь;
	ТекВремя  = ТекущаяДата();
	ТаблИтого = ПолучитьРасчитаннуюДебиторку(ТекКонтрагент);
	
	ПостроительИтоговый 	  = Новый Запрос;
	ПостроительИтоговый.Текст = ПолучитьТекстЗапросИтоговый(2);
		
	ПостроительИтоговый.УстановитьПараметр("Дата1",Дата1);
	ПостроительИтоговый.УстановитьПараметр("Дата2",Дата2);
	ПостроительИтоговый.УстановитьПараметр("Контрагент", Контрагент);
	ПостроительИтоговый.УстановитьПараметр("ВидДоговора",МассивВидовДоговоров);
	ПостроительИтоговый.УстановитьПараметр("ПустойДокумент",МассивДоков);
	...
	ПостроительИтоговый.МенеджерВременныхТаблиц = ЗапросВспомог.МенеджерВременныхТаблиц;
	  
	ПозПакета = Найти(ПостроительИтоговый.Текст,";");
	Если ПозПакета = 0 Тогда
		Сообщить("Сбой в отчете. Обратитесь к администратору");
		возврат Неопределено;
	Иначе
		ПостроительИтоговый.Текст = Сред(ПостроительИтоговый.Текст,ПозПакета+1);
	КонецЕсли;

	ПостроительИтоговый.МенеджерВременныхТаблиц = ЗапросВспомог.МенеджерВременныхТаблиц;
	РезЗапрос = ПостроительИтоговый.Выполнить().Выгрузить();
	
	//РезЗапрос.Колонки.Найти("Договор").Имя = "ДоговорКонтрагента";
	//РезЗапрос.Колонки.Найти("Договор").Имя = "ДоговорКонтрагента";
...
// Эту функцию вызвать чтобы получить таблицу с разнесенной суммой по договорам контрагента
Функция СформироватьТаблицуОплаты(СуммаОплаты, ТекКонтрагент) Экспорт
	
	РезЗапрос = ПросчитатьЗадолженность(ТекКонтрагент);
	
	// Получим список договоров контрагента
	СписокДоговоров = ПолучитьСписокДоговоров(ТекКонтрагент);
	
	Если ЗначениеЗаполнено(РезЗапрос) Тогда
		// Данные какие-то есть
		ИтогВзаиморасчетов = РезЗапрос.Итог("КонОст");
		...
		//		СтруктураДоговоров.Вставить(ТекСтрока.Договор, ТекСтрока.ТекСумма);					
		//	КонецЦикла;
		//Иначе 
		//	Сообщить("Не удалось определить договоры контрагента. Автоматическое распределение суммы невозможно.");
		//КонецЕсли;
		
		Если ИтогВзаиморасчетов <= 0 Тогда
			// Задолженности нет, надо кинуть на основной договор авансом
			Если СуммаОплаты > 0 Тогда 				
				ОснДоговор = ПолучитьОсновнойДоговорКонтрагента(ТекКонтрагент);
				Если ЗначениеЗаполнено(ОснДоговор) Тогда
					СтрокаТекДоговор = СписокДоговоров.Найти(ОснДоговор);
					СтрокаТекДоговор.ТекСумма = СтрокаТекДоговор.ТекСумма + СуммаОплаты;
					Сообщить("Сумма оплаты по контрагенту " + Строка(ТекКонтрагент) + " распределена. На основной договор зачислен аванс");
				КонецЕсли;				
			КонецЕсли;			
		Иначе
			// Задолженность есть, раскидаем по договорам
			СуммаОстатокОплаты = СуммаОплаты;
			СуммаРаспределено  = 0;
			Инд                = 0;			
			// Распределим сумму по документам, у которых есть задолженность
			Для Каждого ТекСтрока Из РезЗапрос Цикл				
				ТекСумма = ТекСтрока.КонОст;
				Если ТекСтрока.КонОст <= 0 Тогда Продолжить КонецЕсли;				
				Если ТекСумма >= СуммаОстатокОплаты Тогда
					СтрокаТекДоговор = СписокДоговоров.Найти(ТекСтрока.ДоговорКонтрагента);
					СтрокаТекДоговор.ТекСумма = СтрокаТекДоговор.ТекСумма + СуммаОстатокОплаты;
					СуммаРаспределено  = СуммаРаспределено + СуммаОстатокОплаты;
					СуммаОстатокОплаты = СуммаОстатокОплаты - СуммаОстатокОплаты;					
				Иначе
					СтрокаТекДоговор = СписокДоговоров.Найти(ТекСтрока.ДоговорКонтрагента);
					СтрокаТекДоговор.ТекСумма = СтрокаТекДоговор.ТекСумма + ТекСумма;
					СуммаРаспределено  = СуммаРаспределено + ТекСумма;
					СуммаОстатокОплаты = СуммаОстатокОплаты - ТекСумма;
					...
					//// На всякий случай проверим если всю сумму распределили
					Если СуммаРаспределено = СуммаОплаты И СуммаОстатокОплаты = 0 Тогда
						// Всю сумму по договорам раскидали
						Сообщить("Сумма оплаты по контрагенту " + Строка(ТекКонтрагент) + " распределена.");
						Прервать;
					ИначеЕсли СуммаРаспределено <> СуммаОплаты И СуммаОстатокОплаты = 0 Тогда
						// Такого не должно произойти, но все же
						Сообщить("При распределении оплаты по контрагенту " + Строка(ТекКонтрагент) + " возникли ошибки. Необходимо обратиться к программистам 1С.");
						Прервать;
					КонецЕсли;					
				КонецЕсли;				
			КонецЦикла;
			
			// Проверим, если задолженность осталась кинем на основной договор авансом
		...
		Возврат ОбработатьТаблицуДоговоров(СписокДоговоров);
	...
	Иначе
		...
		Если СуммаОплаты > 0 Тогда 				
			...
			КонецЕсли;
		Иначе
			Сообщить("У контрагента " + Строка(ТекКонтрагент) + " не установлен основной договор. Необходимо выбрать договор вручную");
		КонецЕсли;
...
Функция ОбработатьТаблицуДоговоров(СтараяТаблица)
	
	НТЗ = СтараяТаблица.Скопировать();
	НТЗ.Очистить();
	
	Для Каждого ТекСтрока Из СтараяТаблица Цикл		
		Если ТекСтрока.ТекСумма > 0 Тогда
...
Функция ПолучитьСписокДоговоров(ТекКонтрагент)
	...
	Возврат ?(РезТЗ.Количество()>0, РезТЗ, Неопределено);
...
Функция ПолучитьОсновнойДоговорКонтрагента(ТекКонтрагент)
	...
	Запрос.УстановитьПараметр("Ссылка", ТекКонтрагент);
	
	РезТЗ = Запрос.Выполнить().Выгрузить();
	
	Возврат ?(РезТЗ.Количество()>0, РезТЗ[0].Договор, Неопределено);
	
КонецФункции

Функция ПолучитьРасчитаннуюДебиторку(ТекКонтрагент = Неопределено)
	
	ТекВремя = ТекущаяДата();
	ДатаНач  = НачалоДня(ТекВремя);
	ДатаКон  = КонецДня(ТекВремя);
	...
	Дата1 = НачалоДня(ДатаНач);
	Дата2 = КонецДня(ДатаКон);
		
	ПостроительЗапроса = Новый Запрос;
	...
	ПостроительЗапроса.УстановитьПараметр("Дата1",Дата1);
	ПостроительЗапроса.УстановитьПараметр("Дата2",Дата2);
	ПостроительЗапроса.УстановитьПараметр("Контрагент", Контрагент);
	ПостроительЗапроса.УстановитьПараметр("ВидДоговора",МассивВидовДоговоров);
	ПостроительЗапроса.УстановитьПараметр("ПустойДокумент",МассивДоков);	
	
	ПостроительЗапроса.Выполнить();	
	РезЗапроса 			   = ПостроительЗапроса.Выполнить().Выгрузить();
	ДатаДляРасчетаПроср    = НачалоДня(Дата2);
	
	ВыборкаДетальная 	   = ПостроительЗапроса.Выполнить().Выбрать();
	...
	КонецЦикла; 
	//Сообщить("Кол: " + ТаблИтого.Количество());
	возврат ТаблИтого;
...
Функция ПолучитьТекстЗапросИтоговый(Пар=1)
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВзаиморасчетыСКонтрагентами.Контрагент,
	|	ВзаиморасчетыСКонтрагентами.Организация,
	...
	|	ВЫБОР
	|		КОГДА ТаблИтого.КонОст > 0
	|			ТОГДА ВЫРАЗИТЬ(ТаблИтого.Просрочено / ТаблИтого.КонОст * 100 КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ 0
...
Функция ПолучитьДанныеПоДоговорамФормированиеПлатежки(ДокументыКИмпорту, НомерДок, ДатаДок, пКонтрагент)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ВРем.ДатаДок КАК ДАТА) КАК ДатаДок,
	|	ВРем.Договор,
	|	ВЫРАЗИТЬ(ВРем.Контрагент КАК Справочник.Контрагенты) КАК Контрагент,
	|	ВЫРАЗИТЬ(ВРем.НомерДок КАК СТРОКА(11)) КАК НомерДок,
	|	ВРем.НомерСтроки,
	|	ВРем.СтатьяДДС,
	|	ВРем.Сумма,
	|	ВРем.СуммаДокумента,
	...
	|	ТекЭл.СтатьяДДС,
	|	ТекЭл.Сумма,
	|	ТекЭл.СуммаДокумента,
	|	ТекЭл.СуммаПоступило
	|ИЗ
	|	ВТабл КАК ТекЭл
	|ГДЕ
	|	ТекЭл.Контрагент = &Контрагент
	|	И ТекЭл.НомерДок = &НомерДок
	|	И ТекЭл.ДатаДок = &ДатаДок";
	Запрос.УстановитьПараметр("Табл",     ДокументыКИмпорту);
	...
	РезТЗ = Запрос.Выполнить().Выгрузить();
	Возврат ?(РезТЗ.Количество()>1, РезТЗ, Неопределено);
...
Функция ПросчитатьЗадолженностьПоСпискуКонтрагентов(СписокКонтрагентов) Экспорт
	
	//СписокКонтрагентов = Новый СписокЗначений;
	ВыводитьВремя = Ложь;
	ТекВремя  = ТекущаяДата();
	ТаблИтого = ПолучитьРасчитаннуюДебиторкуПоСпискуКонтрагентов(СписокКонтрагентов);
	
	ПостроительИтоговый 	  = Новый Запрос;
	ПостроительИтоговый.Текст = ПолучитьТекстЗапросИтоговыйПоСпискуКонтрагентов(2);
		
	ПостроительИтоговый.УстановитьПараметр("Дата1",Дата1);
	ПостроительИтоговый.УстановитьПараметр("Дата2",Дата2);
	ПостроительИтоговый.УстановитьПараметр("СписокКонтрагентов", СписокКонтрагентов);
	ПостроительИтоговый.УстановитьПараметр("ВидДоговора",МассивВидовДоговоров);
	ПостроительИтоговый.УстановитьПараметр("ПустойДокумент",МассивДоков);
	...
	//РезЗапрос.Колонки.Найти("Договор").Имя = "ДоговорКонтрагента";
	//РезЗапрос.Колонки.Найти("Договор").Имя = "ДоговорКонтрагента";
	ТаблицаПДЗ = РезЗапрос;
	Возврат ТаблицаПДЗ;
	
КонецФункции

Функция ПолучитьРасчитаннуюДебиторкуПоСпискуКонтрагентов(СписокЗначений = Неопределено)
	...
	ПостроительЗапроса = Новый Запрос;
	ПостроительЗапроса.Текст = ПолучитьТекстЗапросаПоСпискуКонтрагентов();
	...
	ПостроительЗапроса.УстановитьПараметр("Дата1",Дата1);
	ПостроительЗапроса.УстановитьПараметр("Дата2",Дата2);
	ПостроительЗапроса.УстановитьПараметр("СписокКонтрагентов", СписокЗначений);
	ПостроительЗапроса.УстановитьПараметр("ВидДоговора",МассивВидовДоговоров);
	ПостроительЗапроса.УстановитьПараметр("ПустойДокумент",МассивДоков);	
...
Функция ПолучитьТекстЗапросаПоСпискуКонтрагентов()
	возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	        |	ВзаиморасчетыСКонтрагентами.Организация КАК Организация,
	        |	ВзаиморасчетыСКонтрагентами.ДоговорКонтрагента КАК ДоговорКонтрагента,
	        |	ВзаиморасчетыСКонтрагентами.Контрагент КАК Контрагент,
	        ...
	        |	И ВзаиморасчетыСКонтрагентами.Активность
	        |	И ВзаиморасчетыСКонтрагентами.ДоговорКонтрагента.ВидДоговора В(&ВидДоговора)
	        |	И ВзаиморасчетыСКонтрагентами.Контрагент В (&СписокКонтрагентов)
	        |
	        |СГРУППИРОВАТЬ ПО
	        |	ВзаиморасчетыСКонтрагентами.Организация,
	        |	ВзаиморасчетыСКонтрагентами.ДоговорКонтрагента,
...
Функция ПолучитьТекстЗапросИтоговыйПоСпискуКонтрагентов(Пар=1)
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВзаиморасчетыСКонтрагентами.Контрагент,
	|	ВзаиморасчетыСКонтрагентами.Организация,
	...
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ПроцентПросрочки,
	|	ТаблИтого.Регистратор.Дата КАК РегистраторДата
	|ИЗ
	|	ТаблИтого КАК ТаблИтого
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
	|		ПО ТаблИтого.Контрагент = Контрагенты.Ссылка
	...
	|	И ДоговорыКонтрагентов.Владелец В(&СписокКонтрагентов)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата,
	|	ТаблИтого.Регистратор.Дата";
...
Функция СформироватьТаблицуОплаты_ПоСпискуКонтрагентов(ТаблицаПДЗ, СуммаОплаты, ТекКонтрагент = Неопределено, ОбщаяТаблицаПДЗ = Неопределено) Экспорт
	
	//РезЗапрос = ПросчитатьЗадолженностьПоТекущемуКонтрагенту(СписокКонтрагентов);
	//РезЗапрос = ПросчитатьЗадолженность(СписокКонтрагентов);
	РезЗапрос = ТаблицаПДЗ;
	// Получим список договоров контрагента
	СписокДоговоров = ПолучитьСписокДоговоров(ТекКонтрагент);
	
	Если ЗначениеЗаполнено(ТаблицаПДЗ) Тогда
		// Данные какие-то есть
		ИтогВзаиморасчетов = РезЗапрос.Итог("КонОст");
		...
		Иначе
			...
			Для Каждого ТекСтрока Из РезЗапрос Цикл				
				СуммаТекСтроки = 0;
				ТекСумма = ТекСтрока.КонОст;
				Если ТекСтрока.КонОст <= 0 Тогда					
					ТекСтрокаВОбщейПДЗ = ОбщаяТаблицаПДЗ.Найти(ТекСтрока.Регистратор);
					Если ТекСтрокаВОбщейПДЗ <> Неопределено Тогда
						ТекСтрокаВОбщейПДЗ.Использовалась = Истина;
					КонецЕсли;				
					ТекСтрока.Использовалась = Истина;					
					Продолжить;					
				КонецЕсли;
				...
				Если ТекСумма >= СуммаОстатокОплаты Тогда
					СтрокаТекДоговор = СписокДоговоров.Найти(ТекСтрока.ДоговорКонтрагента);
					СтрокаТекДоговор.ТекСумма = СтрокаТекДоговор.ТекСумма + СуммаОстатокОплаты;
					СуммаТекСтроки     = СуммаОстатокОплаты;
					СуммаРаспределено  = СуммаРаспределено + СуммаОстатокОплаты;
					СуммаОстатокОплаты = СуммаОстатокОплаты - СуммаОстатокОплаты;					
				Иначе
					СтрокаТекДоговор = СписокДоговоров.Найти(ТекСтрока.ДоговорКонтрагента);
				...
				ТекСтрокаВОбщейПДЗ = ОбщаяТаблицаПДЗ.Найти(ТекСтрока.Регистратор);
				Если ТекСтрокаВОбщейПДЗ <> Неопределено Тогда
					ТекСтрокаВОбщейПДЗ.КонОст = ТекСтрокаВОбщейПДЗ.КонОст - СуммаТекСтроки;
				Иначе
					Сообщить("Критическая ошибка! Просьба обратиться к программистам 1С.", СтатусСообщения.ОченьВажное);
				КонецЕсли;
				
				Если ТекСтрока.КонОст - СуммаТекСтроки <= 0 Тогда									
					ТекСтрока.Использовалась = Истина;					
					Продолжить;					
				КонецЕсли;
	...
	Иначе
		...
		// Проверим, если задолженности нет кинем на основной договор авансом
		Если СуммаОплаты > 0 Тогда 				
			ОснДоговор = ПолучитьОсновнойДоговорКонтрагента(ТекКонтрагент);
			Если ЗначениеЗаполнено(ОснДоговор) Тогда
				СтрокаТекДоговор = СписокДоговоров.Найти(ОснДоговор);
