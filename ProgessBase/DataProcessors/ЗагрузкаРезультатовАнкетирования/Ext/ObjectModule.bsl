		...
		Для Каждого СтрокаДерева Из ДеревоАнкет.Строки Цикл  // загружаем из каталога
			...
			Для индФлага = 1 По 2 Цикл
				...
				Если индФлага = 1 И СтрокаДерева.ЗагружатьВСправочник Тогда
					...
					Иначе
						ОбщегоНазначенияЗК.СообщитьОбОшибке("Загрузка файла прервана.");
					КонецЕсли;
...
Функция ПрочитатьФайл(ИмяФайла, ПотокВывода) Экспорт
	...
	Если НЕ ХМЛ.Прочитать() Тогда
		ОбщегоНазначенияЗК.СообщитьОбОшибке("Невозможно прочитать файл!");
		Возврат Ложь;
	КонецЕсли;
	...
	Пока ((ХМЛ.ТипУзла <> ТипУзлаXML.НачалоЭлемента) ИЛИ (ХМЛ.Имя <> "Заголовок")) Цикл
		...
		Если НЕ ХМЛ.Прочитать() Тогда
			ОбщегоНазначенияЗК.СообщитьОбОшибке("Не найден заголовок опроса!");
			Возврат Ложь;
		КонецЕсли;
	...
	Если Версия <> "1.1" Тогда
		ОбщегоНазначенияЗК.СообщитьОбОшибке("Неверная версия формата данных ("+Версия+").");
		Возврат Ложь;
	КонецЕсли;
	...
	Пока ((ХМЛ.ТипУзла <> ТипУзлаXML.НачалоЭлемента) ИЛИ (ХМЛ.Имя <> "РезультатОпроса")) Цикл
		...
		Если НЕ ХМЛ.Прочитать() Тогда
			ОбщегоНазначенияЗК.СообщитьОбОшибке("Не найдены результаты опроса!");
			Возврат Ложь;
		КонецЕсли;
	...
	Если Анкета.Пустая() Тогда
		ОбщегоНазначенияЗК.СообщитьОбОшибке("Не обнаружена анкета с идентификатором " + ИдАнкеты);
		ОбщегоНазначенияЗК.СообщитьОбОшибке("Возможно изменился идентификатор или она удалена");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПоместитьВопросыВОбъектЗагрузки(ХМЛ, ПотокВывода, ДатаЗаписи, Анкета, ТабличныйВопрос = Неопределено, НомерСтрокиВтаблице = 0)
	...
	Для каждого СтрокаОтвет из ОтветыТаблицы Цикл
		...
		Если ЕстьКонтактныеЛица И
			ЗначениеЗаполнено(Фамилия) и ЗначениеЗаполнено(Имя) и ЗначениеЗаполнено(Отчество) и ЗначениеЗаполнено(ДатаРождения) И
			ТипЗнч(ПотокВывода) = Тип("СправочникОбъект.КонтактныеЛица") Тогда
			ПотокВывода = Справочники.КонтактныеЛица.СоздатьЭлемент();
			ПотокВывода.Наименование = Фамилия	+ " " + Имя + " " + Отчество;
			ПотокВывода.ДатаРождения = ДатаРождения;
			ПотокВывода.Записать();
		...
		ИначеЕсли	ТипВидКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес	тогда
			...
			МассивПолей	=	ОбщегоНазначенияЗК.РазложитьСтрокуВМассивПодстрок(Ответ,	"¤");
