...
Функция ВыгрузитьВсеНастройки() Экспорт 
	...
	лДанные.Вставить("Показатели", Новый Массив);
	лВыборка = Справочники.ис_ПоказателиОтложенныхНалогов.Выбрать();
	Пока лВыборка.Следующий() Цикл 
		Если НЕ лВыборка.Ссылка.ПометкаУдаления Тогда 
			лДанные.Показатели.Добавить(ПолучитьДанныеОбъекта(лВыборка.Ссылка));
		КонецЕсли;
	КонецЦикла;
...
Функция ЗагрузитьНастройки(Данные) Экспорт 
	...
	Попытка
		...
		Для Каждого лДанныеОъекта Из лДанные.Показатели Цикл 
