...
Процедура ПроверитьСписокНеобработанныхВходящихВСД(GUIDПлощадки,ПеречитатьСНуля=Ложь)
	...
	Если ТаблицаВСД.Количество() > 0 Тогда 
		...
		//Обработка таблицы
		//Если есть ТТН - то должна остаться 1 строка из нескольких ВСД если ТТН нет - то каждый всд уникален
		СписокВСДСвернутый = ТаблицаВСД.Скопировать();
		СписокВСДСвернутый.Свернуть("ДатаВСД, НомерТТН, ДатаТТН, Статус");
		...
		ВсегоВСд = СписокВСДСвернутый.количество();
		Для й=1 По ВсегоВСд Цикл
			ТекСтр = СписокВСДСвернутый[ВсегоВСд-й];
			Если СокрЛП(ТекСтр.НомерТТН) = "" Тогда
				СписокВСДСвернутый.Удалить(ТекСтр);
			КонецЕсли;
		КонецЦикла;
		
		МассивСтрокСПустымТТН = ТаблицаВСД.НайтиСтроки(Новый Структура("НомерТТН",""));
		Для Каждого ВСДСПустымТТН Из МассивСтрокСПустымТТН Цикл
			НоваяСтрокаСвернутогоСписка = СписокВСДСвернутый.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаСвернутогоСписка,ВСДСПустымТТН);
		КонецЦикла;
		
		//соотнесем с текущим положением дел в справочнике
		ТаблицаВСДКОбработке = ПолучитьНовыеЗаписиОВходящихВСДИзСпискаЗагруженных(СписокВСДСвернутый,ВТаблицеТолькоОформленные,ВычиталиВсеОформленные);
		
		ОсновнойМодуль.Состояние_КонтурEDI("Обрабатываю новые входящие электронные ВСД");
		й=0;
		ВсегоТТН = ТаблицаВСДКОбработке.Количество();
		Для Каждого ТекВСД Из ТаблицаВСДКОбработке Цикл
			й=й+1;
			ОсновнойМодуль.Состояние_КонтурEDI("Обрабатываю новые входящие электронные ВСД "+й+"/"+ВсегоТТН);
			Если ТекВСД.Статус = "CONFIRMED" 
				И ТекВСД.Ссылка = Null Тогда
				
				ВходящееСообщение = ПолучитьСообщениеВходящейТТН();
				
				СтрокиСЭтимиВСД = ТаблицаВСД.НайтиСтроки(Новый Структура("ДатаВСД, НомерТТН, ДатаТТН, Статус",ТекВСД.ДатаВСД,ТекВСД.НомерТТН,ТекВСД.ДатаТТН,ТекВСД.Статус));
...
Функция ПолучитьНовыеЗаписиОВходящихВСДИзСпискаЗагруженных(СписокВСДСвернутый,ВТаблицеТолькоОформленные,ВычиталиВсеОформленные)
	
	Если СписокВСДСвернутый = Неопределено Или СписокВСДСвернутый.Количество() = 0 Тогда
		Возврат СписокВСДСвернутый;
	КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	СписокВСД.ДатаТТН,
	               |	КонтурEDI_Сообщения.Ссылка,
	               |	ЕСТЬNULL(КонтурEDI_Сообщения.ID, СписокВСД.ВсдUUID) КАК ВсдUUID,
	               |	СписокВСД.Статус
	               |ИЗ
	               |	СписокВСД КАК СписокВСД
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КонтурEDI_Сообщения КАК КонтурEDI_Сообщения
	               |		ПО (ИСТИНА)
	               |			И СписокВСД.ДатаТТН = КонтурEDI_Сообщения.ДатаДокумента
	               |			И СписокВСД.ДатаВСД = КонтурEDI_Сообщения.ДатаПоставки
	               |			И (НЕ КонтурEDI_Сообщения.ПометкаУдаления)
	               |			И (КонтурEDI_Сообщения.Направление = ""Входящее"")
	               |			И (КонтурEDI_Сообщения.ТипСообщения = ""M_INC"")
	               |			И (СписокВСД.НомерТТН ПОДОБНО КонтурEDI_Сообщения.НомерДокумента
	               |				ИЛИ СписокВСД.НомерТТН = """"
	               |					И КонтурEDI_Сообщения.НомерДокумента = """"
	               |					И СписокВСД.ВсдUUID = КонтурEDI_Сообщения.ID)";
	
	Запрос.УстановитьПараметр("СписокВСД", СписокВСДСвернутый);
